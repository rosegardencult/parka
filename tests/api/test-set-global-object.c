...
static duk_ret_t test_basic(duk_context* ctx_root, void* udata) {
  ...
  duk_eval_string_noresult(ctx,
                           "var myEval = eval;  // writes 'myEval' to global "
                           "object as a side effect\n");
  dump_global_object_keys(ctx);
  duk_eval_string_noresult(ctx, "myEval('print(this.newGlobal)');");

  /*
   *  Test access to global object through an object environment.
   */
  printf("access through this.xxx and variable lookup xxx\n");
  duk_eval_string_noresult(ctx, "print('this.testName:', this.testName);\n");
  duk_eval_string_noresult(ctx, "print('testName:', testName);\n");
...
static duk_ret_t test_noeval(duk_context* ctx_root, void* udata) {
   ...
   *  Eval with C API.
   */
  (void)duk_peval_string(ctx, "print('hello from C eval'); 123\n");
...
static duk_ret_t test_regexp_prototype_shared(duk_context* ctx_root,
  ...
  (void)duk_peval_string(ctx2,
                         ...
                         "print('bar equals getProto(re2):', getProto(re2).bar "
                         "=== getProto(re2));\n");
  printf("result: %s\n", duk_safe_to_string(ctx2, -1));
  duk_pop(ctx2);
