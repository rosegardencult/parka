...
buffer: dynamic=0, size=16:
\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f index 15, type
7 -> 7, ptr-is-NULL -1, size 0 buffer: dynamic=1, size=0: index 16, type 7 -> 7,
ptr-is-NULL 0, size 16 buffer: dynamic=1, size=16:
\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f index 17, type
8 -> 7, ptr-is-NULL 0, size 4 buffer: dynamic=0, size=4: null index 18, type 8
-> 7, ptr-is-NULL 0, size 10 buffer: dynamic=0, size=10: 0xdeadbeef
...
static void dump_buffer(duk_context* ctx) {
  ...
  printf("buffer: dynamic=%d, size=%lu:", (int)duk_is_dynamic_buffer(ctx, -1),
         (unsigned long)sz);
  for (i = 0; i < sz; i++) {
...
static duk_ret_t test_1(duk_context* ctx, void* udata) {
  ...
  for (i = 0; i < n; i++) {
    ...
    printf("index %ld, type %ld -> %ld, ptr-is-NULL %d, size %lu\n", (long)i,
           (long)t1, (long)t2, (sz == 0 ? -1 : (ptr == NULL ? 1 : 0)),
           (unsigned long)sz);
    dump_buffer(ctx);
    duk_pop(ctx);
