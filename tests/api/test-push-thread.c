...
static duk_ret_t test_1(duk_context* ctx, void* udata) {
  ...
  printf("duk_is_object(%ld) = %d\n", (long)thr_idx,
         (int)duk_is_object(ctx, thr_idx));
  printf("duk_is_thread(%ld) = %d\n", (long)thr_idx,
         (int)duk_is_thread(ctx, thr_idx));
...
static duk_ret_t test_2(duk_context* ctx, void* udata) {
  ...
  duk_peval_string_noresult(ctx_a,
                            "print('context a: ' + String(this.globalFoo));");
  duk_peval_string_noresult(ctx_b,
                            "print('context b: ' + String(this.globalFoo));");
