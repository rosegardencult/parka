...
idx 0: type 1, duk_require_heapptr() -> TypeError: heapobject required, found
undefined (stack index 0) top: 7 idx 1: type 2, duk_get_heapptr() -> NULL idx 1:
type 2, duk_require_heapptr() -> TypeError: heapobject required, found null
(stack index 1) top: 7 idx 2: type 3, duk_get_heapptr() -> NULL idx 2: type 3,
duk_require_heapptr() -> TypeError: heapobject required, found true (stack index
2) top: 7 idx 3: type 4, duk_get_heapptr() -> NULL idx 3: type 4,
duk_require_heapptr() -> TypeError: heapobject required, found 123 (stack index
3) top: 7 idx 4: type 5, duk_get_heapptr() -> non-NULL idx 4: type 5,
duk_require_heapptr() -> non-NULL top: 7 idx 5: type 6, duk_get_heapptr() ->
non-NULL idx 5: type 6, duk_require_heapptr() -> non-NULL top: 7 idx 6: type 7,
duk_get_heapptr() -> non-NULL idx 6: type 7, duk_require_heapptr() -> non-NULL
top: 7
idx 7: type 0, duk_get_heapptr() -> NULL
idx 7: type 5, duk_require_heapptr() -> TypeError: heapobject required, found
none (stack index 7) "test string" {foo:"bar"} |deadbeef| undefined final top: 0
...
static duk_ret_t raw_require_heapptr(duk_context* ctx, void* udata) {
  ...
  printf("idx %ld: type %ld, duk_require_heapptr() -> %s\n", (long)i,
         (long)duk_get_type(ctx, i), (ptr ? "non-NULL" : "NULL"));
...
static duk_ret_t test_basic(duk_context* ctx, void* udata) {
  ...
  for (i = 0; i < n + 1; i++) {
    ...
    ret =
        duk_safe_call(ctx, raw_require_heapptr, NULL, 1 /*nargs*/, 1 /*nrets*/);
    if (ret == DUK_EXEC_SUCCESS) {
      ;
    } else {
      printf("idx %ld: type %ld, duk_require_heapptr() -> %s\n", (long)i,
             (long)duk_get_type(ctx, i), duk_safe_to_string(ctx, -1));
    }
    duk_pop(ctx);
  }
