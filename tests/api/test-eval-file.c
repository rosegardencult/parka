...
static duk_ret_t test_raw(duk_context* ctx, void* udata) {
  ...
  const char* data2 =
      "print('Hello world from a file, with exception'); throw new Error('eval "
      "error');";
  const char* data3 =
      "print('Hello world from a file, with syntax error'); obj = {";
  duk_ret_t rc;
...
void test(duk_context* ctx) { TEST_SAFE_CALL(test_raw); }
