...
test__c_function ok
top: 1
TypeError: undefined required, found function LFUNC() { [lightfunc code] }
(stack index -3) TypeError: null required, found function LFUNC() { [lightfunc
code] } (stack index -3) TypeError: boolean required, found function LFUNC() {
[lightfunc code] } (stack index -3) TypeError: number required, found function
LFUNC() { [lightfunc code] } (stack index -3) TypeError: string required, found
function LFUNC() { [lightfunc code] } (stack index -3) TypeError: buffer
required, found function LFUNC() { [lightfunc code] } (stack index -3)
TypeError: pointer required, found function LFUNC() { [lightfunc code] } (stack
index -3) TypeError: nativefunction required, found function LFUNC() {
[lightfunc code] } (stack index -3) top: 1 TypeError: undefined required, found
[object Function] (stack index -3) TypeError: null required, found [object
Function] (stack index -3) TypeError: boolean required, found [object Function]
(stack index -3) TypeError: number required, found [object Function] (stack
index -3) TypeError: string required, found [object Function] (stack index -3)
TypeError: buffer required, found [object Function] (stack index -3)
TypeError: pointer required, found [object Function] (stack index -3)
...
#define TEST_REQUIRE(fn)                                                     \
  do {                                                                       \
    ...
    if (rc != 0) {                                                           \
      duk_eval_string(                                                       \
          ctx,                                                               \
          "(function (v) { print(String(v).replace(/\\(0x.*?\\)/g, '(PTR)')" \
...
static void test__require_calls(duk_context* ctx) {
  ...
  /* No duk_require_object(), duk_require_array(), duk_require_c_lightfunc(),
   * etc. */
}
...
static duk_ret_t test_2a(duk_context* ctx, void* udata) {
  ...
  duk_safe_call(ctx, test__null, NULL, 0, 1);
  printf("%s\n", duk_safe_to_string(ctx, 0));
  duk_pop(ctx);

  duk_set_top(ctx, 0);
  duk_push_undefined(ctx);
  test__require_calls(ctx);
  duk_set_top(ctx, 0);
  duk_push_null(ctx);
  test__require_calls(ctx);
  duk_set_top(ctx, 0);
  ...
  test__require_calls(ctx);
  duk_set_top(ctx, 0);
  duk_push_c_lightfunc(ctx, dummy_func, 0, 0, 0);
  test__require_calls(ctx);
  duk_set_top(ctx, 0);
  duk_eval_string(ctx, "(function dummy(){})");
  test__require_calls(ctx);
  duk_set_top(ctx, 0);
  duk_push_thread(ctx);
  test__require_calls(ctx);
