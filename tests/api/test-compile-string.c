...
static duk_ret_t test_string(duk_context* ctx, void* udata) {
  ...
  duk_compile_string_filename(ctx, 0,
                              "print(Duktape.act(-2).function.fileName); 234");
  duk_call(ctx, 0);
  printf("return value is: '%s'\n", duk_to_string(ctx, -1));
  duk_pop(ctx);
  ...
  duk_push_string(ctx, "myFile.js");
  rc = duk_pcompile_string_filename(
      ctx, 0, "print(Duktape.act(-2).function.fileName); 234");
  printf("compile rc=%ld\n", (long)rc);
  duk_call(ctx, 0);
  printf("return value is: '%s'\n", duk_to_string(ctx, -1));
  duk_pop(ctx);
