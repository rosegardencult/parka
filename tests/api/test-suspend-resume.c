...
static duk_ret_t test_suspend_resume_throw_trivial(duk_context* ctx,
                                                   void* udata) {
  duk_thread_state st;
...
static duk_ret_t test_suspend_resume_throw_basic(duk_context* ctx,
                                                 void* udata) {
  duk_context* other;
  duk_thread_state st;
...
static duk_ret_t test_suspend_resume_reterr_trivial(duk_context* ctx,
                                                    void* udata) {
  duk_thread_state st;
...
static duk_ret_t test_suspend_resume_reterr_basic(duk_context* ctx,
                                                  void* udata) {
  duk_context* other;
  duk_thread_state st;
  ...
  duk_suspend(ctx, &st);
  duk_eval_string(
      other, "try { throw 'other thread executing'; } catch (e) { print(e); }");
  duk_resume(ctx, &st);
  return DUK_RET_RANGE_ERROR;
}
