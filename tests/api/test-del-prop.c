...
delete arr[2] -> rc=1
final object:
{"bar":"barval","nul\u0000key":"nulval","undefined":"undefinedval"} final array:
["foo","bar",null] final top: 3
...
delete arr[2] -> rc=1
final object:
{"foo":"fooval","bar":"barval","nul\u0000key":"nulval","undefined":"undefinedval"}
...
static void prep(duk_context* ctx) {
  ...
  duk_push_string(
      ...
      "\"nul\\u0000key\": \"nulval\", \"undefined\": \"undefinedval\"}");
...
static duk_ret_t test_delpropheapptr_d_safecall(duk_context* ctx, void* udata) {
  ...
  duk_eval_string(ctx,
                  "({ toString: function () { print('toString() called'); "
                  "return 'foo'; } })");
  ptr = duk_require_heapptr(ctx, -1);
  rc = duk_del_prop_heapptr(ctx, 0, ptr);
  printf("delete obj.foo -> rc=%d\n", (int)rc);
