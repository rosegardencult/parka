...
static duk_ret_t test_to_buffer_1(duk_context* ctx, void* udata) {
  ...
  duk_eval_string(ctx,
                  ...
                  "    b.set([ 0x60, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, "
                  "0x68, 0x69 ]);\n"
                  "    return b;\n"
                  "})()\n");
  ...
  for (i = 0; i <= 3; i++) {
    ...
    duk_eval_string(ctx, "(function (v) { return Uint8Array.plainOf(v); })");
    duk_dup(ctx, i);
    duk_call(ctx, 1);
...
void test(duk_context* ctx) { TEST_SAFE_CALL(test_to_buffer_1); }
