...
function diffText(text1, text2, callback) {
  ...
  child = child_process.exec(cmd.join(" "), function diffDone(
    error,
    stdout,
    stderr
  ) {
...
function executeTest(options, callback) {
  ...
  function compileApiTest() {
    ...
    try {
      fs.writeFileSync(
        tempSource,
        options.engine.cPrefix + fs.readFileSync(options.testPath)
      );
    } catch (e) {
      console.log(e);
      callback(e);
  ...
  function prepareEcmaTest() {
    ...
    try {
      ...
      fs.writeFileSync(
        tempPrologue,
        options.engine.jsPrefix || "/* no prefix */"
      );
    } catch (e) {
      console.log(e);
      callback(e);
...
var DUK_HEADER = "this.__engine__ = 'duk'; ";
...
var RHINO_HEADER = "this.__engine__ = 'rhino'; ";

var SMJS_HEADER = "this.__engine__ = 'smjs'; ";
...
function findTestCasesSync(argList) {
  ...
  argList.forEach(function checkArg(arg) {
    ...
    if (st.isFile()) {
      ...
      if (m[1].substring(0, 5) === "util-") {
        return;
      } // skip utils
      found[m[1]] = true;
      testcases.push(arg);
    } else if (st.isDirectory()) {
      fs.readdirSync(arg).forEach(function check(fn) {
        var m = pat.exec(fn);
        if (!m) {
          return;
        }
...
function testRunnerMain() {
  var argv = require("optimist")
    .usage(
      "Execute one or multiple test cases; dirname to execute all tests in a directory."
    )
    ...
    .describe(
      "test-sleep",
      "sleep time (milliseconds) between testcases, avoid overheating :)"
    )
    ...
    .describe(
      "valgrind",
      "run duktape testcase with valgrind (no effect on other engines)"
    )
    .describe("prep-test-path", "path for test_prep.py")
    .describe(
      "util-include-path",
      "path for util-*.js files (tests/ecmascript usually)"
    )
    ...
    .describe(
      "emduk-trailing-line-hack",
      "strip bogus newline from end of emduk stdout"
    )
    .demand("prep-test-path")
    .demand("util-include-path")
    .demand(1).argv; // at least 1 non-arg
  var testcases;
  var engines;
  var queue1, queue2;
  ...
  function analyzeResults() {
    ...
    iterateResults(function analyze(tn, en, res) {
      ...
      if (res.testcase.meta.skip) {
        ...
        if (
          !res.testcase.meta.knownissue &&
          !res.testcase.meta.specialoptions
        ) {
          summary.exitCode = 1;
        }
        res.status = "fail";
  ...
  function printSummary() {
      ...
      if (res.status === "skip") {
        ...
        if (res.testcase.meta.specialoptions) {
          if (typeof res.testcase.meta.specialoptions === "string") {
            parts.push(
              "requires special feature options: " +
                res.testcase.meta.specialoptions
            );
          } else {
        ...
        engines.forEach(function checkDiffToOther(other) {
          ...
          parts.push(
            other.name +
              " diff " +
              res.diff_other[other.name].split("\n").length +
              " lines"
          );
    ...
    console.log("");
    console.log("SUMMARY: " + countPass + " pass, " + countFail + " fail"); // countSkip is no longer correct
  }
  ...
  engine_api = { name: "api", cPrefix: API_TEST_HEADER };
  ...
  var timer = setInterval(function() {
    ...
    var rate =
      (execStartQueue - queue1.length()) / ((now - execStartTime) / 1000);
    var eta = Math.ceil(queue1.length() / rate);
    console.log(
      "Still running, testcase task queue length: " +
        queue1.length() +
        ", eta " +
        eta +
        " second(s)"
