...
function errCreateTest() {
  ...
  Duktape.errCreate = function(err) {
    if (err === null) {
      print("errCreate:", null);
    } else if (typeof err === "object") {
      print("errCreate:", typeof err, err instanceof Error, err.message);
    } else {
      print("errCreate:", typeof err);
    }
    return err;
  };
...
function errThrowTest() {
  ...
  Duktape.errThrow = function(err) {
    if (err === null) {
      print("errThrow:", null);
    } else if (isPlainBuffer(err)) {
      print("errThrow:", typeof err);
    } else if (typeof err === "object") {
      print("errThrow:", typeof err, err instanceof Error, err.message);
    } else {
      print("errThrow:", typeof err);
    }
    return err;
...
function errCreateAndErrThrowTest() {
  ...
  Duktape.errThrow = function(err) {
    ...
    errPrint(e2);
    print(
      "e1 tracedata existence matches:",
      "tracedata" in err === "tracedata" in e1
    );
    print(
      "e2 tracedata existence matches:",
      "tracedata" in err === "tracedata" in e2
    );
