...
function createPlainBuffer(arg) {
  ...
  if (!isPlainBuffer(res)) {
    throw new Error(
      "internal error, createPlainBuffer() failed to create a plain buffer"
    );
  }
  return res;
}
...
function createArrayBuffer(arg) {
  ...
  if (!(res instanceof ArrayBuffer)) {
    throw new Error(
      "createArrayBuffer() failed to create an ArrayBuffer instance"
    );
  }
  return res;
}
...
function getPrototypeChain(x) {
  ...
  var protos = [
    ...
    {
      ref: Object.getPrototypeOf(Uint8Array.prototype),
    ...
    if (dumpValue) {
      print(
        ...
        typeof x[k] === "object" || typeof x[k] === "function"
          ? Object.prototype.toString.call(x[k])
          : x[k]
      );
    } else {
      print(k);
