...
TOTP.sha1Raw = function(words) {
  ...
  [h0, h1, h2, h3, h4].forEach(function(v) {
    for (var i = 24; i >= 0; i -= 8) {
      res += String.fromCharCode((v >> i) & 0xff);
    }
  });
  return res;
};
...
TOTP.hmacSha1 = function(key, text) {
  ...
  var i, n, t1, t2;
  if (key.length > B) {
    throw new Error("key too long (key hashing not supported)");
  }
