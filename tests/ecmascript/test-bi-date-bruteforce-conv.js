...
var days_in_month_noleap = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
...
function bruteForceConversionTest() {
  ...
  function msg(x) {
    return (
      "brute force conversion test failed: " +
      x +
      " (year=" +
      year +
      ", month=" +
  ...
  function testYearAndMonth() {
    ...
    if (dt.getTime() !== timeval) {
      throw new Error(
        msg("getTime !== timeVal: " + dt.getTime() + " vs " + timeval)
      );
    }
  ...
  for (;;) {
    ...
    timeval +=
      (isLeap ? days_in_month_leap : days_in_month_noleap)[month] * ms_day;
