...
/* Generate pc2line data for an instruction sequence, leaving a buffer on stack
 * top. */
DUK_INTERNAL void duk_hobject_pc2line_pack(duk_hthread* thr,
  ...
  curr_offset = (duk_uint_fast32_t)(
      sizeof(duk_uint32_t) + num_header_entries * sizeof(duk_uint32_t) * 2);
...
#  endif
      ...
      if (diff_line == 0) {
        ...
        duk_be_encode(be_ctx, (duk_uint32_t)((0x06 << 8) + (diff_line + 0x80)),
                      11);
      } else {
        /* 1 1 1 <32 bits>
         ...
         */
        duk_be_encode(
            be_ctx, (duk_uint32_t)((0x07 << 16) + ((next_line >> 16) & 0xffff)),
            19);
 ...
 * returned.
 */
DUK_LOCAL duk_uint_fast32_t duk__hobject_pc2line_query_raw(
    duk_hthread* thr, duk_hbuffer_fixed* buf, duk_uint_fast32_t pc) {
  ...
  if (pc >= pc_limit) {
    ...
    DUK_DD(DUK_DDPRINT(
        "pc2line lookup failed: pc out of bounds (pc=%ld, limit=%ld)", (long)pc,
        (long)pc_limit));
    goto pc2line_error;
  }
  ...
  if ((duk_size_t)start_offset > DUK_HBUFFER_FIXED_GET_SIZE(buf)) {
    DUK_DD(DUK_DDPRINT(
        "pc2line lookup failed: start_offset out of bounds (start_offset=%ld, "
        "buffer_size=%ld)",
...
#  endif
  ...
  DUK_DDD(DUK_DDDPRINT("pc2line lookup result: pc %ld -> line %ld", (long)pc,
                       (long)curr_line));
  return curr_line;
