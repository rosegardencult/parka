...
duk_hstring_char_code_at_raw(duk_hthread* thr, duk_hstring* h, duk_uint_t pos,
  ...
  boff = (duk_uint32_t)duk_heap_strcache_offset_char2byte(thr, h,
                                                          (duk_uint32_t)pos);
  DUK_DDD(DUK_DDDPRINT("charCodeAt: pos=%ld -> boff=%ld, str=%!O", (long)pos,
                       (long)boff, (duk_heaphdr*)h));
  ...
  p = p_start + boff;
  DUK_DDD(DUK_DDDPRINT("p_start=%p, p_end=%p, p=%p", (const void*)p_start,
  ...
  if (duk_unicode_decode_xutf8(thr, &p, p_start, p_end, &cp1)) {
    if (surrogate_aware && cp1 >= 0xd800UL && cp1 <= 0xdbffUL) {
       ...
       */
      cp2 = 0; /* If call fails, this is left untouched and won't match cp2
                  check. */
      (void)duk_unicode_decode_xutf8(thr, &p, p_start, p_end, &cp2);
      if (cp2 >= 0xdc00UL && cp2 <= 0xdfffUL) {
        cp1 = (duk_ucodepoint_t)(((cp1 - 0xd800UL) << 10) + (cp2 - 0xdc00UL) +
                                 0x10000UL);
      }
    }
  } else {
...
DUK_INTERNAL void duk_hstring_init_charlen(duk_hstring* h) {
  ...
  clen = duk_unicode_unvalidated_utf8_length(DUK_HSTRING_GET_DATA(h),
...
#if defined(DUK_USE_HSTRING_LAZY_CLEN)
#  if defined(DUK_USE_HSTRING_CLEN)
DUK_LOCAL DUK_COLD duk_size_t
duk__hstring_get_charlen_slowpath(duk_hstring* h) {
  duk_size_t res;
...
#    endif
  ...
  res = duk_unicode_unvalidated_utf8_length(DUK_HSTRING_GET_DATA(h),
                                            DUK_HSTRING_GET_BYTELEN(h));
#    if defined(DUK_USE_STRLEN16)
...
DUK_INTERNAL duk_bool_t duk_hstring_equals_ascii_cstring(duk_hstring* h,
  ...
  if (duk_memcmp((const void*)cstr, (const void*)DUK_HSTRING_GET_DATA(h),
                 len) == 0) {
    return 1;
  }
  return 0;
