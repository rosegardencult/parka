...
#      define DUK__DEBUG_LOG(lev, ...)                  \
        duk_debug_log((duk_int_t)(lev), DUK_FILE_MACRO, \
...
#      define DUK__DEBUG_STASH(lev)                                           \
        (void)DUK_SNPRINTF(duk_debug_file_stash, DUK_DEBUG_STASH_SIZE, "%s",  \
            ...
            (void)DUK_SNPRINTF(duk_debug_func_stash, DUK_DEBUG_STASH_SIZE,    \
                               "%s", (const char*)DUK_FUNC_MACRO),            \
...
#      else
#        define DUK_DPRINT \
          0 && /* args go here as a comma expression in parens */
#      endif
...
#    if defined(DUK_USE_VARIADIC_MACROS)
DUK_INTERNAL_DECL void duk_debug_log(duk_int_t level, const char* file,
