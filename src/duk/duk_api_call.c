 ...
 * present).
 */
DUK_LOCAL duk_idx_t duk__call_get_idx_func(duk_hthread* thr, duk_idx_t nargs,
                                           duk_idx_t other) {
  duk_idx_t idx_func;
  ...
  if (DUK_UNLIKELY((idx_func | nargs) <
                   0)) { /* idx_func < 0 || nargs < 0; OR sign bits */
...
DUK_LOCAL void duk__call_prop_prep_stack(duk_hthread* thr,
  ...
  DUK_DDD(DUK_DDDPRINT(
      "duk__call_prop_prep_stack, normalized_obj_idx=%ld, nargs=%ld, "
      "stacktop=%ld",
  ...
  duk_dup(thr,
          -nargs -
              1); /* Note: -nargs alone would fail for nargs == 0, this is OK */
...
DUK_EXTERNAL void duk_call_prop(duk_hthread* thr, duk_idx_t obj_idx,
                                duk_idx_t nargs) {
  /*
...
DUK_EXTERNAL duk_int_t duk_pcall(duk_hthread* thr, duk_idx_t nargs) {
  ...
  return duk_safe_call(thr, duk__pcall_raw, (void*)&args /*udata*/,
                       nargs + 1 /*nargs*/, 1 /*nrets*/);
}
...
DUK_INTERNAL duk_int_t duk_pcall_method_flags(duk_hthread* thr, duk_idx_t nargs,
                                              duk_small_uint_t call_flags) {
  duk__pcall_method_args args;
  ...
  return duk_safe_call(thr, duk__pcall_method_raw, (void*)&args /*udata*/,
                       nargs + 2 /*nargs*/, 1 /*nrets*/);
}
...
DUK_EXTERNAL duk_int_t duk_pcall_prop(duk_hthread* thr, duk_idx_t obj_idx,
                                      duk_idx_t nargs) {
  duk__pcall_prop_args args;
  ...
  return duk_safe_call(thr, duk__pcall_prop_raw, (void*)&args /*udata*/,
                       nargs + 1 /*nargs*/, 1 /*nrets*/);
}
...
DUK_EXTERNAL duk_int_t duk_safe_call(duk_hthread* thr,
  ...
  if (DUK_UNLIKELY(
          (nargs | nrets) < 0 || /* nargs < 0 || nrets < 0; OR sign bits */
          thr->valstack_top < thr->valstack_bottom +
                                  nargs || /* nargs too large compared to top */
          thr->valstack_end + nargs <
              thr->valstack_top +
                  nrets)) { /* nrets too large compared to reserve */
    DUK_D(DUK_DPRINT(
        "not enough stack reserve for safe call or invalid arguments: "
        "nargs=%ld < 0 (?), nrets=%ld < 0 (?), top=%ld < bottom=%ld + "
        "nargs=%ld (?), "
        "end=%ld + nargs=%ld < top=%ld + nrets=%ld (?)",
        (long)nargs, (long)nrets, (long)(thr->valstack_top - thr->valstack),
        (long)(thr->valstack_bottom - thr->valstack), (long)nargs,
        (long)(thr->valstack_end - thr->valstack), (long)nargs,
        (long)(thr->valstack_top - thr->valstack), (long)nrets));
    DUK_ERROR_TYPE_INVALID_ARGS(thr);
    DUK_WO_NORETURN(return DUK_EXEC_ERROR;);
  }
...
DUK_EXTERNAL void duk_new(duk_hthread* thr, duk_idx_t nargs) {
  ...
  duk_push_object(
      thr); /* default instance; internal proto updated by call handling */
...
DUK_EXTERNAL duk_int_t duk_pnew(duk_hthread* thr, duk_idx_t nargs) {
  ...
  rc = duk_safe_call(thr, duk__pnew_helper, (void*)&nargs /*udata*/,
                     nargs + 1 /*nargs*/, 1 /*nrets*/);
  return rc;
}
...
DUK_EXTERNAL void duk_set_magic(duk_hthread* thr, duk_idx_t idx,
                                duk_int_t magic) {
  duk_hnatfunc* nf;
