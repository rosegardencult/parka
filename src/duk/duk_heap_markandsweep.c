...
DUK_LOCAL_DECL void duk__mark_tvals(duk_heap* heap, duk_tval* tv,
                                    duk_idx_t count);
...
DUK_LOCAL void duk__mark_hobject(duk_heap* heap, duk_hobject* h) {
  ...
  for (i = 0; i < (duk_uint_fast32_t)DUK_HOBJECT_GET_ENEXT(h); i++) {
    ...
    if (DUK_HOBJECT_E_SLOT_IS_ACCESSOR(heap, h, i)) {
      duk__mark_heaphdr(
          heap, (duk_heaphdr*)DUK_HOBJECT_E_GET_VALUE_PTR(heap, h, i)->a.get);
      duk__mark_heaphdr(
          heap, (duk_heaphdr*)DUK_HOBJECT_E_GET_VALUE_PTR(heap, h, i)->a.set);
    } else {
...
DUK_LOCAL void duk__mark_heaphdr(duk_heap* heap, duk_heaphdr* h) {
  DUK_DDD(DUK_DDDPRINT("duk__mark_heaphdr %p, type %ld", (void*)h,
  ...
  if (!DUK_HEAPHDR_HAS_READONLY(h)) {
    h->h_assert_refcount++; /* Comparison refcount: bump even if already
                               reachable. */
  }
#endif
  ...
  switch (DUK_HEAPHDR_GET_TYPE(h)) {
    ...
    case DUK_HTYPE_BUFFER:
      ...
      break;
    default:
      DUK_D(DUK_DPRINT("attempt to mark heaphdr %p with invalid htype %ld",
                       (void*)h, (long)DUK_HEAPHDR_GET_TYPE(h)));
      DUK_UNREACHABLE();
  }
...
DUK_LOCAL void duk__mark_finalizable(duk_heap* heap) {
  ...
  while (hdr != NULL) {
    ...
    if (!DUK_HEAPHDR_HAS_REACHABLE(hdr) && DUK_HEAPHDR_IS_OBJECT(hdr) &&
      ...
      DUK_DD(
          DUK_DDPRINT("unreachable heap object will be "
...
#  if defined(DUK_USE_DEBUG)
  if (count_finalize_list > 0) {
    DUK_D(
        DUK_DPRINT("marked %ld objects on the finalize_list as reachable "
                   "(previous finalizer run skipped)",
...
DUK_LOCAL void duk__handle_temproot(duk_heap* heap, duk_heaphdr* hdr) {
  ...
  DUK_HEAPHDR_CLEAR_REACHABLE(
      hdr); /* Done so that duk__mark_heaphdr() works correctly. */
#if defined(DUK_USE_ASSERTIONS) && defined(DUK_USE_REFERENCE_COUNTING)
...
#if defined(DUK_USE_DEBUG)
    DUK_DD(DUK_DDPRINT("temproot mark heap scan processed %ld temp roots",
                       (long)count));
#endif
  }
}
  ...
  for (i = 0; i < heap->st_size; i++) {
    ...
    while (h != NULL) {
      ...
      if (DUK_HEAPHDR_HAS_REACHABLE((duk_heaphdr*)h)) {
        ...
        duk_heap_strtable_unlink_prev(heap, (duk_hstring*)h,
                                      (duk_hstring*)prev);
...
DUK_LOCAL void duk__sweep_heap(duk_heap* heap, duk_small_uint_t flags,
                               duk_size_t* out_count_keep) {
...
#  if defined(DUK_USE_REFERENCE_COUNTING)
        DUK_HEAPHDR_PREINC_REFCOUNT(
            curr); /* Bump refcount so that refzero never occurs when pending a
                      finalizer call. */
#  endif
...
#  endif
...
#endif /* DUK_USE_FINALIZER_SUPPORT */
      {
        if (DUK_UNLIKELY(DUK_HEAPHDR_HAS_FINALIZED(curr))) {
          ...
          if (flags & DUK_MS_FLAG_POSTPONE_RESCUE) {
            ...
            DUK_DD(
                DUK_DDPRINT("sweep; reachable, finalized --> rescued after "
                            "finalization: %p",
                            (void*)curr));
#if defined(DUK_USE_FINALIZER_SUPPORT)
      ...
      if (DUK_HEAPHDR_IS_OBJECT(curr) &&
          DUK_HOBJECT_IS_THREAD((duk_hobject*)curr)) {
        ...
        duk_valstack_shrink_check_nothrow(
            thr_curr, flags & DUK_MS_FLAG_EMERGENCY /*snug*/);
      }
      ...
      if (DUK_HEAPHDR_HAS_FINALIZED(curr)) {
        DUK_DD(
            DUK_DDPRINT("sweep; unreachable, finalized --> finalized object "
                        "not rescued: %p",
                        (void*)curr));
      } else {
...
#if defined(DUK_USE_DEBUG)
  DUK_D(
      DUK_DPRINT("mark-and-sweep sweep objects (non-string): %ld freed, %ld "
                 "kept, %ld rescued, %ld queued for finalization",
                 (long)count_free, (long)count_keep, (long)count_rescue,
                 (long)count_finalize));
#endif
  *out_count_keep = count_keep;
}
...
DUK_LOCAL void duk__compact_objects(duk_heap* heap) {
  ...
  duk__compact_object_list(heap, heap->heap_thread, heap->heap_allocated,
                           &count_check, &count_compact, &count_bytes_saved);
#  if defined(DUK_USE_FINALIZER_SUPPORT)
...
#endif
#if defined(DUK_USE_REFERENCE_COUNTING)
  DUK_ASSERT(heap->refzero_list ==
             NULL); /* Always handled to completion inline in DECREF. */
#endif

#if defined(DUK_USE_DEBUG)
  DUK_D(DUK_DPRINT(
      "mark-and-sweep compact objects: %ld checked, %ld compaction attempts, "
      "%ld bytes saved by compaction",
...
DUK_LOCAL void duk__assert_walk_list(duk_heap* heap, duk_heaphdr* start,
                                     duk__gc_heaphdr_assert func) {
  duk_heaphdr* curr;
...
DUK_LOCAL void duk__assert_walk_strtable(duk_heap* heap,
                                         duk__gc_hstring_assert func) {
  duk_uint32_t i;
...
DUK_LOCAL void duk__assert_heaphdr_flags(duk_heap* heap) {
  duk__assert_walk_list(heap, heap->heap_allocated,
                        duk__assert_heaphdr_flags_cb);
#  if defined(DUK_USE_REFERENCE_COUNTING)
  DUK_ASSERT(heap->refzero_list ==
             NULL); /* Always handled to completion inline in DECREF. */
#  endif
...
DUK_LOCAL void duk__assert_valid_refcounts(duk_heap* heap) {
  duk__assert_walk_list(heap, heap->heap_allocated,
                        duk__assert_valid_refcounts_cb);
}
...
DUK_LOCAL void duk__clear_assert_refcounts(duk_heap* heap) {
  duk__assert_walk_list(heap, heap->heap_allocated,
                        duk__clear_assert_refcounts_cb1);
#    if defined(DUK_USE_FINALIZER_SUPPORT)
  duk__assert_walk_list(heap, heap->finalize_list,
                        duk__clear_assert_refcounts_cb1);
#    endif
#    if defined(DUK_USE_REFERENCE_COUNTING)
  duk__assert_walk_list(heap, heap->refzero_list,
                        duk__clear_assert_refcounts_cb1);
#    endif
...
DUK_LOCAL void duk__check_refcount_heaphdr(duk_heaphdr* hdr) {
  ...
  if (DUK_HEAPHDR_IS_STRING(hdr) &&
      DUK_HSTRING_HAS_PINNED_LITERAL((duk_hstring*)hdr)) {
    expect_refc++;
  }
  count_ok = ((duk_size_t)DUK_HEAPHDR_GET_REFCOUNT(hdr) == expect_refc);
  if (!count_ok) {
    DUK_D(
        DUK_DPRINT("refcount mismatch for: %p: header=%ld counted=%ld --> %!iO",
...
DUK_LOCAL void duk__check_assert_refcounts(duk_heap* heap) {
  duk__assert_walk_list(heap, heap->heap_allocated,
                        duk__check_assert_refcounts_cb1);
#    if defined(DUK_USE_FINALIZER_SUPPORT)
  duk__assert_walk_list(heap, heap->finalize_list,
                        duk__check_assert_refcounts_cb1);
#    endif
...
DUK_LOCAL void duk__dump_stats(duk_heap* heap) {
  ...
  DUK_D(DUK_DPRINT(
      "stats mark-and-sweep: try_count=%ld, skip_count=%ld, "
      "emergency_count=%ld",
  ...
  DUK_D(DUK_DPRINT(
      "stats envrec: delayedcreate=%ld, create=%ld, newenv=%ld, oldenv=%ld, "
      "pushclosure=%ld",
      (long)heap->stats_envrec_delayedcreate, (long)heap->stats_envrec_create,
      (long)heap->stats_envrec_newenv, (long)heap->stats_envrec_oldenv,
...
DUK_INTERNAL void duk_heap_mark_and_sweep(duk_heap* heap,
  ...
  DUK_ASSERT(heap->ms_running ==
             0); /* ms_prevent_count is bumped when ms_running is set */
  ...
  DUK_D(DUK_DPRINT(
      "garbage collect (mark-and-sweep) starting, requested flags: 0x%08lx, "
      "effective flags: 0x%08lx",
...
#  endif /* DUK_USE_REFERENCE_COUNTING */
  ...
  duk__mark_finalize_list(
      heap); /* Mark finalizer work list as reachability roots. */
#endif
  duk__mark_temproots_by_heap_scan(heap); /* Temproots. */
...
#  endif /* DUK_USE_LITCACHE_SIZE */
  ...
  heap->ms_trigger_counter =
      (duk_int_t)((tmp * DUK_HEAP_MARK_AND_SWEEP_TRIGGER_MULT) +
                  DUK_HEAP_MARK_AND_SWEEP_TRIGGER_ADD);
  DUK_D(
      DUK_DPRINT("garbage collect (mark-and-sweep) finished: %ld objects kept, "
                 "%ld strings kept, trigger reset to %ld",
                 (long)count_keep_obj, (long)count_keep_str,
                 (long)heap->ms_trigger_counter));
#else
  DUK_D(
      DUK_DPRINT("garbage collect (mark-and-sweep) finished: %ld objects kept, "
                 "%ld strings kept, no voluntary trigger",
                 (long)count_keep_obj, (long)count_keep_str));
#endif
