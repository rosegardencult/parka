...
DUK_LOCAL DUK_NOINLINE void duk__call_c_recursion_limit_check_slowpath(
    duk_hthread* thr) {
  ...
  if (thr->heap->augmenting_error) {
    if (thr->heap->call_recursion_depth <
      ...
      DUK_D(
          DUK_DPRINT("C recursion limit reached but augmenting error and "
                     "within relaxed limit"));
      return;
    }
  }
#endif
...
DUK_LOCAL DUK_ALWAYS_INLINE void duk__call_c_recursion_limit_check(
    duk_hthread* thr) {
...
DUK_LOCAL DUK_NOINLINE void duk__call_callstack_limit_check_slowpath(
    duk_hthread* thr) {
  /* When augmenting an error, the effective limit is a bit higher.
   * Check for it only if the fast path check fails.
   */
  ...
  if (thr->heap->augmenting_error) {
    if (thr->callstack_top <
      ...
      DUK_D(
          DUK_DPRINT("call stack limit reached but augmenting error and within "
                     "relaxed limit"));
      return;
    }
  }
...
DUK_LOCAL DUK_ALWAYS_INLINE void duk__call_callstack_limit_check(
    duk_hthread* thr) {
  /* This check is forcibly inlined because it's very cheap and almost
   * always passes.  The slow path is forcibly noinline.
   */
...
DUK_LOCAL void duk__create_arguments_object(duk_hthread* thr, duk_hobject* func,
    ...
    if (!DUK_HOBJECT_HAS_STRICT(func) && idx < n_formals) {
      ...
      if (!duk_has_prop(thr, i_mappednames)) {
        ...
        DUK_DDD(DUK_DDDPRINT("set mappednames[%s]=%ld",
                             (const char*)duk_get_string(thr, -1), (long)idx));
  ...
  if (need_map) {
    ...
    duk_dup(thr, i_map);
    duk_xdef_prop_stridx(thr, i_arg, DUK_STRIDX_INT_MAP,
    ...
    duk_xdef_prop_stridx(thr, i_arg, DUK_STRIDX_INT_VARENV,
                         DUK_PROPDESC_FLAGS_NONE); /* out of spec, don't care */
  }
...
DUK_INTERNAL void duk_call_construct_postprocess(
    duk_hthread* thr, duk_small_uint_t proxy_invariant) {
  ...
  duk_err_augment_error_create(
      thr, thr, NULL, 0,
      DUK_AUGMENT_FLAG_NOBLAME_FILELINE | DUK_AUGMENT_FLAG_SKIP_ONE);
#endif
}
...
DUK_LOCAL void duk__handle_bound_chain_for_call(
    duk_hthread* thr, duk_idx_t idx_func, duk_bool_t is_constructor_call) {
  duk_tval* tv_func;
  duk_hobject* func;
  duk_idx_t len;
  ...
  if (DUK_TVAL_IS_OBJECT(tv_func)) {
    ...
    if (DUK_HOBJECT_HAS_BOUNDFUNC(func)) {
      ...
      DUK_DDD(
          DUK_DDDPRINT("bound function handled, idx_func=%ld, curr func=%!T",
  ...
  if (DUK_TVAL_IS_OBJECT(tv_func)) {
    ...
    DUK_ASSERT(DUK_HOBJECT_HAS_COMPFUNC(func) ||
               DUK_HOBJECT_HAS_NATFUNC(func) || DUK_HOBJECT_IS_PROXY(func));
  }
#endif
}
...
DUK_LOCAL duk_bool_t duk__handle_specialfuncs_for_call(
  ...
  switch (((duk_hnatfunc*)func)->magic) {
    case 0: { /* 0=Function.prototype.call() */
       ...
       * idx_func + 3: 2nd argument to .call, desired 1st argument for ultimate
       * target
       * ...
       *
      ...
      if (thr->valstack_top < tv_args) {
        ...
        thr->valstack_top =
            tv_args; /* at least target function and 'this' binding present */
      }
      break;
    }
    ...
    case 3: { /* 3=Reflect.construct() */
      ...
      *call_flags |= DUK_CALL_FLAG_CONSTRUCT;
      ...
      if (top > idx_func + 3) {
        ...
        duk_set_top_unsafe(thr,
                           idx_func + 3); /* remove any args beyond argArray */
      }
      DUK_ASSERT(duk_get_top(thr) == idx_func + 3);
      DUK_ASSERT(duk_is_valid_index(thr, idx_func + 2));
      (void)duk_unpack_array_like(
          thr, idx_func + 2); /* XXX: should also remove target to be symmetric
                                 with duk_pack()? */
...
DUK_LOCAL void duk__handle_proxy_for_call(duk_hthread* thr, duk_idx_t idx_func,
  ...
  if (*call_flags & DUK_CALL_FLAG_CONSTRUCT) {
    *call_flags |= DUK_CALL_FLAG_CONSTRUCT_PROXY; /* Enable 'construct' trap
                                                     return invariant check. */
    *call_flags &= ~(DUK_CALL_FLAG_CONSTRUCT); /* Resume as non-constructor call
                                                  to the trap. */
...
DUK_LOCAL void duk__update_func_caller_prop(duk_hthread* thr,
                                            duk_hobject* func) {
  duk_tval* tv_caller;
  duk_hobject* h_tmp;
  ...
  tv_caller = duk_hobject_find_entry_tval_ptr_stridx(thr->heap, func,
                                                     DUK_STRIDX_CALLER);
  if (tv_caller) {
...
DUK_LOCAL DUK_INLINE void duk__coerce_nonstrict_this_binding(
    duk_hthread* thr, duk_idx_t idx_this) {
  duk_tval* tv_this;
  ...
  switch (DUK_TVAL_GET_TAG(tv_this)) {
    ...
    case DUK_TAG_NULL:
      ...
      if (DUK_LIKELY(obj_global != NULL)) {
         ...
         */
        DUK_D(
            DUK_DPRINT("this binding: wanted to use global object, but it is "
                       "NULL -> using undefined instead"));
        DUK_ASSERT(!DUK_TVAL_IS_HEAP_ALLOCATED(
            tv_this));                   /* no need to decref previous value */
...
DUK_LOCAL duk_hobject* duk__resolve_target_func_and_this_binding(
    duk_hthread* thr, duk_idx_t idx_func, duk_small_uint_t* call_flags) {
  duk_tval* tv_func;
  duk_hobject* func;
  duk_bool_t first;
    ...
    if (DUK_TVAL_IS_OBJECT(tv_func)) {
      ...
      if (DUK_HOBJECT_HAS_BOUNDFUNC(func)) {
        ...
        if (DUK_HOBJECT_HAS_EXOTIC_PROXYOBJ(func)) {
           ...
           */
          duk__handle_proxy_for_call(thr, idx_func, (duk_hproxy*)func,
                                     call_flags);
        } else
#endif
        {
          ...
          if (duk__handle_specialfuncs_for_call(thr, idx_func, func, call_flags,
                                                first) != 0) {
...
not_callable:
   ...
   */
  if (DUK_TVAL_IS_OBJECT(tv_func)) {
    duk_tval* tv_wrap = duk_hobject_find_entry_tval_ptr_stridx(
        thr->heap, DUK_TVAL_GET_OBJECT(tv_func), DUK_STRIDX_INT_TARGET);
    if (tv_wrap != NULL) {
...
DUK_LOCAL void duk__safe_call_adjust_valstack(duk_hthread* thr,
  ...
  DUK_DDD(DUK_DDDPRINT(
      "adjust valstack after func call: "
      "num_stack_rets=%ld, num_actual_rets=%ld, stack_top=%ld, "
...
#  endif
  ...
  *out_act = act;
  ...
  if (func == NULL || !DUK_HOBJECT_IS_COMPFUNC(func)) {
    DUK_DDD(
        DUK_DDDPRINT("tail call prevented by target not being ecma function"));
    return 0;
  }
...
#  endif
      ;
  flags2 =
      (duk_small_uint_t)((call_flags & DUK_CALL_FLAG_CONSTRUCT) ? 1 : 0)
#  if defined(DUK_USE_ES6_PROXY)
      |
...
#  endif
  ...
  if (DUK_HOBJECT_HAS_NOTAIL(func)) {
    ...
    DUK_DDD(
        DUK_DDDPRINT("tail call prevented by function having a notail flag"));
    return 0;
  }
...
#  endif
  ...
  *out_vs_min_bytes =
      ...
      sizeof(duk_tval) * ((duk_size_t)*out_nregs + DUK_VALSTACK_INTERNAL_EXTRA);
...
DUK_LOCAL void duk__call_setup_act_not_tailcall(
  ...
  if (act != NULL) {
    ...
    act->retval_byteoff =
        entry_valstack_bottom_byteoff + (duk_size_t)idx_func * sizeof(duk_tval);
  }
      ...
      *out_nregs = *out_nargs;
      if (*out_nargs >= 0) {
        *out_vs_min_bytes =
        ...
        /* Vararg function. */
        duk_size_t valstack_top_byteoff = (duk_size_t)(
            (duk_uint8_t*)thr->valstack_top - ((duk_uint8_t*)thr->valstack));
  ...
  if (DUK_LIKELY(func != NULL)) {
    if (DUK_LIKELY(DUK_HOBJECT_HAS_NEWENV(func))) {
      ...
      if (DUK_LIKELY(!DUK_HOBJECT_HAS_CREATEARGS(func))) {
        ...
        /* third arg: absolute index (to entire valstack) of bottom_byteoff of
         * new activation */
...
DUK_LOCAL duk_int_t duk__handle_call_raw(duk_hthread* thr, duk_idx_t idx_func,
  ...
  if (DUK_LIKELY(duk__resolve_target_fastpath_check(thr, idx_func, &func,
    ...
    func =
        duk__resolve_target_func_and_this_binding(thr, idx_func, &call_flags);
  }
  ...
  if (func != NULL && DUK_HOBJECT_IS_COMPFUNC(func)) {
    ...
    if (call_flags & DUK_CALL_FLAG_ALLOW_ECMATOECMA) {
      ...
      DUK_ASSERT(thr->ptr_curr_pc == NULL);
      thr->heap
          ->call_recursion_depth--; /* No recursion increase for this case. */
...
DUK_INTERNAL duk_int_t duk_handle_call_unprotected(
    duk_hthread* thr, duk_idx_t idx_func, duk_small_uint_t call_flags) {
  DUK_ASSERT(duk_is_valid_index(thr, idx_func));
  DUK_ASSERT(idx_func >= 0);
...
DUK_LOCAL void duk__handle_safe_call_error(
  ...
  DUK_DD(DUK_DDPRINT("safe call error handled, pf_prevent_count updated to %ld",
...
DUK_INTERNAL duk_int_t duk_handle_safe_call(duk_hthread* thr,
  ...
  if (DUK_SETJMP(our_jmpbuf.jb) == 0) {
    ...
    duk__handle_safe_call_error(thr, entry_act,
#if defined(DUK_USE_ASSERTIONS)
                                entry_callstack_top,
#endif
                                entry_curr_thread, entry_thread_state,
                                idx_retbase, num_stack_rets,
...
#  endif
    ...
    try {
      DUK_ERROR_TYPE(
          thr, "caught invalid c++ exception (perhaps thrown by user code)");
      DUK_WO_NORETURN(return 0;);
    } catch (duk_internal_exception exc) {
      DUK_D(
          DUK_DPRINT("caught api error thrown from unexpected c++ exception"));
      DUK_UNREF(exc);
      duk__handle_safe_call_error(
          thr, entry_act,
#  if defined(DUK_USE_ASSERTIONS)
...
#if defined(DUK_USE_VERBOSE_ERRORS)
DUK_INTERNAL DUK_NOINLINE DUK_COLD void duk_call_setup_propcall_error(
    duk_hthread* thr, duk_tval* tv_base, duk_tval* tv_key) {
...
#  if defined(DUK_USE_PARANOID_ERRORS)
  ...
  duk_xdef_prop_stridx(
      thr, -2, DUK_STRIDX_INT_TARGET,
      DUK_PROPDESC_FLAGS_NONE); /* Marker property, reuse _Target. */
