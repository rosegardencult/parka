...
DUK_LOCAL void duk__decref_tvals_norz(duk_hthread* thr, duk_tval* tv,
                                      duk_idx_t count) {
...
DUK_INTERNAL void duk_hobject_refcount_finalize_norz(duk_heap* heap,
  ...
  while (n-- > 0) {
    ...
    if (DUK_UNLIKELY(p_flag[n] & DUK_PROPDESC_FLAG_ACCESSOR)) {
      ...
      DUK_ASSERT(h_getset == NULL ||
                 DUK_HEAPHDR_IS_OBJECT((duk_heaphdr*)h_getset));
  ...
  if (DUK_HOBJECT_IS_COMPFUNC(h)) {
    ...
    DUK_HEAPHDR_DECREF_ALLOWNULL(
        thr, (duk_heaphdr*)DUK_HCOMPFUNC_GET_LEXENV(heap, f));
    DUK_HEAPHDR_DECREF_ALLOWNULL(
        thr, (duk_heaphdr*)DUK_HCOMPFUNC_GET_VARENV(heap, f));
    DUK_HEAPHDR_DECREF_ALLOWNULL(thr,
                                 (duk_hbuffer*)DUK_HCOMPFUNC_GET_DATA(heap, f));
...
#  endif /* DUK_USE_ES6_PROXY */
    ...
    for (act = t->callstack_curr; act != NULL; act = act->parent) {
      DUK_HOBJECT_DECREF_NORZ_ALLOWNULL(thr,
                                        (duk_hobject*)DUK_ACT_GET_FUNC(act));
...
#  endif
  ...
  DUK_ASSERT(DUK_HEAPHDR_GET_PREV(heap, curr) ==
             NULL); /* We're called on initial insert only. */
  /* curr->next is GARBAGE. */

  do {
    duk_heaphdr* prev;

    DUK_DDD(DUK_DDDPRINT("refzero processing %p: %!O", (void*)curr,
                         (duk_heaphdr*)curr));
...
#  endif
    ...
    DUK_ASSERT(DUK_HEAPHDR_GET_TYPE(curr) ==
               DUK_HTYPE_OBJECT); /* currently, always the case */
...
DUK_LOCAL DUK_INLINE void duk__refcount_refzero_hobject(
    duk_heap* heap, duk_hobject* obj, duk_bool_t skip_free_pending) {
  duk_heaphdr* hdr;
  duk_heaphdr* root;
...
DUK_INTERNAL DUK_ALWAYS_INLINE void duk_refzero_check_fast(duk_hthread* thr) {
  ...
  DUK_ASSERT(thr->heap->refzero_list ==
             NULL); /* Processed to completion inline. */
...
#  define DUK__RZ_SUPPRESS_ASSERT1()                                        \
    do {                                                                    \
      ...
      DUK_ASSERT(thr->heap->ms_running == 0 ||                              \
                 thr->heap->heap_thread != NULL);                           \
       ...
       */                                                                   \
      DUK_ASSERT(                                                           \
          !(thr->heap->ms_running == 1 && thr != thr->heap->heap_thread));  \
       ...
       * if that's the case.                                                \
       */                                                                   \
      DUK_ASSERT(thr->heap->heap_initializing == 0 ||                       \
                 thr->heap->ms_prevent_count > 0);                          \
      DUK_ASSERT(thr->heap->heap_initializing == 0 ||                       \
                 thr->heap->pf_prevent_count > 0);                          \
    } while (0)
...
#    define DUK__RZ_SUPPRESS_ASSERT2()                         \
      do {                                                     \
        ...
        DUK_ASSERT(!DUK_HEAP_HAS_DEBUGGER_PAUSED(thr->heap) || \
                   thr->heap->ms_running != 0);                \
      } while (0)
...
#  define DUK__RZ_SUPPRESS_CHECK()                                             \
    do {                                                                       \
      ...
      if (DUK_UNLIKELY(DUK__RZ_SUPPRESS_COND())) {                             \
        DUK_DDD(                                                               \
            DUK_DDDPRINT("refzero handling suppressed (not even queued) when " \
                         "mark-and-sweep running, object: %p",                 \
                         (void*)h));                                           \
        return;                                                                \
      }                                                                        \
    } while (0)
...
DUK_LOCAL DUK__RZ_INLINE void duk__hobject_refzero_helper(
    duk_hthread* thr, duk_hobject* h, duk_bool_t skip_free_pending) {
  duk_heap* heap;
...
DUK_LOCAL DUK__RZ_INLINE void duk__heaphdr_refzero_helper(
    duk_hthread* thr, duk_heaphdr* h, duk_bool_t skip_free_pending) {
  duk_heap* heap;
  ...
  DUK_DDD(DUK_DDDPRINT("ms_running=%ld, heap_thread=%p",
                       (long)thr->heap->ms_running, thr->heap->heap_thread));
  DUK__RZ_SUPPRESS_CHECK();
