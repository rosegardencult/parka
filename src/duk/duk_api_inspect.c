 ...
 * varargs, format string with parsing is often better than string pointer
 * arrays.
 */
DUK_LOCAL void duk__inspect_multiple_uint(duk_hthread* thr, const char* fmt,
                                          duk_int_t* vals) {
  duk_int_t val;
  const char* p;
  const char* p_curr;
...
DUK_EXTERNAL void duk_inspect_value(duk_hthread* thr, duk_idx_t idx) {
  ...
  switch ((duk_small_int_t)DUK_HEAPHDR_GET_TYPE(h)) {
    case DUK_HTYPE_STRING: {
      ...
      vals[DUK__IDX_HBYTES] =
          (duk_int_t)(sizeof(duk_hstring) + DUK_HSTRING_GET_BYTELEN(h_str) + 1);
    ...
    case DUK_HTYPE_OBJECT: {
      ...
      if (DUK_HOBJECT_IS_COMPFUNC(h_obj)) {
        duk_hbuffer* h_data = (duk_hbuffer*)DUK_HCOMPFUNC_GET_DATA(
            thr->heap, (duk_hcompfunc*)h_obj);
        vals[DUK__IDX_BCBYTES] =
            (duk_int_t)(h_data ? DUK_HBUFFER_GET_SIZE(h_data) : 0);
      }
      break;
    }
    case DUK_HTYPE_BUFFER: {
      ...
      if (DUK_HBUFFER_HAS_DYNAMIC(h_buf)) {
        ...
        vals[DUK__IDX_HBYTES] = (duk_int_t)(sizeof(duk_hbuffer_fixed) +
