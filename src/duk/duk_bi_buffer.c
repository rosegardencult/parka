...
DUK_INTERNAL duk_ret_t duk_bi_nodejs_buffer_constructor(duk_hthread* thr) {
  ...
  duk_push_buffer_object(
      thr, -1, 0, DUK_HBUFFER_FIXED_GET_SIZE((duk_hbuffer_fixed*)(void*)h_buf),
      DUK_BUFOBJ_UINT8ARRAY);
...
DUK_INTERNAL duk_ret_t duk_bi_typedarray_constructor(duk_hthread* thr) {
  ...
  if (DUK_TVAL_IS_OBJECT(tv)) {
    ...
    if (DUK_HOBJECT_GET_CLASS_NUMBER(h_obj) == DUK_HOBJECT_CLASS_ARRAYBUFFER) {
      ...
      DUK_DDD(
          DUK_DDDPRINT("selecting copy mode for bufobj arg, "
...
#  if !defined(DUK_USE_PREFER_SIZE)
    ...
    case 1: {
      ...
      DUK_DDD(
          DUK_DDDPRINT("using fast copy: p_src=%p, p_src_end=%p, p_dst=%p, "
      ...
      while (p_src != p_src_end) {
        DUK_DDD(
            DUK_DDDPRINT("fast path per element copy loop: "
...
DUK_INTERNAL duk_ret_t duk_bi_buffer_compare_shared(duk_hthread* thr) {
  ...
  if (DUK_HBUFOBJ_VALID_SLICE(h_bufarg1) &&
    ...
    comp_res = duk_js_data_compare((const duk_uint8_t*)DUK_HBUFFER_GET_DATA_PTR(
                                       thr->heap, h_bufarg1->buf) +
                                       h_bufarg1->offset,
                                   (const duk_uint8_t*)DUK_HBUFFER_GET_DATA_PTR(
                                       thr->heap, h_bufarg2->buf) +
                                       h_bufarg2->offset,
...
#  endif /* !DUK_USE_PREFER_SIZE */
    ...
    DUK_DDD(
        DUK_DDDPRINT("overlap check: p_src_base=%p, src_length=%ld, "
                     "p_dst_base=%p, dst_length=%ld",
                     (void*)p_src_base, (long)src_length, (void*)p_dst_base,
                     (long)dst_length));
    ...
    DUK_DDD(
        DUK_DDDPRINT("after overlap check: p_src_base=%p, src_length=%ld, "
                     "p_dst_base=%p, dst_length=%ld, valstack top=%ld",
                     (void*)p_src_base, (long)src_length, (void*)p_dst_base,
                     (long)dst_length, (long)duk_get_top(thr)));
...
DUK_LOCAL void duk__arraybuffer_plain_slice(duk_hthread* thr,
  ...
  p_copy =
      (duk_uint8_t*)duk_push_fixed_buffer_nozero(thr, (duk_size_t)slice_length);
...
#  endif
  ...
  if (magic_ftype != DUK__FLD_VARINT) {
    DUK_ASSERT(magic_ftype <
               (duk_small_uint_t)(sizeof(duk__buffer_nbytes_from_fldtype) /
                                  sizeof(duk_uint8_t)));
