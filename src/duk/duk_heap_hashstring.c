...
DUK_INTERNAL duk_uint32_t duk_heap_hashstring(duk_heap* heap,
  ...
  if (len <= DUK__STRHASH_SHORTSTRING) {
    ...
    hash =
        duk_util_hashbytes(str, (duk_size_t)DUK__STRHASH_SHORTSTRING, str_seed);
    off = DUK__STRHASH_SHORTSTRING + (skip * (hash % 256)) / 256;

    /* XXX: inefficient loop */
    while (off < len) {
      duk_size_t left = len - off;
      duk_size_t now = (duk_size_t)(
          left > DUK__STRHASH_BLOCKSIZE ? DUK__STRHASH_BLOCKSIZE : left);
...
DUK_INTERNAL duk_uint32_t duk_heap_hashstring(duk_heap* heap,
  ...
  hash = heap->hash_seed ^
         ((duk_uint32_t)len); /* Bernstein hash init value is normally 5381 */
