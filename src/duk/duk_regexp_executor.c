...
DUK_LOCAL duk_uint32_t duk__bc_get_u32(duk_re_matcher_ctx* re_ctx,
  ...
  return (duk_uint32_t)duk_unicode_decode_xutf8_checked(
      re_ctx->thr, pc, re_ctx->bytecode, re_ctx->bytecode_end);
}
...
DUK_LOCAL duk_int32_t duk__bc_get_i32(duk_re_matcher_ctx* re_ctx,
  ...
  t = (duk_uint32_t)duk_unicode_decode_xutf8_checked(
      re_ctx->thr, pc, re_ctx->bytecode, re_ctx->bytecode_end);
  if (t & 1) {
...
DUK_LOCAL duk_codepoint_t duk__inp_get_cp(duk_re_matcher_ctx* re_ctx,
  ...
  res = (duk_codepoint_t)duk_unicode_decode_xutf8_checked(
      re_ctx->thr, sp, re_ctx->input, re_ctx->input_end);
...
DUK_LOCAL const duk_uint8_t* duk__inp_backtrack(duk_re_matcher_ctx* re_ctx,
                                                const duk_uint8_t** sp,
                                                duk_uint_fast32_t count) {
  return duk__utf8_backtrack(re_ctx->thr, sp, re_ctx->input, re_ctx->input_end,
                             count);
}
 ...
 *
 *  Returns 'sp' on successful match (points to character after last matched
 * one), NULL otherwise.
 *
...
#  endif
    ...
    DUK_DDD(DUK_DDDPRINT(
        "match: rec=%ld, steps=%ld, pc (after op)=%ld, sp=%ld, op=%ld",
        (long)re_ctx->recursion_depth, (long)re_ctx->steps_count,
        (long)(pc - re_ctx->bytecode), (long)(sp - re_ctx->input), (long)op));
...
#  endif
      ...
      case DUK_REOP_INVRANGES: {
        ...
        while (n) {
          ...
          DUK_DDD(DUK_DDDPRINT(
              "matching ranges/invranges, n=%ld, r1=%ld, r2=%ld, c=%ld",
      ...
      case DUK_REOP_SQMINIMAL: {
        ...
        DUK_DDD(DUK_DDDPRINT("minimal quantifier, qmin=%lu, qmax=%lu, skip=%ld",
                             (unsigned long)qmin, (unsigned long)qmax,
                             (long)skip));
      ...
      case DUK_REOP_SQGREEDY: {
        ...
        skip = duk__bc_get_i32(re_ctx, &pc);
        DUK_DDD(DUK_DDDPRINT(
            "greedy quantifier, qmin=%lu, qmax=%lu, atomlen=%lu, skip=%ld",
            (unsigned long)qmin, (unsigned long)qmax, (unsigned long)atomlen,
            (long)skip));
      ...
      case DUK_REOP_SAVE: {
        ...
        if (idx >= re_ctx->nsaved) {
          ...
          DUK_D(DUK_DPRINT("internal error, regexp save index insane: idx=%ld",
                           (long)idx));
          goto internal_error;
        }
...
#  endif
        ...
        DUK_DDD(DUK_DDDPRINT(
            "wipe saved range: start=%ld, count=%ld -> [%ld,%ld] (captures "
            "[%ld,%ld])",
            (long)idx_start, (long)idx_count, (long)idx_start,
            (long)(idx_start + idx_count - 1), (long)(idx_start / 2),
            (long)((idx_start + idx_count - 1) / 2)));
        if (idx_start + idx_count > re_ctx->nsaved || idx_count == 0) {
          /* idx is unsigned, < 0 check is not necessary */
          DUK_D(
              DUK_DPRINT("internal error, regexp wipe indices insane: "
                         "idx_start=%ld, idx_count=%ld",
        ...
        range_save = (duk_uint8_t**)duk_push_fixed_buffer_nozero(
            re_ctx->thr, sizeof(duk_uint8_t*) * idx_count);
        DUK_ASSERT(range_save != NULL);
        duk_memcpy(range_save, re_ctx->saved + idx_start,
...
#  else
        duk_memzero((void*)(re_ctx->saved + idx_start),
                    sizeof(duk_uint8_t*) * idx_count);
#  endif
        ...
        if (sub_sp) {
          ...
          DUK_DDD(DUK_DDDPRINT(
              "match: keep wiped/resaved values [%ld,%ld] (captures [%ld,%ld])",
        ...
        DUK_DDD(DUK_DDDPRINT(
            ...
            (long)idx_start, (long)(idx_start + idx_count - 1),
            (long)(idx_start / 2), (long)((idx_start + idx_count - 1) / 2)));
        duk_memcpy((void*)(re_ctx->saved + idx_start), (const void*)range_save,
                   sizeof(duk_uint8_t*) * idx_count);
        duk_pop_unsafe(re_ctx->thr);
        goto fail;
      }
      ...
      case DUK_REOP_LOOKNEG: {
         ...
         *  may be overwritten.  Because the regexp parser does no such
         * analysis, we currently save the entire saved array here.  Lookaheads
         * are thus a bit expensive.  Note that the saved array is not needed
         * for just the lookahead sub-match, but for the matching of the entire
         * sequel.
         *
        ...
        duk_require_stack(re_ctx->thr, 1);
        full_save = (duk_uint8_t**)duk_push_fixed_buffer_nozero(
            re_ctx->thr, sizeof(duk_uint8_t*) * re_ctx->nsaved);
        DUK_ASSERT(full_save != NULL);
        duk_memcpy(full_save, re_ctx->saved,
      ...
      lookahead_fail:
        /* fail: restore saves */
        duk_memcpy((void*)re_ctx->saved, (const void*)full_save,
                   sizeof(duk_uint8_t*) * re_ctx->nsaved);
        duk_pop_unsafe(re_ctx->thr);
        goto fail;
      }
      case DUK_REOP_BACKREFERENCE: {
        ...
        if (!re_ctx->saved[idx] || !re_ctx->saved[idx + 1]) {
          ...
          DUK_DDD(DUK_DDDPRINT(
              "backreference: saved[%ld,%ld] not complete, always match",
              (long)idx, (long)(idx + 1)));
          break;
        }
        DUK_DDD(DUK_DDDPRINT("backreference: match saved[%ld,%ld]", (long)idx,
                             (long)(idx + 1)));
...
/*
 *  Exposed matcher function which provides the semantics of
 * RegExp.prototype.exec().
 *
 *  RegExp.prototype.test() has the same semantics as exec() but does not return
 * the result object (which contains the matching string and capture groups).
 * Currently there is no separate test() helper, so a temporary result object is
 * created and discarded if test() is needed.  This is intentional, to save code
 * space.
 *
...
DUK_LOCAL void duk__regexp_match_helper(duk_hthread* thr,
  ...
  DUK_ASSERT(DUK_HOBJECT_GET_CLASS_NUMBER(h_regexp) ==
  ...
  duk_xget_owndataprop_stridx_short(
      thr, -2, DUK_STRIDX_INT_BYTECODE); /* [ ... re_obj input ] -> [ ... re_obj
                                            input bc ] */
  h_bytecode =
      duk_require_hstring(thr, -1); /* no regexp instance should exist without a
                                       non-configurable bytecode property */
  ...
  DUK_ASSERT(DUK_RE_FLAG_GLOBAL <
             0x10000UL); /* must fit into duk_small_int_t */
  global = (duk_small_int_t)(
      force_global | (duk_small_int_t)(re_ctx.re_flags & DUK_RE_FLAG_GLOBAL));

  DUK_ASSERT(re_ctx.nsaved >= 2);
  DUK_ASSERT((re_ctx.nsaved % 2) == 0);

  p_buf = (duk_uint8_t*)duk_push_fixed_buffer(
      thr, sizeof(duk_uint8_t*) * re_ctx.nsaved); /* rely on zeroing */
  DUK_UNREF(p_buf);
...
#  endif
  ...
  DUK_DDD(
      DUK_DDDPRINT("regexp ctx initialized, flags=0x%08lx, nsaved=%ld, "
                   "recursion_limit=%ld, steps_limit=%ld",
                   (unsigned long)re_ctx.re_flags, (long)re_ctx.nsaved,
                   (long)re_ctx.recursion_limit, (long)re_ctx.steps_limit));
   ...
   *  its value to an arbitrary (garbage) value though; E5 requires that
   * lastIndex be coerced to a number before using.  The code below works even
   * if the property is missing: the value will then be coerced to zero.
   *
   *  Note: lastIndex may be outside Uint32 range even after ToInteger()
   * coercion. For instance, ToInteger(+Infinity) = +Infinity.  We track the
   * match offset as an integer, but pre-check it to be inside the 32-bit range
   * before the loop. If not, the check in E5 Section 15.10.6.2, step 9.a
   * applies.
   */
  ...
  duk_get_prop_stridx_short(thr, -4,
                            DUK_STRIDX_LAST_INDEX); /* -> [ ... re_obj input bc
                                                       saved_buf lastIndex ] */
  ...
  sp = re_ctx.input +
       duk_heap_strcache_offset_char2byte(thr, h_input, char_offset);
  ...
  for (;;) {
    /* char offset in [0, h_input->clen] (both ends inclusive), checked before
     * entry */
    DUK_ASSERT_DISABLE(char_offset >= 0);
    DUK_ASSERT(char_offset <= DUK_HSTRING_GET_CHARLEN(h_input));

    /* Note: re_ctx.steps is intentionally not reset, it applies to the entire
     * unanchored match */
     ...
     *  Note:
     *
     *    - duk__match_regexp() is required not to longjmp() in ordinary
     * "non-match" conditions; a longjmp() will terminate the entire matching
     * process.
     *
    ...
    if (char_offset > DUK_HSTRING_GET_CHARLEN(h_input)) {
       ...
       *  Note:
       *
       *    - Intentionally attempt (empty) match at char_offset ==
       * k_input->clen
       *
       *    - Negative char_offsets have been eliminated and char_offset is
       * duk_uint32_t
    ...
    /* avoid calling at end of input, will DUK_ERROR (above check suffices to
     * avoid this) */
    (void)duk__utf8_advance(thr, &sp, re_ctx.input, re_ctx.input_end,
                            (duk_uint_fast32_t)1);
  }
...
match_over:
  ...
  /*
   *  Matching complete, create result array or return a 'null'.  Update
   * lastIndex if necessary.  See E5 Section 15.10.6.2.
   *
   *  Because lastIndex is a character (not byte) offset, we need the character
   *  length of the match which we conveniently get as a side effect of
   * interning the matching substring (0th index of result array).
   *
   *  saved[0]         start pointer (~ byte offset) of current match
   *  saved[1]         end pointer (~ byte offset) of current match (exclusive)
   *  char_offset      start character offset of current match (-> .index of
   * result) char_end_offset  end character offset (computed below)
   */
...
#  endif
    ...
    DUK_DDD(
        DUK_DDDPRINT("regexp matches at char_offset %ld", (long)char_offset));
...
#  endif
    ...
    for (i = 0; i < re_ctx.nsaved; i += 2) {
       ...
       */
      if (re_ctx.saved[i] && re_ctx.saved[i + 1] &&
          re_ctx.saved[i + 1] >= re_ctx.saved[i]) {
        duk_push_lstring(thr, (const char*)re_ctx.saved[i],
        ...
        if (i == 0) {
           ...
           * correct char length.
           */
          char_end_offset = char_offset + (duk_uint32_t)duk_get_length(
                                              thr, -1); /* add charlen */
        }
      } else {
