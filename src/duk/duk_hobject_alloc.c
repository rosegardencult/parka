...
DUK_LOCAL void duk__init_object_parts(duk_heap* heap, duk_uint_t hobject_flags,
                                      duk_hobject* obj) {
  DUK_ASSERT(obj != NULL);
  /* Zeroed by caller. */

  obj->hdr.h_flags = hobject_flags | DUK_HTYPE_OBJECT;
  DUK_ASSERT(DUK_HEAPHDR_GET_TYPE(&obj->hdr) ==
             DUK_HTYPE_OBJECT); /* Assume zero shift. */
...
DUK_INTERNAL duk_hobject* duk_hobject_alloc_unchecked(
...
DUK_INTERNAL duk_hobject* duk_hobject_alloc(duk_hthread* thr,
  ...
  res = (duk_hobject*)duk__hobject_alloc_init(thr, hobject_flags,
                                              sizeof(duk_hobject));
  return res;
}
...
DUK_INTERNAL duk_hcompfunc* duk_hcompfunc_alloc(duk_hthread* thr,
  ...
  res = (duk_hcompfunc*)duk__hobject_alloc_init(thr, hobject_flags,
                                                sizeof(duk_hcompfunc));
#if defined(DUK_USE_EXPLICIT_NULL_INIT)
#  if defined(DUK_USE_HEAPPTR16)
...
DUK_INTERNAL duk_hnatfunc* duk_hnatfunc_alloc(duk_hthread* thr,
  ...
  res = (duk_hnatfunc*)duk__hobject_alloc_init(thr, hobject_flags,
                                               sizeof(duk_hnatfunc));
#if defined(DUK_USE_EXPLICIT_NULL_INIT)
  res->func = NULL;
#endif
...
DUK_INTERNAL duk_hbufobj* duk_hbufobj_alloc(duk_hthread* thr,
  ...
  res = (duk_hbufobj*)duk__hobject_alloc_init(thr, hobject_flags,
                                              sizeof(duk_hbufobj));
#  if defined(DUK_USE_EXPLICIT_NULL_INIT)
  res->buf = NULL;
  res->buf_prop = NULL;
 ...
 * another thread.
 */
DUK_INTERNAL duk_hthread* duk_hthread_alloc_unchecked(
    duk_heap* heap, duk_uint_t hobject_flags) {
  duk_hthread* res;
...
DUK_INTERNAL duk_harray* duk_harray_alloc(duk_hthread* thr,
  ...
  res = (duk_harray*)duk__hobject_alloc_init(thr, hobject_flags,
                                             sizeof(duk_harray));
...
DUK_INTERNAL duk_hdecenv* duk_hdecenv_alloc(duk_hthread* thr,
  ...
  res = (duk_hdecenv*)duk__hobject_alloc_init(thr, hobject_flags,
                                              sizeof(duk_hdecenv));
#if defined(DUK_USE_EXPLICIT_NULL_INIT)
  res->thread = NULL;
  res->varmap = NULL;
...
DUK_INTERNAL duk_hobjenv* duk_hobjenv_alloc(duk_hthread* thr,
  ...
  res = (duk_hobjenv*)duk__hobject_alloc_init(thr, hobject_flags,
                                              sizeof(duk_hobjenv));
#if defined(DUK_USE_EXPLICIT_NULL_INIT)
  res->target = NULL;
#endif
...
DUK_INTERNAL duk_hproxy* duk_hproxy_alloc(duk_hthread* thr,
  ...
  res = (duk_hproxy*)duk__hobject_alloc_init(thr, hobject_flags,
                                             sizeof(duk_hproxy));
