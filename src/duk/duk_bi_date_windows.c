...
DUK_LOCAL void duk__convert_systime_to_ularge(const SYSTEMTIME* st,
                                              ULARGE_INTEGER* res) {
  FILETIME ft;
...
DUK_LOCAL void duk__convert_filetime_to_ularge(const FILETIME* ft,
                                               ULARGE_INTEGER* res) {
  res->LowPart = ft->dwLowDateTime;
  res->HighPart = ft->dwHighDateTime;
}
...
#endif /* defined(DUK_USE_DATE_NOW_WINDOWS) || \
          defined(DUK_USE_DATE_TZO_WINDOWS) */

#if defined(DUK_USE_DATE_NOW_WINDOWS)
DUK_INTERNAL duk_double_t duk_bi_date_get_now_windows(void) {
  /* Suggested step-by-step method from documentation of
   ...
   */
  return (duk_double_t)((LONGLONG)tmp1.QuadPart - (LONGLONG)tmp2.QuadPart) /
         10000.0;
}
...
DUK_INTERNAL duk_int_t duk_bi_date_get_local_tzoffset_windows(duk_double_t d) {
  ...
  duk__convert_systime_to_ularge((const SYSTEMTIME*)&st1, &tmp1);
  tmp2.QuadPart =
      (ULONGLONG)(d * 10000.0); /* millisec -> 100ns units since jan 1, 1970 */
  ...
  if (SystemTimeToTzSpecificLocalTime((LPTIME_ZONE_INFORMATION)NULL, &st2,
                                      &st3) == 0) {
    DUK_D(DUK_DPRINT(
  ...
  return (duk_int_t)(((LONGLONG)tmp3.QuadPart - (LONGLONG)tmp2.QuadPart) /
                     DUK_I64_CONSTANT(10000000)); /* seconds */
}
...
duk_bi_date_get_local_tzoffset_windows_no_dst(duk_double_t d) {
  ...
  return (duk_int_t)(((LONGLONG)tmp2.QuadPart - (LONGLONG)tmp1.QuadPart) /
                     DUK_I64_CONSTANT(10000000)); /* seconds */
}
