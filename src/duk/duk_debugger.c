...
DUK_LOCAL void duk__debug_null_most_callbacks(duk_hthread* thr) {
  ...
  heap->dbg_read_cb = NULL;
  heap->dbg_write_cb = NULL; /* this is especially critical to avoid another
                                write call in detach1() */
  heap->dbg_peek_cb = NULL;
  heap->dbg_read_flush_cb = NULL;
  heap->dbg_write_flush_cb = NULL;
  heap->dbg_request_cb = NULL;
...
DUK_LOCAL void duk__debug_set_pause_state(duk_hthread* thr, duk_heap* heap,
  ...
  if (line == 0) {
    ...
    DUK_D(
        DUK_DPRINT("no line info for current activation, disable line-based "
                   "pause flags: 0x%08lx -> 0x%08lx",
  ...
  DUK_D(
      DUK_DPRINT("set state for automatic pause triggers, flags=0x%08lx, "
                 "act=%p, startline=%ld",
...
fail:
  DUK_D(
      DUK_DPRINT("debug connection error: failed to decode any pointer "
...
#  endif
    default:
      ...
      DUK_DD(DUK_DDPRINT(
          "i32=%ld du1=%02x%02x%02x%02x%02x%02x%02x%02x "
          "du2=%02x%02x%02x%02x%02x%02x%02x%02x",
          (long)i32, (unsigned int)du1.uc[0], (unsigned int)du1.uc[1],
...
DUK_INTERNAL void duk_debug_send_status(duk_hthread* thr) {
  ...
  if (act == NULL) {
    ...
    duk_debug_write_uint(thr,
                         (duk_uint32_t)duk_hthread_get_act_curr_pc(thr, act));
  }
  ...
  if (duk_is_error(thr, -1)) {
    ...
    if (act != NULL) {
      ...
      duk_debug_write_uint(
          thr, (duk_uint32_t)duk_hobject_pc2line_query(thr, -2, pc));
      duk_pop_2(thr);
    } else {
 ...
 */
DUK_LOCAL duk_activation* duk__debug_read_level_get_activation(
    duk_hthread* thr) {
  duk_activation* act;
  duk_int32_t level;
...
DUK_LOCAL void duk__debug_handle_apprequest(duk_hthread* thr, duk_heap* heap) {
  ...
  if (heap->dbg_request_cb != NULL) {
    ...
    DUK_D(
        DUK_DPRINT("calling into AppRequest request_cb with nvalues=%ld, "
                   "old_top=%ld, top=%ld",
                   (long)nvalues, (long)old_top, (long)duk_get_top(thr)));
    nrets = heap->dbg_request_cb(thr, heap->dbg_udata, nvalues);
    DUK_D(DUK_DPRINT(
        "returned from AppRequest request_cb; nvalues=%ld -> nrets=%ld, "
        "old_top=%ld, top=%ld",
    ...
    if (nrets >= 0) {
      ...
      if (duk_get_top(thr) < old_top + 1) {
        DUK_D(
            DUK_DPRINT("request callback return value doesn't match value "
                       "stack configuration"));
        goto fail;
      }
...
#    endif
  ...
  switch (DUK_HEAPHDR_GET_TYPE(hdr)) {
    ...
    case DUK_HTYPE_OBJECT: {
      ...
      duk_debug_write_heapptr(thr,
                              (duk_heaphdr*)DUK_HOBJECT_GET_PROTOTYPE(heap, h));
      ...
      for (i = 0; i < (duk_uint_fast32_t)DUK_HOBJECT_GET_ENEXT(h); i++) {
        duk_debug_write_uint(thr,
                             (duk_uint32_t)DUK_HOBJECT_E_GET_FLAGS(heap, h, i));
        ...
        if (DUK_HOBJECT_E_SLOT_IS_ACCESSOR(heap, h, i)) {
          ...
          duk_debug_write_heapptr(
              thr,
              (duk_heaphdr*)DUK_HOBJECT_E_GET_VALUE_PTR(heap, h, i)->a.get);
          duk_debug_write_heapptr(
              thr,
              (duk_heaphdr*)DUK_HOBJECT_E_GET_VALUE_PTR(heap, h, i)->a.set);
        } else {
          duk_debug_write_int(thr, 0); /* isAccessor */
...
DUK_LOCAL void duk__debug_dump_heap_allocated(duk_hthread* thr,
                                              duk_heap* heap) {
  duk_heaphdr* hdr;
...
DUK_LOCAL void duk__debug_handle_get_bytecode(duk_hthread* thr,
                                              duk_heap* heap) {
  duk_activation* act;
  ...
  switch (DUK_HEAPHDR_GET_TYPE(h)) {
    case DUK_HTYPE_STRING: {
      ...
      duk__debug_getinfo_prop_uint(thr, "bytelen",
                                   (duk_uint_t)DUK_HSTRING_GET_BYTELEN(h_str));
      duk__debug_getinfo_prop_uint(thr, "charlen",
                                   (duk_uint_t)DUK_HSTRING_GET_CHARLEN(h_str));
...
#  endif
  ...
  DUK_D(DUK_DPRINT(
      "process debug messages: read_cb=%s, no_block=%ld, detaching=%ld, "
      "processing=%ld",
  ...
  for (;;) {
    ...
    while (thr->heap->dbg_read_cb == NULL && thr->heap->dbg_detaching) {
      ...
      DUK_D(
          DUK_DPRINT("detach pending (dbg_read_cb == NULL, dbg_detaching != "
                     "0), call detach2"));
      ...
      DUK_D(
          DUK_DPRINT("after detach2 (and possible reattach): dbg_read_cb=%s, "
                     "dbg_detaching=%ld",
    ...
    if (thr->heap->dbg_read_cb == NULL) {
      DUK_D(
          DUK_DPRINT("debug connection broken (and not detaching), stop "
                     "processing messages"));
      break;
    }
    ...
    if (!DUK_HEAP_HAS_DEBUGGER_PAUSED(thr->heap) || no_block) {
      if (!duk_debug_read_peek(thr)) {
         ...
         */
        DUK_D(
            DUK_DPRINT("processing debug message, peek indicated no data, stop "
                       "processing messages"));
        break;
      }
      DUK_D(DUK_DPRINT(
...
DUK_INTERNAL duk_bool_t duk_debug_remove_breakpoint(
...
DUK_INTERNAL void duk_debug_set_paused(duk_heap* heap) {
  if (duk_debug_is_paused(heap)) {
    DUK_D(
        DUK_DPRINT("trying to set paused state when already paused, ignoring"));
  } else {
