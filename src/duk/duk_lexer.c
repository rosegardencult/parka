...
#define DUK__MAX_RE_QUANT_DIGITS                                               \
  9 /* Does not allow e.g. 2**31-1, but one more would allow overflows of u32. \
     */
 ...
 *  Future work:
 *
 *    * Reject other invalid Unicode sequences (see Wikipedia entry for
 * examples) in strict UTF-8 mode.
 *
...
#  endif
  ...
  cp = (duk_lexer_codepoint*)(void*)((duk_uint8_t*)lex_ctx->buffer +
                                     start_offset_bytes);
    ...
    if (DUK_LIKELY(x < 0x80UL)) {
      ...
      if (DUK_UNLIKELY(x <= 0x000dUL)) {
        if ((x == 0x000aUL) ||
           ...
           *   PS
           *
           * For CR LF, CR is ignored if it is followed by LF, and the LF will
...
DUK_LOCAL void duk__advance_bytes(duk_lexer_ctx* lex_ctx,
  ...
  DUK_ASSERT(count_bytes <= (duk_small_uint_t)(DUK_LEXER_WINDOW_SIZE *
  ...
  DUK_ASSERT((duk_uint8_t*)lex_ctx->window + count_bytes <=
             (duk_uint8_t*)lex_ctx->buffer +
                 DUK_LEXER_BUFFER_SIZE * sizeof(duk_lexer_codepoint));
  ...
  lex_ctx->window =
      (duk_lexer_codepoint*)(void*)((duk_uint8_t*)lex_ctx->window +
                                    count_bytes); /* avoid multiply */
  used_bytes = (duk_small_uint_t)((duk_uint8_t*)lex_ctx->window -
                                  (duk_uint8_t*)lex_ctx->buffer);
  avail_bytes =
      DUK_LEXER_BUFFER_SIZE * sizeof(duk_lexer_codepoint) - used_bytes;
  if (avail_bytes <
      (duk_small_uint_t)(DUK_LEXER_WINDOW_SIZE * sizeof(duk_lexer_codepoint))) {
    /* Not enough data to provide a full window, so "scroll" window to
     * start of buffer and fill up the rest.
     */
    duk_memmove((void*)lex_ctx->buffer, (const void*)lex_ctx->window,
                (size_t)avail_bytes);
...
#  endif
  ...
  if (DUK_LIKELY(x < 0x80UL)) {
    ...
    if (DUK_UNLIKELY(x <= 0x000dUL)) {
      if ((x == 0x000aUL) ||
          ((x == 0x000dUL) &&
           (lex_ctx->input_offset >= lex_ctx->input_length ||
...
#  endif
  ...
  if ((duk_size_t)len >
      (duk_size_t)(lex_ctx->input_length - lex_ctx->input_offset)) {
    goto error_clipped;
  }
...
DUK_LOCAL void duk__advance_bytes(duk_lexer_ctx* lex_ctx,
  ...
  keep_bytes =
      DUK_LEXER_WINDOW_SIZE * sizeof(duk_lexer_codepoint) - count_bytes;
...
DUK_LOCAL void duk__init_lexer_window(duk_lexer_ctx* lex_ctx) {
  ...
  duk__advance_bytes(
      lex_ctx,
      DUK_LEXER_WINDOW_SIZE * sizeof(duk_lexer_codepoint)); /* fill window */
}
...
DUK_LOCAL duk_hstring* duk__internbuffer(duk_lexer_ctx* lex_ctx,
  ...
  DUK_ASSERT(valstack_idx == lex_ctx->slot1_idx ||
             valstack_idx == lex_ctx->slot2_idx);
...
DUK_LOCAL duk_codepoint_t duk__lexer_parse_escape(duk_lexer_ctx* lex_ctx,
  ...
  duk_small_int_t digits; /* Initial value 2 or 4 for fixed length escapes, 0
                             for ES2015 \u{H+}. */
 ...
 */
DUK_LOCAL duk_codepoint_t
duk__lexer_parse_legacy_octal(duk_lexer_ctx* lex_ctx, duk_small_uint_t* out_adv,
  ...
  DUK_ASSERT(DUK__LOOKUP(lex_ctx, 1) >= DUK_ASC_0 &&
             DUK__LOOKUP(lex_ctx, 1) <= DUK_ASC_9);
  ...
  DUK_DDD(
      DUK_DDDPRINT("final lookup_idx=%ld, cp=%ld", (long)lookup_idx, (long)cp));

  adv = lookup_idx;
  if (lookup_idx == 1) {
    DUK_DDD(DUK_DDDPRINT(
        "\\8 or \\9 -> treat as literal, accept in strict mode too"));
    ...
    if (reject_annex_b) {
      DUK_DDD(DUK_DDDPRINT(
          "non-zero octal literal %ld -> reject in strict-mode", (long)cp));
      cp = -1;
    } else {
      ...
      switch (x) {
        ...
        default: {
          if (duk_unicode_is_line_terminator(x)) {
             ...
             *    \0                                     (lookahead not
             * OctalDigit) \1 ... \7                              (lookahead not
             * OctalDigit) \ZeroToThree OctalDigit                (lookahead not
             * OctalDigit) \FourToSeven OctalDigit                (no lookahead
             * restrictions) \ZeroToThree OctalDigit OctalDigit     (no
             * lookahead restrictions)
             *
             *  Zero escape is part of the standard syntax.  Octal escapes are
             *  defined in E5 Section B.1.2, and are only allowed in non-strict
             * mode. Any other productions starting with a decimal digit are
             * invalid but are in practice treated like identity escapes.
             *
             *  Parse octal (up to 3 digits) from the lookup window.
             */

            emitcp = duk__lexer_parse_legacy_octal(
                lex_ctx, &adv, strict_mode /*reject_annex_b*/);
            if (emitcp < 0) {
              goto fail_escape;
            }
          } else if (x < 0) {
...
void duk_lexer_parse_js_input_element(duk_lexer_ctx* lex_ctx,
  ...
  duk_codepoint_t
      x; /* temporary, must be signed and 32-bit to hold Unicode code points */
  duk_small_uint_t advtok =
      0; /* (advance << 8) + token_type, updated at function end,
          * init is unnecessary but suppresses "may be used uninitialized"
          * warnings.
          */
  duk_bool_t got_lineterm =
...
restart:
  ...
  switch (x) {
    ...
    case DUK_ASC_HT: /* fast paths for space and tab */
      DUK__ADVANCECHARS(lex_ctx, 1);
      goto restart;
    case DUK_ASC_LF: /* LF line terminator; CR LF and Unicode lineterms are
    ...
    case DUK_ASC_HASH: /* '#' */
      if (DUK__L1() == DUK_ASC_EXCLAMATION && lex_ctx->window[0].offset == 0 &&
        ...
        /* DUK__ADVANCECHARS(lex_ctx, 2) would be correct here, but not
         * necessary */
    ...
    case DUK_ASC_SLASH: /* '/' */
      if (DUK__L1() == DUK_ASC_SLASH) {
         ...
         *
         *  RegExp parsing is a bit complex.  First, the regexp body is
         * delimited by forward slashes, but the body may also contain forward
         * slashes as part of an escape sequence or inside a character class
         * (delimited by square brackets).  A mini state machine is used to
         * implement these.
         *
         *  Further, an early (parse time) error must be thrown if the regexp
         *  would cause a run-time error when used in the expression new
         * RegExp(...). Parsing here simply extracts the (candidate) regexp, and
         * also accepts invalid regular expressions (which are delimited
         * properly).  The caller (compiler) must perform final validation and
         * regexp compilation.
         *
         *  RegExp first char may not be '/' (single line comment) or '*'
         * (multi- line comment).  These have already been checked above, so
         * there is no need below for special handling of the first regexp
         * character as in the E5 productions.
         *
    ...
    case DUK_ASC_LANGLE: /* '<' */
      ...
      if (DUK__L1() == DUK_ASC_EXCLAMATION && DUK__L2() == DUK_ASC_MINUS &&
          DUK__L3() == DUK_ASC_MINUS) {
        /*
         *  ES2015: B.1.3, handle "<!--" SingleLineHTMLOpenComment
         */

        /* DUK__ADVANCECHARS(lex_ctx, 4) would be correct here, but not
         * necessary */
        duk__lexer_skip_to_endofline(lex_ctx);
        goto restart; /* line terminator will be handled on next round */
      } else
      ...
      break;
    case DUK_ASC_RANGLE: /* '>' */
      if (DUK__L1() == DUK_ASC_RANGLE && DUK__L2() == DUK_ASC_RANGLE &&
          DUK__L3() == DUK_ASC_EQUALS) {
    ...
    case DUK_ASC_MINUS: /* '-' */
#if defined(DUK_USE_HTML_COMMENTS)
      if (got_lineterm && DUK__L1() == DUK_ASC_MINUS &&
          DUK__L2() == DUK_ASC_RANGLE) {
        /*
        ...
        /* DUK__ADVANCECHARS(lex_ctx, 3) would be correct here, but not
         * necessary */
        duk__lexer_skip_to_endofline(lex_ctx);
        goto restart; /* line terminator will be handled on next round */
      } else
    ...
    case DUK_ASC_SINGLEQUOTE: { /* '\'' */
      DUK__INITBUFFER(lex_ctx);
      duk__lexer_parse_string_literal(lex_ctx, out_token, x /*quote*/,
                                      strict_mode);
  ...
  if (duk_unicode_is_line_terminator(x)) {
    ...
    for (;;) {
      ...
      if (DUK__L0() == DUK_ASC_BACKSLASH) {
         ...
         */
        if (!(first ? duk_unicode_is_identifier_start(esc_cp)
                    : duk_unicode_is_identifier_part(esc_cp))) {
          goto fail_escape;
        }
    ...
    i_end = (strict_mode ? DUK_STRIDX_END_RESERVED
    ...
    /* Note: decimal number may start with a period, but must be followed by a
     * digit */
    ...
    if (s2n_radix != 10) {
      ...
      s2n_flags = DUK_S2N_FLAG_ALLOW_EXP | DUK_S2N_FLAG_ALLOW_FRAC |
    ...
    /* Section 7.8.3 (note): NumericLiteral must be followed by something other
     * than IdentifierStart or DecimalDigit.
     */
...
skip_slow_path:
   ...
   * or EOF).
   */
  if (got_lineterm || out_token->t == DUK_TOK_RCURLY ||
      out_token->t == DUK_TOK_EOF) {
...
DUK_INTERNAL void duk_lexer_parse_re_token(duk_lexer_ctx* lex_ctx,
  ...
  duk_small_uint_t advtok = 0; /* init is unnecessary but suppresses "may be
  ...
  DUK_DDD(
      DUK_DDDPRINT("parsing regexp token, L0=%ld, L1=%ld", (long)x, (long)y));
...
#  endif
    ...
    case DUK_ASC_BACKSLASH: {
       ...
       * ES2015 Annex B relaxes the rules to allow these (and other) real world
       * forms.
       */

      advtok = DUK__ADVTOK(
          2, DUK_RETOK_ATOM_CHAR); /* default: char escape (two chars) */
      if (y == DUK_ASC_LC_B) {
         ...
         */
        out_token->num =
            (duk_uint32_t)duk__lexer_parse_escape(lex_ctx, 0 /*allow_es6*/);
        ...
        if (y == DUK_ASC_0) {
          ...
          /* DUK__L0() cannot be a digit, because the loop doesn't terminate if
           * it is */
...
#  else                                /* DUK_USE_ES6_REGEXP_SYNTAX */
                 ...
                 y == DUK_UNICODE_CP_ZWNJ || y == DUK_UNICODE_CP_ZWJ) {
...
#  endif                               /* DUK_USE_ES6_REGEXP_SYNTAX */
    ...
    case DUK_ASC_LPAREN: {
      /* XXX: naming is inconsistent: ATOM_END_GROUP ends an
       * ASSERT_START_LOOKAHEAD */
...
#  if !defined(DUK_USE_ES6_REGEXP_SYNTAX)
    case DUK_ASC_RCURLY:
    case DUK_ASC_RBRACKET: {
      /* Although these could be parsed as PatternCharacters unambiguously
       * (here), E5 Section 15.10.1 grammar explicitly forbids these as
       * PatternCharacters.
       */
...
void duk__emit_u16_direct_ranges(duk_lexer_ctx* lex_ctx,
                                 ...
                                 void* userdata, const duk_uint16_t* ranges,
                                 duk_small_int_t num) {
  ...
  while (ranges < ranges_end) {
    ...
    gen_range(userdata, (duk_codepoint_t)ranges[0], (duk_codepoint_t)ranges[1],
              1);
    ranges += 2;
  }
}
...
DUK_INTERNAL void duk_lexer_parse_re_ranges(duk_lexer_ctx* lex_ctx,
  ...
  for (;;) {
    ...
    ch = -1; /* not strictly necessary, but avoids "uninitialized variable"
                warnings */
    DUK_UNREF(ch);
    ...
    if (x < 0) {
      ...
      /*
       *  The escapes are same as outside a character class, except that \b has
       * a different meaning, and \B and backreferences are prohibited (see E5
      ...
      if (x == DUK_ASC_LC_B) {
         ...
         * '\B' is not allowed and is caught by the
         * duk_unicode_is_identifier_part() check below.
         */
        ch = 0x0008;
        ...
        duk__emit_u16_direct_ranges(
            lex_ctx, gen_range, userdata, duk_unicode_re_ranges_not_wordchar,
