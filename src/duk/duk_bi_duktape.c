...
DUK_INTERNAL duk_ret_t duk_bi_duktape_object_enc(duk_hthread* thr) {
  ...
  h_str = duk_require_hstring(
      thr,
      0); /* Could reject symbols, but no point: won't match comparisons. */
...
#  if defined(DUK_USE_JSON_SUPPORT) && defined(DUK_USE_JX)
    ...
    duk_bi_json_stringify_helper(
        thr, 1 /*idx_value*/, 2 /*idx_replacer*/, 3 /*idx_space*/,
...
DUK_INTERNAL duk_ret_t duk_bi_duktape_object_dec(duk_hthread* thr) {
  ...
  h_str = duk_require_hstring(
      thr, 0); /* Could reject symbols, but no point: won't match comparisons */
  duk_require_valid_index(thr, 1);
...
#  if defined(DUK_USE_JSON_SUPPORT) && defined(DUK_USE_JX)
    ...
    duk_bi_json_parse_helper(thr, 1 /*idx_value*/, 2 /*idx_replacer*/,
