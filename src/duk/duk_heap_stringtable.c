...
#  define DUK__HEAPPTR_ENC16(heap, ptr) \
    DUK_USE_HEAPPTR_ENC16((heap)->heap_udata, (ptr))
#  define DUK__HEAPPTR_DEC16(heap, val) \
    DUK_USE_HEAPPTR_DEC16((heap)->heap_udata, (val))
...
#  endif
  ...
  for (i = 0; i < heap->st_size; i++) {
    ...
    count_chain_max =
        (count_chain > count_chain_max ? count_chain : count_chain_max);
    count_chain_min =
        (count_chain < count_chain_min ? count_chain : count_chain_min);
  ...
  DUK_D(DUK_DPRINT(
      ...
      (double)count_total / (double)heap->st_size, (unsigned long)count_len[0],
      (unsigned long)count_len[1], (unsigned long)count_len[2],
      (unsigned long)count_len[3], (unsigned long)count_len[4],
      (unsigned long)count_len[5], (unsigned long)count_len[6],
...
#  endif
...
#endif /* DUK_USE_HSTRING_EXTDATA && DUK_USE_EXTSTR_INTERN_CHECK */
  {
    ...
    DUK_ASSERT(sizeof(duk_hstring) + blen + 1 >
               blen); /* No wrap, limits ensure. */
  ...
  if (dummy != DUK_HSTRING_NO_ARRAY_INDEX) {
    ...
    DUK_ASSERT(duk_unicode_unvalidated_utf8_length(data, (duk_size_t)blen) ==
               blen);
  } else {
  ...
  DUK_DDD(
      DUK_DDDPRINT("interned string, hash=0x%08lx, blen=%ld, has_arridx=%ld, "
                   "has_extdata=%ld",
...
#  endif
  ...
  DUK_DD(DUK_DDPRINT("grow in-place: %lu -> %lu", (unsigned long)heap->st_size,
...
#  if defined(DUK_USE_STRTAB_PTRCOMP)
  new_ptr = (duk_uint16_t*)DUK_REALLOC(heap, heap->strtable16,
                                       sizeof(duk_uint16_t) * new_st_size);
#  else
  new_ptr = (duk_hstring**)DUK_REALLOC(heap, heap->strtable,
                                       sizeof(duk_hstring*) * new_st_size);
#  endif
...
#  endif
  ...
  DUK_DD(DUK_DDPRINT("shrink in-place: %lu -> %lu",
...
#if defined(DUK__STRTAB_RESIZE_CHECK)
DUK_LOCAL DUK_COLD DUK_NOINLINE void duk__strtable_resize_check(
    duk_heap* heap) {
...
#  endif
  ...
  DUK_DD(
      DUK_DDPRINT("resize check string table: size=%lu, count=%lu, "
                  "load_factor=%lu (fixed point .4; float %lf)",
  ...
  if (load_factor >= DUK_USE_STRTAB_GROW_LIMIT) {
    if (heap->st_size >= DUK_USE_STRTAB_MAXSIZE) {
      DUK_DD(
          DUK_DDPRINT("want to grow strtable (based on load factor) but "
...
#  endif
    ...
    if (heap->st_size <= DUK_USE_STRTAB_MINSIZE) {
      DUK_DD(
          DUK_DDPRINT("want to shrink strtable (based on load factor) but "
                      "already minimum size"));
    } else {
...
DUK_LOCAL duk_hstring* duk__strtable_do_intern(duk_heap* heap,
  ...
  DUK_DDD(
      DUK_DDDPRINT("do_intern: heap=%p, str=%p, blen=%lu, strhash=%lx, "
                   "st_size=%lu, st_count=%lu, load=%lf",
                   (void*)heap, (const void*)str, (unsigned long)blen,
                   (unsigned long)strhash, (unsigned long)heap->st_size,
...
#if defined(DUK_USE_HSTRING_EXTDATA) && defined(DUK_USE_EXTSTR_INTERN_CHECK)
  extdata = (const duk_uint8_t*)DUK_USE_EXTSTR_INTERN_CHECK(
      heap->heap_udata, (void*)DUK_LOSE_CONST(str), (duk_size_t)blen);
#else
...
#endif
  DUK_ASSERT(res->hdr.h_next ==
             NULL); /* This is the case now, but unnecessary zeroing/NULLing. */
...
DUK_LOCAL duk_hstring* duk__strtab_romstring_lookup(duk_heap* heap,
                                                    const duk_uint8_t* str,
                                                    duk_size_t blen,
                                                    duk_uint32_t strhash) {
  ...
  while (curr != NULL) {
    ...
    if (strhash == DUK_HSTRING_GET_HASH(curr) &&
        ...
        duk_memcmp_unsafe((const void*)str,
                          (const void*)DUK_HSTRING_GET_DATA(curr), blen) == 0) {
      DUK_DDD(
          DUK_DDDPRINT("intern check: rom string: %!O, computed hash 0x%08lx, "
                       "rom hash 0x%08lx",
                       curr, (unsigned long)strhash,
...
DUK_INTERNAL duk_hstring* duk_heap_strtable_intern(duk_heap* heap,
  ...
  DUK_DDD(DUK_DDDPRINT("intern check: heap=%p, str=%p, blen=%lu", (void*)heap,
                       (const void*)str, (unsigned long)blen));
  ...
  DUK_ASSERT(
      blen <=
      DUK_HSTRING_MAX_BYTELEN); /* Caller is responsible for ensuring this. */
...
DUK_INTERNAL duk_hstring* duk_heap_strtable_intern_u32(duk_heap* heap,
  ...
  return duk_heap_strtable_intern(heap, (const duk_uint8_t*)p,
                                  (duk_uint32_t)((buf + sizeof(buf)) - p));
}
...
DUK_INTERNAL duk_hstring* duk_heap_strtable_intern_checked(
...
DUK_LOCAL duk_uint_t duk__strtable_litcache_key(const duk_uint8_t* str,
  ...
  key &= (duk_uintptr_t)(DUK_USE_LITCACHE_SIZE -
                         1); /* Assumes size is power of 2. */
  /* Due to masking, cast is in 32-bit range. */
  DUK_ASSERT(key <= DUK_UINT_MAX);
  return (duk_uint_t)key;
}

DUK_INTERNAL duk_hstring* duk_heap_strtable_intern_literal_checked(
    duk_hthread* thr, const duk_uint8_t* str, duk_uint32_t blen) {
  duk_uint_t key;
  ...
  if (ent->addr == str) {
    DUK_DD(
        DUK_DDPRINT("intern check for cached, pinned literal: str=%p, blen=%ld "
                    "-> duk_hstring %!O",
  ...
  if (!DUK_HSTRING_HAS_PINNED_LITERAL(h)) {
    DUK_DD(DUK_DDPRINT("pin duk_hstring because it is a literal: %!O",
                       (duk_heaphdr*)h));
...
DUK_INTERNAL duk_hstring* duk_heap_strtable_intern_u32_checked(
...
#  endif
  ...
  DUK_DDD(DUK_DDDPRINT(
      "remove: heap=%p, h=%p, blen=%lu, strhash=%lx", (void*)heap, (void*)h,
...
DUK_INTERNAL void duk_heap_strtable_unlink_prev(duk_heap* heap, duk_hstring* h,
                                                duk_hstring* prev) {
#if defined(DUK_USE_STRTAB_PTRCOMP)
  duk_uint16_t* slot;
#else
  ...
  DUK_DDD(
      DUK_DDDPRINT("remove: heap=%p, prev=%p, h=%p, blen=%lu, strhash=%lx",
