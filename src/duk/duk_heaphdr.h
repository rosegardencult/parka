...
#  if defined(DUK_USE_DOUBLE_LINKED_HEAP)
  /* refcounting requires direct heap frees, which in turn requires a dual
   * linked heap */
#    if defined(DUK_USE_HEAPPTR16)
...
#  define DUK_HEAPHDR_FLAG_REACHABLE \
    DUK_HEAPHDR_HEAP_FLAG(0) /* mark-and-sweep: reachable */
#  define DUK_HEAPHDR_FLAG_TEMPROOT \
    DUK_HEAPHDR_HEAP_FLAG(1) /* mark-and-sweep: children not processed */
#  define DUK_HEAPHDR_FLAG_FINALIZABLE                                        \
    DUK_HEAPHDR_HEAP_FLAG(2) /* mark-and-sweep: finalizable (on current pass) \
                              */
#  define DUK_HEAPHDR_FLAG_FINALIZED                                         \
    DUK_HEAPHDR_HEAP_FLAG(3) /* mark-and-sweep: finalized (on previous pass) \
                              */
#  define DUK_HEAPHDR_FLAG_READONLY \
    DUK_HEAPHDR_HEAP_FLAG(4) /* read-only object, in code section */
...
#      define DUK_HEAPHDR_SET_PREV(heap, h, val)                       \
        do {                                                           \
          (h)->h_prev16 =                                              \
              DUK_USE_HEAPPTR_ENC16((heap)->heap_udata, (void*)(val)); \
        } while (0)
#    else
...
#    define DUK_HEAPHDR_PREDEC_REFCOUNT(h) \
      (--(h)->h_refcount) /* result: updated refcount */
#  else
/* refcount macros not defined without refcounting, caller must #if defined()
 * now */
#  endif /* DUK_USE_REFERENCE_COUNTING */
...
#  define DUK_HEAPHDR_GET_FLAG_RANGE(h, m, n) \
    (((h)->h_flags >> (m)) & ((1UL << (n)) - 1UL))

#  define DUK_HEAPHDR_SET_FLAG_RANGE(h, m, n, v)                            \
    do {                                                                    \
      (h)->h_flags =                                                        \
          ((h)->h_flags & (~(((1UL << (n)) - 1UL) << (m)))) | ((v) << (m)); \
    } while (0)
