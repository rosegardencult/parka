...
DUK_LOCAL void duk__bw_update_ptrs(duk_hthread* thr, duk_bufwriter_ctx* bw_ctx,
  ...
  DUK_ASSERT(p != NULL || (DUK_HBUFFER_DYNAMIC_GET_SIZE(bw_ctx->buf) == 0 &&
                           curr_offset == 0 && new_length == 0));
  bw_ctx->p = p + curr_offset;
  bw_ctx->p_base = p;
  bw_ctx->p_limit = p + new_length;
}

DUK_INTERNAL void duk_bw_init(duk_hthread* thr, duk_bufwriter_ctx* bw_ctx,
...
DUK_INTERNAL duk_uint8_t* duk_bw_resize(duk_hthread* thr,
  ...
  DUK_DD(DUK_DDPRINT("resize bufferwriter from %ld to %ld (add_sz=%ld)",
                     (long)curr_off, (long)new_sz, (long)add_sz));
...
DUK_INTERNAL void duk_bw_write_raw_slice(duk_hthread* thr,
                                         ...
                                         duk_size_t src_off, duk_size_t len) {
  duk_uint8_t* p_base;
...
DUK_INTERNAL void duk_bw_insert_raw_slice(duk_hthread* thr,
  ...
  duk_memmove_unsafe((void*)(p_base + dst_off + len),
                     (const void*)(p_base + dst_off), (size_t)move_sz);
  duk_memcpy_unsafe((void*)(p_base + dst_off), (const void*)(p_base + src_off),
                    (size_t)len);
  bw->p += len;
}
...
DUK_INTERNAL void duk_bw_remove_raw_slice(duk_hthread* thr,
                                          duk_bufwriter_ctx* bw, duk_size_t off,
                                          duk_size_t len) {
  duk_size_t move_sz;
...
DUK_INTERNAL void duk_bw_assert_valid(duk_hthread* thr,
                                      duk_bufwriter_ctx* bw_ctx) {
  DUK_UNREF(thr);
  DUK_ASSERT(bw_ctx != NULL);
  DUK_ASSERT(bw_ctx->buf != NULL);
  DUK_ASSERT((DUK_HBUFFER_DYNAMIC_GET_SIZE(bw_ctx->buf) == 0) ||
             (bw_ctx->p != NULL && bw_ctx->p_base != NULL &&
              bw_ctx->p_limit != NULL && bw_ctx->p_limit >= bw_ctx->p_base &&
