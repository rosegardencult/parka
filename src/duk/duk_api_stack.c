...
DUK_LOCAL_DECL duk_idx_t duk__push_c_function_raw(duk_hthread* thr,
                                                  duk_c_function func,
                                                  duk_idx_t nargs,
                                                  duk_uint_t flags,
...
DUK_LOCAL const duk_uint_t duk__type_from_tag[] = {
    ...
    DUK_TYPE_UNDEFINED, DUK_TYPE_NULL,      DUK_TYPE_BOOLEAN,
    DUK_TYPE_POINTER,   DUK_TYPE_LIGHTFUNC, DUK_TYPE_NONE,
    DUK_TYPE_STRING,    DUK_TYPE_OBJECT,    DUK_TYPE_BUFFER,
};
DUK_LOCAL const duk_uint_t duk__type_mask_from_tag[] = {
    ...
    DUK_TYPE_MASK_POINTER,   DUK_TYPE_MASK_LIGHTFUNC, DUK_TYPE_MASK_NONE,
    DUK_TYPE_MASK_STRING,    DUK_TYPE_MASK_OBJECT,    DUK_TYPE_MASK_BUFFER,
};
...
DUK_LOCAL_DECL duk_heaphdr* duk__get_tagged_heaphdr_raw(duk_hthread* thr,
                                                        duk_idx_t idx,
                                                        duk_uint_t tag);

DUK_LOCAL duk_int_t duk__api_coerce_d2i(duk_hthread* thr, duk_idx_t idx,
                                        duk_int_t def_value,
                                        duk_bool_t require) {
  duk_tval* tv;
  duk_small_int_t c;
...
DUK_LOCAL duk_uint_t duk__api_coerce_d2ui(duk_hthread* thr, duk_idx_t idx,
 ...
 */
DUK_INTERNAL void duk_set_top_and_wipe(duk_hthread* thr, duk_idx_t top,
 ...
 */
DUK_LOCAL DUK_COLD DUK_NOINLINE duk_bool_t
duk__resize_valstack(duk_hthread* thr, duk_size_t new_size) {
  ...
  DUK_ASSERT((duk_size_t)(thr->valstack_top - thr->valstack) <=
             new_size);                           /* can't resize below 'top' */
  DUK_ASSERT(new_size <= DUK_USE_VALSTACK_LIMIT); /* valstack limit caller has
                                                     check, prevents wrapping */
  DUK_ASSERT(new_size <=
             DUK_SIZE_MAX /
                 sizeof(duk_tval)); /* specific assert for wrapping */
  ...
  new_valstack = (duk_tval*)DUK_REALLOC_INDIRECT(
      thr->heap, duk_hthread_get_valstack_ptr, (void*)thr, new_alloc_size);
   ...
   */
  ptr_diff =
      (duk_ptrdiff_t)((duk_uint8_t*)new_valstack - (duk_uint8_t*)thr->valstack);
  tv_prev_alloc_end =
      (duk_tval*)(void*)((duk_uint8_t*)thr->valstack_alloc_end + ptr_diff);
  thr->valstack = new_valstack;
  thr->valstack_bottom =
      (duk_tval*)(void*)((duk_uint8_t*)thr->valstack_bottom + ptr_diff);
  thr->valstack_top =
      (duk_tval*)(void*)((duk_uint8_t*)thr->valstack_top + ptr_diff);
  thr->valstack_end =
      (duk_tval*)(void*)((duk_uint8_t*)thr->valstack_end + ptr_diff);
  thr->valstack_alloc_end =
      (duk_tval*)(void*)((duk_uint8_t*)new_valstack + new_alloc_size);
  ...
  DUK_D(DUK_DPRINT(
      ...
      (unsigned long)(pre_alloc_end - pre_valstack), (unsigned long)new_size,
      (unsigned long)((duk_uint8_t*)pre_alloc_end - (duk_uint8_t*)pre_valstack),
      (unsigned long)new_alloc_size, (void*)pre_valstack, (void*)thr->valstack,
      (void*)pre_bottom, (void*)thr->valstack_bottom,
      (long)(thr->valstack_bottom - thr->valstack), (void*)pre_top,
      (void*)thr->valstack_top, (long)(thr->valstack_top - thr->valstack),
      (void*)pre_end, (void*)thr->valstack_end,
      (long)(thr->valstack_end - thr->valstack), (void*)pre_alloc_end,
      (void*)thr->valstack_alloc_end,
      (long)(thr->valstack_alloc_end - thr->valstack), (void*)tv_prev_alloc_end,
...
DUK_LOCAL DUK_COLD DUK_NOINLINE duk_bool_t duk__valstack_grow(
    duk_hthread* thr, duk_size_t min_bytes, duk_bool_t throw_on_error) {
  duk_size_t min_size;
  duk_size_t new_size;
  ...
  if (DUK_UNLIKELY(new_size > DUK_USE_VALSTACK_LIMIT ||
                   new_size < min_size /*wrap*/)) {
    /* Note: may be triggered even if minimal new_size would not reach the
     * limit, plan limit accordingly.
     */
    if (throw_on_error) {
 ...
 */
DUK_INTERNAL DUK_INLINE void duk_valstack_grow_check_throw(
    duk_hthread* thr, duk_size_t min_bytes) {
  duk_tval* tv;
...
DUK_INTERNAL DUK_INLINE duk_bool_t
...
DUK_INTERNAL void duk_valstack_shrink_check_nothrow(duk_hthread* thr,
  ...
  alloc_bytes = (duk_size_t)((duk_uint8_t*)thr->valstack_alloc_end -
                             (duk_uint8_t*)thr->valstack);
  reserve_bytes = (duk_size_t)((duk_uint8_t*)thr->valstack_end -
                               (duk_uint8_t*)thr->valstack);
...
#  endif
  ...
  DUK_D(
      DUK_DPRINT("valstack shrink check: alloc_bytes=%ld, reserve_bytes=%ld, "
                 "shrink_bytes=%ld (unvalidated)",
  ...
  DUK_ASSERT(shrink_bytes / sizeof(duk_tval) * sizeof(duk_tval) ==
             shrink_bytes);

  DUK_D(DUK_DPRINT("valstack shrink check: decided to shrink, snug: %ld",
                   (long)snug));
...
DUK_EXTERNAL duk_bool_t duk_check_stack_top(duk_hthread* thr, duk_idx_t top) {
  ...
  DUK_ASSERT(top >= 0);
  min_new_bytes =
      (duk_size_t)((duk_uint8_t*)thr->valstack_bottom -
                   (duk_uint8_t*)thr->valstack) +
...
DUK_INTERNAL void duk_insert_undefined_n(duk_hthread* thr, duk_idx_t idx,
                                         duk_idx_t count) {
  duk_tval *tv, *tv_end;
...
DUK_EXTERNAL void duk_copy(duk_hthread* thr, duk_idx_t from_idx,
                           duk_idx_t to_idx) {
  duk_tval* tv1;
  duk_tval* tv2;
...
DUK_EXTERNAL void duk_remove(duk_hthread* thr, duk_idx_t idx) {
  ...
  nbytes = (duk_size_t)(((duk_uint8_t*)q) -
                        ((duk_uint8_t*)p)); /* Note: 'q' is top-1 */
  duk_memmove((void*)p, (const void*)(p + 1), (size_t)nbytes);
...
DUK_INTERNAL void duk_remove_n(duk_hthread* thr, duk_idx_t idx,
                               duk_idx_t count) {
#if defined(DUK_USE_PREFER_SIZE)
...
DUK_INTERNAL void duk_remove_n_unsafe(duk_hthread* thr, duk_idx_t idx,
                                      duk_idx_t count) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL void duk_xcopymove_raw(duk_hthread* to_thr, duk_hthread* from_thr,
                                    duk_idx_t count, duk_bool_t is_copy) {
  void* src;
  duk_size_t nbytes;
  duk_tval* p;
  duk_tval* q;
  ...
  if (DUK_UNLIKELY((duk_size_t)((duk_uint8_t*)to_thr->valstack_end -
                                (duk_uint8_t*)to_thr->valstack_top) < nbytes)) {
 ...
 */
DUK_INTERNAL duk_tval* duk_reserve_gap(duk_hthread* thr, duk_idx_t idx_base,
                                       duk_idx_t count) {
  duk_tval* tv_src;
  duk_tval* tv_dst;
  ...
  DUK_ASSERT_API_ENTRY(thr);
  DUK_ASSERT(count >= 0);
  DUK_ASSERT((duk_size_t)(thr->valstack_end - thr->valstack_top) >=
             (duk_size_t)count);
  ...
  copy_bytes =
      (duk_size_t)((duk_uint8_t*)thr->valstack_top - (duk_uint8_t*)tv_src);
  thr->valstack_top =
      (duk_tval*)(void*)((duk_uint8_t*)thr->valstack_top + gap_bytes);
...
DUK_LOCAL DUK_ALWAYS_INLINE duk_bool_t
duk__get_boolean_raw(duk_hthread* thr, duk_idx_t idx, duk_bool_t def_value) {
  duk_bool_t ret;
  duk_tval* tv;
...
DUK_EXTERNAL duk_bool_t duk_get_boolean_default(duk_hthread* thr, duk_idx_t idx,
...
DUK_EXTERNAL duk_bool_t duk_opt_boolean(duk_hthread* thr, duk_idx_t idx,
                                        duk_bool_t def_value) {
  DUK_ASSERT_API_ENTRY(thr);

  if (duk_check_type_mask(thr, idx,
                          DUK_TYPE_MASK_NONE | DUK_TYPE_MASK_UNDEFINED)) {
    return def_value;
  }
  return duk_require_boolean(thr, idx);
}

DUK_LOCAL DUK_ALWAYS_INLINE duk_double_t
duk__get_number_raw(duk_hthread* thr, duk_idx_t idx, duk_double_t def_value) {
...
DUK_EXTERNAL duk_double_t duk_opt_number(duk_hthread* thr, duk_idx_t idx,
                                         duk_double_t def_value) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL duk_int_t duk_get_int(duk_hthread* thr, duk_idx_t idx) {
  ...
  return (duk_int_t)duk__api_coerce_d2i(thr, idx, 0 /*def_value*/,
                                        0 /*require*/);
}
...
DUK_EXTERNAL duk_uint_t duk_get_uint(duk_hthread* thr, duk_idx_t idx) {
  ...
  return (duk_uint_t)duk__api_coerce_d2ui(thr, idx, 0 /*def_value*/,
                                          0 /*require*/);
}

DUK_EXTERNAL duk_int_t duk_get_int_default(duk_hthread* thr, duk_idx_t idx,
                                           duk_int_t def_value) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL duk_uint_t duk_get_uint_default(duk_hthread* thr, duk_idx_t idx,
                                             duk_uint_t def_value) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL duk_int_t duk_opt_int(duk_hthread* thr, duk_idx_t idx,
                                   duk_int_t def_value) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL duk_uint_t duk_opt_uint(duk_hthread* thr, duk_idx_t idx,
                                     duk_uint_t def_value) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL const char* duk_get_lstring(duk_hthread* thr, duk_idx_t idx,
                                         duk_size_t* out_len) {
  duk_hstring* h;
  const char* ret;
  duk_size_t len;
...
DUK_EXTERNAL const char* duk_require_lstring(duk_hthread* thr, duk_idx_t idx,
                                             duk_size_t* out_len) {
  duk_hstring* h;
...
DUK_EXTERNAL const char* duk_opt_lstring(duk_hthread* thr, duk_idx_t idx,
                                         duk_size_t* out_len,
                                         const char* def_ptr,
                                         duk_size_t def_len) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL const char* duk_opt_string(duk_hthread* thr, duk_idx_t idx,
                                        const char* def_ptr) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL const char* duk_get_string_default(duk_hthread* thr, duk_idx_t idx,
...
DUK_LOCAL void* duk__get_pointer_raw(duk_hthread* thr, duk_idx_t idx,
                                     void* def_value) {
  duk_tval* tv;
  void* p;
...
DUK_EXTERNAL void* duk_opt_pointer(duk_hthread* thr, duk_idx_t idx,
                                   void* def_value) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL void* duk_get_pointer_default(duk_hthread* thr, duk_idx_t idx,
                                           void* def_value) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_LOCAL void* duk__get_buffer_helper(duk_hthread* thr, duk_idx_t idx,
                                       duk_size_t* out_size, void* def_ptr,
                                       duk_size_t def_size,
...
DUK_EXTERNAL void* duk_get_buffer(duk_hthread* thr, duk_idx_t idx,
                                  duk_size_t* out_size) {
  DUK_ASSERT_API_ENTRY(thr);

  return duk__get_buffer_helper(thr, idx, out_size, NULL /*def_ptr*/,
                                0 /*def_size*/, 0 /*throw_flag*/);
}

DUK_EXTERNAL void* duk_opt_buffer(duk_hthread* thr, duk_idx_t idx,
                                  duk_size_t* out_size, void* def_ptr,
                                  duk_size_t def_size) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL void* duk_get_buffer_default(duk_hthread* thr, duk_idx_t idx,
                                          duk_size_t* out_size, void* def_ptr,
                                          duk_size_t def_len) {
  DUK_ASSERT_API_ENTRY(thr);

  return duk__get_buffer_helper(thr, idx, out_size, def_ptr, def_len,
                                0 /*throw_flag*/);
}

DUK_EXTERNAL void* duk_require_buffer(duk_hthread* thr, duk_idx_t idx,
                                      duk_size_t* out_size) {
  DUK_ASSERT_API_ENTRY(thr);

  return duk__get_buffer_helper(thr, idx, out_size, NULL /*def_ptr*/,
                                0 /*def_size*/, 1 /*throw_flag*/);
}
 ...
 */
DUK_INTERNAL void* duk_get_buffer_data_raw(duk_hthread* thr, duk_idx_t idx,
                                           duk_size_t* out_size, void* def_ptr,
                                           duk_size_t def_size,
                                           duk_bool_t throw_flag,
  ...
  if (DUK_TVAL_IS_BUFFER(tv)) {
    ...
    return (void*)DUK_HBUFFER_GET_DATA_PTR(
        thr->heap, h); /* may be NULL (but only if size is 0) */
  }
...
DUK_EXTERNAL void* duk_get_buffer_data(duk_hthread* thr, duk_idx_t idx,
                                       duk_size_t* out_size) {
  DUK_ASSERT_API_ENTRY(thr);
  return duk_get_buffer_data_raw(thr, idx, out_size, NULL /*def_ptr*/,
                                 0 /*def_size*/, 0 /*throw_flag*/, NULL);
}

DUK_EXTERNAL void* duk_get_buffer_data_default(duk_hthread* thr, duk_idx_t idx,
                                               duk_size_t* out_size,
                                               void* def_ptr,
                                               duk_size_t def_size) {
  DUK_ASSERT_API_ENTRY(thr);
  return duk_get_buffer_data_raw(thr, idx, out_size, def_ptr, def_size,
                                 0 /*throw_flag*/, NULL);
}

DUK_EXTERNAL void* duk_opt_buffer_data(duk_hthread* thr, duk_idx_t idx,
                                       duk_size_t* out_size, void* def_ptr,
                                       duk_size_t def_size) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL void* duk_require_buffer_data(duk_hthread* thr, duk_idx_t idx,
                                           duk_size_t* out_size) {
  DUK_ASSERT_API_ENTRY(thr);
  return duk_get_buffer_data_raw(thr, idx, out_size, NULL /*def_ptr*/,
                                 0 /*def_size*/, 1 /*throw_flag*/, NULL);
}
...
DUK_INTERNAL duk_hcompfunc* duk_require_hcompfunc(duk_hthread* thr,
  ...
  h = (duk_hobject*)duk__get_tagged_heaphdr_raw(thr, idx, DUK_TAG_OBJECT);
  if (DUK_UNLIKELY(!(h != NULL && DUK_HOBJECT_IS_COMPFUNC(h)))) {
    DUK_ERROR_REQUIRE_TYPE_INDEX(thr, idx, "compiledfunction",
...
DUK_INTERNAL duk_hnatfunc* duk_require_hnatfunc(duk_hthread* thr,
  ...
  h = (duk_hobject*)duk__get_tagged_heaphdr_raw(thr, idx, DUK_TAG_OBJECT);
  if (DUK_UNLIKELY(!(h != NULL && DUK_HOBJECT_IS_NATFUNC(h)))) {
    DUK_ERROR_REQUIRE_TYPE_INDEX(thr, idx, "nativefunction",
                                 DUK_STR_NOT_NATFUNC);
    DUK_WO_NORETURN(return NULL;);
  }
  return (duk_hnatfunc*)h;
...
DUK_EXTERNAL duk_c_function duk_opt_c_function(duk_hthread* thr, duk_idx_t idx,
...
DUK_EXTERNAL duk_c_function duk_get_c_function_default(
    duk_hthread* thr, duk_idx_t idx, duk_c_function def_value) {
  duk_c_function ret;
...
DUK_EXTERNAL void duk_require_constructable(duk_hthread* thr, duk_idx_t idx) {
  ...
  if (DUK_UNLIKELY(h != NULL && !DUK_HOBJECT_HAS_CONSTRUCTABLE(h))) {
    DUK_ERROR_REQUIRE_TYPE_INDEX(thr, idx, "constructable",
...
DUK_EXTERNAL duk_hthread* duk_opt_context(duk_hthread* thr, duk_idx_t idx,
...
DUK_EXTERNAL void* duk_opt_heapptr(duk_hthread* thr, duk_idx_t idx,
                                   void* def_value) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL void* duk_get_heapptr_default(duk_hthread* thr, duk_idx_t idx,
                                           void* def_value) {
  void* ret;
...
DUK_EXTERNAL void* duk_require_heapptr(duk_hthread* thr, duk_idx_t idx) {
  ...
  DUK_ASSERT(tv != NULL);
  if (DUK_UNLIKELY(!DUK_TVAL_IS_HEAP_ALLOCATED(tv))) {
    DUK_ERROR_REQUIRE_TYPE_INDEX(thr, idx, "heapobject",
...
DUK_INTERNAL duk_hobject* duk_get_hobject_promote_mask(duk_hthread* thr,
                                                       ...
                                                       duk_uint_t type_mask) {
  DUK_ASSERT_API_ENTRY(thr);
  return duk__get_hobject_promote_mask_raw(thr, idx,
                                           type_mask | DUK_TYPE_MASK_PROMOTE);
}

/* Like duk_get_hobject_promote_mask() but throw a TypeError instead of
 * returning a NULL.
 */
DUK_INTERNAL duk_hobject* duk_require_hobject_promote_mask(
    duk_hthread* thr, duk_idx_t idx, duk_uint_t type_mask) {
  DUK_ASSERT_API_ENTRY(thr);
  return duk__get_hobject_promote_mask_raw(
      thr, idx, type_mask | DUK_TYPE_MASK_THROW | DUK_TYPE_MASK_PROMOTE);
}

/* Require a duk_hobject * at 'idx'; if the value is not an object but matches
 * the supplied 'type_mask', return a NULL instead.  Otherwise throw a
 * TypeError.
 */
DUK_INTERNAL duk_hobject* duk_require_hobject_accept_mask(
    duk_hthread* thr, duk_idx_t idx, duk_uint_t type_mask) {
  DUK_ASSERT_API_ENTRY(thr);
  return duk__get_hobject_promote_mask_raw(thr, idx,
                                           type_mask | DUK_TYPE_MASK_THROW);
}

DUK_INTERNAL duk_hobject* duk_get_hobject_with_class(
    duk_hthread* thr, duk_idx_t idx, duk_small_uint_t classnum) {
  duk_hobject* h;
...
DUK_INTERNAL duk_hobject* duk_require_hobject_with_class(
    duk_hthread* thr, duk_idx_t idx, duk_small_uint_t classnum) {
  duk_hobject* h;
  ...
  h = (duk_hobject*)duk__get_tagged_heaphdr_raw(thr, idx, DUK_TAG_OBJECT);
  if (DUK_UNLIKELY(
          !(h != NULL && DUK_HOBJECT_GET_CLASS_NUMBER(h) == classnum))) {
    duk_hstring* h_class;
    h_class = DUK_HTHREAD_GET_STRING(
        thr, DUK_HOBJECT_CLASS_NUMBER_TO_STRIDX(classnum));
    DUK_UNREF(h_class);

    DUK_ERROR_REQUIRE_TYPE_INDEX(thr, idx,
...
DUK_EXTERNAL void duk_set_length(duk_hthread* thr, duk_idx_t idx,
                                 duk_size_t len) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_LOCAL duk_bool_t duk__defaultvalue_coerce_attempt(
    duk_hthread* thr, duk_idx_t idx, duk_small_uint_t func_stridx) {
...
/* E5 Section 9.1 */
DUK_LOCAL const char* const duk__toprim_hint_strings[3] = {"default", "string",
                                                           "number"};
DUK_LOCAL void duk__to_primitive_helper(duk_hthread* thr, duk_idx_t idx,
                                        duk_int_t hint,
  ...
  DUK_ASSERT_API_ENTRY(thr);
  DUK_ASSERT(hint == DUK_HINT_NONE || hint == DUK_HINT_NUMBER ||
  ...
  if (!duk_check_type_mask(thr, idx,
                           DUK_TYPE_MASK_OBJECT | DUK_TYPE_MASK_LIGHTFUNC |
                               DUK_TYPE_MASK_BUFFER)) {
    DUK_ASSERT(
        !duk_is_buffer(thr, idx)); /* duk_to_string() relies on this behavior */
    return;
  }
   ...
   * which mimic objects.
   */
  if (check_symbol && duk_get_method_stridx(
                          thr, idx, DUK_STRIDX_WELLKNOWN_SYMBOL_TO_PRIMITIVE)) {
    DUK_ASSERT(hint >= 0 &&
               (duk_size_t)hint <
                   sizeof(duk__toprim_hint_strings) / sizeof(const char*));
    duk_dup(thr, idx);
...
DUK_EXTERNAL void duk_to_primitive(duk_hthread* thr, duk_idx_t idx,
                                   duk_int_t hint) {
  duk__to_primitive_helper(thr, idx, hint, 1 /*check_symbol*/);
}

#if defined(DUK_USE_SYMBOL_BUILTIN)
DUK_INTERNAL void duk_to_primitive_ordinary(duk_hthread* thr, duk_idx_t idx,
                                            duk_int_t hint) {
...
DUK_EXTERNAL duk_double_t duk_to_number(duk_hthread* thr, duk_idx_t idx) {
  ...
  d = duk_js_tonumber(
      thr,
      tv); /* XXX: fastint coercion? now result will always be a non-fastint */
...
DUK_LOCAL duk_double_t duk__to_int_uint_helper(duk_hthread* thr, duk_idx_t idx,
  ...
  /* Relookup in case coerce_func() has side effects, e.g. ends up coercing an
   * object */
  tv = duk_require_tval(thr, idx);
  DUK_TVAL_SET_NUMBER_UPDREF(thr, tv, d); /* side effects */
  return d;
...
DUK_INTERNAL duk_uint8_t duk_to_uint8clamped(duk_hthread* thr, duk_idx_t idx) {
  ...
  if (duk_double_equals(t, 0.5)) {
    ...
    ret = (duk_uint8_t)d;
    ret = (ret + 1) &
          0xfe; /* Example: d=3.5, t=0.5 -> ret = (3 + 1) & 0xfe = 4 & 0xfe = 4
...
DUK_EXTERNAL const char* duk_to_lstring(duk_hthread* thr, duk_idx_t idx,
                                        duk_size_t* out_len) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL const char* duk_safe_to_lstring(duk_hthread* thr, duk_idx_t idx,
                                             duk_size_t* out_len) {
  DUK_ASSERT_API_ENTRY(thr);
  ...
  duk_dup(thr, idx);
  (void)duk_safe_call(thr, duk__safe_to_string_raw, NULL /*udata*/, 1 /*nargs*/,
                      1 /*nrets*/);
  if (!duk_is_string(thr, -1)) {
    /* Error: try coercing error to string once. */
    (void)duk_safe_call(thr, duk__safe_to_string_raw, NULL /*udata*/,
                        1 /*nargs*/, 1 /*nrets*/);
    if (!duk_is_string(thr, -1)) {
      /* Double error */
      duk_pop_unsafe(thr);
...
DUK_EXTERNAL const char* duk_safe_to_stacktrace(duk_hthread* thr,
  ...
  duk_dup(thr, idx);
  rc = duk_safe_call(thr, duk__safe_to_stacktrace_raw, NULL /*udata*/,
                     1 /*nargs*/, 1 /*nrets*/);
  if (rc != 0) {
...
DUK_INTERNAL duk_hstring* duk_to_property_key_hstring(duk_hthread* thr,
  ...
  duk_to_primitive(thr, idx,
                   DUK_HINT_STRING); /* needed for e.g. Symbol objects */
  h = duk_get_hstring(thr, idx);
  if (h == NULL) {
...
DUK_INTERNAL void duk_push_class_string_tval(duk_hthread* thr, duk_tval* tv,
  ...
  if (duk_js_isarray_hobject(h_obj)) {
    ...
    if (!avoid_side_effects) {
      (void)duk_get_prop_stridx(thr, -1,
...
DUK_INTERNAL duk_int_t duk_to_int_clamped_raw(duk_hthread* thr, duk_idx_t idx,
                                              duk_int_t minval,
                                              duk_int_t maxval,
...
DUK_INTERNAL duk_int_t duk_to_int_clamped(duk_hthread* thr, duk_idx_t idx,
                                          duk_idx_t minval, duk_idx_t maxval) {
  duk_bool_t dummy;
...
DUK_INTERNAL duk_int_t duk_to_int_check_range(duk_hthread* thr, duk_idx_t idx,
                                              duk_int_t minval,
                                              duk_int_t maxval) {
  DUK_ASSERT_API_ENTRY(thr);
  return duk_to_int_clamped_raw(
      thr, idx, minval, maxval,
      NULL); /* out_clamped==NULL -> RangeError if outside range */
}
...
DUK_EXTERNAL const char* duk_to_string(duk_hthread* thr, duk_idx_t idx) {
  ...
  switch (DUK_TVAL_GET_TAG(tv)) {
    ...
    case DUK_TAG_BUFFER: /* Go through Uint8Array.prototype.toString() for
                            coercion. */
    case DUK_TAG_OBJECT: {
    ...
    default: {
      ...
      duk_push_tval(thr, tv);
      duk_numconv_stringify(thr, 10 /*radix*/, 0 /*precision:shortest*/,
                            0 /*force_exponential*/);
      break;
    }
  }
...
DUK_EXTERNAL void* duk_to_buffer_raw(duk_hthread* thr, duk_idx_t idx,
                                     duk_size_t* out_size, duk_uint_t mode) {
  duk_hbuffer* h_buf;
  ...
  if (h_buf != NULL) {
    ...
    tmp = (DUK_HBUFFER_HAS_DYNAMIC(h_buf) ? DUK_BUF_MODE_DYNAMIC
                                          : DUK_BUF_MODE_FIXED);
  ...
  dst_data = (duk_uint8_t*)duk_push_buffer(
      thr, src_size, (mode == DUK_BUF_MODE_DYNAMIC) /*dynamic*/);
...
DUK_LOCAL void duk__push_func_from_lightfunc(duk_hthread* thr,
  ...
  flags = DUK_HOBJECT_FLAG_EXTENSIBLE | DUK_HOBJECT_FLAG_CONSTRUCTABLE |
          DUK_HOBJECT_FLAG_CALLABLE | DUK_HOBJECT_FLAG_FASTREFS |
          DUK_HOBJECT_FLAG_NATFUNC | DUK_HOBJECT_FLAG_NEWENV |
  ...
  if ((duk_idx_t)lf_len != nargs) {
    ...
    duk_xdef_prop_stridx_short(thr, -2, DUK_STRIDX_LENGTH,
...
DUK_LOCAL duk_bool_t duk__tag_check(duk_hthread* thr, duk_idx_t idx,
...
#  endif
  ...
  DUK_ASSERT(sizeof(duk__type_from_tag) / sizeof(duk_uint_t) ==
             DUK_TAG_MAX - DUK_TAG_MIN + 1);
...
DUK_LOCAL const char* const duk__type_names[] = {
    "none",   "undefined", "null",   "boolean", "number",
...
DUK_INTERNAL const char* duk_get_type_name(duk_hthread* thr, duk_idx_t idx) {
  ...
  DUK_ASSERT(DUK_TYPE_MIN == 0 &&
             sizeof(duk__type_names) / sizeof(const char*) == DUK_TYPE_MAX + 1);
...
DUK_EXTERNAL duk_bool_t duk_check_type(duk_hthread* thr, duk_idx_t idx,
                                       duk_int_t type) {
  DUK_ASSERT_API_ENTRY(thr);
...
#else  /* DUK_USE_PACKED_TVAL */
  DUK_ASSERT(DUK_TVAL_IS_VALID_TAG(tv));
  DUK_ASSERT(sizeof(duk__type_mask_from_tag) / sizeof(duk_uint_t) ==
             DUK_TAG_MAX - DUK_TAG_MIN + 1);
...
DUK_EXTERNAL duk_bool_t duk_check_type_mask(duk_hthread* thr, duk_idx_t idx,
                                            duk_uint_t mask) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL duk_bool_t duk_is_thread(duk_hthread* thr, duk_idx_t idx) {
  ...
  obj = duk_get_hobject(thr, idx);
  if (obj) {
    return (DUK_HOBJECT_GET_CLASS_NUMBER(obj) == DUK_HOBJECT_CLASS_THREAD ? 1
                                                                          : 0);
  }
  return 0;
}
...
DUK_EXTERNAL const char* duk_push_lstring(duk_hthread* thr, const char* str,
                                          duk_size_t len) {
  duk_hstring* h;
  duk_tval* tv_slot;
  ...
  h = duk_heap_strtable_intern_checked(thr, (const duk_uint8_t*)str,
                                       (duk_uint32_t)len);
  DUK_ASSERT(h != NULL);
...
DUK_EXTERNAL const char* duk_push_literal_raw(duk_hthread* thr, const char* str,
                                              duk_size_t len) {
  duk_hstring* h;
  duk_tval* tv_slot;
  ...
  h = duk_heap_strtable_intern_literal_checked(thr, (const duk_uint8_t*)str,
                                               (duk_uint32_t)len);
  DUK_ASSERT(h != NULL);
...
DUK_INTERNAL duk_hstring* duk_push_uint_to_hstring(duk_hthread* thr,
  ...
  /* XXX: this could be a direct DUK_SPRINTF to a buffer followed by
   * duk_push_string() */
...
DUK_LOCAL void duk__push_this_helper(duk_hthread* thr,
  ...
  DUK_ASSERT(DUK_TVAL_IS_UNDEFINED(
      thr->valstack_top)); /* because of valstack init policy */
  tv_slot = thr->valstack_top++;
...
DUK_INTERNAL duk_hstring* duk_push_this_coercible_to_string(duk_hthread* thr) {
  ...
  duk__push_this_helper(thr, 1 /*check_object_coercible*/);
  return duk_to_hstring_m1(
      thr); /* This will reject all Symbol values; accepts Symbol objects. */
}
...
DUK_INTERNAL duk_tval* duk_get_borrowed_this_tval(duk_hthread* thr) {
  ...
  DUK_ASSERT(thr->valstack_bottom - 1 >=
             thr->valstack); /* 'this' binding exists */
...
DUK_LOCAL void duk__push_stash(duk_hthread* thr) {
  if (!duk_xget_owndataprop_stridx_short(thr, -1, DUK_STRIDX_INT_VALUE)) {
    ...
    duk_xdef_prop_stridx_short(
        thr, -3, DUK_STRIDX_INT_VALUE,
        DUK_PROPDESC_FLAGS_C); /* [ ... parent stash stash ] -> [ ... parent
                                  stash ] */
  }
  duk_remove_m2(thr);
}
...
DUK_LOCAL duk_int_t duk__try_push_vsprintf(duk_hthread* thr, void* buf,
                                           duk_size_t sz, const char* fmt,
                                           va_list ap) {
  duk_int_t len;
...
DUK_EXTERNAL const char* duk_push_vsprintf(duk_hthread* thr, const char* fmt,
                                           va_list ap) {
   ...
   */
  res = duk_push_lstring(thr, (const char*)buf,
                         (duk_size_t)len); /* [ buf? res ] */
  if (pushed_buf) {
    duk_remove_m2(thr);
  }
  return res;
...
DUK_INTERNAL duk_hobject* duk_push_object_helper(
    duk_hthread* thr, duk_uint_t hobject_flags_and_class,
  ...
  DUK_DDD(DUK_DDDPRINT("created object with flags: 0x%08lx",
                       (unsigned long)h->hdr.h_flags));
  ...
  if (prototype_bidx >= 0) {
    DUK_HOBJECT_SET_PROTOTYPE_INIT_INCREF(thr, h,
                                          thr->builtins[prototype_bidx]);
  } else {
    DUK_ASSERT(prototype_bidx == -1);
...
DUK_INTERNAL duk_hobject* duk_push_object_helper_proto(
    duk_hthread* thr, duk_uint_t hobject_flags_and_class, duk_hobject* proto) {
  duk_hobject* h;
...
DUK_EXTERNAL duk_idx_t duk_push_array(duk_hthread* thr) {
  ...
  flags = DUK_HOBJECT_FLAG_EXTENSIBLE | DUK_HOBJECT_FLAG_FASTREFS |
          DUK_HOBJECT_FLAG_ARRAY_PART | DUK_HOBJECT_FLAG_EXOTIC_ARRAY |
...
#  endif
#endif
  DUK_DDD(DUK_DDDPRINT("created thread object with flags: 0x%08lx",
  ...
  /* default prototype */
  DUK_HOBJECT_SET_PROTOTYPE_INIT_INCREF(
      thr, (duk_hobject*)obj, obj->builtins[DUK_BIDX_THREAD_PROTOTYPE]);
...
DUK_INTERNAL duk_hcompfunc* duk_push_hcompfunc(duk_hthread* thr) {
  ...
  obj = duk_hcompfunc_alloc(
      thr, DUK_HOBJECT_FLAG_EXTENSIBLE | DUK_HOBJECT_FLAG_CALLABLE |
  ...
  DUK_DDD(DUK_DDDPRINT("created compiled function object with flags: 0x%08lx",
                       (unsigned long)obj->obj.hdr.h_flags));
  ...
  DUK_HOBJECT_SET_PROTOTYPE_INIT_INCREF(
      thr, (duk_hobject*)obj, thr->builtins[DUK_BIDX_FUNCTION_PROTOTYPE]);
...
DUK_INTERNAL duk_hboundfunc* duk_push_hboundfunc(duk_hthread* thr) {
  ...
  DUK__CHECK_SPACE();
  obj = duk_hboundfunc_alloc(
      thr->heap, DUK_HOBJECT_FLAG_EXTENSIBLE | DUK_HOBJECT_FLAG_BOUNDFUNC |
...
DUK_LOCAL duk_idx_t duk__push_c_function_raw(duk_hthread* thr,
                                             duk_c_function func,
                                             duk_idx_t nargs, duk_uint_t flags,
                                             duk_small_uint_t proto_bidx) {
  duk_hnatfunc* obj;
  duk_idx_t ret;
  duk_tval* tv_slot;
  ...
  DUK_DDD(DUK_DDDPRINT(
      "created native function object with flags: 0x%08lx, nargs=%ld",
  ...
  DUK_HOBJECT_SET_PROTOTYPE_INIT_INCREF(thr, (duk_hobject*)obj,
                                        thr->builtins[proto_bidx]);
  return ret;
...
DUK_EXTERNAL duk_idx_t duk_push_c_function(duk_hthread* thr,
   ...
   */
  return duk__push_c_function_raw(thr, func, nargs, flags,
                                  DUK_BIDX_NATIVE_FUNCTION_PROTOTYPE);
}
...
DUK_INTERNAL void duk_push_c_function_builtin_noconstruct(duk_hthread* thr,
  ...
  flags = DUK_HOBJECT_FLAG_EXTENSIBLE | DUK_HOBJECT_FLAG_CALLABLE |
          DUK_HOBJECT_FLAG_FASTREFS | DUK_HOBJECT_FLAG_NATFUNC |
          DUK_HOBJECT_FLAG_NEWENV | DUK_HOBJECT_FLAG_STRICT |
...
DUK_EXTERNAL duk_idx_t duk_push_c_lightfunc(duk_hthread* thr,
                                            duk_c_function func,
                                            duk_idx_t nargs, duk_idx_t length,
                                            duk_int_t magic) {
  duk_small_uint_t lf_flags;
  duk_tval* tv_slot;
  ...
  if (DUK_UNLIKELY(!(length >= DUK_LFUNC_LENGTH_MIN &&
                     length <= DUK_LFUNC_LENGTH_MAX))) {
    goto api_error;
  }
  if (DUK_UNLIKELY(
  ...
  lf_flags =
      DUK_LFUNC_FLAGS_PACK((duk_small_int_t)magic, (duk_small_uint_t)length,
...
#if defined(DUK_USE_BUFFEROBJECT_SUPPORT)
DUK_INTERNAL duk_hbufobj* duk_push_bufobj_raw(
    duk_hthread* thr, duk_uint_t hobject_flags_and_class,
    duk_small_int_t prototype_bidx) {
  duk_hbufobj* obj;
  duk_tval* tv_slot;
...
DUK_EXTERNAL void duk_push_buffer_object(duk_hthread* thr, duk_idx_t idx_buffer,
  ...
  if (sizeof(duk_size_t) != sizeof(duk_uint_t)) {
    if (DUK_UNLIKELY((duk_size_t)uint_offset != byte_offset ||
                     (duk_size_t)uint_length != byte_length)) {
      goto range_error;
    }
  }
  ...
  if (h_arraybuf != NULL &&              /* argument is an object */
      ...
      DUK_HOBJECT_GET_CLASS_NUMBER(h_arraybuf) ==
          DUK_HOBJECT_CLASS_ARRAYBUFFER /* argument is ArrayBuffer */) {
...
DUK_EXTERNAL duk_idx_t duk_push_error_object_va_raw(
    duk_hthread* thr, duk_errcode_t err_code, const char* filename,
  ...
  duk_err_augment_error_create(thr, thr, filename, line,
                               augment_flags); /* may throw an error */
#endif
...
DUK_EXTERNAL void* duk_push_buffer_raw(duk_hthread* thr, duk_size_t size,
...
#  endif
   ...
   * restriction.)
   */
  h_target = duk_require_hobject_promote_mask(
      thr, -2, DUK_TYPE_MASK_LIGHTFUNC | DUK_TYPE_MASK_BUFFER);
   ...
   */
  h_handler = duk_require_hobject_promote_mask(
  ...
  DUK_ASSERT(DUK_HOBJECT_GET_PROTOTYPE(thr->heap, (duk_hobject*)h_proxy) ==
             NULL);
  ...
  DUK_TVAL_SET_UNDEFINED(
      tv_slot); /* [ ... target handler ] -> [ ... proxy undefined ] */
...
#  if defined(DUK_USE_FINALIZER_SUPPORT)
  for (curr = thr->heap->finalize_list; curr != NULL;
...
#  endif
    ...
    for (curr = thr->heap->heap_allocated; curr != NULL;
         curr = DUK_HEAPHDR_GET_NEXT(thr->heap, curr)) {
      if (curr == h) {
        DUK_ASSERT(found == 0); /* Would indicate corrupted lists. */
        found = 1;
...
DUK_EXTERNAL duk_idx_t duk_push_heapptr(duk_hthread* thr, void* ptr) {
  ...
  if (DUK_UNLIKELY(DUK_HEAPHDR_HAS_FINALIZABLE((duk_heaphdr*)ptr))) {
     ...
     */
#if defined(DUK_USE_REFERENCE_COUNTING)
    DUK_ASSERT(DUK_HEAPHDR_GET_REFCOUNT(curr) >=
               1); /* Preincremented on finalize_list insert. */
...
DUK_EXTERNAL void duk_pop_n(duk_hthread* thr, duk_idx_t count) {
  ...
  if (DUK_UNLIKELY((duk_uidx_t)(thr->valstack_top - thr->valstack_bottom) <
                   (duk_uidx_t)count)) {
...
DUK_INTERNAL void duk_pack(duk_hthread* thr, duk_idx_t count) {
  ...
  DUK_ASSERT((duk_size_t)count <=
             DUK_SIZE_MAX / sizeof(duk_tval)); /* no wrapping */

  tv_dst = duk_push_harray_with_size_outptr(
      thr, (duk_uint32_t)count); /* XXX: uninitialized would be OK */
  ...
  duk_memcpy_unsafe((void*)tv_dst, (const void*)tv_src,
                    (size_t)count * sizeof(duk_tval));
  ...
  if (DUK_LIKELY(DUK_TVAL_IS_OBJECT(tv))) {
    ...
    if (DUK_LIKELY(DUK_HOBJECT_IS_ARRAY(h) &&
      ...
      if (DUK_UNLIKELY(len != h_arr->length ||
                       h_arr->length >
                           DUK_HOBJECT_GET_ASIZE((duk_hobject*)h_arr))) {
        goto skip_fast;
      }
...
#endif
  DUK_DDD(DUK_DDDPRINT("THROW ERROR (API): %!dT (after throw augment)",
                       (duk_tval*)duk_get_tval(thr, -1)));
...
DUK_EXTERNAL void duk_error_va_raw(duk_hthread* thr, duk_errcode_t err_code,
...
DUK_EXTERNAL void duk_error_raw(duk_hthread* thr, duk_errcode_t err_code,
                                const char* filename, duk_int_t line,
                                const char* fmt, ...) {
  va_list ap;
...
#if !defined(DUK_USE_VARIADIC_MACROS)
DUK_NORETURN(DUK_LOCAL_DECL void duk__throw_error_from_stash(
    duk_hthread* thr, duk_errcode_t err_code, const char* fmt, va_list ap));
...
DUK_EXTERNAL duk_ret_t duk_error_stash(duk_hthread* thr, duk_errcode_t err_code,
                                       const char* fmt, ...) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL duk_bool_t duk_equals(duk_hthread* thr, duk_idx_t idx1,
                                   duk_idx_t idx2) {
  duk_tval *tv1, *tv2;
...
DUK_EXTERNAL duk_bool_t duk_strict_equals(duk_hthread* thr, duk_idx_t idx1,
                                          duk_idx_t idx2) {
  duk_tval *tv1, *tv2;
...
DUK_EXTERNAL duk_bool_t duk_samevalue(duk_hthread* thr, duk_idx_t idx1,
                                      duk_idx_t idx2) {
  duk_tval *tv1, *tv2;
...
DUK_EXTERNAL duk_bool_t duk_instanceof(duk_hthread* thr, duk_idx_t idx1,
                                       duk_idx_t idx2) {
  duk_tval *tv1, *tv2;
...
DUK_INTERNAL void duk_push_lightfunc_tostring(duk_hthread* thr, duk_tval* tv) {
  ...
  DUK_TVAL_GET_LIGHTFUNC(
      tv, func, lf_flags); /* read before 'tv' potentially invalidated */
...
DUK_INTERNAL void duk_push_string_funcptr(duk_hthread* thr, duk_uint8_t* ptr,
                                          duk_size_t sz) {
...
DUK_LOCAL void duk__push_hstring_readable_unicode(duk_hthread* thr,
  ...
  duk_uint8_t
      buf[DUK_UNICODE_MAX_XUTF8_LENGTH * DUK__READABLE_SUMMARY_MAXCHARS +
          2 /*quotes*/ + 3 /*periods*/];
  duk_uint8_t* q;
  duk_ucodepoint_t cp;
  ...
  *q++ = (duk_uint8_t)DUK_ASC_SINGLEQUOTE;
  for (;;) {
    ...
    if (duk_unicode_decode_xutf8(thr, &p, p_start, p_end, &cp)) {
      if (cp < 0x20 || cp == 0x7f || cp == DUK_ASC_SINGLEQUOTE ||
          cp == DUK_ASC_BACKSLASH) {
  ...
  if (tv == NULL) {
    ...
    switch (DUK_TVAL_GET_TAG(tv)) {
      ...
      case DUK_TAG_OBJECT: {
        ...
        if (error_aware && duk_hobject_prototype_chain_contains(
                               thr, h, thr->builtins[DUK_BIDX_ERROR_PROTOTYPE],
                               1 /*ignore_loop*/)) {
           ...
           */
          duk_tval* tv_msg;
          tv_msg = duk_hobject_find_entry_tval_ptr_stridx(thr->heap, h,
                                                          DUK_STRIDX_MESSAGE);
          if (tv_msg != NULL && DUK_TVAL_IS_STRING(tv_msg)) {
            /* It's critical to avoid recursion so
             * only summarize a string .message.
             */
            duk__push_hstring_readable_unicode(thr, DUK_TVAL_GET_STRING(tv_msg),
...
DUK_INTERNAL void duk_copy_tvals_incref(duk_hthread* thr, duk_tval* tv_dst,
                                        duk_tval* tv_src, duk_size_t count) {
  duk_tval* tv;
