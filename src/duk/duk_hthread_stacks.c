...
DUK_INTERNAL void duk_hthread_catcher_unwind_norz(duk_hthread* thr,
  ...
  DUK_DDD(DUK_DDDPRINT("unwinding catch stack entry %p (lexenv check is done)",
                       (void*)cat));
...
DUK_INTERNAL void duk_hthread_catcher_unwind_nolexenv_norz(
  ...
  DUK_DDD(DUK_DDDPRINT(
      "unwinding catch stack entry %p (lexenv check is not done)", (void*)cat));
...
#if defined(DUK_USE_CACHE_CATCHER)
DUK_INTERNAL DUK_INLINE duk_catcher* duk_hthread_catcher_alloc(
    duk_hthread* thr) {
  duk_catcher* cat;
...
#if defined(DUK_USE_CACHE_ACTIVATION)
DUK_INTERNAL DUK_INLINE duk_activation* duk_hthread_activation_alloc(
    duk_hthread* thr) {
  duk_activation* act;
 ...
 */
DUK_INTERNAL duk_activation* duk_hthread_get_activation_for_level(
    duk_hthread* thr, duk_int_t level) {
  duk_activation* act;
