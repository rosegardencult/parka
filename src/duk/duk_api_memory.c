...
DUK_EXTERNAL void* duk_realloc_raw(duk_hthread* thr, void* ptr,
                                   duk_size_t size) {
  DUK_ASSERT_API_ENTRY(thr);
...
DUK_EXTERNAL void duk_gc(duk_hthread* thr, duk_uint_t flags) {
  ...
  DUK_ASSERT(DUK_GC_COMPACT ==
             DUK_MS_FLAG_EMERGENCY); /* Compact flag is 1:1 with emergency flag
                                        which forces compaction. */
