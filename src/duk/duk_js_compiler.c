...
#define DUK__FUNC_FLAG_GETSET \
  (1 << 1) /* Parsing an object literal getter/setter. */
#define DUK__FUNC_FLAG_METDEF \
  (1 << 2) /* Parsing an object literal method definition shorthand. */
#define DUK__FUNC_FLAG_PUSHNAME_PASS1 \
  (1 << 3) /* Push function name when creating template (first pass only). */
#define DUK__FUNC_FLAG_USE_PREVTOKEN                                          \
  (1 << 4) /* Use prev_token to start function parsing (workaround for object \
              literal). */
...
#define DUK__TOKEN_LBP_FLAG_NO_REGEXP \
  (1 << 5) /* regexp literal must not follow this token */
#define DUK__TOKEN_LBP_FLAG_TERMINATES \
  (1 << 6) /* terminates expression; e.g. post-increment/-decrement */
...
DUK_LOCAL void duk__convert_to_func_template(duk_compiler_ctx* comp_ctx) {
  ...
  DUK_DDD(DUK_DDDPRINT(
      "consts_count=%ld, funcs_count=%ld, code_size=%ld -> "
      "data_size=%ld*%ld + %ld*%ld + %ld = %ld",
      (long)consts_count, (long)funcs_count, (long)code_size,
      (long)consts_count, (long)sizeof(duk_tval), (long)funcs_count,
      (long)sizeof(duk_hobject*), (long)code_size, (long)data_size));
  ...
  for (i = 0; i < funcs_count; i++) {
    ...
    *p_func++ = h;
    ...
    DUK_DDD(
        DUK_DDDPRINT("inner function: %p -> %!iO", (void*)h, (duk_heaphdr*)h));
  }
  ...
  if (formals_length != (duk_size_t)h_res->nargs) {
     ...
     */
    DUK_DD(
        DUK_DDPRINT("keeping _Formals because of direct eval or explicit "
...
#define DUK__EMIT_FLAG_A_IS_SOURCE \
  (1 << 11) /* slot A is a source (default: target) */
#define DUK__EMIT_FLAG_B_IS_TARGET \
  (1 << 12) /* slot B is a target (default: source) */
#define DUK__EMIT_FLAG_C_IS_TARGET \
  (1 << 13) /* slot C is a target (default: source) */
#define DUK__EMIT_FLAG_BC_REGCONST (1 << 14) /* slots B and C are reg/const */
#define DUK__EMIT_FLAG_RESERVE_JUMPSLOT                                        \
  (1 << 15) /* reserve a jumpslot after instr before target spilling, used for \
               NEXTENUM */
...
DUK_LOCAL void duk__emit(duk_compiler_ctx* comp_ctx, duk_instr_t ins) {
  ...
  DUK_DDD(
      DUK_DDDPRINT("duk__emit: 0x%08lx curr_token.start_line=%ld "
...
DUK_LOCAL void duk__emit_a_b_c(duk_compiler_ctx* comp_ctx,
  ...
  if (a <= DUK_BC_A_MAX) {
    ...
    DUK_D(
        DUK_DPRINT("out of regs: 'a' (reg) needs shuffling but does not fit "
                   "into BC, a: %ld",
                   (long)a));
    goto error_outofregs;
  }
  ...
  if ((b & DUK__CONST_MARKER) != 0) {
    ...
    if (b <= 0xff) {
      ...
      DUK_D(
          DUK_DPRINT("out of regs: 'b' (const) needs shuffling but does not "
                     "fit into BC, b: %ld",
                     (long)b));
      goto error_outofregs;
    }
    ...
    if (b <= 0xff) {
      ...
      if (b > DUK_BC_B_MAX) {
        ...
        DUK_D(
            DUK_DPRINT("out of regs: 'b' (reg) needs shuffling but shuffle "
                       "prohibited, b: %ld",
                       (long)b));
        goto error_outofregs;
      }
  ...
  if ((c & DUK__CONST_MARKER) != 0) {
    ...
    if (c <= 0xff) {
      ...
      DUK_D(
          DUK_DPRINT("out of regs: 'c' (const) needs shuffling but does not "
                     "fit into BC, c: %ld",
                     (long)c));
      goto error_outofregs;
    }
    ...
    if (c <= 0xff) {
      ...
      if (c > DUK_BC_C_MAX) {
        ...
        DUK_D(
            DUK_DPRINT("out of regs: 'c' (reg) needs shuffling but shuffle "
                       "prohibited, c: %ld",
                       (long)c));
        goto error_outofregs;
      }
...
DUK_LOCAL void duk__patch_trycatch(duk_compiler_ctx* comp_ctx,
  ...
  if (const_varname & DUK__CONST_MARKER) {
    ...
    if (reg_catch > DUK_BC_BC_MAX || const_varname > DUK_BC_BC_MAX) {
       ...
       */
      DUK_D(
          DUK_DPRINT("failed to patch trycatch: flags=%ld, reg_catch=%ld, "
...
#define DUK__IVAL_FLAG_ALLOW_CONST \
  (1 << 0) /* allow a constant to be returned */
#define DUK__IVAL_FLAG_REQUIRE_TEMP                                    \
  (1 << 1) /* require a (mutable) temporary as a result (or a const if \
              allowed) */
#define DUK__IVAL_FLAG_REQUIRE_SHORT                                         \
  (1 << 2) /* require a short (8-bit) reg/const which fits into bytecode B/C \
              slot */
...
duk_regconst_t duk__ispec_toregconst_raw(duk_compiler_ctx* comp_ctx,
  ...
  DUK_DDD(
      DUK_DDDPRINT("duk__ispec_toregconst_raw(): x={%ld:%ld:%!T}, "
...
duk__lookup_active_register_binding(duk_compiler_ctx* comp_ctx) {
  ...
  if (duk_is_number(thr, -1)) {
    ...
    if (comp_ctx->curr_func.catch_depth > 0 ||
       ...
       */
      DUK_DDD(
          DUK_DDDPRINT("slow path access outside of try-catch and with, no "
                       "need for _Varmap"));
...
#define DUK__OBJ_LIT_KEY_PLAIN \
  (1 << 0)                            /* key encountered as a plain property */
      ...
      if (args_op == DUK_OP_NONE) {
        ...
        if (reg_varbind >= 0) {
          ...
          if (pc_after_rhs == pc_before_rhs) {
             ...
             */
            DUK_DD(
                DUK_DDPRINT("rhs is side effect free, rewind and avoid "
...
#define DUK__HAS_TERM \
  (1 << 1) /* stmt has explicit/implicit semicolon terminator */
#define DUK__ALLOW_AUTO_SEMI_ALWAYS                                           \
  (1 << 2) /* allow automatic semicolon even without lineterm (compatibility) \
            */
#define DUK__STILL_PROLOGUE \
  (1 << 3) /* statement does not terminate directive prologue */
#define DUK__IS_TERMINAL                                                      \
  (1 << 4) /* statement is guaranteed to be terminal (control doesn't flow to \
              next statement) */
...
DUK_LOCAL void duk__parse_for_stmt(duk_compiler_ctx* comp_ctx, duk_ivalue* res,
  ...
  if (comp_ctx->curr_token.t == DUK_TOK_VAR) {
    ...
    if (comp_ctx->curr_token.t == DUK_TOK_IN) {
      ...
      DUK_DDD(
          DUK_DDDPRINT("detected for variant 4: for (var "
    ...
    if (comp_ctx->curr_token.t == DUK_TOK_IN) {
      ...
      DUK_DDD(
          DUK_DDDPRINT("detected for variant 1: for (ExpressionNoIn_opt; "
                       "Expression_opt; Expression_opt) Statement"));
      duk__ivalue_toplain_ignore(comp_ctx, res);
      goto parse_1_or_2;
    }
...
retry_parse:
  ...
  DUK_DDD(
      DUK_DDDPRINT("try stmt parse, stmt_id=%ld, label_id=%ld, "
...
#else
            DUK_DDD(
                DUK_DDDPRINT("use strict detected but strict declarations "
                             "disabled, ignoring"));
#endif
...
DUK_LOCAL void duk__init_varmap_and_prologue_for_pass2(
  ...
  DUK_DDD(
      DUK_DDDPRINT("varmap: %!T, is_arguments_shadowed=%ld",
                   (duk_tval*)duk_get_tval(thr, comp_ctx->curr_func.varmap_idx),
