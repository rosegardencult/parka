...
DUK_INTERNAL void duk_err_create_and_throw(duk_hthread* thr,
  ...
  DUK_DD(DUK_DDPRINT(
      ...
      (long)code, (const char*)msg, (const char*)filename, (long)line));
#else
  ...
  if (thr->heap->creating_error) {
    ...
    if (h_err != NULL) {
      DUK_D(
          DUK_DPRINT("double fault detected -> use built-in fixed 'double "
                     "error' instance"));
      DUK_TVAL_SET_OBJECT(&tv_val, h_err);
    } else {
      DUK_D(
          DUK_DPRINT("double fault detected; there is no built-in fixed "
                     "'double error' instance "
     ...
     */
#if defined(DUK_USE_VERBOSE_ERRORS)
    duk_push_error_object_raw(thr, code | DUK_ERRCODE_FLAG_NOBLAME_FILELINE,
                              filename, line, "%s", (const char*)msg);
#else
    duk_push_error_object_raw(thr, code | DUK_ERRCODE_FLAG_NOBLAME_FILELINE,
                              NULL, 0, NULL);
#endif
  ...
  DUK_DDD(DUK_DDDPRINT(
      "THROW ERROR (INTERNAL): %!iT, %!iT (after throw augment)",
