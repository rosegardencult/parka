...
#    define DUK_HCOMPFUNC_GET_DATA(heap, h)                                 \
      ((duk_hbuffer_fixed*)(void*)DUK_USE_HEAPPTR_DEC16((heap)->heap_udata, \
                                                        (h)->data16))
...
#  define DUK_HCOMPFUNC_GET_CODE_END(heap, h)                                 \
    ((duk_instr_t*)(void*)(DUK_HBUFFER_FIXED_GET_DATA_PTR(                    \
                               (heap), DUK_HCOMPFUNC_GET_DATA((heap), (h))) + \
                           DUK_HBUFFER_GET_SIZE(                              \
                               (duk_hbuffer*)DUK_HCOMPFUNC_GET_DATA((heap),   \
                                                                    h))))
...
#  define DUK_HCOMPFUNC_GET_CONSTS_COUNT(heap, h)              \
    ((duk_size_t)(DUK_HCOMPFUNC_GET_CONSTS_SIZE((heap), (h)) / \
                  sizeof(duk_tval)))

#  define DUK_HCOMPFUNC_GET_FUNCS_COUNT(heap, h)              \
    ((duk_size_t)(DUK_HCOMPFUNC_GET_FUNCS_SIZE((heap), (h)) / \
                  sizeof(duk_hobject*)))

#  define DUK_HCOMPFUNC_GET_CODE_COUNT(heap, h)              \
    ((duk_size_t)(DUK_HCOMPFUNC_GET_CODE_SIZE((heap), (h)) / \
                  sizeof(duk_instr_t)))
