...
#    define DUK_UTIL_GET_RANDOM_DOUBLE(thr) \
      DUK_USE_GET_RANDOM_DOUBLE((thr)->heap_udata)
#  else
...
#  define DUK_RAW_WRITE_FLOAT_BE(ptr, val) \
...
#  define DUK_RAW_WRITEINC_CESU8(ptr, val) \
...
#  define DUK_BW_PUSH_AS_STRING(thr, bw_ctx)                          \
    do {                                                              \
      duk_push_lstring((thr), (const char*)(bw_ctx)->p_base,          \
...
DUK_INTERNAL_DECL void duk_bw_assert_valid(duk_hthread* thr,
...
#  define DUK_BW_ASSERT_SPACE_EXPR(thr, bw_ctx, sz)                   \
    (DUK_BW_ASSERT_VALID_EXPR((thr), (bw_ctx)),                       \
     DUK_ASSERT_EXPR((duk_size_t)((bw_ctx)->p_limit - (bw_ctx)->p) >= \
                     (duk_size_t)(sz)))
...
#  define DUK_BW_WRITE_RAW_U8_6(thr, bw_ctx, val1, val2, val3, val4, val5, \
                                val6)                                      \
    do {                                                                   \
      duk_uint8_t* duk__p;                                                 \
...
#  define DUK_BW_WRITE_RAW_XUTF8(thr, bw_ctx, cp)                    \
    do {                                                             \
      ...
      DUK_BW_ASSERT_SPACE((thr), (bw_ctx),                           \
                          duk_unicode_get_xutf8_length(duk__cp));    \
...
#  define DUK_BW_WRITE_RAW_CESU8(thr, bw_ctx, cp)                    \
    do {                                                             \
      ...
      duk__cp = (duk_ucodepoint_t)(cp);                              \
      DUK_BW_ASSERT_SPACE((thr), (bw_ctx),                           \
                          duk_unicode_get_cesu8_length(duk__cp));    \
...
#  define DUK_BW_WRITE_RAW_CSTRING(thr, bw_ctx, val)                 \
    do {                                                             \
      ...
      duk_memcpy_unsafe((void*)((bw_ctx)->p), (const void*)duk__val, \
                        duk__val_len);                               \
...
#  define DUK_BW_WRITE_RAW_HSTRING(thr, bw_ctx, val)              \
    do {                                                          \
      ...
      duk_memcpy_unsafe((void*)((bw_ctx)->p),                     \
                        (const void*)DUK_HSTRING_GET_DATA((val)), \
                        duk__val_len);                            \
      (bw_ctx)->p += duk__val_len;                                \
    } while (0)
...
#  define DUK_BW_WRITE_ENSURE_U8_5(thr, bw_ctx, val1, val2, val3, val4, val5) \
    do {                                                                      \
      ...
      DUK_BW_WRITE_RAW_U8_5((thr), (bw_ctx), (val1), (val2), (val3), (val4),  \
                            (val5));                                          \
    } while (0)
#  define DUK_BW_WRITE_ENSURE_U8_6(thr, bw_ctx, val1, val2, val3, val4, val5, \
                                   val6)                                      \
    do {                                                                      \
      DUK_BW_ENSURE((thr), (bw_ctx), 6);                                      \
      DUK_BW_WRITE_RAW_U8_6((thr), (bw_ctx), (val1), (val2), (val3), (val4),  \
                            (val5), (val6));                                  \
    } while (0)
...
DUK_INTERNAL_DECL duk_uint32_t duk_util_hashbytes(const duk_uint8_t* data,
                                                  duk_size_t len,
                                                  duk_uint32_t seed);
#  endif

DUK_INTERNAL_DECL duk_uint32_t duk_bd_decode(duk_bitdecoder_ctx* ctx,
                                             duk_small_int_t bits);
DUK_INTERNAL_DECL duk_small_uint_t duk_bd_decode_flag(duk_bitdecoder_ctx* ctx);
DUK_INTERNAL_DECL duk_uint32_t duk_bd_decode_flagged(duk_bitdecoder_ctx* ctx,
                                                     duk_small_int_t bits,
                                                     duk_uint32_t def_value);
DUK_INTERNAL_DECL duk_int32_t duk_bd_decode_flagged_signed(
    duk_bitdecoder_ctx* ctx, duk_small_int_t bits, duk_int32_t def_value);
DUK_INTERNAL_DECL duk_uint32_t duk_bd_decode_varuint(duk_bitdecoder_ctx* ctx);
DUK_INTERNAL_DECL duk_small_uint_t
duk_bd_decode_bitpacked_string(duk_bitdecoder_ctx* bd, duk_uint8_t* out);

DUK_INTERNAL_DECL void duk_be_encode(duk_bitencoder_ctx* ctx, duk_uint32_t data,
                                     duk_small_int_t bits);
...
DUK_INTERNAL_DECL void duk_bw_init(duk_hthread* thr, duk_bufwriter_ctx* bw_ctx,
...
DUK_INTERNAL_DECL void duk_raw_write_double_be(duk_uint8_t* p,
                                               duk_double_t val);
DUK_INTERNAL_DECL duk_small_int_t duk_raw_write_xutf8(duk_uint8_t* p,
                                                      duk_ucodepoint_t val);
DUK_INTERNAL_DECL duk_small_int_t duk_raw_write_cesu8(duk_uint8_t* p,
                                                      duk_ucodepoint_t val);
DUK_INTERNAL_DECL void duk_raw_writeinc_u16_be(duk_uint8_t** p,
                                               duk_uint16_t val);
DUK_INTERNAL_DECL void duk_raw_writeinc_u32_be(duk_uint8_t** p,
                                               duk_uint32_t val);
DUK_INTERNAL_DECL void duk_raw_writeinc_float_be(duk_uint8_t** p,
                                                 duk_float_t val);
DUK_INTERNAL_DECL void duk_raw_writeinc_double_be(duk_uint8_t** p,
                                                  duk_double_t val);
DUK_INTERNAL_DECL void duk_raw_writeinc_xutf8(duk_uint8_t** p,
                                              duk_ucodepoint_t val);
DUK_INTERNAL_DECL void duk_raw_writeinc_cesu8(duk_uint8_t** p,
                                              duk_ucodepoint_t val);

#  if defined( \
      DUK_USE_DEBUGGER_SUPPORT) /* For now only needed by the debugger. */
...
DUK_INTERNAL_DECL duk_small_int_t duk_memcmp(const void* s1, const void* s2,
                                             duk_size_t len);
DUK_INTERNAL_DECL duk_small_int_t duk_memcmp_unsafe(const void* s1,
                                                    const void* s2,
                                                    duk_size_t len);

DUK_INTERNAL_DECL duk_bool_t
duk_is_whole_get_int32_nonegzero(duk_double_t x, duk_int32_t* ival);
...
DUK_INTERNAL_DECL duk_bool_t duk_double_same_sign(duk_double_t x,
                                                  duk_double_t y);
