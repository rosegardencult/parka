...
DUK_INTERNAL void duk_proxy_ownkeys_postprocess(duk_hthread* thr,
  ...
  for (i = 0; i < len; i++) {
    ...
    if (!(flags & DUK_ENUM_INCLUDE_NONENUMERABLE)) {
       ...
       * descriptor.
       */
      if (duk_hobject_get_own_propdesc(thr, h_proxy_target,
                                       duk_known_hstring(thr, -1), &desc,
                                       0 /*flags*/)) {
        if ((desc.flags & DUK_PROPDESC_FLAG_ENUMERABLE) == 0) {
          DUK_DDD(DUK_DDDPRINT("ignore non-enumerable property: %!T",
                               duk_get_tval(thr, -1)));
          goto skip_key;
        }
      } else {
        DUK_DDD(DUK_DDDPRINT("ignore non-existent property: %!T",
                             duk_get_tval(thr, -1)));
        goto skip_key;
      }
    }
    if (DUK_UNLIKELY(DUK_HSTRING_HAS_SYMBOL(h))) {
      if (!(flags & DUK_ENUM_INCLUDE_SYMBOLS)) {
        DUK_DDD(
            DUK_DDDPRINT("ignore symbol property: %!T", duk_get_tval(thr, -1)));
        goto skip_key;
      }
      if (DUK_HSTRING_HAS_HIDDEN(h) && !(flags & DUK_ENUM_INCLUDE_HIDDEN)) {
        DUK_DDD(DUK_DDDPRINT("ignore hidden symbol property: %!T",
                             duk_get_tval(thr, -1)));
        goto skip_key;
      }
    } else {
      if (flags & DUK_ENUM_EXCLUDE_STRINGS) {
        DUK_DDD(
            DUK_DDDPRINT("ignore string property: %!T", duk_get_tval(thr, -1)));
        goto skip_key;
      }
    }
  ...
  /* XXX: The key enumerability check should trigger the
   * "getOwnPropertyDescriptor" trap which has not yet been implemented.  In the
   * absence of such a trap, the enumerability should be checked from the target
   * object; this is handled above.
   */
}
