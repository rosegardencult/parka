...
#define DUK__IEEE_DOUBLE_EXP_MIN \
  (-1022) /* biased exp == 0 -> denormal, exp -1022 */
...
DUK_LOCAL const duk__exp_limits duk__str2num_exp_limits[] = {
    {957, -1147}, {605, -725}, {479, -575}, {414, -496}, {372, -446},
    {342, -411},  {321, -384}, {304, -364}, {291, -346}, {279, -334},
    {268, -323},  {260, -312}, {252, -304}, {247, -296}, {240, -289},
    {236, -283},  {231, -278}, {227, -273}, {223, -267}, {220, -263},
    {216, -260},  {213, -256}, {210, -253}, {208, -249}, {205, -246},
    {203, -244},  {201, -241}, {198, -239}, {196, -237}, {195, -234},
    {193, -232},  {191, -230}, {190, -228}, {188, -226}, {187, -225},
};
 ...
 * bits, with the caller responsible for ensuring this is never exceeded.  No
 * memory allocation (except stack) is needed for bigint computation. Operations
...
DUK_LOCAL void duk__bi_copy(duk__bigint* x, duk__bigint* y) {
  ...
  duk_memcpy((void*)x->v, (const void*)y->v,
             (size_t)(sizeof(duk_uint32_t) * (size_t)n));
}
...
DUK_LOCAL void duk__bi_add(duk__bigint* x, duk__bigint* y, duk__bigint* z) {
  ...
  for (i = 0; i < ny; i++) {
     ...
     *  - If carry not added: 0x12345678 + 0 + 0xffffffff = 0x12345677 (<
     * 0x12345678)
     *  - If carry added:     0x12345678 + 1 + 0xffffffff = 0x12345678 (==
     * 0x12345678)
     */
    if (carry) {
      tmp2++;
...
/* x <- y + z */
DUK_LOCAL void duk__bi_add_small(duk__bigint* x, duk__bigint* y,
                                 duk_uint32_t z) {
  duk__bigint tmp;
...
DUK_LOCAL void duk__bi_sub(duk__bigint* x, duk__bigint* y, duk__bigint* z) {
  ...
  for (i = 0; i < ny; i++) {
     ...
     *  - If borrow not subtracted: 0x12345678 - 0 - 0xffffffff = 0x12345679 (>
     * 0x12345678)
     *  - If borrow subtracted:     0x12345678 - 1 - 0xffffffff = 0x12345678 (==
     * 0x12345678)
     */
    if (borrow) {
      tmp2--;
...
/* x <- x - y, use t as temp */
DUK_LOCAL void duk__bi_sub_copy(duk__bigint* x, duk__bigint* y,
                                duk__bigint* t) {
  duk__bi_sub(t, x, y);
  duk__bi_copy(x, t);
}
...
/* x <- y * z */
DUK_LOCAL void duk__bi_mul_small(duk__bigint* x, duk__bigint* y,
                                 duk_uint32_t z) {
  duk__bigint tmp;
...
/* x <- x * y, use t as temp */
DUK_LOCAL void duk__bi_mul_copy(duk__bigint* x, duk__bigint* y,
                                duk__bigint* t) {
  duk__bi_mul(t, x, y);
  duk__bi_copy(x, t);
}
...
/* x <- x * y, use t as temp */
DUK_LOCAL void duk__bi_mul_small_copy(duk__bigint* x, duk_uint32_t y,
                                      duk__bigint* t) {
...
DUK_LOCAL duk_small_int_t duk__bi_is_2to52(duk__bigint* x) {
  DUK_ASSERT(duk__bi_is_valid(x));
  return (duk_small_int_t)(x->n == 2) && (x->v[0] == 0U) &&
         (x->v[1] == (1U << (52 - 32)));
}
...
DUK_LOCAL void duk__bi_exp_small(duk__bigint* x, duk_small_int_t b,
                                 duk_small_int_t y, duk__bigint* t1,
                                 duk__bigint* t2) {
  /* Fast path the binary case */

  DUK_ASSERT(x != t1 && x != t2 &&
             t1 != t2); /* distinct bignums, easy mistake to make */
  DUK_ASSERT(b >= 0);
  DUK_ASSERT(y >= 0);
...
#define DUK__MAX_OUTPUT_DIGITS \
  1040 /* (Number.MAX_VALUE).toString(2).length == 1024, + slack */

/* Maximum number of characters in formatted value. */
#define DUK__MAX_FORMATTED_LENGTH \
  1040 /* (-Number.MAX_VALUE).toString(2).length == 1025, + slack */
...
#define DUK__NUMCONV_CTX_BIGINTS_SIZE \
  (sizeof(duk__bigint) * DUK__NUMCONV_CTX_NUM_BIGINTS)
...
typedef struct {
  ...
  duk_small_int_t is_s2n; /* if 1, doing a string-to-number; else doing a
                             number-to-string */
  duk_small_int_t
      is_fixed; /* if 1, doing a fixed format output (not free format) */
  duk_small_int_t
      req_digits; /* requested number of output digits; 0 = free-format */
...
DUK_LOCAL void duk__dragon4_prepare(duk__numconv_stringify_ctx* nc_ctx) {
  ...
  if (nc_ctx->e >= 0) {
    ...
    if (lowest_mantissa) {
      ...
      duk__bi_exp_small(&nc_ctx->mm, nc_ctx->b, nc_ctx->e, &nc_ctx->t1,
                        &nc_ctx->t2); /* mm <- b^e */
      duk__bi_mul_small(&nc_ctx->mp, &nc_ctx->mm,
                        (duk_uint32_t)nc_ctx->b); /* mp <- b^(e+1) */
      duk__bi_mul_small(&nc_ctx->t1, &nc_ctx->f, 2);
      duk__bi_mul(&nc_ctx->r, &nc_ctx->t1,
                  &nc_ctx->mp); /* r <- (2 * f) * b^(e+1) */
      duk__bi_set_small(&nc_ctx->s,
                        (duk_uint32_t)(nc_ctx->b * 2)); /* s <- 2 * b */
    ...
    if (nc_ctx->e > DUK__IEEE_DOUBLE_EXP_MIN /*not minimum exponent*/ &&
      ...
      duk__bi_mul_small(&nc_ctx->r, &nc_ctx->f,
                        (duk_uint32_t)(nc_ctx->b * 2)); /* r <- (2 * b) * f */
      duk__bi_exp_small(&nc_ctx->t1, nc_ctx->b, 1 - nc_ctx->e, &nc_ctx->s,
      ...
      duk__bi_exp_small(&nc_ctx->t1, nc_ctx->b, -nc_ctx->e, &nc_ctx->s,
                        &nc_ctx->t2); /* NB: use 's' as temp on purpose */
...
DUK_LOCAL void duk__dragon4_scale(duk__numconv_stringify_ctx* nc_ctx) {
  ...
  DUK_DDD(DUK_DDDPRINT("scale: B=%ld, low_ok=%ld, high_ok=%ld", (long)nc_ctx->B,
  ...
  for (;;) {
    ...
    duk__bi_add(&nc_ctx->t1, &nc_ctx->r, &nc_ctx->mp); /* t1 = (+ r m+) */
    duk__bi_mul_small(&nc_ctx->t2, &nc_ctx->t1,
                      (duk_uint32_t)nc_ctx->B); /* t2 = (* (+ r m+) B) */
    if (duk__bi_compare(&nc_ctx->t2, &nc_ctx->s) <=
        (nc_ctx->high_ok ? -1 : 0)) {
      ...
      if (nc_ctx->unequal_gaps) {
        ...
        duk__bi_mul_small_copy(&nc_ctx->mm, (duk_uint32_t)nc_ctx->B,
                               &nc_ctx->t1);
      }
      k--;
    } else {
...
DUK_LOCAL void duk__dragon4_generate(duk__numconv_stringify_ctx* nc_ctx) {
  ...
  for (;;) {
    DUK_DDD(DUK_DDDPRINT(
        "generate loop, count=%ld, k=%ld, B=%ld, low_ok=%ld, high_ok=%ld",
        (long)count, (long)nc_ctx->k, (long)nc_ctx->B, (long)nc_ctx->low_ok,
        (long)nc_ctx->high_ok));
    DUK__BI_PRINT("r", &nc_ctx->r);
    DUK__BI_PRINT("s", &nc_ctx->s);
    DUK__BI_PRINT("m+", &nc_ctx->mp);
    ...
    duk__bi_mul_small(&nc_ctx->t1, &nc_ctx->r,
                      (duk_uint32_t)nc_ctx->B); /* t1 <- (* r B) */
    d = 0;
    for (;;) {
    ...
    duk__bi_mul_small_copy(&nc_ctx->mp, (duk_uint32_t)nc_ctx->B,
                           &nc_ctx->t2); /* m+ <- (* m+ B) */
    duk__bi_mul_small_copy(&nc_ctx->mm, (duk_uint32_t)nc_ctx->B,
                           &nc_ctx->t2); /* m- <- (* m- B) */
    ...
    if (nc_ctx->is_fixed == 0) {
      /* free-form */
      tc1 = (duk__bi_compare(&nc_ctx->r, &nc_ctx->mm) <=
             (nc_ctx->low_ok ? 0 : -1));

      duk__bi_add(&nc_ctx->t1, &nc_ctx->r, &nc_ctx->mp); /* t1 <- (+ r m+) */
      tc2 = (duk__bi_compare(&nc_ctx->t1, &nc_ctx->s) >=
             (nc_ctx->high_ok ? 0 : 1));
    ...
    if (tc1) {
      if (tc2) {
        ...
        if (duk__bi_compare(&nc_ctx->t1, &nc_ctx->s) < 0) { /* (< (* r 2) s) */
          DUK_DDD(DUK_DDDPRINT(
              "tc1=true, tc2=true, 2r > s: output d --> %ld (k=%ld)", (long)d,
              (long)nc_ctx->k));
          DUK__DRAGON4_OUTPUT_PREINC(nc_ctx, count, d);
        } else {
          DUK_DDD(DUK_DDDPRINT(
              "tc1=true, tc2=true, 2r <= s: output d+1 --> %ld (k=%ld)",
    ...
    if (nc_ctx->is_fixed) {
      if (nc_ctx->abs_pos) {
        int pos = nc_ctx->k - count +
                  1; /* count is already incremented, take into account */
        DUK_DDD(DUK_DDDPRINT(
            "fixed format, absolute: abs pos=%ld, k=%ld, count=%ld, req=%ld",
            (long)pos, (long)nc_ctx->k, (long)count, (long)nc_ctx->req_digits));
        if (pos <= nc_ctx->req_digits) {
          DUK_DDD(DUK_DDDPRINT(
              "digit position reached req_digits, end generate loop"));
          break;
        }
      } else {
        DUK_DDD(DUK_DDDPRINT(
            "fixed format, relative: k=%ld, count=%ld, req=%ld",
            (long)nc_ctx->k, (long)count, (long)nc_ctx->req_digits));
        if (count >= nc_ctx->req_digits) {
          DUK_DDD(DUK_DDDPRINT(
              "digit count reached req_digits, end generate loop"));
          break;
        }
      }
    }
 ...
 */
DUK_LOCAL duk_small_int_t duk__dragon4_fixed_format_round(
    duk__numconv_stringify_ctx* nc_ctx, duk_small_int_t round_idx) {
  duk_small_int_t t;
  duk_uint8_t* p;
  ...
  if (round_idx >= nc_ctx->count) {
    DUK_DDD(DUK_DDDPRINT(
        "round_idx out of bounds (%ld >= %ld (count)) -> no rounding",
    ...
    for (;;) {
      *p = 0;
      if (p == &nc_ctx->digits[0]) {
        DUK_DDD(DUK_DDDPRINT(
            "carry propagated to first digit -> special case handling"));
      ...
      DUK_DDD(
          DUK_DDDPRINT("fixed-format rounding carry: B=%ld, roundup_limit=%ld, "
                       "p=%p, digits=%p",
                       (long)nc_ctx->B, (long)roundup_limit, (void*)p,
                       (void*)nc_ctx->digits));
      p--;
      t = *p;
...
DUK_LOCAL void duk__dragon4_convert_and_push(
    duk__numconv_stringify_ctx* nc_ctx, duk_hthread* thr, duk_small_int_t radix,
    duk_small_int_t digits, duk_small_uint_t flags, duk_small_int_t neg) {
  duk_small_int_t k;
   ...
   * fake k such that one leading digit is generated (e.g. digits=123 ->
   * "1.23").
   *
  ...
  if (!nc_ctx->abs_pos /* toFixed() */) {
    if ((flags & DUK_N2S_FLAG_FORCE_EXP) ||    /* exponential notation forced */
        ((flags & DUK_N2S_FLAG_NO_ZERO_PAD) && /* fixed precision and zero
  ...
  DUK_DDD(DUK_DDDPRINT(
      "expt=%ld, k=%ld, count=%ld, pos=%ld, pos_end=%ld, is_fixed=%ld, "
  ...
  /* Digit generation */
  while (pos > pos_end) {
    DUK_DDD(DUK_DDDPRINT("digit generation: pos=%ld, pos_end=%ld", (long)pos,
                         (long)pos_end));
    if (pos == 0) {
      *q++ = (duk_uint8_t)'.';
    }
...
DUK_LOCAL void duk__dragon4_double_to_ctx(duk__numconv_stringify_ctx* nc_ctx,
  ...
  if (expt == 0) {
    ...
    DUK_ASSERT(duk__bi_is_valid(
        &nc_ctx->f)); /* true, because v[1] has at least one bit set */
  }
...
recheck_exp:
  ...
  if (expt > 1023) {
    ...
    /* denormal or zero */
    bitstart =
        1023 + expt; /* expt==-1023 -> bitstart=0 (leading 1);
  ...
  DUK_DDD(DUK_DDDPRINT("ieee expt=%ld, bitstart=%ld, bitround=%ld", (long)expt,
                       (long)bitstart, (long)bitround));
  ...
  if (!skip_round) {
    if (duk__dragon4_fixed_format_round(nc_ctx, bitround)) {
       ...
       */
      DUK_DDD(DUK_DDDPRINT(
          "rounding caused exponent to be bumped, recheck exponent"));
      skip_round = 1;
      goto recheck_exp;
    }
  }
  ...
  DUK_DDD(DUK_DDDPRINT("mantissa is complete: %08lx %08lx", (unsigned long)t,
...
DUK_LOCAL DUK_NOINLINE void duk__numconv_stringify_raw(duk_hthread* thr,
  ...
  if (duk_double_equals((double)uval, x) && /* integer number in range */
    ...
    DUK_ASSERT(DUK__NUMCONV_CTX_BIGINTS_SIZE >=
  ...
  if (c == DUK_FP_ZERO) {
    ...
    if (nc_ctx->is_fixed) {
      if (nc_ctx->abs_pos) {
        count =
            digits + 2; /* lead zero + 'digits' fractions + 1 for rounding */
      } else {
...
zero_skip:
  ...
  if (flags & DUK_N2S_FLAG_FIXED_FORMAT) {
    ...
    if (flags & DUK_N2S_FLAG_FRACTION_DIGITS) {
       ...
       */
      roundpos =
          -digits; /* absolute position for digit considered for rounding */
      roundpos = nc_ctx->k - roundpos;
    } else {
      roundpos = digits;
    }
    DUK_DDD(DUK_DDDPRINT("rounding: k=%ld, count=%ld, digits=%ld, roundpos=%ld",
                         (long)nc_ctx->k, (long)nc_ctx->count, (long)digits,
                         (long)roundpos));
...
DUK_INTERNAL void duk_numconv_stringify(duk_hthread* thr, duk_small_int_t radix,
                                        duk_small_int_t digits,
                                        duk_small_uint_t flags) {
...
DUK_LOCAL DUK_NOINLINE void duk__numconv_parse_raw(duk_hthread* thr,
  ...
  DUK_DDD(DUK_DDDPRINT("parse number: %!T, radix=%ld, flags=0x%08lx",
                       (duk_tval*)duk_get_tval(thr, -1), (long)radix,
                       (unsigned long)flags));

  DUK_ASSERT(radix >= 2 && radix <= 36);
  DUK_ASSERT(radix - 2 <
             (duk_small_int_t)sizeof(duk__str2num_digits_for_radix));
  ...
  if ((flags & DUK_S2N_FLAG_ALLOW_INF) &&
      DUK_STRNCMP((const char*)p, "Infinity", 8) == 0) {
    ...
    if ((flags & DUK_S2N_FLAG_ALLOW_GARBAGE) == 0 && p[8] != DUK_ASC_NUL) {
      DUK_DDD(
          DUK_DDDPRINT("parse failed: trailing garbage after matching "
  ...
  if (ch == (duk_small_int_t)'0') {
    ...
    if ((flags & DUK_S2N_FLAG_ALLOW_AUTO_HEX_INT) && ch == DUK_ASC_LC_X) {
      DUK_DDD(
          DUK_DDDPRINT("detected 0x/0X hex prefix, changing radix and "
                       "preventing fractions and exponent"));
      detect_radix = 16;
#if 0
			...
			p += 1;
#endif
    } else if ((flags & DUK_S2N_FLAG_ALLOW_AUTO_OCT_INT) &&
               ch == DUK_ASC_LC_O) {
      DUK_DDD(
          DUK_DDDPRINT("detected 0o oct prefix, changing radix and preventing "
                       "fractions and exponent"));
      detect_radix = 8;
    } else if ((flags & DUK_S2N_FLAG_ALLOW_AUTO_BIN_INT) &&
               ch == DUK_ASC_LC_B) {
      DUK_DDD(
          DUK_DDDPRINT("detected 0b bin prefix, changing radix and preventing "
                       "fractions and exponent"));
      detect_radix = 2;
    }
    if (detect_radix > 0) {
      radix = detect_radix;
      /* Clear empty as zero flag: interpret e.g. '0x' and '0xg' as a NaN (=
       * parse error) */
      ...
      flags |= DUK_S2N_FLAG_ALLOW_LEADING_ZERO; /* allow e.g. '0x0009' and
                                                   '0b00010001' */
      p += 2;
    }
  }
  ...
  for (;;) {
    ...
    DUK_DDD(DUK_DDDPRINT(
        "parse digits: p=%p, ch='%c' (%ld), expt=%ld, expt_adj=%ld, "
        "dig_whole=%ld, dig_frac=%ld, dig_expt=%ld, dig_lzero=%ld, "
        "dig_prec=%ld",
  ...
  if (dig_lzero > 0 && dig_whole > 1) {
    if ((flags & DUK_S2N_FLAG_ALLOW_LEADING_ZERO) == 0) {
      DUK_DDD(DUK_DDDPRINT(
          "parse failed: leading zeroes not allowed in integer part"));
      goto parse_fail;
    }
  }
  ...
  if (dig_whole == 0) {
    if (dig_frac == 0) {
      ...
      DUK_DDD(DUK_DDDPRINT(
          "parse failed: plain period without leading or trailing digits"));
      goto parse_fail;
    } else if (dig_frac > 0) {
      /* ".123" */
      ...
      if ((flags & DUK_S2N_FLAG_ALLOW_EMPTY_AS_ZERO) == 0) {
        ...
        DUK_DDD(DUK_DDDPRINT(
            "parse failed: no digits, but not empty (had a +/- sign)"));
        goto parse_fail;
      }
    }
  } else {
  ...
  while (dig_prec < duk__str2num_digits_for_radix[radix - 2]) {
     ...
     */
    DUK_DDD(DUK_DDDPRINT("dig_prec=%ld, pad significand with zero",
                         (long)dig_prec));
...
DUK_INTERNAL void duk_numconv_parse(duk_hthread* thr, duk_small_int_t radix,
                                    duk_small_uint_t flags) {
  duk_native_stack_check(thr);
