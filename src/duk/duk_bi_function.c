...
DUK_INTERNAL duk_ret_t duk_bi_function_constructor(duk_hthread* thr) {
  ...
  duk_push_hstring_stridx(thr, DUK_STRIDX_COMPILE);
  /* XXX: copy from caller? */ /* XXX: ignored now */
  h_sourcecode = duk_require_hstring(
      thr, -2); /* no symbol check needed; -2 is concat'd code */
  duk_js_compile(thr, (const duk_uint8_t*)DUK_HSTRING_GET_DATA(h_sourcecode),
                 (duk_size_t)DUK_HSTRING_GET_BYTELEN(h_sourcecode), comp_flags);
...
DUK_INTERNAL duk_ret_t duk_bi_function_prototype_to_string(duk_hthread* thr) {
  ...
  if (DUK_TVAL_IS_OBJECT(tv)) {
    ...
    if (DUK_HOBJECT_IS_COMPFUNC(obj)) {
      duk_push_sprintf(thr, "function %s() { [ecmascript code] }",
                       (const char*)func_name);
    } else if (DUK_HOBJECT_IS_NATFUNC(obj)) {
      duk_push_sprintf(thr, "function %s() { [native code] }",
                       (const char*)func_name);
    } else if (DUK_HOBJECT_IS_BOUNDFUNC(obj)) {
      duk_push_sprintf(thr, "function %s() { [bound code] }",
                       (const char*)func_name);
    } else {
      goto type_error;
    }
...
DUK_INTERNAL duk_ret_t duk_bi_function_prototype_bind(duk_hthread* thr) {
  ...
  DUK_ASSERT(DUK_HOBJECT_GET_PROTOTYPE(thr->heap, (duk_hobject*)h_bound) ==
             NULL);
  ...
  if (DUK_TVAL_IS_OBJECT(tv_tmp)) {
    ...
    DUK_HOBJECT_SET_PROTOTYPE_INIT_INCREF(thr, (duk_hobject*)h_bound,
                                          bound_proto);
      ...
      DUK_ASSERT(DUK_TVAL_IS_LIGHTFUNC(&h_boundtarget->target) ||
                 (DUK_TVAL_IS_OBJECT(&h_boundtarget->target) &&
                  DUK_HOBJECT_IS_CALLABLE(
                      DUK_TVAL_GET_OBJECT(&h_boundtarget->target)) &&
  ...
  DUK_TVAL_INCREF(
      thr, &h_bound->target); /* old values undefined, no decref needed */
  ...
  tv_res = (duk_tval*)DUK_ALLOC_CHECKED(
      thr, ((duk_size_t)bound_nargs) * sizeof(duk_tval));
  ...
  DUK_ASSERT(nargs >= 0);
  duk_copy_tvals_incref(thr, tv_res + n_prevbound, DUK_GET_TVAL_POSIDX(thr, 1),
                        (duk_size_t)nargs);
  ...
  DUK_TVAL_SET_U32(tv_tmp,
                   (duk_uint32_t)bound_len); /* in-place update, fastint */
  duk_xdef_prop_stridx_short(
      thr, -2, DUK_STRIDX_LENGTH,
      DUK_PROPDESC_FLAGS_C); /* attrs in E6 Section 9.2.4 */
...
#  endif
  ...
  DUK_DDD(DUK_DDDPRINT("created bound function: %!iT",
...
DUK_INTERNAL duk_ret_t duk_bi_native_function_length(duk_hthread* thr) {
  ...
  if (DUK_TVAL_IS_OBJECT(tv)) {
    ...
    func_nargs = h->nargs;
    duk_push_int(thr,
                 func_nargs == DUK_HNATFUNC_NARGS_VARARGS ? 0 : func_nargs);
  } else if (DUK_TVAL_IS_LIGHTFUNC(tv)) {
...
DUK_INTERNAL duk_ret_t duk_bi_function_prototype_hasinstance(duk_hthread* thr) {
  ...
  ret = duk_js_instanceof_ordinary(thr, DUK_GET_TVAL_POSIDX(thr, 0),
                                   DUK_GET_THIS_TVAL_PTR(thr));
