...
DUK_LOCAL duk_uint8_t* duk__dump_hbuffer_raw(duk_hthread* thr, duk_uint8_t* p,
  ...
  duk_memcpy_unsafe((void*)p,
                    (const void*)DUK_HBUFFER_GET_DATA_PTR(thr->heap, h), len);
  p += len;
  return p;
}
...
DUK_LOCAL duk_uint8_t* duk__dump_string_prop(duk_hthread* thr, duk_uint8_t* p,
  ...
  tv = duk_hobject_find_entry_tval_ptr_stridx(thr->heap, (duk_hobject*)func,
                                              stridx);
...
DUK_LOCAL duk_uint8_t* duk__dump_buffer_prop(duk_hthread* thr, duk_uint8_t* p,
  ...
  if (tv != NULL && DUK_TVAL_IS_BUFFER(tv)) {
    ...
    DUK_ASSERT(DUK_HBUFFER_MAX_BYTELEN <=
  ...
  if (h != NULL) {
    ...
    for (i = 0; i < (duk_uint_fast32_t)DUK_HOBJECT_GET_ENEXT(h); i++) {
      ...
      DUK_ASSERT(DUK_TVAL_IS_NUMBER(
          tv_val)); /* known to be number; in fact an integer */
#  if defined(DUK_USE_FASTINT)
      DUK_ASSERT(DUK_TVAL_IS_FASTINT(tv_val));
      DUK_ASSERT(DUK_TVAL_GET_FASTINT(tv_val) ==
                 (duk_int64_t)DUK_TVAL_GET_FASTINT_U32(
                     tv_val)); /* known to be 32-bit */
...
#  endif
      ...
      DUK_ASSERT(DUK_HSTRING_MAX_BYTELEN <=
                 0x7fffffffUL); /* ensures no overflow */
      p = DUK_BW_ENSURE_RAW(thr, bw_ctx, 4U + DUK_HSTRING_GET_BYTELEN(key) + 4U,
                            p);
...
DUK_LOCAL duk_uint8_t* duk__dump_formals(duk_hthread* thr, duk_uint8_t* p,
  ...
  if (h != NULL) {
    ...
    for (i = 0; i < h->length; i++) {
      ...
      DUK_ASSERT(DUK_HSTRING_MAX_BYTELEN <=
                 0x7fffffffUL); /* ensures no overflow */
      p = DUK_BW_ENSURE_RAW(thr, bw_ctx, 4U + DUK_HSTRING_GET_BYTELEN(varname),
                            p);
      p = duk__dump_hstring_raw(p, varname);
    }
  } else {
    DUK_DD(
        DUK_DDPRINT("dumping function without _Formals, emit marker to "
                    "indicate missing _Formals"));
    p = DUK_BW_ENSURE_RAW(thr, bw_ctx, 4U, p);
...
static duk_uint8_t* duk__dump_func(duk_hthread* thr, duk_hcompfunc* func,
                                   duk_bufwriter_ctx* bw_ctx, duk_uint8_t* p) {
  duk_tval *tv, *tv_end;
  ...
  DUK_DD(
      DUK_DDPRINT("dumping function %p to %p: "
  ...
  p = DUK_BW_ENSURE_RAW(thr, bw_ctx,
                        3U * 4U + 2U * 2U + 3U * 4U + count_instr * 4U, p);
...
#  else
  ...
  DUK_RAW_WRITEINC_U32_BE(p, 0);
#  endif
  tmp32 = DUK_HEAPHDR_GET_FLAGS(
      (duk_heaphdr*)func); /* masks flags, only duk_hobject flags */
  ...
  while (tv != tv_end) {
    ...
    if (DUK_TVAL_IS_STRING(tv)) {
      ...
      DUK_ASSERT(DUK_HSTRING_MAX_BYTELEN <=
                 0x7fffffffUL); /* ensures no overflow */
      p = DUK_BW_ENSURE_RAW(thr, bw_ctx,
                            1U + 4U + DUK_HSTRING_GET_BYTELEN(h_str), p);
      *p++ = DUK__SER_STRING;
      p = duk__dump_hstring_raw(p, h_str);
    } else {
...
#  endif
  ...
  DUK_DD(DUK_DDPRINT("serialized function %p -> final pointer %p", (void*)func,
                     (void*)p));
...
static const duk_uint8_t* duk__load_func(duk_hthread* thr, const duk_uint8_t* p,
  ...
  DUK_DD(DUK_DDPRINT("loading function, p=%p, p_end=%p", (const void*)p,
                     (const void*)p_end));
  ...
  DUK_DD(DUK_DDPRINT("instr=%ld, const=%ld, funcs=%ld, data_size=%ld",
                     (long)count_instr, (long)count_const, (long)count_const,
                     (long)data_size));
  ...
  DUK_ASSERT(DUK_HOBJECT_GET_PROTOTYPE(thr->heap, (duk_hobject*)h_fun) ==
...
#  endif
  ...
  DUK_HEAPHDR_SET_FLAGS(
      (duk_heaphdr*)h_fun,
      tmp32); /* masks flags to only change duk_hobject flags */
...
#  if defined(DUK_USE_INTEGER_BE)
  q = fun_data + sizeof(duk_tval) * count_const +
      sizeof(duk_hobject*) * count_funcs;
...
#  endif
  ...
  tv1 = duk_get_tval(
      thr, idx_base + 2); /* may be NULL if no constants or inner funcs */
...
#  if defined(DUK_USE_FUNC_NAME_PROPERTY)
  ...
  if (DUK_HOBJECT_HAS_NAMEBINDING((duk_hobject*)h_fun)) {
    ...
    duk_xdef_prop(thr, -3,
                  DUK_PROPDESC_FLAGS_NONE); /* -> [ func funcname env ] */
...
#  endif /* DUK_USE_FUNC_FILENAME_PROPERTY */
  ...
  if (DUK_HOBJECT_HAS_CONSTRUCTABLE((duk_hobject*)h_fun)) {
    ...
    duk_compact_m1(thr);
    duk_xdef_prop_stridx_short(thr, -2, DUK_STRIDX_PROTOTYPE,
