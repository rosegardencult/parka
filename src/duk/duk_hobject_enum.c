...
DUK_LOCAL duk__sort_key_t duk__hstring_sort_key(duk_hstring* x) {
   ...
   */
  DUK_ASSERT(x != NULL);
  DUK_ASSERT(!DUK_HSTRING_HAS_SYMBOL(x) ||
             DUK_HSTRING_GET_ARRIDX_FAST(x) == DUK_HSTRING_NO_ARRAY_INDEX);

  val = (duk__sort_key_t)DUK_HSTRING_GET_ARRIDX_FAST(x);

#if defined(DUK_USE_SYMBOL_BUILTIN)
  val = val + (duk__sort_key_t)(DUK_HEAPHDR_GET_FLAGS_RAW((duk_heaphdr*)x) &
...
DUK_LOCAL duk_bool_t duk__sort_compare_es6(duk_hstring* a, duk_hstring* b,
...
DUK_LOCAL void duk__sort_enum_keys_es6(duk_hthread* thr, duk_hobject* h_obj,
  ...
  for (idx = idx_start + 1; idx < idx_end; idx++) {
    ...
    val_curr =
        duk__hstring_sort_key(h_curr); /* Remains same during scanning. */
...
DUK_INTERNAL void duk_hobject_enumerator_create(duk_hthread* thr,
  ...
  duk_put_prop_stridx_short(
      thr, -2, DUK_STRIDX_INT_TARGET); /* Target is bare, plain put OK. */

  /* Initialize index so that we skip internal control keys. */
  duk_push_int(thr, DUK__ENUM_START_INDEX);
  duk_put_prop_stridx_short(
      thr, -2, DUK_STRIDX_INT_NEXT); /* Target is bare, plain put OK. */
  ...
  if (DUK_LIKELY(!duk_hobject_proxy_check(enum_target, &h_proxy_target,
                                          &h_proxy_handler))) {
    goto skip_proxy;
  }
  ...
  if (!duk_get_prop_stridx_short(thr, -1, DUK_STRIDX_OWN_KEYS)) {
    ...
    duk_push_hobject(
        thr,
        enum_target); /* -> [ ... enum_target res handler undefined target ] */
    duk_put_prop_stridx_short(
        thr, -4, DUK_STRIDX_INT_TARGET); /* Target is bare, plain put OK. */
  ...
  duk_insert(thr, -2);
  duk_push_hobject(
      thr, h_proxy_target); /* -> [ ... enum_target res trap handler target ] */
  duk_call_method(thr,
...
skip_proxy:
  ...
  while (curr) {
    ...
    cond =
        DUK_HOBJECT_HAS_EXOTIC_STRINGOBJ(curr) || DUK_HOBJECT_IS_BUFOBJ(curr);
#else
    ...
    if (cond) {
      ...
      if (DUK_HOBJECT_HAS_EXOTIC_STRINGOBJ(curr)) {
        ...
        DUK_ASSERT(
            h_val !=
            NULL); /* string objects must not created without internal value */
    ...
    if (cond) {
      for (i = 0; i < (duk_uint_fast32_t)DUK_HOBJECT_GET_ASIZE(curr); i++) {
        ...
        k = duk_heap_strtable_intern_u32_checked(
            thr, (duk_uint32_t)i); /* Fragile reachability. */
        DUK_ASSERT(k);
    ...
    for (i = 0; i < (duk_uint_fast32_t)DUK_HOBJECT_GET_ENEXT(curr); i++) {
      ...
      if (DUK_UNLIKELY(DUK_HSTRING_HAS_SYMBOL(k))) {
        ...
        DUK_ASSERT(
            !DUK_HSTRING_HAS_HIDDEN(k)); /* would also have symbol flag */
      ...
      DUK_ASSERT(DUK_HOBJECT_E_SLOT_IS_ACCESSOR(thr->heap, curr, i) ||
                 !DUK_TVAL_IS_UNUSED(
                     &DUK_HOBJECT_E_GET_VALUE_PTR(thr->heap, curr, i)->v));
...
#if defined(DUK_USE_PREFER_SIZE)
      duk__sort_enum_keys_es6(thr, res, (duk_int_fast32_t)sort_start_index,
  ...
  if (enum_flags & DUK_ENUM_SORT_ARRAY_INDICES) {
     ...
     * also for mixed keys.
     */
    duk__sort_enum_keys_es6(thr, res, (duk_int_fast32_t)DUK__ENUM_START_INDEX,
...
compact_and_return:
  ...
  DUK_DDD(DUK_DDDPRINT("created enumerator object: %!iT",
...
DUK_INTERNAL duk_bool_t duk_hobject_enumerator_next(duk_hthread* thr,
  ...
  DUK_DDD(DUK_DDDPRINT(
      "getting next enum value, enum_target=%!iO, enumerator=%!iT",
  ...
  for (;;) {
    ...
    DUK_ASSERT(
        !DUK_TVAL_IS_UNUSED(&DUK_HOBJECT_E_GET_VALUE(thr->heap, e, idx).v));
    ...
    DUK_DDD(
        DUK_DDDPRINT("enumeration: found element, key: %!O", (duk_heaphdr*)k));
    res = k;
    break;
  }
