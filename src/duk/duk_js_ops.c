...
#else
      DUK_ASSERT(duk_double_is_nan_or_zero(d) == 0 ||
                 duk_double_is_nan_or_zero(d) == 1);
...
DUK_LOCAL duk_double_t duk__tonumber_string_raw(duk_hthread* thr) {
  ...
  s2n_flags = DUK_S2N_FLAG_TRIM_WHITE | DUK_S2N_FLAG_ALLOW_EXP |
              ...
              DUK_S2N_FLAG_ALLOW_INF | DUK_S2N_FLAG_ALLOW_FRAC |
              DUK_S2N_FLAG_ALLOW_NAKED_FRAC | DUK_S2N_FLAG_ALLOW_EMPTY_FRAC |
              DUK_S2N_FLAG_ALLOW_EMPTY_AS_ZERO |
              DUK_S2N_FLAG_ALLOW_LEADING_ZERO |
              DUK_S2N_FLAG_ALLOW_AUTO_HEX_INT |
              DUK_S2N_FLAG_ALLOW_AUTO_OCT_INT | DUK_S2N_FLAG_ALLOW_AUTO_BIN_INT;
  ...
  DUK_ASSERT(DUK_TVAL_IS_DOUBLE(
      thr->valstack_top)); /* no fastint conversion in numconv now */
...
DUK_INTERNAL duk_int32_t duk_js_toint32(duk_hthread* thr, duk_tval* tv) {
  ...
  DUK_ASSERT(DUK_FPCLASSIFY(d) == DUK_FP_ZERO ||
             DUK_FPCLASSIFY(d) == DUK_FP_NORMAL);
  DUK_ASSERT(d >= -2147483648.0 &&
             d <= 2147483647.0); /* [-0x80000000,0x7fffffff] */
  DUK_ASSERT(duk_double_equals(
      d, (duk_double_t)((duk_int32_t)d))); /* whole, won't clip */
...
DUK_INTERNAL duk_uint32_t duk_js_touint32(duk_hthread* thr, duk_tval* tv) {
  ...
  DUK_ASSERT(duk_double_equals(
      d, (duk_double_t)((duk_uint32_t)d))); /* whole, won't clip */
...
DUK_LOCAL duk_bool_t duk__js_equals_number(duk_double_t x, duk_double_t y) {
  ...
  if (duk_double_equals(x, y)) {
     ...
     * are caught above.
     */
    DUK_ASSERT(!(DUK_FPCLASSIFY(x) == DUK_FP_ZERO &&
                 DUK_FPCLASSIFY(y) == DUK_FP_ZERO));
    return 0;
  }
...
DUK_INTERNAL duk_bool_t duk_js_equals_helper(duk_hthread* thr, duk_tval* tv_x,
                                             duk_tval* tv_y,
  ...
  if ((type_mask_x & DUK_TYPE_MASK_NUMBER) &&
      (type_mask_y & DUK_TYPE_MASK_STRING)) {
  ...
  if ((type_mask_x & DUK_TYPE_MASK_STRING) &&
      (type_mask_y & DUK_TYPE_MASK_NUMBER)) {
  ...
  if (type_mask_x & DUK_TYPE_MASK_BOOLEAN) {
    DUK_ASSERT(DUK_TVAL_GET_BOOLEAN(tv_x) == 0 ||
               DUK_TVAL_GET_BOOLEAN(tv_x) == 1);
    duk_push_uint(thr, DUK_TVAL_GET_BOOLEAN(tv_x));
    duk_push_tval(thr, tv_y);
    goto recursive_call;
  }
  if (type_mask_y & DUK_TYPE_MASK_BOOLEAN) {
    DUK_ASSERT(DUK_TVAL_GET_BOOLEAN(tv_y) == 0 ||
               DUK_TVAL_GET_BOOLEAN(tv_y) == 1);
    duk_push_tval(thr, tv_x);
    duk_push_uint(thr, DUK_TVAL_GET_BOOLEAN(tv_y));
    goto recursive_call;
  }

  /* String-number-symbol/object -> coerce object to primitive (apparently
   * without hint), then try again. */
...
recursive_call:
  ...
  {
    duk_bool_t rc;
    rc = duk_js_equals_helper(thr, DUK_GET_TVAL_NEGIDX(thr, -2),
...
DUK_INTERNAL duk_small_int_t duk_js_data_compare(const duk_uint8_t* buf1,
   ...
   * inputs.
   */
  rc = duk_memcmp_unsafe((const void*)buf1, (const void*)buf2,
                         (size_t)prefix_len);
...
#if defined(DUK_USE_FASTINT)
DUK_LOCAL duk_bool_t duk__compare_fastint(duk_bool_t retval, duk_int64_t v1,
                                          duk_int64_t v2) {
...
#if defined(DUK_USE_PARANOID_MATH)
DUK_LOCAL duk_bool_t duk__compare_number(duk_bool_t retval, duk_double_t d1,
                                         duk_double_t d2) {
...
DUK_INTERNAL duk_bool_t duk_js_compare_helper(duk_hthread* thr, duk_tval* tv_x,
                                              duk_tval* tv_y,
                                              duk_small_uint_t flags) {
  duk_double_t d1, d2;
  duk_small_int_t rc;
  ...
  DUK_ASSERT(DUK_COMPARE_FLAG_NEGATE ==
             1); /* Rely on this flag being lowest. */
  ...
  if (DUK_LIKELY(DUK_TVAL_IS_FASTINT(tv_x) && DUK_TVAL_IS_FASTINT(tv_y))) {
    return duk__compare_fastint(retval, DUK_TVAL_GET_FASTINT(tv_x),
  ...
  if (DUK_LIKELY(DUK_TVAL_IS_NUMBER(tv_x) && DUK_TVAL_IS_NUMBER(tv_y))) {
    return duk__compare_number(retval, DUK_TVAL_GET_NUMBER(tv_x),
  ...
  if (DUK_TVAL_IS_STRING(tv_x) && DUK_TVAL_IS_STRING(tv_y)) {
    ...
    if (DUK_LIKELY(!DUK_HSTRING_HAS_SYMBOL(h1) &&
                   !DUK_HSTRING_HAS_SYMBOL(h2))) {
...
DUK_LOCAL duk_bool_t duk__js_instanceof_helper(duk_hthread* thr, duk_tval* tv_x,
                                               duk_tval* tv_y,
   ...
   */
  if (!skip_sym_check) {
    if (duk_get_method_stridx(thr, -1,
  ...
  duk_get_prop_stridx_short(
      thr, -1, DUK_STRIDX_PROTOTYPE); /* -> [ ... lval rval rval.prototype ] */
...
DUK_INTERNAL duk_bool_t duk_js_instanceof(duk_hthread* thr, duk_tval* tv_x,
                                          duk_tval* tv_y) {
...
DUK_INTERNAL duk_bool_t duk_js_in(duk_hthread* thr, duk_tval* tv_x,
                                  duk_tval* tv_y) {
  duk_bool_t retval;
  ...
  duk_require_type_mask(
      thr, -1,
      DUK_TYPE_MASK_OBJECT | DUK_TYPE_MASK_LIGHTFUNC | DUK_TYPE_MASK_BUFFER);

  (void)duk_to_property_key_hstring(thr, -2);

  retval = duk_hobject_hasprop(thr, DUK_GET_TVAL_NEGIDX(thr, -1),
...
/* Get array index for a string which is known to be an array index.  This
 * helper is needed when duk_hstring doesn't concretely store the array index,
 * but strings are flagged as array indices at intern time.
 */
DUK_INTERNAL duk_uarridx_t
duk_js_to_arrayindex_hstring_fast_known(duk_hstring* h) {
  const duk_uint8_t* p;
  duk_uarridx_t res;
  duk_uint8_t t;
