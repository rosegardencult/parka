...
#  define DUK__FAILED(msg)                                             \
    do {                                                               \
      DUK_D(DUK_DPRINT("self test failed: " #msg " at " DUK_FILE_MACRO \
...
#  define DUK__DBLUNION_CMP_TRUE(a, b)                              \
    do {                                                            \
      if (duk_memcmp((const void*)(a), (const void*)(b),            \
                     sizeof(duk__test_double_union)) != 0) {        \
...
#    define DUK__DOUBLE_COMPARE(u, a, b, c, d, e, f, g, h)         \
      ((u)->x[0] == (h) && (u)->x[1] == (g) && (u)->x[2] == (f) && \
       (u)->x[3] == (e) && (u)->x[4] == (d) && (u)->x[5] == (c) && \
       (u)->x[6] == (b) && (u)->x[7] == (a))
...
#    define DUK__DOUBLE_COMPARE(u, a, b, c, d, e, f, g, h)         \
      ((u)->x[0] == (d) && (u)->x[1] == (c) && (u)->x[2] == (b) && \
       (u)->x[3] == (a) && (u)->x[4] == (h) && (u)->x[5] == (g) && \
       (u)->x[6] == (f) && (u)->x[7] == (e))
...
#    define DUK__DOUBLE_COMPARE(u, a, b, c, d, e, f, g, h)         \
      ((u)->x[0] == (a) && (u)->x[1] == (b) && (u)->x[2] == (c) && \
       (u)->x[3] == (d) && (u)->x[4] == (e) && (u)->x[5] == (f) && \
       (u)->x[6] == (g) && (u)->x[7] == (h))
#  else
...
DUK_LOCAL duk_uint_t duk__selftest_types(void) {
  ...
  if (!(sizeof(duk_int8_t) == 1 && sizeof(duk_uint8_t) == 1 &&
        sizeof(duk_int16_t) == 2 && sizeof(duk_uint16_t) == 2 &&
...
#  else
  DUK_D(
      DUK_DPRINT("skip double aliasing self test when duk_tval is not packed"));
  return 0;
#  endif
}
...
#  endif
  ...
  c.d = a.d + b.d;
  if (!DUK__DOUBLE_COMPARE(&c, 0xf0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00)) {
    DUK_D(DUK_DPRINT(
        "broken result (native endiannesss): %02x %02x %02x %02x %02x %02x "
        "%02x %02x",
        (unsigned int)c.x[0], (unsigned int)c.x[1], (unsigned int)c.x[2],
        (unsigned int)c.x[3], (unsigned int)c.x[4], (unsigned int)c.x[5],
  ...
  duk_memset((void*)&c, 0, sizeof(c));
  c.d = a.d + b.d;
  if (!DUK__DOUBLE_COMPARE(&c, 0x3f, 0xf0, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x02)) {
    DUK_D(DUK_DPRINT(
        "broken result (native endiannesss): %02x %02x %02x %02x %02x %02x "
        "%02x %02x",
...
DUK_LOCAL duk_uint_t duk__selftest_cast_double_to_small_uint(void) {
  ...
  if (!(duk_double_equals(d1, 1.0) && u == 1 && duk_double_equals(d2, 1.0) &&
        duk_double_equals(d1, d2))) {
  ...
  if (!(duk_double_equals(d1v, 1.0) && uv == 1 && duk_double_equals(d2v, 1.0) &&
        duk_double_equals(d1v, d2v))) {
    DUK__FAILED("double to duk_small_uint_t cast failed");
  }
...
DUK_LOCAL duk_uint_t duk__selftest_alloc_funcs(
    duk_alloc_function alloc_func, duk_realloc_function realloc_func,
    duk_free_function free_func, void* udata) {
  duk_uint_t error_count = 0;
  void* ptr;
  void* new_ptr;
  duk_small_int_t i, j;
...
DUK_INTERNAL duk_uint_t duk_selftest_run_tests(
    duk_alloc_function alloc_func, duk_realloc_function realloc_func,
    duk_free_function free_func, void* udata) {
  duk_uint_t error_count = 0;
  ...
  error_count +=
      duk__selftest_alloc_funcs(alloc_func, realloc_func, free_func, udata);

  DUK_D(DUK_DPRINT("self test complete, total error count: %ld",
                   (long)error_count));
