...
DUK_INTERNAL void duk_free_hbuffer(duk_heap* heap, duk_hbuffer* h) {
  ...
  if (DUK_HBUFFER_HAS_DYNAMIC(h) && !DUK_HBUFFER_HAS_EXTERNAL(h)) {
    ...
    DUK_DDD(DUK_DDDPRINT("free dynamic buffer %p",
                         (void*)DUK_HBUFFER_DYNAMIC_GET_DATA_PTR(heap, g)));
...
DUK_INTERNAL void duk_free_hstring(duk_heap* heap, duk_hstring* h) {
  ...
  if (DUK_HSTRING_HAS_EXTDATA(h)) {
    DUK_DDD(DUK_DDDPRINT("free extstr: hstring %!O, extdata: %p", h,
                         DUK_HSTRING_GET_EXTDATA((duk_hstring_external*)h)));
    DUK_USE_EXTSTR_FREE(heap->heap_udata, (const void*)DUK_HSTRING_GET_EXTDATA(
                                              (duk_hstring_external*)h));
  }
#endif
  DUK_FREE(heap, (void*)h);
}
...
DUK_INTERNAL void duk_heap_free_heaphdr_raw(duk_heap* heap, duk_heaphdr* hdr) {
  ...
  DUK_DDD(DUK_DDDPRINT("free heaphdr %p, htype %ld", (void*)hdr,
...
DUK_LOCAL void duk__free_finalize_list(duk_heap* heap) {
  ...
  while (curr) {
    DUK_DDD(
        DUK_DDDPRINT("FINALFREE (finalize_list): %!iO", (duk_heaphdr*)curr));
    next = DUK_HEAPHDR_GET_NEXT(heap, curr);
    duk_heap_free_heaphdr_raw(heap, curr);
    curr = next;
  }
...
#  if defined(DUK_USE_REFERENCE_COUNTING)
  DUK_ASSERT(heap->refzero_list ==
             NULL); /* refzero not running -> must be empty */
#  endif
  ...
  heap->ms_prevent_count = 1; /* Bump, because mark-and-sweep assumes it's
                                 bumped when ms_running is set. */
  ...
  for (round_no = 0;; round_no++) {
      ...
      if (DUK_HEAPHDR_IS_OBJECT(curr)) {
        ...
        if (DUK_HOBJECT_HAS_FINALIZER_FAST(heap, (duk_hobject*)curr)) {
          if (!DUK_HEAPHDR_HAS_FINALIZED((duk_heaphdr*)curr)) {
            DUK_ASSERT(DUK_HEAP_HAS_FINALIZER_NORESCUE(
                heap)); /* maps to finalizer 2nd argument */
    ...
    DUK_D(
        DUK_DPRINT("finalizer round %ld complete, %ld objects, tried to "
                   "execute %ld finalizers, current limit is %ld",
                   (long)round_no, (long)count_all, (long)count_finalized,
                   (long)curr_limit));
    ...
    if (count_finalized >= curr_limit) {
      DUK_D(
          DUK_DPRINT("finalizer count above limit, potentially runaway "
                     "finalizer; skip remaining finalizers"));
      break;
    }
  }
...
DUK_INTERNAL void duk_heap_free(duk_heap* heap) {
  ...
  duk_heap_mark_and_sweep(
      heap,
      0); /* Skip finalizers; queue finalizable objects to heap_allocated. */
...
#if defined(DUK_USE_REFERENCE_COUNTING)
  DUK_ASSERT(heap->refzero_list ==
             NULL); /* Always processed to completion inline. */
#endif
#if defined(DUK_USE_FINALIZER_SUPPORT)
  DUK_ASSERT(heap->finalize_list ==
             NULL); /* Last mark-and-sweep with skip_finalizers. */
#endif
...
#  if defined(DUK_USE_ASSERTIONS)
  for (i = 0; i < sizeof(duk_rom_strings_lookup) / sizeof(const duk_hstring*);
       i++) {
    const duk_hstring* h;
    duk_uint32_t hash;
    ...
    while (h != NULL) {
      hash =
          duk_heap_hashstring(heap, (const duk_uint8_t*)DUK_HSTRING_GET_DATA(h),
...
DUK_LOCAL duk_bool_t duk__init_heap_thread(duk_heap* heap) {
  ...
  thr = duk_hthread_alloc_unchecked(
      heap, DUK_HOBJECT_FLAG_EXTENSIBLE |
...
#  endif
  ...
  DUK_HOBJECT_SET_PROTOTYPE_INIT_INCREF(
      thr, (duk_hobject*)thr, thr->builtins[DUK_BIDX_THREAD_PROTOTYPE]);
...
#  define DUK__DUMPLM_SIGNED_RAW(t, a, b)                              \
    do {                                                               \
      DUK_D(DUK_DPRINT(t "=[%ld,%ld]=[%lf,%lf]", (long)(a), (long)(b), \
                       (double)(a), (double)(b)));                     \
    } while (0)
#  define DUK__DUMPLM_UNSIGNED_RAW(t, a, b)                            \
    do {                                                               \
      DUK_D(DUK_DPRINT(t "=[%lu,%lu]=[%lf,%lf]", (unsigned long)(a),   \
                       (unsigned long)(b), (double)(a), (double)(b))); \
    } while (0)
...
duk_heap* duk_heap_alloc(duk_alloc_function alloc_func,
  ...
  DUK_ASSERT((DUK_HTYPE_OBJECT & 0x01U) ==
             1); /* DUK_HEAPHDR_IS_OBJECT() relies ont his. */
  ...
  if (duk_selftest_run_tests(alloc_func, realloc_func, free_func, heap_udata) >
      0) {
...
#  endif
    fatal_func(heap_udata,
               "sizeof(duk_tval) not 8 or 16, cannot use "
               "DUK_USE_EXEC_REGCONST_OPTIMIZE option");
  }
...
#if defined(DUK_USE_INJECT_HEAP_ALLOC_ERROR) && \
    (DUK_USE_INJECT_HEAP_ALLOC_ERROR == 1)
  goto failed;
#endif
...
#  if !defined(DUK_USE_STRHASH_DENSE)
  res->hash_seed ^= 5381; /* Bernstein hash init value is normally 5381; XOR it
                             in in case pointer low bits are 0 */
#  endif
#endif /* DUK_USE_ROM_STRINGS */
...
#if defined(DUK_USE_STRTAB_PTRCOMP)
  res->strtable16 =
      (duk_uint16_t*)alloc_func(heap_udata, sizeof(duk_uint16_t) * st_initsize);
  if (res->strtable16 == NULL) {
    goto failed;
  }
#else
  res->strtable =
      (duk_hstring**)alloc_func(heap_udata, sizeof(duk_hstring*) * st_initsize);
...
#if defined(DUK_USE_INJECT_HEAP_ALLOC_ERROR) && \
    (DUK_USE_INJECT_HEAP_ALLOC_ERROR == 2)
  goto failed;
#endif
...
#if defined(DUK_USE_INJECT_HEAP_ALLOC_ERROR) && \
    (DUK_USE_INJECT_HEAP_ALLOC_ERROR == 3)
  goto failed;
#endif
...
#if defined(DUK_USE_INJECT_HEAP_ALLOC_ERROR) && \
    (DUK_USE_INJECT_HEAP_ALLOC_ERROR == 4)
  goto failed;
#endif
  DUK_D(DUK_DPRINT("heap init: initialize heap object"));
  DUK_ASSERT(res->heap_thread != NULL);
  res->heap_object = duk_hobject_alloc_unchecked(
      res, DUK_HOBJECT_FLAG_EXTENSIBLE | DUK_HOBJECT_FLAG_FASTREFS |
...
#  else
  res->rnd_state[0] =
      (duk_uint64_t)duk_time_get_ecmascript_time(res->heap_thread);
  DUK_ASSERT(res->rnd_state[1] ==
             0); /* Not filled here, filled in by seed preparation. */
...
#    endif
   ...
   */
  {
    ...
    tmp_u64 = 0;
    duk_memcpy(
        (void*)&tmp_u64, (const void*)&res,
        (size_t)(sizeof(void*) >= sizeof(duk_uint64_t) ? sizeof(duk_uint64_t)
                                                       : sizeof(void*)));
