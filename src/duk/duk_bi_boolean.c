...
duk_bi_boolean_prototype_tostring_shared(duk_hthread* thr) {
  duk_tval* tv;
  duk_hobject* h;
...
DUK_INTERNAL duk_ret_t duk_bi_boolean_constructor(duk_hthread* thr) {
  ...
  if (duk_is_constructor_call(thr)) {
    /* XXX: helper; rely on Boolean.prototype as being non-writable,
     * non-configurable */
    duk_push_this(thr);
    h_this = duk_known_hobject(thr, -1);
    DUK_ASSERT(DUK_HOBJECT_GET_PROTOTYPE(thr->heap, h_this) ==
               thr->builtins[DUK_BIDX_BOOLEAN_PROTOTYPE]);
    ...
    duk_xdef_prop_stridx_short(
        ...
        DUK_PROPDESC_FLAGS_NONE); /* XXX: proper flags? */
  }                               /* unbalanced stack */
