...
#    if defined(DUK_USE_ROM_GLOBAL_INHERIT)
  ...
  h_global = duk_push_object_helper(
      thr,
...
#    elif defined(DUK_USE_ROM_GLOBAL_CLONE)
  ...
  duk_memcpy((void*)props,
             (const void*)DUK_HOBJECT_GET_PROPS(thr->heap, h_oldglobal),
             alloc_size);
...
#    endif
  ...
  DUK_ASSERT(!DUK_HEAPHDR_NEEDS_REFCOUNT_UPDATE(
      (duk_heaphdr*)
          thr->builtins[DUK_BIDX_GLOBAL])); /* no need to decref: ROM object */
   ...
   * global object.
   */
  h_objenv = (duk_hobject*)duk_hobjenv_alloc(
  ...
  DUK_ASSERT(!DUK_HEAPHDR_NEEDS_REFCOUNT_UPDATE(
      (duk_heaphdr*)thr
          ->builtins[DUK_BIDX_GLOBAL_ENV])); /* no need to decref: ROM object */
...
DUK_INTERNAL void duk_hthread_create_builtin_objects(duk_hthread* thr) {
  ...
  DUK_D(DUK_DPRINT(
      ...
      (int)DUK_NUM_BUILTINS, (int)DUK_NUM_ALL_BUILTINS));
  ...
  for (i = 0; i < DUK_NUM_ALL_BUILTINS; i++) {
    ...
    len = (duk_small_int_t)duk_bd_decode_flagged_signed(
        bd, DUK__LENGTH_PROP_BITS, (duk_int32_t)-1 /*def_value*/);
    ...
    if (class_num == DUK_HOBJECT_CLASS_FUNCTION) {
      ...
      c_nargs = (duk_small_int_t)duk_bd_decode_flagged_signed(
          bd, DUK__NARGS_BITS, len /*def_value*/);
...
#  endif
      ...
      (void)duk_push_object_helper(
          thr, DUK_HOBJECT_FLAG_FASTREFS | DUK_HOBJECT_FLAG_EXTENSIBLE,
    ...
    DUK_ASSERT(!DUK_HOBJECT_HAS_ARRAY_PART(h) ||
               class_num == DUK_HOBJECT_CLASS_ARRAY);
    /* DUK_HOBJECT_FLAG_STRICT varies */
    DUK_ASSERT(
        !DUK_HOBJECT_HAS_NATFUNC(h) || /* all native functions have NEWENV */
    ...
    DUK_DDD(DUK_DDDPRINT("created built-in %ld, class=%ld, length=%ld", (long)i,
                         (long)class_num, (long)len));
  }
  ...
  for (i = 0; i < DUK_NUM_ALL_BUILTINS; i++) {
    ...
    if (t > 0) {
      ...
      DUK_HOBJECT_SET_PROTOTYPE_UPDREF(thr, h,
                                       duk_known_hobject(thr, (duk_idx_t)t));
      ...
      DUK_HOBJECT_SET_PROTOTYPE_UPDREF(
          thr, h, thr->builtins[DUK_BIDX_FUNCTION_PROTOTYPE]);
    }

    t = (duk_small_uint_t)duk_bd_decode_varuint(bd);
    if (t > 0) {
      /* 'prototype' property for all built-in objects (which have it) has
       * attributes:
       *  [[Writable]] = false,
       *  [[Enumerable]] = false,
       *  [[Configurable]] = false
      ...
      duk_xdef_prop_stridx(thr, (duk_idx_t)i, DUK_STRIDX_PROTOTYPE,
                           DUK_PROPDESC_FLAGS_NONE);
    }

    t = (duk_small_uint_t)duk_bd_decode_varuint(bd);
    if (t > 0) {
      /* 'constructor' property for all built-in objects (which have it) has
       * attributes:
       *  [[Writable]] = true,
       *  [[Enumerable]] = false,
       *  [[Configurable]] = true
      ...
      duk_dup(thr, (duk_idx_t)t);
      duk_xdef_prop_stridx(thr, (duk_idx_t)i, DUK_STRIDX_CONSTRUCTOR,
                           DUK_PROPDESC_FLAGS_WC);
    }

    /* normal valued properties */
    num = (duk_small_uint_t)duk_bd_decode_varuint(bd);
    DUK_DDD(DUK_DDDPRINT("built-in object %ld, %ld normal valued properties",
                         (long)i, (long)num));
    for (j = 0; j < num; j++) {
      ...
      defprop_flags = (duk_small_uint_t)duk_bd_decode_flagged(
          bd, DUK__PROP_FLAGS_BITS, (duk_uint32_t)DUK_PROPDESC_FLAGS_WC);
      defprop_flags |=
          DUK_DEFPROP_FORCE | DUK_DEFPROP_HAVE_VALUE |
      ...
      DUK_DDD(
          DUK_DDDPRINT("built-in %ld, normal-valued property %ld, key %!T, "
                       "flags 0x%02lx, type %ld",
                       (long)i, (long)j, duk_get_tval(thr, -1),
                       (unsigned long)defprop_flags, (long)t));
      ...
      switch (t) {
        ...
        case DUK__PROP_TYPE_ACCESSOR: {
          duk_small_uint_t natidx_getter =
              (duk_small_uint_t)duk_bd_decode_varuint(bd);
          duk_small_uint_t natidx_setter =
              (duk_small_uint_t)duk_bd_decode_varuint(bd);
          duk_small_uint_t accessor_magic =
              (duk_small_uint_t)duk_bd_decode_varuint(bd);
          duk_c_function c_func_getter;
          duk_c_function c_func_setter;

          DUK_DDD(
              DUK_DDDPRINT("built-in accessor property: objidx=%ld, key=%!T, "
                           "getteridx=%ld, setteridx=%ld, flags=0x%04lx",
                           (long)i, duk_get_tval(thr, -1), (long)natidx_getter,
                           (long)natidx_setter, (unsigned long)defprop_flags));

          c_func_getter = duk_bi_native_functions[natidx_getter];
          if (c_func_getter != NULL) {
            duk_push_c_function_builtin_noconstruct(thr, c_func_getter,
                                                    0); /* always 0 args */
          ...
          if (c_func_setter != NULL) {
            duk_push_c_function_builtin_noconstruct(thr, c_func_setter,
                                                    1); /* always 1 arg */
            duk_set_magic(thr, -1, (duk_int_t)accessor_magic);
          ...
          DUK_ASSERT((defprop_flags & DUK_PROPDESC_FLAG_WRITABLE) ==
    ...
    num = (duk_small_uint_t)duk_bd_decode_varuint(bd);
    DUK_DDD(DUK_DDDPRINT("built-in object %ld, %ld function valued properties",
                         (long)i, (long)num));
    for (j = 0; j < num; j++) {
      duk_hstring* h_key;
      duk_small_uint_t natidx;
...
#  endif
      ...
      c_nargs = (duk_int_t)duk_bd_decode_flagged(
          bd, DUK__NARGS_BITS, (duk_uint32_t)c_length /*def_value*/);
      if (c_nargs == DUK__NARGS_VARARGS_MARKER) {
        c_nargs = DUK_VARARGS;
      }
      ...
      DUK_DDD(DUK_DDDPRINT(
          "built-in %ld, function-valued property %ld, key %!O, natidx %ld, "
          "length %ld, nargs %ld",
...
#  if defined(DUK_USE_LIGHTFUNC_BUILTINS)
      lightfunc_eligible =
          ((c_nargs >= DUK_LFUNC_NARGS_MIN && c_nargs <= DUK_LFUNC_NARGS_MAX) ||
           (c_nargs == DUK_VARARGS)) &&
...
#    endif
      ...
      if (lightfunc_eligible) {
        duk_tval tv_lfunc;
        duk_small_uint_t lf_nargs = (duk_small_uint_t)(
            c_nargs == DUK_VARARGS ? DUK_LFUNC_NARGS_VARARGS : c_nargs);
        duk_small_uint_t lf_flags =
            DUK_LFUNC_FLAGS_PACK(magic, c_length, lf_nargs);
        DUK_TVAL_SET_LIGHTFUNC(&tv_lfunc, c_func, lf_flags);
        duk_push_tval(thr, &tv_lfunc);
        DUK_D(
            DUK_DPRINT("built-in function eligible as light function: i=%d, "
                       "j=%d c_length=%ld, c_nargs=%ld, magic=%ld -> %!iT",
                       (int)i, (int)j, (long)c_length, (long)c_nargs,
                       (long)magic, duk_get_tval(thr, -1)));
        goto lightfunc_skip;
      }

      DUK_D(DUK_DPRINT(
          "built-in function NOT ELIGIBLE as light function: i=%d, j=%d "
          "c_length=%ld, c_nargs=%ld, magic=%ld",
          (int)i, (int)j, (long)c_length, (long)c_nargs, (long)magic));
...
#  endif
      ...
      defprop_flags = (duk_small_uint_t)duk_bd_decode_flagged(
          bd, DUK__PROP_FLAGS_BITS, (duk_uint32_t)DUK_PROPDESC_FLAGS_WC);
      defprop_flags |= DUK_DEFPROP_FORCE | DUK_DEFPROP_HAVE_VALUE |
                       DUK_DEFPROP_HAVE_WRITABLE | DUK_DEFPROP_HAVE_ENUMERABLE |
...
#  if defined(DUK_USE_DATE_BUILTIN)
  ...
  duk_xdef_prop_stridx_short(thr, DUK_BIDX_DATE_PROTOTYPE,
...
#  if !defined(DUK_USE_ES6_OBJECT_PROTO_PROPERTY)
  DUK_DD(
      DUK_DDPRINT("delete Object.prototype.__proto__ built-in which is not "
                  "enabled in features"));
  (void)duk_hobject_delprop_raw(thr, thr->builtins[DUK_BIDX_OBJECT_PROTOTYPE],
...
#  if !defined(DUK_USE_ES6_OBJECT_SETPROTOTYPEOF)
  DUK_DD(
      DUK_DDPRINT("delete Object.setPrototypeOf built-in which is not enabled "
                  "in features"));
  (void)duk_hobject_delprop_raw(thr, thr->builtins[DUK_BIDX_OBJECT_CONSTRUCTOR],
...
#  elif defined(DUK_USE_INTEGER_BE)
                  "b"
#  elif defined( \
      DUK_USE_INTEGER_ME) /* integer mixed endian not really used now */
                  "m"
#  else
                  "?"
#  endif
...
#  endif
                  ...
                  DUK_USE_ARCH_STRING " " DUK_USE_OS_STRING
                  " " DUK_USE_COMPILER_STRING);
  duk_xdef_prop_stridx_short(thr, DUK_BIDX_DUKTAPE, DUK_STRIDX_ENV,
                             DUK_PROPDESC_FLAGS_WC);
...
DUK_INTERNAL void duk_hthread_copy_builtin_objects(duk_hthread* thr_from,
  ...
  for (i = 0; i < DUK_NUM_BUILTINS; i++) {
    ...
    DUK_HOBJECT_INCREF_ALLOWNULL(thr_to,
                                 thr_to->builtins[i]); /* side effect free */
  }
}
