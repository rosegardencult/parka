...
/* XXX: currently defines unnecessary symbols when DUK_USE_DATE_BUILTIN is
 * disabled. */
...
DUK_LOCAL_DECL duk_double_t duk__push_this_get_timeval_tzoffset(
    duk_hthread* thr, duk_small_uint_t flags, duk_int_t* out_tzoffset);
DUK_LOCAL_DECL duk_double_t duk__push_this_get_timeval(duk_hthread* thr,
                                                       duk_small_uint_t flags);
DUK_LOCAL_DECL void duk__twodigit_year_fixup(duk_hthread* thr,
                                             duk_idx_t idx_val);
DUK_LOCAL_DECL duk_ret_t duk__set_this_timeval_from_dparts(
    duk_hthread* thr, duk_double_t* dparts, duk_small_uint_t flags);
...
#define DUK__DPRINT_PARTS_AND_DPARTS(parts, dparts)                            \
  do {                                                                         \
    DUK_D(DUK_DPRINT(                                                          \
        "parts: %ld %ld %ld %ld %ld %ld %ld %ld, dparts: %lf %lf %lf %lf %lf " \
        "%lf %lf %lf",                                                         \
        (long)(parts)[0], (long)(parts)[1], (long)(parts)[2],                  \
        (long)(parts)[3], (long)(parts)[4], (long)(parts)[5],                  \
        (long)(parts)[6], (long)(parts)[7], (double)(dparts)[0],               \
        (double)(dparts)[1], (double)(dparts)[2], (double)(dparts)[3],         \
        (double)(dparts)[4], (double)(dparts)[5], (double)(dparts)[6],         \
        (double)(dparts)[7]));                                                 \
  } while (0)
...
#define DUK__CF_ACCEPT_NUL \
  (1 << 2) /* accept string if next char is NUL (otherwise reject) */

#define DUK__PACK_RULE(partmask, sepmask, nextpart, flags)       \
  ((duk_uint32_t)(partmask) + (((duk_uint32_t)(sepmask)) << 9) + \
   (((duk_uint32_t)(nextpart)) << 17) + (((duk_uint32_t)(flags)) << 21))
...
DUK_LOCAL const duk_uint8_t duk__parse_iso8601_seps[] = {
    DUK_ASC_PLUS /*0*/,  DUK_ASC_MINUS /*1*/, DUK_ASC_UC_T /*2*/,
    DUK_ASC_SPACE /*3*/, DUK_ASC_COLON /*4*/, DUK_ASC_PERIOD /*5*/,
    DUK_ASC_UC_Z /*6*/,  DUK_ASC_NUL /*7*/
};
...
DUK_LOCAL const duk_uint32_t duk__parse_iso8601_control[] = {
    ...
    DUK__PACK_RULE(DUK__PM_YEAR | DUK__PM_MONTH | DUK__PM_DAY,
                   DUK__SM_T | DUK__SM_SPACE, DUK__PI_HOUR, 0),
    ...
    DUK__PACK_RULE(DUK__PM_YEAR | DUK__PM_MONTH | DUK__PM_DAY |
                       DUK__PM_HOUR /*Note1*/ | DUK__PM_MINUTE |
                       DUK__PM_SECOND | DUK__PM_MILLISECOND,
                   DUK__SM_PLUS, DUK__PI_TZHOUR, 0),
    DUK__PACK_RULE(DUK__PM_YEAR | DUK__PM_MONTH | DUK__PM_DAY |
                       DUK__PM_HOUR /*Note1*/ | DUK__PM_MINUTE |
                       DUK__PM_SECOND | DUK__PM_MILLISECOND,
                   DUK__SM_MINUS, DUK__PI_TZHOUR, DUK__CF_NEG),
    DUK__PACK_RULE(DUK__PM_YEAR | DUK__PM_MONTH | DUK__PM_DAY |
                       DUK__PM_HOUR /*Note1*/ | DUK__PM_MINUTE |
                       DUK__PM_SECOND | DUK__PM_MILLISECOND,
                   DUK__SM_Z, 0, DUK__CF_ACCEPT_NUL),
    DUK__PACK_RULE(DUK__PM_YEAR | DUK__PM_MONTH | DUK__PM_DAY |
                       DUK__PM_HOUR /*Note1*/ | DUK__PM_MINUTE |
                       DUK__PM_SECOND | DUK__PM_MILLISECOND |
                       DUK__PM_TZHOUR /*Note2*/ | DUK__PM_TZMINUTE,
                   DUK__SM_NUL, 0, DUK__CF_ACCEPT)
     ...
     *        with just hours ("HH"), but accept it here, e.g.
     * "2012-01-02T03:04:05+02"
     */
};
...
DUK_LOCAL duk_bool_t duk__parse_string_iso8601_subset(duk_hthread* thr,
  ...
  DUK_ASSERT(parts[DUK_DATE_IDX_YEAR] ==
  ...
  for (;;) {
    ...
    DUK_DDD(DUK_DDDPRINT(
        "parsing, part_idx=%ld, char=%ld ('%c')", (long)part_idx, (long)ch,
    ...
    if (ch >= DUK_ASC_0 && ch <= DUK_ASC_9) {
      ...
      DUK_DDD(DUK_DDDPRINT("wrote part %ld -> value %ld", (long)part_idx,
                           (long)accum));

      accum = 0;
      ndigits = 0;

      for (i = 0; i < (duk_small_uint_t)(sizeof(duk__parse_iso8601_seps) /
                                         sizeof(duk_uint8_t));
           i++) {
      ...
      if (i == (duk_small_uint_t)(sizeof(duk__parse_iso8601_seps) /
                                  sizeof(duk_uint8_t))) {
      ...
      sep_idx = i;
      match_val = (1UL << part_idx) +
                  (1UL << (sep_idx + 9)); /* match against rule part/sep bits */

      for (i = 0; i < (duk_small_uint_t)(sizeof(duk__parse_iso8601_control) /
                                         sizeof(duk_uint32_t));
           i++) {
        ...
        DUK_DDD(
            DUK_DDDPRINT("part_idx=%ld, sep_idx=%ld, match_val=0x%08lx, "
                         "considering rule=0x%08lx",
        ...
        DUK_DDD(DUK_DDDPRINT(
            "rule match -> part_idx=%ld, sep_idx=%ld, match_val=0x%08lx, "
            "rule=0x%08lx -> nextpart=%ld, cflags=0x%02lx",
            (long)part_idx, (long)sep_idx, (unsigned long)match_val,
            (unsigned long)rule, (long)nextpart, (unsigned long)cflags));
      ...
      if (i == (duk_small_uint_t)(sizeof(duk__parse_iso8601_control) /
                                  sizeof(duk_uint32_t))) {
      ...
      if (ch == 0) {
         ...
         */
        DUK_DDD(DUK_DDDPRINT(
            "NUL after rule matching (should not happen) -> reject"));
        goto reject;
      }
    } /* if-digit-else-ctrl */
  }   /* char loop */
...
accept:
   ...
   *
   * Note: DUK_DATE_IDX_WEEKDAY is initialized with a bogus value
   * (DUK__PI_TZHOUR) on purpose.  It won't be actually used by
   * duk_bi_date_get_timeval_from_dparts(), but will make the value initialized
   * just in case, and avoid any potential for Valgrind issues.
   */
...
DUK_INTERNAL duk_bool_t duk_bi_date_timeval_in_leeway_range(duk_double_t x) {
  return (x >= -DUK_DATE_MSEC_100M_DAYS_LEEWAY &&
          x <= DUK_DATE_MSEC_100M_DAYS_LEEWAY);
}
...
DUK_LOCAL duk_int_t duk__day_from_year(duk_int_t year) {
  /* Note: in integer arithmetic, (x / 4) is same as floor(x / 4) for
   * non-negative values, but is incorrect for negative ones.
   */
  return 365 * (year - 1970) + duk__div_floor(year - 1969, 4) -
         duk__div_floor(year - 1901, 100) + duk__div_floor(year - 1601, 400);
}
...
DUK_LOCAL duk_int_t duk__year_from_day(duk_int_t day,
  ...
  for (;;) {
    ...
    DUK_DDD(DUK_DDDPRINT("year=%ld day=%ld, diff_days=%ld", (long)year,
                         (long)day, (long)diff_days));
    if (diff_days <= 0) {
      DUK_ASSERT(-diff_days < 366); /* fits into duk_small_int_t */
      *out_day_within_year = -diff_days;
      DUK_DDD(DUK_DDDPRINT("--> year=%ld, day-within-year=%ld", (long)year,
                           (long)*out_day_within_year));
      DUK_ASSERT(*out_day_within_year >= 0);
      DUK_ASSERT(*out_day_within_year <
                 (duk_bi_date_is_leap_year(year) ? 366 : 365));
      return year;
    }
 ...
 */
DUK_LOCAL duk_double_t duk__make_day(duk_double_t year, duk_double_t month,
                                     duk_double_t day) {
  duk_int_t day_num;
  duk_bool_t is_leap;
  duk_small_int_t i, n;
 ...
 */
DUK_INTERNAL void duk_bi_date_timeval_to_parts(duk_double_t d, duk_int_t* parts,
                                               duk_double_t* dparts,
                                               duk_small_uint_t flags) {
  duk_double_t d1, d2;
  duk_int_t t1, t2;
  ...
  DUK_ASSERT(parts[DUK_DATE_IDX_MILLISECOND] >= 0 &&
             parts[DUK_DATE_IDX_MILLISECOND] <= 999);
  DUK_ASSERT(parts[DUK_DATE_IDX_SECOND] >= 0 &&
             parts[DUK_DATE_IDX_SECOND] <= 59);
  DUK_ASSERT(parts[DUK_DATE_IDX_MINUTE] >= 0 &&
             parts[DUK_DATE_IDX_MINUTE] <= 59);
  DUK_ASSERT(parts[DUK_DATE_IDX_HOUR] >= 0 && parts[DUK_DATE_IDX_HOUR] <= 23);

  DUK_DDD(DUK_DDDPRINT(
      "d=%lf, d1=%lf, d2=%lf, t1=%ld, t2=%ld, parts: hour=%ld min=%ld sec=%ld "
      "msec=%ld",
      (double)d, (double)d1, (double)d2, (long)t1, (long)t2,
      (long)parts[DUK_DATE_IDX_HOUR], (long)parts[DUK_DATE_IDX_MINUTE],
      (long)parts[DUK_DATE_IDX_SECOND], (long)parts[DUK_DATE_IDX_MILLISECOND]));
  ...
  parts[DUK_DATE_IDX_WEEKDAY] =
      (t2 + 4 + DUK__WEEKDAY_MOD_ADDER) % 7; /* E5.1 Section 15.9.1.6 */
  DUK_ASSERT(parts[DUK_DATE_IDX_WEEKDAY] >= 0 &&
             parts[DUK_DATE_IDX_WEEKDAY] <= 6);
  ...
  for (month = 0; month < 12; month++) {
    ...
    DUK_DDD(DUK_DDDPRINT("month=%ld, dim=%ld, day=%ld", (long)month, (long)dim,
                         (long)day));
    if (day < dim) {
      break;
    }
  ...
  if ((flags & DUK_DATE_FLAG_EQUIVYEAR) && (year < 1971 || year > 2037)) {
    ...
    jan1_since_epoch =
        day_since_epoch - day_in_year; /* day number for Jan 1 since epoch */
    DUK_ASSERT(jan1_since_epoch + DUK__WEEKDAY_MOD_ADDER >= 0);
    jan1_weekday = (jan1_since_epoch + 4 + DUK__WEEKDAY_MOD_ADDER) %
    ...
    DUK_ASSERT(arridx >= 0 &&
               arridx < (duk_small_int_t)(sizeof(duk__date_equivyear) /
                                          sizeof(duk_uint8_t)));
 ...
 */
DUK_INTERNAL duk_double_t duk_bi_date_get_timeval_from_dparts(
  ...
  tmp_time +=
      dparts[DUK_DATE_IDX_MINUTE] * ((duk_double_t)DUK_DATE_MSEC_MINUTE);
  tmp_time +=
      dparts[DUK_DATE_IDX_SECOND] * ((duk_double_t)DUK_DATE_MSEC_SECOND);
  tmp_time += dparts[DUK_DATE_IDX_MILLISECOND];

  /* MakeDay */
  tmp_day = duk__make_day(dparts[DUK_DATE_IDX_YEAR], dparts[DUK_DATE_IDX_MONTH],
                          dparts[DUK_DATE_IDX_DAY]);

  /* MakeDate */
  d = tmp_day * ((duk_double_t)DUK_DATE_MSEC_DAY) + tmp_time;

  DUK_DDD(DUK_DDDPRINT("time=%lf day=%lf --> timeval=%lf", (double)tmp_time,
                       (double)tmp_day, (double)d));
  ...
  if (flags & DUK_DATE_FLAG_LOCALTIME) {
    ...
    for (i = 0; i < DUK__LOCAL_TZOFFSET_MAXITER; i++) {
      ...
      if (tzoff == tzoffprev1) {
        DUK_DDD(DUK_DDDPRINT(
            "tzoffset iteration finished, i=%d, tzoff=%ld, tzoffprev1=%ld, "
            "tzoffprev2=%ld",
            (int)i, (long)tzoff, (long)tzoffprev1, (long)tzoffprev2));
        break;
         ...
         */
        DUK_DDD(DUK_DDDPRINT(
            "tzoffset iteration two-value cycle, i=%d, tzoff=%ld, "
            "tzoffprev1=%ld, tzoffprev2=%ld",
            (int)i, (long)tzoff, (long)tzoffprev1, (long)tzoffprev2));
        if (tzoffprev1 > tzoff) {
          tzoff = tzoffprev1;
        }
 ...
 */
DUK_LOCAL duk_double_t duk__push_this_get_timeval_tzoffset(
    duk_hthread* thr, duk_small_uint_t flags, duk_int_t* out_tzoffset) {
  duk_hobject* h;
  duk_double_t d;
  ...
  duk_push_this(thr);
  h = duk_get_hobject(
      thr, -1); /* XXX: getter with class check, useful in built-ins */
...
DUK_LOCAL duk_ret_t duk__set_this_timeval_from_dparts(duk_hthread* thr,
   ...
   */
  duk_xdef_prop_stridx_short(thr, -3, DUK_STRIDX_INT_VALUE,
...
DUK_LOCAL void duk__format_parts_iso8601(duk_int_t* parts, duk_int_t tzoffset,
                                         duk_small_uint_t flags,
                                         duk_uint8_t* out_buf) {
  ...
  DUK_ASSERT(parts[DUK_DATE_IDX_YEAR] >= -999999 &&
             parts[DUK_DATE_IDX_YEAR] <= 999999);

  /* Note: %06d for positive value, %07d for negative value to include
   * sign and 6 digits.
   */
  DUK_SNPRINTF(
      yearstr, sizeof(yearstr),
  ...
  if (flags & DUK_DATE_FLAG_LOCALTIME) {
    ...
    arg_hours = tmp / 60;
    arg_minutes = tmp % 60;
    DUK_ASSERT(arg_hours <=
               24); /* Even less is actually guaranteed for a valid tzoffset. */
    arg_hours = arg_hours &
                0x3f; /* For [0,24] this is a no-op, but fixes GCC 7 warning,
                         see https://github.com/svaarala/duktape/issues/1602. */
   ...
   * is portable.
   */
  if ((flags & DUK_DATE_FLAG_TOSTRING_DATE) &&
      (flags & DUK_DATE_FLAG_TOSTRING_TIME)) {
    DUK_SPRINTF((char*)out_buf, "%s-%02d-%02d%c%02d:%02d:%02d.%03d%s",
                (const char*)yearstr, (int)parts[DUK_DATE_IDX_MONTH],
                (int)parts[DUK_DATE_IDX_DAY], (int)sep,
                (int)parts[DUK_DATE_IDX_HOUR], (int)parts[DUK_DATE_IDX_MINUTE],
                (int)parts[DUK_DATE_IDX_SECOND],
                (int)parts[DUK_DATE_IDX_MILLISECOND], (const char*)tzstr);
  } else if (flags & DUK_DATE_FLAG_TOSTRING_DATE) {
    DUK_SPRINTF((char*)out_buf, "%s-%02d-%02d", (const char*)yearstr,
                (int)parts[DUK_DATE_IDX_MONTH], (int)parts[DUK_DATE_IDX_DAY]);
  } else {
...
DUK_LOCAL duk_ret_t duk__get_part_helper(duk_hthread* thr,
  ...
  duk_double_t d;
  duk_int_t parts[DUK_DATE_IDX_NUM_PARTS];
  duk_small_uint_t idx_part = (duk_small_uint_t)(
      flags_and_idx >> DUK_DATE_FLAG_VALUE_SHIFT); /* unpack args */
  ...
  duk_bi_date_timeval_to_parts(d, parts, NULL,
                               flags_and_idx); /* no need to mask idx portion */
   ...
   * unconditionally.
   */
  duk_push_int(thr, (flags_and_idx & DUK_DATE_FLAG_SUB1900)
                        ? parts[idx_part] - 1900
                        : parts[idx_part]);
  return 1;
}
...
DUK_LOCAL duk_ret_t duk__set_part_helper(duk_hthread* thr,
  ...
  duk_double_t dparts[DUK_DATE_IDX_NUM_PARTS];
  duk_idx_t nargs;
  duk_small_uint_t maxnargs = (duk_small_uint_t)(
      flags_and_maxnargs >> DUK_DATE_FLAG_VALUE_SHIFT); /* unpack args */
  ...
  for (i = 0; i < maxnargs; i++) {
    ...
    if (idx == DUK_DATE_IDX_YEAR &&
        (flags_and_maxnargs & DUK_DATE_FLAG_YEAR_FIXUP)) {
 ...
 */
DUK_LOCAL void duk__set_parts_from_args(duk_hthread* thr, duk_double_t* dparts,
                                        duk_idx_t nargs) {
  duk_double_t d;
  duk_small_uint_t i;
  ...
  DUK_DDD(DUK_DDDPRINT("parts from args -> %lf %lf %lf %lf %lf %lf %lf %lf",
                       (double)dparts[0], (double)dparts[1], (double)dparts[2],
                       (double)dparts[3], (double)dparts[4], (double)dparts[5],
                       (double)dparts[6], (double)dparts[7]));
}
...
static duk_uint16_t duk__date_magics[] = {
    ...
    /* 3: toLocaleString */
    DUK_DATE_FLAG_TOSTRING_DATE + DUK_DATE_FLAG_TOSTRING_TIME +
        DUK_DATE_FLAG_TOSTRING_LOCALE + DUK_DATE_FLAG_LOCALTIME,

    /* 4: toLocaleDateString */
    DUK_DATE_FLAG_TOSTRING_DATE + DUK_DATE_FLAG_TOSTRING_LOCALE +
        DUK_DATE_FLAG_LOCALTIME,
    ...
    /* 7: toISOString */
    DUK_DATE_FLAG_TOSTRING_DATE + DUK_DATE_FLAG_TOSTRING_TIME +
        DUK_DATE_FLAG_NAN_TO_RANGE_ERROR + DUK_DATE_FLAG_SEP_T,
    ...
    /* 12: getDate */
    DUK_DATE_FLAG_ONEBASED + DUK_DATE_FLAG_LOCALTIME +
        (DUK_DATE_IDX_DAY << DUK_DATE_FLAG_VALUE_SHIFT),
    ...
    /* 14: getDay */
    DUK_DATE_FLAG_LOCALTIME +
        (DUK_DATE_IDX_WEEKDAY << DUK_DATE_FLAG_VALUE_SHIFT),
    ...
    /* 18: getMinutes */
    DUK_DATE_FLAG_LOCALTIME +
        (DUK_DATE_IDX_MINUTE << DUK_DATE_FLAG_VALUE_SHIFT),
    ...
    /* 20: getSeconds */
    DUK_DATE_FLAG_LOCALTIME +
        (DUK_DATE_IDX_SECOND << DUK_DATE_FLAG_VALUE_SHIFT),
    ...
    /* 22: getMilliseconds */
    DUK_DATE_FLAG_LOCALTIME +
        (DUK_DATE_IDX_MILLISECOND << DUK_DATE_FLAG_VALUE_SHIFT),

    /* 23: getUTCMilliseconds */
    0 + (DUK_DATE_IDX_MILLISECOND << DUK_DATE_FLAG_VALUE_SHIFT),

    /* 24: setMilliseconds */
    DUK_DATE_FLAG_TIMESETTER + DUK_DATE_FLAG_LOCALTIME +
    ...
    /* 36: setFullYear */
    DUK_DATE_FLAG_NAN_TO_ZERO + DUK_DATE_FLAG_LOCALTIME +
        (3 << DUK_DATE_FLAG_VALUE_SHIFT),
    ...
    /* 38: getYear */
    DUK_DATE_FLAG_LOCALTIME + DUK_DATE_FLAG_SUB1900 +
        (DUK_DATE_IDX_YEAR << DUK_DATE_FLAG_VALUE_SHIFT),

    /* 39: setYear */
    DUK_DATE_FLAG_NAN_TO_ZERO + DUK_DATE_FLAG_YEAR_FIXUP +
        (3 << DUK_DATE_FLAG_VALUE_SHIFT),
};
...
DUK_LOCAL duk_small_uint_t duk__date_get_indirect_magic(duk_hthread* thr) {
  ...
  DUK_ASSERT(magicidx < (duk_small_int_t)(sizeof(duk__date_magics) /
                                          sizeof(duk_uint16_t)));
...
DUK_INTERNAL duk_ret_t duk_bi_date_constructor(duk_hthread* thr) {
  ...
  DUK_DDD(DUK_DDDPRINT("Date constructor, nargs=%ld, is_cons=%ld", (long)nargs,
                       (long)is_cons));
  ...
  (void)duk__set_this_timeval_from_dparts(
      thr, dparts,
      DUK_DATE_FLAG_LOCALTIME /*flags*/); /* -> [ ... this timeval ] */
  duk_pop(thr);                           /* -> [ ... this ] */
  return 1;
}
...
DUK_INTERNAL duk_ret_t duk_bi_date_constructor_now(duk_hthread* thr) {
  ...
  DUK_ASSERT(duk_double_equals(duk__timeclip(d),
                               d)); /* TimeClip() should never be necessary */
  duk_push_number(thr, d);
  return 1;
}
...
DUK_INTERNAL duk_ret_t duk_bi_date_prototype_value_of(duk_hthread* thr) {
  ...
  duk_double_t d =
      duk__push_this_get_timeval(thr, 0 /*flags*/); /* -> [ this ] */
  DUK_ASSERT(DUK_ISFINITE(d) || DUK_ISNAN(d));
  duk_push_number(thr, d);
  return 1;
}
...
duk_bi_date_prototype_get_timezone_offset(duk_hthread* thr) {
  /*
