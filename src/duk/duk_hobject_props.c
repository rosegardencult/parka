 ...
 *      and an explicit DUK_REFZERO_CHECK_xxx() if achieving correct order is
 * hard.
 */
...
duk__check_arguments_map_for_get(duk_hthread* thr, duk_hobject* obj,
                                 duk_hstring* key, duk_propdesc* temp_desc);
DUK_LOCAL_DECL void duk__check_arguments_map_for_put(duk_hthread* thr,
                                                     duk_hobject* obj,
                                                     duk_hstring* key,
                                                     duk_propdesc* temp_desc,
                                                     duk_bool_t throw_flag);
DUK_LOCAL_DECL void duk__check_arguments_map_for_delete(
    duk_hthread* thr, duk_hobject* obj, duk_hstring* key,
    duk_propdesc* temp_desc);

DUK_LOCAL_DECL duk_bool_t duk__handle_put_array_length_smaller(
    duk_hthread* thr, duk_hobject* obj, duk_uint32_t old_len,
    duk_uint32_t new_len, duk_bool_t force_flag, duk_uint32_t* out_result_len);
DUK_LOCAL_DECL duk_bool_t duk__handle_put_array_length(duk_hthread* thr,
                                                       duk_hobject* obj);

DUK_LOCAL_DECL duk_bool_t duk__get_propdesc(duk_hthread* thr, duk_hobject* obj,
                                            duk_hstring* key,
                                            duk_propdesc* out_desc,
                                            duk_small_uint_t flags);
DUK_LOCAL_DECL duk_bool_t duk__get_own_propdesc_raw(
    duk_hthread* thr, duk_hobject* obj, duk_hstring* key, duk_uint32_t arr_idx,
    duk_propdesc* out_desc, duk_small_uint_t flags);

DUK_LOCAL_DECL void duk__abandon_array_part(duk_hthread* thr, duk_hobject* obj);
DUK_LOCAL_DECL void duk__grow_props_for_array_item(
    duk_hthread* thr, duk_hobject* obj, duk_uint32_t highest_arr_idx);
 ...
 *
 * Also check if it's a valid array index and return that (or
 * DUK__NO_ARRAY_INDEX if not).
 */
DUK_LOCAL duk_uint32_t duk__to_property_key(duk_hthread* thr, duk_idx_t idx,
                                            duk_hstring** out_h) {
...
DUK_LOCAL duk_uint32_t duk__get_default_h_size(duk_uint32_t e_size) {
  ...
  if (e_size >= DUK_USE_HOBJECT_HASH_PROP_LIMIT) {
    ...
    DUK_ASSERT((DUK_HOBJECT_MAX_PROPERTIES << 2U) >
               DUK_HOBJECT_MAX_PROPERTIES); /* Won't wrap, even shifted by 2. */
...
DUK_LOCAL duk_uint32_t duk__get_min_grow_e(duk_uint32_t e_size) {
  ...
  res = (e_size + DUK_USE_HOBJECT_ENTRY_MINGROW_ADD) /
...
DUK_LOCAL duk_uint32_t duk__get_min_grow_a(duk_uint32_t a_size) {
  ...
  res = (a_size + DUK_USE_HOBJECT_ARRAY_MINGROW_ADD) /
...
/* Fast check for extending array: check whether or not a slow density check is
 * required. */
DUK_LOCAL duk_bool_t duk__abandon_array_slow_check_required(
    duk_uint32_t arr_idx, duk_uint32_t old_size) {
   ...
   *    new_size > limit'' * (old_size / 8)      || limit'' = 9 -> max 25%
   * increase arr_idx + 1 > limit'' * (old_size / 8)
   *
  ...
  return (new_size_min >= DUK_USE_HOBJECT_ARRAY_ABANDON_MINSIZE) &&
         (arr_idx >
          DUK_USE_HOBJECT_ARRAY_FAST_RESIZE_LIMIT * ((old_size + 7) >> 3));
}
...
DUK_LOCAL duk_bool_t duk__abandon_array_check(duk_hthread* thr,
  ...
  if (!duk__abandon_array_slow_check_required(arr_idx,
...
DUK_LOCAL duk_tval* duk__obtain_arridx_slot_slowpath(duk_hthread* thr,
  ...
  DUK_DDD(DUK_DDDPRINT(
      "write to new array requires array resize, decide whether to do a "
...
DUK_LOCAL duk_bool_t duk__proxy_check_prop(duk_hthread* thr, duk_hobject* obj,
   ...
   * such behavior and to get access to internal key strings.  This is not a
   * problem as such because internal key strings can be created in other ways
   * too (e.g. through buffers).  The best fix is to change Duktape internal
   * lookups to skip proxy behavior.  Until that, internal property accesses
   * bypass the proxy and are applied to the target (as if the handler did not
   * exist). This has some side effects, see test-bi-proxy-internal-keys.js.
   */
  ...
  if (DUK_TVAL_IS_STRING(tv_key)) {
    ...
    if (DUK_HSTRING_HAS_HIDDEN(h_key)) {
       ...
       */
      DUK_DDD(
          DUK_DDDPRINT("hidden key, skip proxy handler and apply to target"));
      return 0;
    }
  }
...
DUK_INTERNAL void duk_hobject_realloc_props(duk_hthread* thr, duk_hobject* obj,
  ...
  DUK_ASSERT(thr != NULL);
  DUK_ASSERT(obj != NULL);
  DUK_ASSERT(!abandon_array ||
             new_a_size == 0); /* if abandon_array, new_a_size must be 0 */
  DUK_ASSERT(
      DUK_HOBJECT_GET_PROPS(thr->heap, obj) != NULL ||
      (DUK_HOBJECT_GET_ESIZE(obj) == 0 && DUK_HOBJECT_GET_ASIZE(obj) == 0));
  DUK_ASSERT(new_h_size == 0 ||
             new_h_size >=
                 new_e_size); /* required to guarantee success of rehashing,
  ...
  DUK_DDD(DUK_DDDPRINT("using layout 1, but no need to pad e_size: %ld",
                       (long)new_e_size));
  new_e_size_adjusted = new_e_size;
#elif defined(DUK_USE_HOBJECT_LAYOUT_1) && \
    ((DUK_HOBJECT_ALIGN_TARGET == 4) || (DUK_HOBJECT_ALIGN_TARGET == 8))
  new_e_size_adjusted =
      (new_e_size + (duk_uint32_t)DUK_HOBJECT_ALIGN_TARGET - 1U) &
      (~((duk_uint32_t)DUK_HOBJECT_ALIGN_TARGET - 1U));
  DUK_DDD(
      DUK_DDDPRINT("using layout 1, and alignment target is %ld, adjusted "
                   "e_size: %ld -> %ld",
                   (long)DUK_HOBJECT_ALIGN_TARGET, (long)new_e_size,
...
#  error invalid hobject layout defines
  ...
  DUK_DDD(DUK_DDDPRINT(
      "attempt to resize hobject %p props (%ld -> %ld bytes), from "
      "{p=%p,e_size=%ld,e_next=%ld,a_size=%ld,h_size=%ld} to "
      "{e_size=%ld,a_size=%ld,h_size=%ld}, abandon_array=%ld, unadjusted "
      "new_e_size=%ld",
      (void*)obj,
      (long)DUK_HOBJECT_P_COMPUTE_SIZE(DUK_HOBJECT_GET_ESIZE(obj),
                                       DUK_HOBJECT_GET_ASIZE(obj),
                                       DUK_HOBJECT_GET_HSIZE(obj)),
      (long)DUK_HOBJECT_P_COMPUTE_SIZE(new_e_size_adjusted, new_a_size,
                                       new_h_size),
      (void*)DUK_HOBJECT_GET_PROPS(thr->heap, obj),
      (long)DUK_HOBJECT_GET_ESIZE(obj), (long)DUK_HOBJECT_GET_ENEXT(obj),
      (long)DUK_HOBJECT_GET_ASIZE(obj), (long)DUK_HOBJECT_GET_HSIZE(obj),
      (long)new_e_size_adjusted, (long)new_a_size, (long)new_h_size,
      (long)abandon_array, (long)new_e_size));
  ...
  thr->heap->ms_base_flags |=
      DUK_MS_FLAG_NO_OBJECT_COMPACTION; /* Avoid attempt to compact the current
                                           object (all objects really). */
  ...
  DUK_DDD(
      DUK_DDDPRINT("new hobject allocation size is %ld", (long)new_alloc_size));
  if (new_alloc_size == 0) {
   ...
   */
  DUK_HOBJECT_P_SET_REALLOC_PTRS(new_p, new_e_k, new_e_pv, new_e_f, new_a,
                                 new_h, new_e_size_adjusted, new_a_size,
                                 new_h_size);
  ...
  DUK_DDD(
      DUK_DDDPRINT("new alloc size %ld, new_e_k=%p, new_e_pv=%p, new_e_f=%p, "
                   "new_a=%p, new_h=%p",
                   (long)new_alloc_size, (void*)new_e_k, (void*)new_e_pv,
                   (void*)new_e_f, (void*)new_a, (void*)new_h));
  ...
  if (abandon_array) {
    ...
    for (i = 0; i < DUK_HOBJECT_GET_ASIZE(obj); i++) {
      ...
      DUK_ASSERT(new_p != NULL && new_e_k != NULL && new_e_pv != NULL &&
                 new_e_f != NULL);
      ...
      duk_push_hstring(
          thr, key); /* keep key reachable for GC etc; guaranteed not to fail */
    ...
    DUK_DDD(DUK_DDDPRINT("abandon array: pop %ld key temps from valstack",
                         (long)new_e_next));
  ...
  /* the entries [new_e_next, new_e_size_adjusted[ are left uninitialized on
   * purpose (ok, not gc reachable) */
   ...
   * critical to ensuring the hash part never fills up.
   */
  ...
  if (new_h_size == 0) {
    ...
    for (i = 0; i < new_e_next; i++) {
      ...
      for (;;) {
        ...
        DUK_DDD(
            DUK_DDDPRINT("rebuild miss %ld, step %ld", (long)j, (long)step));
        j = (j + step) & mask;
  ...
  DUK_DD(DUK_DDPRINT(
      "resized hobject %p props (%ld -> %ld bytes), from "
      "{p=%p,e_size=%ld,e_next=%ld,a_size=%ld,h_size=%ld} to "
      "{p=%p,e_size=%ld,e_next=%ld,a_size=%ld,h_size=%ld}, abandon_array=%ld, "
      "unadjusted new_e_size=%ld",
      (void*)obj,
      (long)DUK_HOBJECT_P_COMPUTE_SIZE(DUK_HOBJECT_GET_ESIZE(obj),
                                       DUK_HOBJECT_GET_ASIZE(obj),
                                       DUK_HOBJECT_GET_HSIZE(obj)),
      (long)new_alloc_size, (void*)DUK_HOBJECT_GET_PROPS(thr->heap, obj),
      (long)DUK_HOBJECT_GET_ESIZE(obj), (long)DUK_HOBJECT_GET_ENEXT(obj),
      (long)DUK_HOBJECT_GET_ASIZE(obj), (long)DUK_HOBJECT_GET_HSIZE(obj),
      (void*)new_p, (long)new_e_size_adjusted, (long)new_e_next,
      (long)new_a_size, (long)new_h_size, (long)abandon_array,
      (long)new_e_size));
  ...
  DUK_FREE_CHECKED(
      thr, DUK_HOBJECT_GET_PROPS(thr->heap, obj)); /* NULL obj->p is OK */
...
DUK_LOCAL void duk__grow_props_for_array_item(duk_hthread* thr,
  ...
  DUK_ASSERT(new_a_size >=
             highest_arr_idx + 1); /* duk__get_min_grow_a() is always >= 1 */
...
DUK_INTERNAL void duk_hobject_compact_props(duk_hthread* thr,
  ...
  DUK_DD(DUK_DDPRINT(
      ...
      (long)e_size, (long)a_used, (long)a_size, (long)a_used, (long)a_size,
  ...
  DUK_DD(DUK_DDPRINT(
      "compacting hobject -> new e_size %ld, new a_size=%ld, new h_size=%ld, "
      "abandon_array=%ld",
...
DUK_INTERNAL duk_bool_t duk_hobject_find_entry(duk_heap* heap, duk_hobject* obj,
                                               duk_hstring* key,
                                               duk_int_t* e_idx,
                                               duk_int_t* h_idx) {
  DUK_ASSERT(obj != NULL);
  ...
  if (DUK_LIKELY(DUK_HOBJECT_GET_HSIZE(obj) == 0)) {
    ...
    DUK_DDD(
        DUK_DDDPRINT("duk_hobject_find_entry() using linear scan for lookup"));
    ...
    DUK_DDD(
        DUK_DDDPRINT("duk_hobject_find_entry() using hash part for lookup"));
...
DUK_INTERNAL duk_tval* duk_hobject_find_entry_tval_ptr_stridx(
    duk_heap* heap, duk_hobject* obj, duk_small_uint_t stridx) {
  return duk_hobject_find_entry_tval_ptr(heap, obj,
                                         DUK_HEAP_GET_STRING(heap, stridx));
}

/* For internal use: get non-accessor entry value and attributes */
DUK_INTERNAL duk_tval* duk_hobject_find_entry_tval_ptr_and_attrs(
    duk_heap* heap, duk_hobject* obj, duk_hstring* key, duk_uint_t* out_attrs) {
  duk_int_t e_idx;
  duk_int_t h_idx;
  ...
  if (DUK_UNLIKELY(DUK_HOBJECT_GET_HSIZE(obj) > 0)) {
    ...
    for (;;) {
      ...
      if (t == DUK__HASH_UNUSED || t == DUK__HASH_DELETED) {
        DUK_DDD(
            DUK_DDDPRINT("duk__hobject_alloc_entry_checked() inserted key into "
                         "hash part, %ld -> %ld",
                         (long)i, (long)idx));
        DUK_ASSERT_DISABLE(i >= 0); /* unsigned */
        DUK_ASSERT(i < DUK_HOBJECT_GET_HSIZE(obj));
      ...
      DUK_DDD(
          DUK_DDDPRINT("duk__hobject_alloc_entry_checked() miss %ld", (long)i));
      i = (i + step) & mask;
 ...
 * need to incref OR decref.  No proxies or accessors are invoked, no prototype
 * walk.
 */

DUK_INTERNAL duk_tval* duk_hobject_get_internal_value_tval_ptr(
    duk_heap* heap, duk_hobject* obj) {
  return duk_hobject_find_entry_tval_ptr_stridx(heap, obj,
                                                DUK_STRIDX_INT_VALUE);
}

DUK_LOCAL duk_heaphdr* duk_hobject_get_internal_value_heaphdr(
    duk_heap* heap, duk_hobject* obj) {
  duk_tval* tv;
...
DUK_INTERNAL duk_hstring* duk_hobject_get_internal_value_string(
    duk_heap* heap, duk_hobject* obj) {
  duk_hstring* h;
...
DUK_LOCAL duk_hobject* duk__hobject_get_entry_object_stridx(
    duk_heap* heap, duk_hobject* obj, duk_small_uint_t stridx) {
  duk_tval* tv;
  duk_hobject* h;
...
DUK_INTERNAL duk_harray* duk_hobject_get_formals(duk_hthread* thr,
  ...
  h = (duk_harray*)duk__hobject_get_entry_object_stridx(thr->heap, obj,
...
DUK_INTERNAL duk_hobject* duk_hobject_get_varmap(duk_hthread* thr,
  ...
  h = duk__hobject_get_entry_object_stridx(thr->heap, obj,
 ...
 */
DUK_LOCAL
duk_bool_t duk__lookup_arguments_map(duk_hthread* thr, duk_hobject* obj,
                                     duk_hstring* key, duk_propdesc* temp_desc,
                                     duk_hobject** out_map,
  ...
  if (!duk_hobject_get_own_propdesc(thr, obj, DUK_HTHREAD_STRING_INT_MAP(thr),
                                    temp_desc, DUK_GETDESC_FLAG_PUSH_VALUE)) {
  ...
  /* [... varname] */
  DUK_DDD(
      DUK_DDDPRINT("-> 'map' exists, and contains key, key is mapped to "
                   "argument/variable binding %!T",
  ...
  rc =
      duk_hobject_get_own_propdesc(thr, obj, DUK_HTHREAD_STRING_INT_VARENV(thr),
                                   temp_desc, DUK_GETDESC_FLAG_PUSH_VALUE);
  DUK_UNREF(rc);
  DUK_ASSERT(
      rc !=
...
DUK_LOCAL duk_bool_t duk__check_arguments_map_for_get(duk_hthread* thr,
  ...
  DUK_DDD(
      DUK_DDDPRINT("arguments object automatic getvar for a bound variable; "
                   "key=%!O, varname=%!O",
                   (duk_heaphdr*)key, (duk_heaphdr*)varname));
 ...
 * mapped. Used in E5 Section 10.6 algorithm for [[DefineOwnProperty]] (used by
 * [[Put]]). Assumes stack top contains 'put' value (which is NOT popped).
 */
DUK_LOCAL void duk__check_arguments_map_for_put(duk_hthread* thr,
  ...
  duk_js_putvar_envrec(thr, varenv, varname, duk_require_tval(thr, -1),
                       throw_flag);
...
DUK_LOCAL void duk__check_arguments_map_for_delete(duk_hthread* thr,
  ...
  if (!duk_hobject_get_own_propdesc(thr, obj, DUK_HTHREAD_STRING_INT_MAP(thr),
                                    temp_desc, DUK_GETDESC_FLAG_PUSH_VALUE)) {
    DUK_DDD(
        DUK_DDDPRINT("arguments: key not mapped, no exotic delete behavior"));
    return;
  }
 ...
 * DUK_GETDESC_FLAG_PUSH_VALUE set)
 *    - Returns zero
 *
 *  Notes:
 *
...
DUK_LOCAL duk_bool_t duk__get_own_propdesc_raw(
    duk_hthread* thr, duk_hobject* obj, duk_hstring* key, duk_uint32_t arr_idx,
    duk_propdesc* out_desc, duk_small_uint_t flags) {
  duk_tval* tv;

  DUK_DDD(DUK_DDDPRINT(
      "duk_hobject_get_own_propdesc: thr=%p, obj=%p, key=%p, out_desc=%p, "
      "flags=%lx, "
      "arr_idx=%ld (obj -> %!O, key -> %!O)",
      (void*)thr, (void*)obj, (void*)key, (void*)out_desc, (long)flags,
      (long)arr_idx, (duk_heaphdr*)obj, (duk_heaphdr*)key));
  ...
  if (duk_hobject_find_entry(thr->heap, obj, key, &out_desc->e_idx,
                             &out_desc->h_idx)) {
  ...
  if (DUK_HOBJECT_HAS_ARRAY_PART(obj) && arr_idx != DUK__NO_ARRAY_INDEX) {
    if (arr_idx < DUK_HOBJECT_GET_ASIZE(obj)) {
      ...
      if (!DUK_TVAL_IS_UNUSED(tv)) {
        ...
        out_desc->h_idx = -1;
        out_desc->a_idx =
            (duk_int_t)arr_idx; /* XXX: limit 2G due to being signed */
        goto prop_found;
      }
    }
  }
  ...
  if (DUK_HOBJECT_HAS_EXOTIC_ARRAY(obj)) {
    ...
    DUK_DDD(DUK_DDDPRINT(
        "array object exotic property get for key: %!O, arr_idx: %ld",
    ...
    if (key == DUK_HTHREAD_STRING_LENGTH(thr)) {
      DUK_DDD(
          DUK_DDDPRINT("-> found, key is 'length', length exotic behavior"));
    ...
    DUK_DDD(DUK_DDDPRINT(
        "string object exotic property get for key: %!O, arr_idx: %ld",
        (duk_heaphdr*)key, (long)arr_idx));
    ...
    if (arr_idx != DUK__NO_ARRAY_INDEX) {
      ...
      if (arr_idx < DUK_HSTRING_GET_CHARLEN(h_val)) {
        ...
        out_desc->flags =
            DUK_PROPDESC_FLAG_ENUMERABLE | /* E5 Section 15.5.5.2 */
    ...
    if (arr_idx != DUK__NO_ARRAY_INDEX &&
      ...
      if (arr_idx < (h_bufobj->length >> h_bufobj->shift)) {
        ...
        if (flags & DUK_GETDESC_FLAG_PUSH_VALUE) {
          ...
          if (h_bufobj->buf != NULL && DUK_HBUFOBJ_VALID_BYTEOFFSET_EXCL(
                                           h_bufobj, byte_off + elem_size)) {
            ...
            DUK_D(
                DUK_DPRINT("bufobj access out of underlying buffer, ignoring "
                           "(read zero)"));
            duk_push_uint(thr, 0);
          }
        }
        out_desc->flags =
            DUK_PROPDESC_FLAG_WRITABLE | DUK_PROPDESC_FLAG_VIRTUAL;
        if (DUK_HOBJECT_GET_CLASS_NUMBER(obj) !=
        ...
        DUK_ASSERT(!DUK_HOBJECT_HAS_EXOTIC_ARGUMENTS(obj));
        goto prop_found_noexotic; /* cannot be e.g. arguments exotic, since
                                     exotic 'traits' are mutually exclusive */
      } else {
...
prop_found:
  DUK_DDD(DUK_DDDPRINT(
      "-> property found, checking for arguments exotic post-behavior"));
  ...
  if (DUK_UNLIKELY(DUK_HOBJECT_HAS_EXOTIC_ARGUMENTS(obj) &&
    ...
    if (duk__check_arguments_map_for_get(thr, obj, key, &temp_desc)) {
      DUK_DDD(DUK_DDDPRINT(
          "-> arguments exotic behavior overrides result: %!T -> %!T",
...
DUK_INTERNAL duk_bool_t duk_hobject_get_own_propdesc(duk_hthread* thr,
  ...
  return duk__get_own_propdesc_raw(
      thr, obj, key, DUK_HSTRING_GET_ARRIDX_SLOW(key), out_desc, flags);
}
...
DUK_LOCAL duk_bool_t duk__get_propdesc(duk_hthread* thr, duk_hobject* obj,
                                       duk_hstring* key, duk_propdesc* out_desc,
                                       duk_small_uint_t flags) {
  duk_hobject* curr;
  duk_uint32_t arr_idx;
  duk_uint_t sanity;
...
DUK_LOCAL duk_tval* duk__getprop_shallow_fastpath_array_tval(duk_hthread* thr,
  ...
  if (!(DUK_HOBJECT_HAS_ARRAY_PART(obj) &&
        ...
        !DUK_HOBJECT_HAS_EXOTIC_STRINGOBJ(obj) && !DUK_HOBJECT_IS_BUFOBJ(obj) &&
  ...
  DUK_DDD(
      DUK_DDDPRINT("fast path attempt (no exotic string/arguments/buffer "
...
DUK_LOCAL duk_bool_t duk__putprop_shallow_fastpath_array_tval(
    duk_hthread* thr, duk_hobject* obj, duk_tval* tv_key, duk_tval* tv_val) {
  duk_tval* tv;
  duk_harray* a;
  duk_uint32_t idx;
  duk_uint32_t old_len, new_len;

  if (!(DUK_HOBJECT_HAS_EXOTIC_ARRAY(obj) && DUK_HOBJECT_HAS_ARRAY_PART(obj) &&
...
#  endif
  ...
  if (idx >= DUK_HOBJECT_GET_ASIZE(
                 obj)) { /* for resizing of array part, use slow path */
    return 0;
  }
  DUK_ASSERT(idx != 0xffffffffUL);
  DUK_ASSERT(idx != DUK__NO_ARRAY_INDEX);
...
#  endif
  ...
  byte_off =
      idx << h_bufobj->shift; /* no wrap assuming h_bufobj->length is valid */
  elem_size = (duk_small_uint_t)(1U << h_bufobj->shift);
...
DUK_LOCAL duk_bool_t duk__putprop_fastpath_bufobj_tval(duk_hthread* thr,
  ...
  DUK_ASSERT(!DUK_HEAPHDR_HAS_READONLY(
      (duk_heaphdr*)
          obj)); /* caller ensures; rom objects are never bufobjs now */
...
DUK_INTERNAL duk_bool_t duk_hobject_getprop(duk_hthread* thr, duk_tval* tv_obj,
                                            duk_tval* tv_key) {
  ...
  DUK_DDD(DUK_DDDPRINT(
      "getprop: thr=%p, obj=%p, key=%p (obj -> %!T, key -> %!T)", (void*)thr,
      (void*)tv_obj, (void*)tv_key, (duk_tval*)tv_obj, (duk_tval*)tv_key));
  ...
  switch (DUK_TVAL_GET_TAG(tv_obj)) {
    ...
    case DUK_TAG_BOOLEAN: {
      DUK_DDD(DUK_DDDPRINT(
          "base object is a boolean, start lookup from boolean prototype"));
    ...
    case DUK_TAG_STRING: {
      ...
      if (DUK_UNLIKELY(DUK_HSTRING_HAS_SYMBOL(h))) {
        ...
        DUK_DDD(DUK_DDDPRINT(
            "base object is a symbol, start lookup from symbol prototype"));
      ...
      if (DUK_TVAL_IS_FASTINT(tv_key)) {
        ...
        DUK_ASSERT(key != NULL);
        DUK_DDD(DUK_DDDPRINT(
            "base object string, key is a non-fast-path number; after "
      ...
      if (arr_idx != DUK__NO_ARRAY_INDEX &&
        ...
        DUK_DDD(DUK_DDDPRINT(
            "-> %!T (base is string, key is an index inside string length "
      ...
      if (key == DUK_HTHREAD_STRING_LENGTH(thr)) {
        ...
        duk_push_uint(thr,
                      (duk_uint_t)DUK_HSTRING_GET_CHARLEN(h)); /* [] -> [res] */

        DUK_STATS_INC(thr->heap, stats_getprop_stringlen);
        DUK_DDD(DUK_DDDPRINT(
            "-> %!T (base is string, key is 'length' after coercion -> "
      ...
      DUK_DDD(DUK_DDDPRINT(
          "base object is a string, start lookup from string prototype"));
    ...
    case DUK_TAG_OBJECT: {
      ...
      if (duk__getprop_fastpath_bufobj_tval(thr, curr, tv_key) != 0) {
        ...
        DUK_DDD(
            DUK_DDDPRINT("-> %!T (base is bufobj, key is a number, bufobj "
                         "fast path)",
                         (duk_tval*)duk_get_tval(thr, -1)));
      ...
      if (DUK_UNLIKELY(DUK_HOBJECT_IS_PROXY(curr))) {
        ...
        if (duk__proxy_check_prop(thr, curr, DUK_STRIDX_GET, tv_key,
                                  &h_target)) {
          /* -> [ ... trap handler ] */
          DUK_DDD(DUK_DDDPRINT("-> proxy object 'get' for key %!T",
                               (duk_tval*)tv_key));
          ...
          if (duk__get_own_propdesc_raw(thr, h_target, key, arr_idx, &desc,
                                        DUK_GETDESC_FLAG_PUSH_VALUE)) {
            duk_tval* tv_hook = duk_require_tval(thr, -3); /* value from hook */
            duk_tval* tv_targ =
                duk_require_tval(thr, -1); /* value from target */
            duk_bool_t datadesc_reject;
            duk_bool_t accdesc_reject;

            DUK_DDD(DUK_DDDPRINT(
                "proxy 'get': target has matching property %!O, check for "
                "conflicting property; tv_hook=%!T, tv_targ=%!T, "
                "desc.flags=0x%08lx, "
                "desc.get=%p, desc.set=%p",
                (duk_heaphdr*)key, (duk_tval*)tv_hook, (duk_tval*)tv_targ,
                (unsigned long)desc.flags, (void*)desc.get, (void*)desc.set));
      ...
      if (DUK_HOBJECT_HAS_EXOTIC_ARGUMENTS(curr)) {
        ...
        if (duk__check_arguments_map_for_get(thr, curr, key, &desc)) {
          DUK_DDD(DUK_DDDPRINT(
              "-> %!T (base is object with arguments exotic behavior, "
              "key matches magically bound property -> skip standard "
              "Get with replacement value)",
              (duk_tval*)duk_get_tval(thr, -1)));

          /* no need for 'caller' post-check, because 'key' must be an array
           * index */
    ...
    case DUK_TAG_BUFFER: {
      ...
      if (DUK_TVAL_IS_FASTINT(tv_key)) {
        ...
        DUK_ASSERT(key != NULL);
        DUK_DDD(DUK_DDDPRINT(
            "base object buffer, key is a non-fast-path number; after "
            "coercion key is %!T, arr_idx %ld",
            (duk_tval*)duk_get_tval(thr, -1), (long)arr_idx));
        pop_count = 1;
      }
      ...
      if (arr_idx != DUK__NO_ARRAY_INDEX && arr_idx < DUK_HBUFFER_GET_SIZE(h)) {
        duk_pop_n_unsafe(thr, pop_count);
        duk_push_uint(thr, ((duk_uint8_t*)DUK_HBUFFER_GET_DATA_PTR(
                               thr->heap, h))[arr_idx]);
        DUK_STATS_INC(thr->heap, stats_getprop_bufferidx);
        DUK_DDD(DUK_DDDPRINT(
            "-> %!T (base is buffer, key is an index inside buffer length "
      ...
      if (key == DUK_HTHREAD_STRING_LENGTH(thr)) {
        duk_pop_unsafe(thr); /* [key] -> [] */
        duk_push_uint(thr,
                      (duk_uint_t)DUK_HBUFFER_GET_SIZE(h)); /* [] -> [res] */
        DUK_STATS_INC(thr->heap, stats_getprop_bufferlen);

        DUK_DDD(
            DUK_DDDPRINT("-> %!T (base is buffer, key is 'length' "
    ...
    case DUK_TAG_POINTER: {
      DUK_DDD(DUK_DDDPRINT(
          "base object is a pointer, start lookup from pointer prototype"));
      curr = thr->builtins[DUK_BIDX_POINTER_PROTOTYPE];
      break;
    }

    case DUK_TAG_LIGHTFUNC: {
      /* Lightfuncs inherit getter .name and .length from
    ...
    default: {
      /* number */
      DUK_DDD(DUK_DDDPRINT(
          "base object is a number, start lookup from number prototype"));
...
lookup:
  ...
  sanity = DUK_HOBJECT_PROTOTYPE_CHAIN_SANITY;
  do {
    if (!duk__get_own_propdesc_raw(thr, curr, key, arr_idx, &desc,
                                   DUK_GETDESC_FLAG_PUSH_VALUE)) {
      goto next_in_chain;
    }
...
DUK_INTERNAL duk_bool_t duk_hobject_hasprop(duk_hthread* thr, duk_tval* tv_obj,
                                            duk_tval* tv_key) {
  duk_tval tv_key_copy;
  duk_hobject* obj;
  duk_hstring* key;
  ...
  DUK_DDD(DUK_DDDPRINT(
      "hasprop: thr=%p, obj=%p, key=%p (obj -> %!T, key -> %!T)", (void*)thr,
      (void*)tv_obj, (void*)tv_key, (duk_tval*)tv_obj, (duk_tval*)tv_key));
  ...
  if (DUK_TVAL_IS_OBJECT(tv_obj)) {
    ...
    arr_idx = duk__push_tval_to_property_key(thr, tv_key, &key);
    if (duk__key_is_plain_buf_ownprop(thr, DUK_TVAL_GET_BUFFER(tv_obj), key,
                                      arr_idx)) {
      rc = 1;
      goto pop_and_return;
    }
  ...
  if (DUK_UNLIKELY(DUK_HOBJECT_IS_PROXY(obj))) {
    ...
    if (duk__proxy_check_prop(thr, obj, DUK_STRIDX_HAS, tv_key, &h_target)) {
      /* [ ... key trap handler ] */
      DUK_DDD(
          DUK_DDDPRINT("-> proxy object 'has' for key %!T", (duk_tval*)tv_key));
      duk_push_hobject(thr, h_target); /* target */
      duk_push_tval(thr, tv_key);      /* P */
      ...
      if (!tmp_bool) {
        ...
        if (duk__get_own_propdesc_raw(thr, h_target, key, arr_idx, &desc,
          ...
          DUK_DDD(DUK_DDDPRINT(
              "proxy 'has': target has matching property %!O, check for "
              "conflicting property; desc.flags=0x%08lx, "
              "desc.get=%p, desc.set=%p",
              (duk_heaphdr*)key, (unsigned long)desc.flags, (void*)desc.get,
              (void*)desc.set));
           ...
           */
          if (!((desc.flags &
                 DUK_PROPDESC_FLAG_CONFIGURABLE) && /* property is configurable
                                                       and */
                DUK_HOBJECT_HAS_EXTENSIBLE(
                    h_target))) { /* ... target is extensible */
            DUK_ERROR_TYPE(thr, DUK_STR_PROXY_REJECTED);
            DUK_WO_NORETURN(return 0;);
          }
        }
  ...
  rc = duk__get_propdesc(thr, obj, key, &desc,
                         0 /*flags*/); /* don't push value */
  /* fall through */
...
DUK_INTERNAL duk_bool_t duk_hobject_hasprop_raw(duk_hthread* thr,
  ...
  return duk__get_propdesc(
      thr, obj, key, &dummy,
      DUK_GETDESC_FLAG_IGNORE_PROTOLOOP); /* don't push value */
}
 ...
 */
DUK_LOCAL
duk_bool_t duk__handle_put_array_length_smaller(
    duk_hthread* thr, duk_hobject* obj, duk_uint32_t old_len,
    duk_uint32_t new_len, duk_bool_t force_flag, duk_uint32_t* out_result_len) {
  ...
  DUK_DDD(
      DUK_DDDPRINT("new array length smaller than old (%ld -> %ld), "
  ...
  if (DUK_HOBJECT_HAS_ARRAY_PART(obj)) {
    ...
    *out_result_len = new_len;
    ...
    DUK_DDD(DUK_DDDPRINT(
        "array length write, no array part, stage 1: find target_len "
        "(highest preventing non-configurable entry (if any))"));

    target_len = new_len;
    if (force_flag) {
      DUK_DDD(DUK_DDDPRINT(
          "array length write, no array part; force flag -> skip stage 1"));
      goto skip_stage1;
    }
    for (i = 0; i < DUK_HOBJECT_GET_ENEXT(obj); i++) {
      key = DUK_HOBJECT_E_GET_KEY(thr->heap, obj, i);
      ...
      if (!DUK_HSTRING_HAS_ARRIDX(key)) {
        DUK_DDD(DUK_DDDPRINT("skip entry index %ld: key not an array index",
                             (long)i));
        continue;
      }

      DUK_ASSERT(
          DUK_HSTRING_HAS_ARRIDX(key)); /* XXX: macro checks for array index
                                           flag, which is unnecessary here */
      ...
      if (arr_idx < new_len) {
        DUK_DDD(DUK_DDDPRINT(
            "skip entry index %ld: key is array index %ld, below new_len",
            (long)i, (long)arr_idx));
        continue;
      }
      if (DUK_HOBJECT_E_SLOT_IS_CONFIGURABLE(thr->heap, obj, i)) {
        DUK_DDD(
            DUK_DDDPRINT("skip entry index %ld: key is a relevant array index "
                         "%ld, but configurable",
                         (long)i, (long)arr_idx));
        continue;
      }
      ...
      if (arr_idx >= target_len) {
        DUK_DDD(DUK_DDDPRINT(
            "entry at index %ld has arr_idx %ld, is not configurable, "
            "update target_len %ld -> %ld",
            (long)i, (long)arr_idx, (long)target_len, (long)(arr_idx + 1)));
  ...
  skip_stage1:
    ...
    for (i = 0; i < DUK_HOBJECT_GET_ENEXT(obj); i++) {
      ...
      if (arr_idx < target_len) {
        DUK_DDD(DUK_DDDPRINT(
            "skip entry index %ld: key is array index %ld, below target_len",
            (long)i, (long)arr_idx));
        continue;
      }
      DUK_ASSERT(force_flag || DUK_HOBJECT_E_SLOT_IS_CONFIGURABLE(
                                   thr->heap, obj, i)); /* stage 1 guarantees */
       ...
       */
      rc = duk_hobject_delprop_raw(thr, obj, key,
                                   force_flag ? DUK_DELPROP_FLAG_FORCE : 0);
      DUK_UNREF(rc);
      DUK_ASSERT(rc != 0);
    }
    ...
    DUK_DDD(
        DUK_DDDPRINT("array length write, no array part, stage 3: update "
                     "length (done by caller)"));
...
DUK_LOCAL duk_bool_t duk__handle_put_array_length(duk_hthread* thr,
  ...
  DUK_DDD(DUK_DDDPRINT(
      "handling a put operation to array 'length' exotic property, "
      "new val: %!T",
      (duk_tval*)duk_get_tval(thr, -1)));
  ...
  DUK_DDD(
      DUK_DDDPRINT("old_len=%ld, new_len=%ld", (long)old_len, (long)new_len));
  ...
  if (new_len >= old_len) {
    DUK_DDD(
        DUK_DDDPRINT("new length is same or higher than old length, just "
                     "update length, no deletions"));
    a->length = new_len;
    return 1;
  }
  ...
  rc = duk__handle_put_array_length_smaller(thr, obj, old_len, new_len,
...
DUK_INTERNAL duk_bool_t duk_hobject_putprop(duk_hthread* thr, duk_tval* tv_obj,
                                            duk_tval* tv_key, duk_tval* tv_val,
                                            duk_bool_t throw_flag) {
  duk_tval tv_obj_copy;
  duk_tval tv_key_copy;
  ...
  DUK_DDD(DUK_DDDPRINT(
      "putprop: thr=%p, obj=%p, key=%p, val=%p, throw=%ld "
      "(obj -> %!T, key -> %!T, val -> %!T)",
      (void*)thr, (void*)tv_obj, (void*)tv_key, (void*)tv_val, (long)throw_flag,
      (duk_tval*)tv_obj, (duk_tval*)tv_key, (duk_tval*)tv_val));
  ...
  switch (DUK_TVAL_GET_TAG(tv_obj)) {
    ...
    case DUK_TAG_NULL: {
      /* Note: unconditional throw */
      DUK_DDD(DUK_DDDPRINT(
          "base object is undefined or null -> reject (object=%!iT)",
          (duk_tval*)tv_obj));
#if defined(DUK_USE_PARANOID_ERRORS)
      DUK_ERROR_TYPE(thr, DUK_STR_INVALID_BASE);
#else
    ...
    case DUK_TAG_OBJECT: {
...
#if defined(DUK_USE_ARRAY_PROP_FASTPATH)
      if (duk__putprop_shallow_fastpath_array_tval(thr, orig, tv_key, tv_val) !=
          0) {
      ...
      if (duk__putprop_fastpath_bufobj_tval(thr, orig, tv_key, tv_val) != 0) {
        DUK_DDD(
            DUK_DDDPRINT("base is bufobj, key is a number, bufobj fast path"));
      ...
      if (DUK_UNLIKELY(DUK_HOBJECT_IS_PROXY(orig))) {
        ...
        if (duk__proxy_check_prop(thr, orig, DUK_STRIDX_SET, tv_key,
                                  &h_target)) {
          /* -> [ ... trap handler ] */
          DUK_DDD(DUK_DDDPRINT("-> proxy object 'set' for key %!T",
                               (duk_tval*)tv_key));
          ...
          if (duk__get_own_propdesc_raw(thr, h_target, key, arr_idx, &desc,
            ...
            DUK_DDD(DUK_DDDPRINT(
                "proxy 'set': target has matching property %!O, check for "
                "conflicting property; tv_val=%!T, tv_targ=%!T, "
                "desc.flags=0x%08lx, "
                "desc.get=%p, desc.set=%p",
    ...
    case DUK_TAG_BUFFER: {
      ...
      if (arr_idx != DUK__NO_ARRAY_INDEX && arr_idx < DUK_HBUFFER_GET_SIZE(h)) {
        duk_uint8_t* data;
        DUK_DDD(
            DUK_DDDPRINT("writing to buffer data at index %ld", (long)arr_idx));
...
lookup:
  ...
  do {
    ...
    if (curr != orig) {
      ...
      if (!DUK_HOBJECT_HAS_EXTENSIBLE(orig)) {
        DUK_DD(
            DUK_DDPRINT("found existing inherited plain property, but original "
                        "object is not extensible"));
        goto fail_not_extensible;
      }
      if (!(desc.flags & DUK_PROPDESC_FLAG_WRITABLE)) {
        DUK_DD(DUK_DDPRINT(
            "found existing inherited plain property, original object is "
            "extensible, but inherited property is not writable"));
        goto fail_not_writable;
      }
      DUK_DD(
          DUK_DDPRINT("put to new property, object extensible, inherited "
                      "property found and is writable"));
      goto create_new;
    } else {
      /*
      ...
      if (!(desc.flags & DUK_PROPDESC_FLAG_WRITABLE)) {
        DUK_DD(
            DUK_DDPRINT("found existing own (non-inherited) plain property, "
                        "but property is not writable"));
        goto fail_not_writable;
      }
      if (desc.flags & DUK_PROPDESC_FLAG_VIRTUAL) {
        DUK_DD(
            DUK_DDPRINT("found existing own (non-inherited) virtual property, "
        ...
        if (DUK_HOBJECT_IS_ARRAY(curr)) {
           ...
           */
          DUK_ASSERT(key == DUK_HTHREAD_STRING_LENGTH(
                                thr)); /* only virtual array property */

          DUK_DDD(
              DUK_DDDPRINT("writing existing 'length' property to array "
                           "exotic, invoke complex helper"));
          ...
          if (arr_idx < (h_bufobj->length >> h_bufobj->shift) &&
              DUK_HBUFOBJ_HAS_VIRTUAL_INDICES(h_bufobj)) {
            duk_uint8_t* data;
            DUK_DDD(DUK_DDDPRINT("writing to buffer data at index %ld",
                                 (long)arr_idx));

            DUK_ASSERT(arr_idx !=
                       DUK__NO_ARRAY_INDEX); /* index/length check guarantees */
            byte_off = arr_idx
                       << h_bufobj->shift; /* no wrap assuming h_bufobj->length
                                              is valid */
            elem_size = (duk_small_uint_t)(1U << h_bufobj->shift);
  ...
  if (!DUK_HOBJECT_HAS_EXTENSIBLE(orig)) {
    DUK_DD(
        DUK_DDPRINT("put to a new property (not found in prototype chain), but "
                    "original object not extensible"));
    goto fail_not_extensible;
  }
...
update_old:
  ...
  DUK_ASSERT(
      !(DUK_HOBJECT_IS_ARRAY(orig) && key == DUK_HTHREAD_STRING_LENGTH(thr)));
  ...
  if (desc.e_idx >= 0) {
    ...
    DUK_DD(
        DUK_DDPRINT("put to an existing entry at index %ld -> new value %!iT",
...
create_new:
  ...
  if (arr_idx != DUK__NO_ARRAY_INDEX && DUK_HOBJECT_HAS_ARRAY_PART(orig)) {
    ...
    DUK_DD(DUK_DDPRINT("put to new array entry: %ld -> %!T", (long)arr_idx,
                       (duk_tval*)tv));
...
entry_updated:
  ...
  if (new_array_length > 0) {
    ...
    DUK_DDD(
        DUK_DDDPRINT("write successful, pending array length update to: %ld",
...
success_with_arguments_exotic:
  ...
  if (arr_idx != DUK__NO_ARRAY_INDEX &&
    ...
    DUK_DDD(
        DUK_DDDPRINT("putprop successful, arguments exotic behavior needed"));
...
DUK_INTERNAL duk_bool_t duk_hobject_delprop_raw(duk_hthread* thr,
  ...
  DUK_DDD(
      DUK_DDDPRINT("delprop_raw: thr=%p, obj=%p, key=%p, throw=%ld, force=%ld "
                   "(obj -> %!O, key -> %!O)",
                   (void*)thr, (void*)obj, (void*)key, (long)throw_flag,
                   (long)force_flag, (duk_heaphdr*)obj, (duk_heaphdr*)key));
  ...
  if (!duk__get_own_propdesc_raw(thr, obj, key, arr_idx, &desc,
                                 0 /*flags*/)) { /* don't push value */
  ...
  if (desc.a_idx < 0 && desc.e_idx < 0) {
     ...
     */
    DUK_DD(
        DUK_DDPRINT("delete failed: property found, force flag, but virtual "
                    "(and implicitly non-configurable)"));
    goto fail_virtual;
  }
  ...
  if (desc.a_idx >= 0) {
    ...
    if (desc.h_idx >= 0) {
      ...
      DUK_DDD(
          DUK_DDDPRINT("removing hash entry at h_idx %ld", (long)desc.h_idx));
     ...
     */
    DUK_DDD(
        DUK_DDDPRINT("before removing value, e_idx %ld, key %p, key at slot %p",
                     (long)desc.e_idx, (void*)key,
    ...
    /* Remove key. */
    DUK_DDD(
        DUK_DDDPRINT("before removing key, e_idx %ld, key %p, key at slot %p",
                     (long)desc.e_idx, (void*)key,
                     (void*)DUK_HOBJECT_E_GET_KEY(thr->heap, obj, desc.e_idx)));
...
success:
  ...
  DUK_DDD(
      DUK_DDDPRINT("delete successful, check for arguments exotic behavior"));
  ...
  if (arr_idx != DUK__NO_ARRAY_INDEX && DUK_HOBJECT_HAS_EXOTIC_ARGUMENTS(obj)) {
    ...
    DUK_DDD(
        DUK_DDDPRINT("delete successful, arguments exotic behavior needed"));
...
DUK_INTERNAL duk_bool_t duk_hobject_delprop(duk_hthread* thr, duk_tval* tv_obj,
                                            duk_tval* tv_key,
                                            duk_bool_t throw_flag) {
  duk_hstring* key = NULL;
#if defined(DUK_USE_ES6_PROXY)
  ...
  DUK_DDD(DUK_DDDPRINT(
      "delprop: thr=%p, obj=%p, key=%p (obj -> %!T, key -> %!T)", (void*)thr,
      (void*)tv_obj, (void*)tv_key, (duk_tval*)tv_obj, (duk_tval*)tv_key));
  ...
  if (DUK_TVAL_IS_OBJECT(tv_obj)) {
    ...
    if (DUK_UNLIKELY(DUK_HOBJECT_IS_PROXY(obj))) {
      ...
      if (duk__proxy_check_prop(thr, obj, DUK_STRIDX_DELETE_PROPERTY, tv_key,
                                &h_target)) {
        /* -> [ ... obj key trap handler ] */
        DUK_DDD(DUK_DDDPRINT("-> proxy object 'deleteProperty' for key %!T",
                             (duk_tval*)tv_key));
        duk_push_hobject(thr, h_target); /* target */
        duk_dup_m4(thr);                 /* P */
        duk_call_method(thr, 2 /*nargs*/);
        ...
        if (duk__get_own_propdesc_raw(thr, h_target, key, arr_idx, &desc,
          ...
          DUK_DDD(
              DUK_DDDPRINT("proxy 'deleteProperty': target has matching "
                           "property %!O, check for "
                           "conflicting property; desc.flags=0x%08lx, "
                           "desc.get=%p, desc.set=%p",
    ...
    rc = duk_hobject_delprop_raw(thr, obj, key,
                                 throw_flag ? DUK_DELPROP_FLAG_THROW : 0);
    goto done_rc;
...
DUK_INTERNAL void duk_hobject_define_property_internal(duk_hthread* thr,
  ...
  duk_small_uint_t propflags =
      flags & DUK_PROPDESC_FLAGS_MASK; /* mask out flags not actually stored */

  DUK_DDD(
      DUK_DDDPRINT("define new property (internal): thr=%p, obj=%!O, key=%!O, "
                   "flags=0x%02lx, val=%!T",
  ...
  if (duk__get_own_propdesc_raw(thr, obj, key, arr_idx, &desc,
    ...
    if (desc.e_idx >= 0) {
      if (flags & DUK_PROPDESC_FLAG_NO_OVERWRITE) {
        DUK_DDD(DUK_DDDPRINT(
            "property already exists in the entry part -> skip as requested"));
        goto pop_exit;
      }
      DUK_DDD(
          DUK_DDDPRINT("property already exists in the entry part -> update "
                       "value and attributes"));
      if (DUK_UNLIKELY(
              DUK_HOBJECT_E_SLOT_IS_ACCESSOR(thr->heap, obj, desc.e_idx))) {
      ...
      if (flags & DUK_PROPDESC_FLAG_NO_OVERWRITE) {
        DUK_DDD(DUK_DDDPRINT(
            "property already exists in the array part -> skip as requested"));
        goto pop_exit;
      }
      DUK_DDD(
          DUK_DDDPRINT("property already exists in the array part -> update "
                       "value (assert attributes)"));
      ...
      if (flags & DUK_PROPDESC_FLAG_NO_OVERWRITE) {
        DUK_DDD(DUK_DDDPRINT(
            "property already exists but is virtual -> skip as requested"));
        goto pop_exit;
      }
      if (key == DUK_HTHREAD_STRING_LENGTH(thr) &&
          DUK_HOBJECT_HAS_EXOTIC_ARRAY(obj)) {
        duk_uint32_t new_len;
...
#endif
        new_len =
            duk__to_new_array_length_checked(thr, DUK_GET_TVAL_NEGIDX(thr, -1));
        ((duk_harray*)obj)->length = new_len;
        DUK_DD(DUK_DDPRINT(
            "internal define property for array .length: %ld -> %ld",
  ...
  if (DUK_HOBJECT_HAS_ARRAY_PART(obj)) {
    if (arr_idx != DUK__NO_ARRAY_INDEX) {
      DUK_DDD(DUK_DDDPRINT(
          "property does not exist, object has array part -> possibly extend "
          "array part and write value (assert attributes)"));
...
write_to_entry_part:
  DUK_DDD(
      DUK_DDDPRINT("property does not exist, object belongs in entry part -> "
                   "allocate new entry and write value and attributes"));
  e_idx = duk__hobject_alloc_entry_checked(thr, obj,
                                           key); /* increases key refcount */
  DUK_ASSERT(e_idx >= 0);
...
write_value:
  ...
  DUK_DDD(DUK_DDDPRINT("writing/updating value: %!T -> %!T", (duk_tval*)tv1,
                       (duk_tval*)tv2));
...
DUK_INTERNAL void duk_hobject_define_property_internal_arridx(
  ...
  if (DUK_HOBJECT_HAS_ARRAY_PART(obj) && arr_idx != DUK__NO_ARRAY_INDEX &&
    ...
    DUK_ASSERT((flags & DUK_PROPDESC_FLAG_NO_OVERWRITE) ==
...
DUK_INTERNAL duk_size_t duk_hobject_get_length(duk_hthread* thr,
  ...
  (void)duk_hobject_getprop(thr, DUK_GET_TVAL_NEGIDX(thr, -2),
...
DUK_INTERNAL duk_bool_t duk_hobject_has_finalizer_fast_raw(duk_heap* heap,
                                                           duk_hobject* obj) {
#else
DUK_INTERNAL duk_bool_t duk_hobject_has_finalizer_fast_raw(duk_hobject* obj) {
  ...
  do {
    ...
    if (DUK_UNLIKELY(sanity-- == 0)) {
      DUK_D(
          DUK_DPRINT("prototype loop when checking for finalizer existence; "
                     "returning false"));
      return 0;
    }
#if defined(DUK_USE_HEAPPTR16)
...
DUK_INTERNAL void duk_hobject_object_get_own_property_descriptor(
    duk_hthread* thr, duk_idx_t obj_idx) {
  duk_hobject* obj;
  duk_hstring* key;
  duk_propdesc pd;
  ...
  obj = duk_require_hobject_promote_mask(
      thr, obj_idx, DUK_TYPE_MASK_LIGHTFUNC | DUK_TYPE_MASK_BUFFER);
  key = duk_to_property_key_hstring(thr, -1);
  DUK_ASSERT(key != NULL);

  DUK_ASSERT_VALSTACK_SPACE(thr, DUK__VALSTACK_SPACE);

  if (!duk_hobject_get_own_propdesc(thr, obj, key, &pd,
                                    DUK_GETDESC_FLAG_PUSH_VALUE)) {
    duk_push_undefined(thr);
    duk_remove_m2(thr);
    return;
...
DUK_INTERNAL
void duk_hobject_prepare_property_descriptor(duk_hthread* thr, duk_idx_t idx_in,
  ...
  DUK_ASSERT(
      idx_in <=
      0x7fffL); /* short variants would be OK, but not used to avoid shifts */
...
DUK_INTERNAL
duk_bool_t duk_hobject_define_property_helper(
    duk_hthread* thr, duk_uint_t defprop_flags, duk_hobject* obj,
    duk_hstring* key, duk_idx_t idx_value, duk_hobject* get, duk_hobject* set,
    duk_bool_t throw_flag) {
  duk_uint32_t arr_idx;
  duk_tval tv;
  ...
  duk_propdesc curr;
  duk_uint32_t
      arridx_new_array_length; /* != 0 => post-update for array 'length' (used
                                  when key is an array index) */
  ...
  DUK_DDD(DUK_DDDPRINT(
      "has_enumerable=%ld is_enumerable=%ld "
      ...
      (long)has_enumerable, (long)is_enumerable, (long)has_configurable,
      (long)is_configurable, (long)has_writable, (long)is_writable,
      (long)has_value,
      (duk_tval*)(idx_value >= 0 ? duk_get_tval(thr, idx_value) : NULL),
      (long)has_get, (void*)get, (duk_heaphdr*)get, (long)has_set, (void*)set,
      (duk_heaphdr*)set, (long)arr_idx, (long)throw_flag));

  /*
   *  Array exotic behaviors can be implemented at this point.  The local
   * variables are essentially a 'value copy' of the input descriptor (Desc),
   * which is modified by the Array [[DefineOwnProperty]] (E5 Section 15.4.5.1).
   */
  ...
  if (key == DUK_HTHREAD_STRING_LENGTH(thr)) {
    ...
    /* E5 Section 15.4.5.1, step 3, steps a - i are implemented here, j - n at
     * the end */
    if (!has_value) {
      DUK_DDD(
          DUK_DDDPRINT("exotic array behavior for 'length', but no value in "
                       "descriptor -> normal behavior"));
      goto skip_array_exotic;
    }

    DUK_DDD(
        DUK_DDDPRINT("exotic array behavior for 'length', value present in "
                     "descriptor -> exotic behavior"));
    ...
    arrlen_new_len = duk__to_new_array_length_checked(
        thr, DUK_GET_TVAL_POSIDX(thr, idx_value));
    duk_push_u32(thr, arrlen_new_len);
    duk_replace(thr, idx_value); /* step 3.e: replace 'Desc.[[Value]]' */

    DUK_DDD(DUK_DDDPRINT("old_len=%ld, new_len=%ld", (long)arrlen_old_len,
                         (long)arrlen_new_len));

    if (arrlen_new_len >= arrlen_old_len) {
      /* standard behavior, step 3.f.i */
      DUK_DDD(DUK_DDDPRINT(
          "new length is same or higher as previous => standard behavior"));
      goto skip_array_exotic;
    }
    DUK_DDD(DUK_DDDPRINT(
        "new length is smaller than previous => exotic post behavior"));
    ...
    if (has_writable && !is_writable) {
      DUK_DDD(
          DUK_DDDPRINT("desc writable is false, force it back to true, and "
                       "flag pending write protect"));
      is_writable = 1;
      pending_write_protect = 1;
    }

    /* remaining actual steps are carried out if standard DefineOwnProperty
     * succeeds */
...
skip_array_exotic:
  ...
  if (!duk__get_own_propdesc_raw(thr, obj, key, arr_idx, &curr,
                                 DUK_GETDESC_FLAG_PUSH_VALUE)) {
    ...
    if (has_set || has_get) {
      ...
      new_flags =
          DUK_PROPDESC_FLAG_ACCESSOR; /* defaults, E5 Section 8.6.1, Table 7 */
      ...
      if (arr_idx != DUK__NO_ARRAY_INDEX && DUK_HOBJECT_HAS_ARRAY_PART(obj)) {
        if (new_flags == DUK_PROPDESC_FLAGS_WEC) {
          DUK_DDD(
              DUK_DDDPRINT("new data property attributes match array defaults, "
                           "attempt to write to array part"));
          tv2 = duk__obtain_arridx_slot(thr, arr_idx, obj);
          if (tv2 == NULL) {
            DUK_DDD(
                DUK_DDDPRINT("failed writing to array part, abandoned array"));
          } else {
          ...
          DUK_DDD(DUK_DDDPRINT(
              "new data property cannot go to array part, abandon array"));
          duk__abandon_array_part(thr, obj);
        }
        /* fall through */
      }
  ...
  /* we currently assume virtual properties are not configurable (as none of
   * them are) */
  DUK_ASSERT((curr.e_idx >= 0 || curr.a_idx >= 0) ||
             !(curr.flags & DUK_PROPDESC_FLAG_CONFIGURABLE));
...
need_check:
  ...
  if (DUK_HEAPHDR_HAS_READONLY((duk_heaphdr*)obj)) {
    DUK_DD(
        DUK_DDPRINT("attempt to define property on read-only target object"));
    goto fail_not_configurable;
  }
#endif
  ...
  if (has_set || has_get) {
    ...
    if (curr.flags & DUK_PROPDESC_FLAG_ACCESSOR) {
      ...
      if (curr.a_idx >= 0) {
        DUK_DDD(
            DUK_DDDPRINT("property to convert is stored in an array entry, "
                         "abandon array and re-lookup"));
        duk__abandon_array_part(thr, obj);
        duk_pop_unsafe(thr); /* remove old value */
        rc = duk__get_own_propdesc_raw(thr, obj, key, arr_idx, &curr,
                                       DUK_GETDESC_FLAG_PUSH_VALUE);
        DUK_UNREF(rc);
        DUK_ASSERT(rc != 0);
        DUK_ASSERT(curr.e_idx >= 0 && curr.a_idx < 0);
      ...
      DUK_DDD(DUK_DDDPRINT(
          "flags after data->accessor conversion: 0x%02lx",
    ...
    if (curr.flags & DUK_PROPDESC_FLAG_ACCESSOR) {
      ...
      DUK_TVAL_SET_UNDEFINED(
          DUK_HOBJECT_E_GET_VALUE_TVAL_PTR(thr->heap, obj, curr.e_idx));
      DUK_HOBJECT_E_SLOT_CLEAR_WRITABLE(thr->heap, obj, curr.e_idx);
      DUK_HOBJECT_E_SLOT_CLEAR_ACCESSOR(thr->heap, obj, curr.e_idx);

      DUK_DDD(DUK_DDDPRINT(
          "flags after accessor->data conversion: 0x%02lx",
          (unsigned long)DUK_HOBJECT_E_GET_FLAGS(thr->heap, obj, curr.e_idx)));
      ...
      if (!(curr.flags & DUK_PROPDESC_FLAG_CONFIGURABLE) && !force_flag) {
        if (!(curr.flags & DUK_PROPDESC_FLAG_WRITABLE) && has_writable &&
            is_writable) {
          goto fail_not_configurable;
        }
  ...
  if (curr.a_idx >= 0) {
    ...
    if (new_flags == DUK_PROPDESC_FLAGS_WEC) {
      ...
      DUK_DDD(
          DUK_DDDPRINT("array index, new property attributes match array "
                       "defaults, update in-place"));

      DUK_ASSERT(
          curr.flags ==
          DUK_PROPDESC_FLAGS_WEC); /* must have been, since in array part */
      DUK_ASSERT(!has_set);
      DUK_ASSERT(!has_get);
      DUK_ASSERT(idx_value >=
                 0); /* must be: if attributes match and we get here the value
                        must differ (otherwise no change) */

      tv2 = duk_require_tval(thr, idx_value);
      tv1 = DUK_HOBJECT_A_GET_VALUE_PTR(thr->heap, obj, curr.a_idx);
      DUK_TVAL_SET_TVAL_UPDREF(thr, tv1,
                               tv2); /* side effects; may invalidate a_idx */
      goto success_exotics;
    }

    DUK_DDD(
        DUK_DDDPRINT("array index, new property attributes do not match array "
                     "defaults, abandon array and re-lookup"));
    duk__abandon_array_part(thr, obj);
    duk_pop_unsafe(thr); /* remove old value */
    rc = duk__get_own_propdesc_raw(thr, obj, key, arr_idx, &curr,
                                   DUK_GETDESC_FLAG_PUSH_VALUE);
  ...
  if (curr.e_idx >= 0) {
    ...
    if (key == DUK_HTHREAD_STRING_LENGTH(thr) &&
      ...
      a = (duk_harray*)obj;
      DUK_DD(
          DUK_DDPRINT("Object.defineProperty() attribute update for duk_harray "
                      ".length -> %02lx",
                      (unsigned long)new_flags));
      DUK_HARRAY_ASSERT_VALID(a);
      if ((new_flags & DUK_PROPDESC_FLAGS_EC) !=
          (curr.flags & DUK_PROPDESC_FLAGS_EC)) {
        DUK_D(DUK_DPRINT(
            "Object.defineProperty() attempt to change virtual array .length "
            "enumerable or configurable attribute, fail"));
        goto fail_virtual;
      }
  ...
  if (has_set) {
    ...
    DUK_HOBJECT_INCREF_ALLOWNULL(thr, set);
    DUK_HOBJECT_DECREF_ALLOWNULL(thr,
                                 tmp); /* side effects; may invalidate e_idx */
  }
  if (has_get) {
    duk_hobject* tmp;
  ...
  if (has_value) {
    ...
    if (curr.e_idx >= 0) {
      ...
      tv1 = DUK_HOBJECT_E_GET_VALUE_TVAL_PTR(thr->heap, obj, curr.e_idx);
      DUK_TVAL_SET_TVAL_UPDREF(thr, tv1,
                               tv2); /* side effects; may invalidate e_idx */
    } else {
      DUK_ASSERT(curr.a_idx <
                 0); /* array part case handled comprehensively previously */
      ...
      if (key == DUK_HTHREAD_STRING_LENGTH(thr) &&
        ...
        a = (duk_harray*)obj;
        DUK_DD(
            DUK_DDPRINT("Object.defineProperty() value update for duk_harray "
                        ".length -> %ld",
                        (long)arrlen_new_len));
        DUK_HARRAY_ASSERT_VALID(a);
  ...
  /*
   *  Standard algorithm succeeded without errors, check for exotic
   * post-behaviors.
   *
   ...
   *    - for an array index key (e.g. "10") the final 'length' update occurs
   * here
...
success_exotics:
  ...
  if (DUK_HOBJECT_HAS_EXOTIC_ARRAY(obj)) {
    ...
    if (arridx_new_array_length > 0) {
      ...
      DUK_DDD(DUK_DDDPRINT(
          "defineProperty successful, pending array length update to: %ld",
    ...
    if (key == DUK_HTHREAD_STRING_LENGTH(thr) &&
      ...
      rc = duk__handle_put_array_length_smaller(
          thr, obj, arrlen_old_len, arrlen_new_len, force_flag, &result_len);
      ...
      if (pending_write_protect) {
        DUK_DDD(DUK_DDDPRINT(
            "setting array length non-writable (pending writability update)"));
        DUK_HARRAY_SET_LENGTH_NONWRITABLE(a);
      }
    ...
    DUK_ASSERT(!DUK_HOBJECT_HAS_EXOTIC_ARRAY(
        obj)); /* traits are separate; in particular, arguments not an array */
    ...
    if (has_set || has_get) {
      ...
      DUK_DDD(DUK_DDDPRINT(
          "defineProperty successful, key mapped to arguments 'map' "
          "changed to an accessor, delete arguments binding"));

      (void)duk_hobject_delprop_raw(thr, map, key, 0); /* ignore result */
    } else {
      /* Note: this order matters (final value before deleting map entry must be
       * done) */
      DUK_DDD(DUK_DDDPRINT(
          "defineProperty successful, key mapped to arguments 'map', "
      ...
      if (has_value) {
        ...
        DUK_DDD(DUK_DDDPRINT(
            "arguments object automatic putvar for a bound variable; "
            "key=%!O, varname=%!O, value=%!T",
            (duk_heaphdr*)key, (duk_heaphdr*)varname,
            (duk_tval*)duk_require_tval(thr, idx_value)));

        /* strict flag for putvar comes from our caller (currently: fixed) */
        duk_js_putvar_envrec(thr, varenv, varname,
                             duk_require_tval(thr, idx_value),
                             1 /*throw_flag*/);
      }
      if (has_writable && !is_writable) {
        DUK_DDD(DUK_DDDPRINT(
    ...
    /* 'varname' is in stack in this else branch, leaving an unbalanced stack
     * below, but this doesn't matter now.
     */
  }
...
DUK_INTERNAL duk_bool_t duk_hobject_object_ownprop_helper(
    duk_hthread* thr, duk_small_uint_t required_desc_flags) {
  duk_hstring* h_v;
  duk_hobject* h_obj;
  duk_propdesc desc;
  duk_bool_t ret;
  ...
  ret = duk_hobject_get_own_propdesc(thr, h_obj, h_v, &desc,
                                     0 /*flags*/); /* don't push value */

  duk_push_boolean(
      thr, ret && ((desc.flags & required_desc_flags) == required_desc_flags));
  return 1;
}
...
DUK_INTERNAL void duk_hobject_object_seal_freeze_helper(duk_hthread* thr,
  ...
  for (i = 0; i < DUK_HOBJECT_GET_ENEXT(obj); i++) {
    ...
    /* since duk__abandon_array_part() causes a resize, there should be no gaps
     * in keys */
 ...
 *  and non-writable (and there are no accessor virtual properties), so they
 * don't need to be considered here now.
 */

DUK_INTERNAL duk_bool_t duk_hobject_object_is_sealed_frozen_helper(
  ...
  for (i = 0; i < DUK_HOBJECT_GET_ENEXT(obj); i++) {
    ...
    if (is_frozen && !(flags & DUK_PROPDESC_FLAG_ACCESSOR) &&
...
/*
 *  Object.preventExtensions() and Object.isExtensible()  (E5
 * Sections 15.2.3.10, 15.2.3.13)
 *
