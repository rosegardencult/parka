...
#    define DUK_ERROR(thr, err, msg)                                          \
      do {                                                                    \
        ...
        duk_err_handle_error(                                                 \
            (thr), DUK_FILE_MACRO,                                            \
            (((duk_uint_t)duk__err) << 24) | ((duk_uint_t)duk__line), (msg)); \
      } while (0)
...
#    define DUK_ERROR_FMT1(thr, err, fmt, arg1)                              \
      do {                                                                   \
        ...
        duk_err_handle_error_fmt(                                            \
            (thr), DUK_FILE_MACRO,                                           \
            (((duk_uint_t)duk__err) << 24) | ((duk_uint_t)duk__line), (fmt), \
            (arg1));                                                         \
      } while (0)
...
#    define DUK_ERROR_RAW_FMT4(thr, file, line, err, fmt, arg1, arg2, arg3,  \
                               arg4)                                         \
      do {                                                                   \
        duk_errcode_t duk__err = (err);                                      \
        duk_int_t duk__line = (duk_int_t)(line);                             \
...
#      define DUK_ERROR_REQUIRE_TYPE_INDEX(thr, idx, expectname, lowmemstr) \
        do {                                                                \
          duk_err_require_type_index((thr), DUK_FILE_MACRO,                 \
                                     (duk_int_t)DUK_LINE_MACRO, (idx),      \
                                     (expectname));                         \
        } while (0)
...
#    define DUK_ERROR_INTERNAL(thr)                        \
      do {                                                 \
        duk_err_error_internal((thr), DUK_FILE_MACRO,      \
...
#    define DUK_ERROR_ALLOC_FAILED(thr)                        \
      do {                                                     \
        duk_err_error_alloc_failed((thr), DUK_FILE_MACRO,      \
                                   (duk_int_t)DUK_LINE_MACRO); \
      } while (0)
...
#    define DUK_ERROR_ERROR(thr, msg)                                   \
      do {                                                              \
        duk_err_error((thr), DUK_FILE_MACRO, (duk_int_t)DUK_LINE_MACRO, \
                      (msg));                                           \
      } while (0)
#    define DUK_ERROR_RANGE_INDEX(thr, idx)                                   \
      do {                                                                    \
        duk_err_range_index((thr), DUK_FILE_MACRO, (duk_int_t)DUK_LINE_MACRO, \
                            (idx));                                           \
      } while (0)
#    define DUK_ERROR_RANGE_PUSH_BEYOND(thr)                  \
      do {                                                    \
        duk_err_range_push_beyond((thr), DUK_FILE_MACRO,      \
                                  (duk_int_t)DUK_LINE_MACRO); \
      } while (0)
...
#    define DUK_ERROR_RANGE(thr, msg)                                   \
      do {                                                              \
        duk_err_range((thr), DUK_FILE_MACRO, (duk_int_t)DUK_LINE_MACRO, \
                      (msg));                                           \
      } while (0)
...
#    define DUK_ERROR_TYPE_INVALID_ARGS(thr)                  \
      do {                                                    \
        duk_err_type_invalid_args((thr), DUK_FILE_MACRO,      \
                                  (duk_int_t)DUK_LINE_MACRO); \
      } while (0)
...
#    define DUK_ERROR_TYPE_INVALID_STATE(thr)                  \
      do {                                                     \
        duk_err_type_invalid_state((thr), DUK_FILE_MACRO,      \
                                   (duk_int_t)DUK_LINE_MACRO); \
      } while (0)
...
#    define DUK_ERROR_TYPE_INVALID_TRAP_RESULT(thr)                  \
      do {                                                           \
        duk_err_type_invalid_trap_result((thr), DUK_FILE_MACRO,      \
                                         (duk_int_t)DUK_LINE_MACRO); \
      } while (0)
      ...
      do {                                                  \
        if (!(x)) {                                         \
          DUK_FATAL_WITHOUT_CONTEXT(                        \
              "assertion failed: " #x " (" DUK_FILE_MACRO   \
...
#    define DUK_ASSERT_REFCOUNT_NONZERO_HEAPHDR(h)                   \
      do {                                                           \
        DUK_ASSERT((h) == NULL ||                                    \
                   DUK_HEAPHDR_GET_REFCOUNT((duk_heaphdr*)(h)) > 0); \
      } while (0)
#    define DUK_ASSERT_REFCOUNT_NONZERO_TVAL(tv)                            \
      do {                                                                  \
        if ((tv) != NULL && DUK_TVAL_IS_HEAP_ALLOCATED((tv))) {             \
          DUK_ASSERT(DUK_HEAPHDR_GET_REFCOUNT(DUK_TVAL_GET_HEAPHDR((tv))) > \
                     0);                                                    \
        }                                                                   \
      } while (0)
#  else
 ...
 * entries required for its own use, and any child calls which are not (a)
 * Duktape API calls or (b) Duktape calls which involve extending the valstack
 * (e.g. getter call).
 */
...
#  if defined(DUK_USE_VERBOSE_ERRORS)
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_handle_error(
    duk_hthread* thr, const char* filename, duk_uint_t line_and_code,
    const char* msg));
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_handle_error_fmt(
    duk_hthread* thr, const char* filename, duk_uint_t line_and_code,
    const char* fmt, ...));
#  else  /* DUK_USE_VERBOSE_ERRORS */
...
#  if defined(DUK_USE_VERBOSE_ERRORS)
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_create_and_throw(
    duk_hthread* thr, duk_errcode_t code, const char* msg, const char* filename,
    duk_int_t line));
#  else
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_create_and_throw(
    duk_hthread* thr, duk_errcode_t code));
#  endif

DUK_NORETURN(DUK_INTERNAL_DECL void duk_error_throw_from_negative_rc(
    duk_hthread* thr, duk_ret_t rc));

#  define DUK_AUGMENT_FLAG_NOBLAME_FILELINE \
    (1U                                     \
     << 0) /* if set, don't blame C file/line for .fileName and .lineNumber */
#  define DUK_AUGMENT_FLAG_SKIP_ONE \
    (1U << 1) /* if set, skip topmost activation in traceback construction */
...
#  if defined(DUK_USE_VERBOSE_ERRORS)
#    if defined(DUK_USE_PARANOID_ERRORS)
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_require_type_index(
    duk_hthread* thr, const char* filename, duk_int_t linenumber, duk_idx_t idx,
...
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_require_type_index(
    ...
    const char* expect_name));
#    endif
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_error_internal(
    duk_hthread* thr, const char* filename, duk_int_t linenumber));
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_error_alloc_failed(
    duk_hthread* thr, const char* filename, duk_int_t linenumber));
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_error(duk_hthread* thr,
                                                  const char* filename,
                                                  duk_int_t linenumber,
                                                  const char* message));
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_range_index(duk_hthread* thr,
                                                        const char* filename,
                                                        duk_int_t linenumber,
                                                        duk_idx_t idx));
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_range_push_beyond(
    duk_hthread* thr, const char* filename, duk_int_t linenumber));
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_range(duk_hthread* thr,
                                                  const char* filename,
                                                  duk_int_t linenumber,
                                                  const char* message));
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_type_invalid_args(
    duk_hthread* thr, const char* filename, duk_int_t linenumber));
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_type_invalid_state(
    duk_hthread* thr, const char* filename, duk_int_t linenumber));
DUK_NORETURN(DUK_INTERNAL_DECL void duk_err_type_invalid_trap_result(
    duk_hthread* thr, const char* filename, duk_int_t linenumber));
...
DUK_NORETURN(DUK_INTERNAL_DECL void duk_default_fatal_handler(void* udata,
                                                              const char* msg));

DUK_INTERNAL_DECL void duk_err_setup_ljstate1(duk_hthread* thr,
...
DUK_INTERNAL_DECL duk_hobject* duk_error_prototype_from_code(
    duk_hthread* thr, duk_errcode_t err_code);
