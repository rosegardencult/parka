...
#  define DUK_JSON_FLAG_ASCII_ONLY \
    (1U << 0) /* escape any non-ASCII characters */
#  define DUK_JSON_FLAG_AVOID_KEY_QUOTES \
    (1U << 1) /* avoid key quotes when key is an ASCII Identifier */
#  define DUK_JSON_FLAG_EXT_CUSTOM \
    (1U << 2) /* extended types: custom encoding */
#  define DUK_JSON_FLAG_EXT_COMPATIBLE \
    (1U << 3) /* extended types: compatible encoding */
...
#  endif
  ...
  duk_uint_t mask_for_undefined; /* type bit mask: types which certainly produce
                                    'undefined' */
#  if defined(DUK_USE_JX) || defined(DUK_USE_JC)
...
#  endif
  duk_hobject*
      visiting[DUK_JSON_ENC_LOOPARRAY]; /* indexed by recursion_depth */
} duk_json_enc_ctx;
