...
#  define DUK_COMPILER_TOKEN_LIMIT \
    100000000L /* 1e8: protects against deeply nested inner functions */
...
#  define DUK_IVAL_ARITH \
    2 /* binary arithmetic; DUK_OP_ADD, DUK_OP_EQ, other binary ops */
...
struct duk_compiler_func {
  ...
  duk_hstring*
      h_name; /* function name (borrowed reference), ends up in _name */
  /* h_code: held in bw_code */
  duk_hobject* h_consts; /* array */
  duk_hobject* h_funcs;  /* array of function templates: [func1, offset1, line1,
                          * func2, offset2, line2]  offset/line points to closing
                          * brace to allow skipping on pass 2
                          */
  duk_hobject* h_decls;  /* array of declarations: [ name1, val1, name2, val2,
                          * ... ]  valN = (typeN) | (fnum << 8), where fnum is
                          * inner func number (0 for vars)  record function and
                          * variable declarations in pass 1
                          */
  ...
  duk_hobject* h_varmap;   /* variable map for pass 2 (identifier -> register
                              number or null (unmapped)) */
  ...
  /* Temp reg handling. */
  duk_regconst_t
      temp_first; /* first register that is a temporary (below: variables) */
  duk_regconst_t temp_next; /* next temporary register to allocate */
  duk_regconst_t temp_max; /* highest value of temp_reg (temp_max - 1 is highest
                              used reg) */
  ...
  duk_regconst_t
      reg_stmt_value; /* register for writing value of 'non-empty' statements
                         (global or eval code), -1 is marker */
...
#  endif
  ...
  duk_uint8_t in_directive_prologue; /* parsing in "directive prologue",
  ...
  duk_uint8_t
      id_access_arguments;    /* function refers to 'arguments' identifier */
  duk_uint8_t id_access_slow; /* function makes one or more slow path accesses
                                 that won't match own static variables */
  duk_uint8_t
      id_access_slow_own; /* function makes one or more slow path accesses that
                             may match own static variables */
  duk_uint8_t is_arguments_shadowed; /* argument/function declaration shadows
                                        'arguments' */
  ...
  duk_uint8_t
      reject_regexp_in_adv; /* reject RegExp literal on next advance() call;
                               needed for handling IdentifierName productions */
  duk_uint8_t
      allow_regexp_in_adv; /* allow RegExp literal on next advance() call */
};
...
struct duk_compiler_ctx {
  ...
  /* current function being compiled (embedded instead of pointer for more
   * compact access) */
