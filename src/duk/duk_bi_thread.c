...
DUK_INTERNAL duk_ret_t duk_bi_thread_resume(duk_hthread* ctx) {
  ...
  DUK_DDD(DUK_DDDPRINT(
      "Duktape.Thread.resume(): thread=%!T, value=%!T, is_error=%!T",
  ...
  if (thr->callstack_top < 2) {
    DUK_DD(
        DUK_DDPRINT("resume state invalid: callstack should contain at least 2 "
                    "entries (caller and Duktape.Thread.resume)"));
    goto state_error;
  }
  ...
  DUK_ASSERT(DUK_ACT_GET_FUNC(thr->callstack_curr->parent) !=
             NULL); /* caller */
...
#  endif
  ...
  DUK_ASSERT(thr->heap->lj.jmpbuf_ptr !=
             NULL);     /* call is from executor, so we know we have a jmpbuf */
...
DUK_INTERNAL duk_ret_t duk_bi_thread_yield(duk_hthread* thr) {
  ...
  if (thr->callstack_top < 2) {
    DUK_DD(
        DUK_DDPRINT("yield state invalid: callstack should contain at least 2 "
                    "entries (caller and Duktape.Thread.yield)"));
    goto state_error;
  }
  DUK_ASSERT(thr->callstack_curr != NULL);
  DUK_ASSERT(thr->callstack_curr->parent != NULL);
  ...
  DUK_ASSERT(thr->callstack_preventcount >=
             1); /* should never be zero, because we (Duktape.Thread.yield) are
                    on the stack */
  if (thr->callstack_preventcount != 1) {
    /* Note: the only yield-preventing call is Duktape.Thread.yield(), hence
     * check for 1, not 0 */
    DUK_DD(
        DUK_DDPRINT("yield state invalid: there must be no yield-preventing "
                    "calls in current thread callstack (preventcount is %ld)",
