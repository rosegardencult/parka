...
DUK_LOCAL void duk__err_augment_user(duk_hthread* thr,
  ...
  if (tv_hnd == NULL) {
    DUK_DD(
        DUK_DDPRINT("error handler does not exist or is not a plain value: %!T",
                    (duk_tval*)tv_hnd));
    return;
  }
...
DUK_LOCAL void duk__add_traceback(duk_hthread* thr, duk_hthread* thr_callstack,
                                  const char* c_filename, duk_int_t c_line,
  ...
  DUK_ASSERT(thr_callstack->callstack_top <=
             DUK_INT_MAX); /* callstack limits */
  ...
  if (thr->compile_ctx != NULL && thr->compile_ctx->h_filename != NULL) {
    ...
    u32 = (duk_uint32_t)thr->compile_ctx->curr_token
              .start_line; /* (flags<<32) + (line), flags = 0 */
  ...
  for (; depth-- > 0; act = act->parent) {
    ...
    DUK_ASSERT(act !=
               NULL); /* depth check above, assumes book-keeping is correct */
    ...
    DUK_ASSERT(
        (duk_double_t)pc <
        DUK_DOUBLE_2TO32); /* assume PC is at most 32 bits and non-negative */
...
#  endif
  ...
  duk_xdef_prop_stridx_short_wec(
      thr, -2, DUK_STRIDX_INT_TRACEDATA); /* -> [ ... error ] */
}
  ...
  if (thr->compile_ctx != NULL && thr->compile_ctx->h_filename != NULL) {
    ...
    for (act = thr_callstack->callstack_curr; depth-- > 0; act = act->parent) {
      ...
      pc = duk_hthread_get_act_prev_pc(
          thr, act); /* thr argument only used for thr->heap, so specific thread
                        doesn't matter */
      DUK_UNREF(pc);
      DUK_ASSERT_DISABLE(pc >= 0); /* unsigned */
      DUK_ASSERT(
...
#  endif
  duk_xdef_prop_stridx_short(
      thr, -3, DUK_STRIDX_FILE_NAME,
      DUK_PROPDESC_FLAGS_C | DUK_PROPDESC_FLAG_NO_OVERWRITE);
  duk_xdef_prop_stridx_short(
      thr, -2, DUK_STRIDX_LINE_NUMBER,
      DUK_PROPDESC_FLAGS_C | DUK_PROPDESC_FLAG_NO_OVERWRITE);
}
...
#  endif
    ...
    DUK_D(DUK_DPRINT(
        ...
        "lex.input_length=%ld, window[0].codepoint=%ld, "
        "window[1].codepoint=%ld",
        (long)at_end, (long)thr->compile_ctx->curr_token.start_offset,
...
DUK_LOCAL void duk__err_augment_builtin_create(
    duk_hthread* thr, duk_hthread* thr_callstack, const char* c_filename,
    duk_int_t c_line, duk_hobject* obj, duk_small_uint_t flags) {
#  if defined(DUK_USE_ASSERTIONS)
  duk_int_t entry_top;
#  endif
...
#  if defined(DUK_USE_TRACEBACKS)
   ...
   */
  if (duk_hobject_find_entry_tval_ptr_stridx(
          thr->heap, obj, DUK_STRIDX_INT_TRACEDATA) != NULL) {
...
DUK_INTERNAL void duk_err_augment_error_create(duk_hthread* thr,
  ...
  if (!obj) {
    DUK_DDD(DUK_DDDPRINT(
        "value is not an object, skip both built-in and user augment"));
    return;
  }
  if (!duk_hobject_prototype_chain_contains(
          thr, obj, thr->builtins[DUK_BIDX_ERROR_PROTOTYPE],
          1 /*ignore_loop*/)) {
  ...
  if (DUK_HOBJECT_HAS_EXTENSIBLE(obj)) {
    ...
    duk__err_augment_builtin_create(thr, thr_callstack, c_filename, c_line, obj,
                                    flags);
  } else {
