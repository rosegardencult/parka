...
typedef struct {
  ...
  duk_uint_t attrs; /* property attributes for identifier (relevant if value !=
                       NULL) */
  duk_bool_t
      has_this; /* for object-bound identifiers: provide 'this' binding */
...
DUK_LOCAL const duk_uint16_t duk__closure_copy_proplist[] = {
    ...
    DUK_STRIDX_INT_VARMAP, DUK_STRIDX_INT_FORMALS,
...
DUK_INTERNAL
void duk_js_push_closure(duk_hthread* thr, duk_hcompfunc* fun_temp,
  ...
  DUK_ASSERT(DUK_HOBJECT_GET_PROTOTYPE(thr->heap, (duk_hobject*)fun_clos) ==
  ...
  DUK_HCOMPFUNC_SET_DATA(thr->heap, fun_clos,
                         DUK_HCOMPFUNC_GET_DATA(thr->heap, fun_temp));
  DUK_HCOMPFUNC_SET_FUNCS(thr->heap, fun_clos,
                          DUK_HCOMPFUNC_GET_FUNCS(thr->heap, fun_temp));
  DUK_HCOMPFUNC_SET_BYTECODE(thr->heap, fun_clos,
                             DUK_HCOMPFUNC_GET_BYTECODE(thr->heap, fun_temp));
   ...
   */
  DUK_ASSERT(DUK_HOBJECT_GET_PROTOTYPE(thr->heap, &fun_clos->obj) ==
             thr->builtins[DUK_BIDX_FUNCTION_PROTOTYPE]);
#if 0
  ...
  DUK_HEAPHDR_SET_FLAGS((duk_heaphdr*)fun_clos,
                        DUK_HEAPHDR_GET_FLAGS_RAW((duk_heaphdr*)fun_temp));
  DUK_DD(DUK_DDPRINT(
  ...
  if (DUK_HOBJECT_HAS_NEWENV(&fun_clos->obj)) {
    ...
    if (DUK_HOBJECT_HAS_NAMEBINDING(&fun_clos->obj)) {
       ...
       *  b) { funcname: <func>, __prototype:  <globalenv> }  (if outer_lex_env
       * missing)
       */
      ...
      duk_xdef_prop(
          thr, -3,
          DUK_PROPDESC_FLAGS_NONE); /* -> [ ... closure template env ] */
    ...
    DUK_HOBJECT_INCREF(
        thr, outer_lex_env); /* NULLs not allowed; asserted on entry */
  ...
  for (i = 0; i < (duk_small_uint_t)(sizeof(duk__closure_copy_proplist) /
                                     sizeof(duk_uint16_t));
       i++) {
    ...
    if (duk_xget_owndataprop_stridx_short(thr, -1, stridx)) {
      ...
      DUK_DDD(
          DUK_DDDPRINT("copying property, stridx=%ld -> found", (long)stridx));
      duk_xdef_prop_stridx_short(thr, -3, stridx, DUK_PROPDESC_FLAGS_C);
    } else {
      DUK_DDD(DUK_DDDPRINT("copying property, stridx=%ld -> not found",
                           (long)stridx));
      duk_pop_unsafe(thr);
    }
  }
  ...
  if (formals) {
    ...
    DUK_DD(DUK_DDPRINT("closure length defaulted from nargs -> %ld",
                       (long)len_value));
  }
  ...
  if (DUK_HOBJECT_HAS_STRICT(&fun_clos->obj)) {
    ...
    DUK_DDD(
        DUK_DDDPRINT("function is non-strict and non-standard 'caller' "
                     "property in use, add initial 'null' value"));
    duk_push_null(thr);
    duk_xdef_prop_stridx_short(thr, -3, DUK_STRIDX_CALLER,
  ...
  if (duk_get_prop_stridx_short(thr, -1, DUK_STRIDX_NAME)) {
    ...
    DUK_DD(DUK_DDPRINT("setting function instance name to %!T",
                       duk_get_tval(thr, -1)));
    duk_xdef_prop_stridx_short(
        thr, -3, DUK_STRIDX_NAME,
        DUK_PROPDESC_FLAGS_C); /* -> [ ... closure template ] */
  } else {
  ...
  DUK_ASSERT(DUK_HOBJECT_GET_CLASS_NUMBER(&fun_clos->obj) ==
             DUK_HOBJECT_CLASS_FUNCTION);
  DUK_ASSERT(DUK_HOBJECT_GET_PROTOTYPE(thr->heap, &fun_clos->obj) ==
             thr->builtins[DUK_BIDX_FUNCTION_PROTOTYPE]);
  DUK_ASSERT(DUK_HOBJECT_HAS_EXTENSIBLE(&fun_clos->obj));
  DUK_ASSERT(duk_has_prop_stridx(thr, -2, DUK_STRIDX_LENGTH) != 0);
  DUK_ASSERT(add_auto_proto == 0 ||
             duk_has_prop_stridx(thr, -2, DUK_STRIDX_PROTOTYPE) != 0);
  /* May be missing .name */
  ...
  DUK_DDD(DUK_DDDPRINT(
      "created function instance: template=%!iT -> closure=%!iT",
...
DUK_LOCAL void duk__preallocate_env_entries(duk_hthread* thr,
  ...
  for (i = 0; i < (duk_uint_fast32_t)DUK_HOBJECT_GET_ENEXT(varmap); i++) {
    ...
    DUK_ASSERT(!DUK_HOBJECT_E_SLOT_IS_ACCESSOR(thr->heap, varmap,
                                               i)); /* assume plain values */
...
/* shared helper */
DUK_INTERNAL
duk_hobject* duk_create_activation_environment_record(
    duk_hthread* thr, duk_hobject* func, duk_size_t bottom_byteoff) {
  duk_hdecenv* env;
  duk_hobject* parent;
  duk_hcompfunc* f;
...
void duk_js_init_activation_environment_records_delayed(duk_hthread* thr,
  ...
  DUK_ASSERT(func != NULL);
  DUK_ASSERT(!DUK_HOBJECT_HAS_BOUNDFUNC(
      func)); /* bound functions are never in act 'func' */
  ...
  env =
      duk_create_activation_environment_record(thr, func, act->bottom_byteoff);
  DUK_ASSERT(env != NULL);
...
DUK_INTERNAL void duk_js_close_environment_record(duk_hthread* thr,
  ...
  if (DUK_UNLIKELY(!DUK_HOBJECT_IS_DECENV(env))) {
    DUK_DDD(
        DUK_DDDPRINT("env not a declarative record: %!iO", (duk_heaphdr*)env));
    return;
  }
   ...
   * unless the environment may have "escaped" (referenced in a function
   * closure). With delayed environments, the existence is probably good enough
   * of a check.
   */
  ...
  DUK_DDD(DUK_DDDPRINT("copying bound register values, %ld bound regs",
                       (long)DUK_HOBJECT_GET_ENEXT(varmap)));
  ...
  for (i = 0; i < (duk_uint_fast32_t)DUK_HOBJECT_GET_ENEXT(varmap); i++) {
    ...
    DUK_ASSERT(DUK_TVAL_GET_NUMBER(tv) <=
               (duk_double_t)DUK_UINT32_MAX); /* limits */
    ...
    DUK_ASSERT((duk_uint8_t*)thr->valstack + regbase_byteoff +
                   sizeof(duk_tval) * regnum >=
     ...
     */
    duk_push_tval(
        thr, (duk_tval*)(void*)((duk_uint8_t*)thr->valstack + regbase_byteoff +
                                sizeof(duk_tval) * regnum));
    DUK_DDD(DUK_DDDPRINT("closing identifier %!O -> reg %ld, value %!T",
                         (duk_heaphdr*)key, (long)regnum,
                         (duk_tval*)duk_get_tval(thr, -1)));
    duk_hobject_define_property_internal(thr, env, key, DUK_PROPDESC_FLAGS_WE);
  }
...
DUK_LOCAL
duk_bool_t duk__getid_open_decl_env_regs(duk_hthread* thr, duk_hstring* name,
                                         duk_hdecenv* env,
  ...
  tv = (duk_tval*)(void*)((duk_uint8_t*)env->thread->valstack +
                          env->regbase_byteoff + sizeof(duk_tval) * reg_rel);
  DUK_ASSERT(tv >= env->thread->valstack &&
             tv < env->thread->valstack_end); /* XXX: more accurate? */
...
DUK_LOCAL
duk_bool_t duk__getid_activation_regs(duk_hthread* thr, duk_hstring* name,
                                      duk_activation* act,
                                      duk__id_lookup_result* out) {
  duk_tval* tv;
  duk_hobject* func;
...
DUK_LOCAL
duk_bool_t duk__get_identifier_reference(duk_hthread* thr, duk_hobject* env,
                                         duk_hstring* name, duk_activation* act,
                                         duk_bool_t parents,
                                         duk__id_lookup_result* out) {
  duk_tval* tv;
  duk_uint_t sanity;
  ...
  if (env == NULL && act != NULL) {
    ...
    if (duk__getid_activation_regs(thr, name, act, out)) {
      DUK_DDD(DUK_DDDPRINT(
          "duk__get_identifier_reference successful: "
          "name=%!O -> value=%!T, attrs=%ld, has_this=%ld, env=%!O, holder=%!O "
          "(found from register bindings when env=NULL)",
          (duk_heaphdr*)name, (duk_tval*)out->value, (long)out->attrs,
          (long)out->has_this, (duk_heaphdr*)out->env,
    ...
    if (!parents) {
      DUK_DDD(DUK_DDDPRINT(
          "duk__get_identifier_reference failed, no parent traversal "
  ...
  while (env != NULL) {
    ...
    DUK_DDD(DUK_DDDPRINT(
        ...
        (duk_heaphdr*)name, (void*)env, (duk_heaphdr*)env));
    ...
    if (cl == DUK_HOBJECT_CLASS_DECENV) {
      ...
      if (duk__getid_open_decl_env_regs(thr, name, (duk_hdecenv*)env, out)) {
        DUK_DDD(DUK_DDDPRINT(
            "duk__get_identifier_reference successful: "
            "name=%!O -> value=%!T, attrs=%ld, has_this=%ld, env=%!O, "
            "holder=%!O "
      ...
      tv = duk_hobject_find_entry_tval_ptr_and_attrs(thr->heap, env, name,
                                                     &attrs);
      if (tv) {
        out->value = tv;
        out->attrs = attrs;
       ...
       * this binding is undefined.  If _This, assumes this binding is _This,
       * and target is also _This.  One property would then be enough.
       */
      ...
      if (found) {
        ...
        DUK_DDD(
            DUK_DDDPRINT("duk__get_identifier_reference successful: "
                         "name=%!O -> value=%!T, attrs=%ld, has_this=%ld, "
                         "env=%!O, holder=%!O "
                         "(object environment record)",
...
DUK_LOCAL
duk_bool_t duk__getvar_helper(duk_hthread* thr, duk_hobject* env,
                              duk_activation* act, duk_hstring* name,
  ...
  DUK_DDD(
      DUK_DDDPRINT("getvar: thr=%p, env=%p, act=%p, name=%!O "
                   "(env -> %!dO)",
                   (void*)thr, (void*)env, (void*)act, (duk_heaphdr*)name,
                   (duk_heaphdr*)env));
  ...
  if (duk__get_identifier_reference(thr, env, name, act, parents, &ref)) {
    ...
    if (throw_flag) {
      DUK_ERROR_FMT1(thr, DUK_ERR_REFERENCE_ERROR, "identifier '%s' undefined",
...
DUK_INTERNAL
duk_bool_t duk_js_getvar_envrec(duk_hthread* thr, duk_hobject* env,
                                duk_hstring* name, duk_bool_t throw_flag) {
  return duk__getvar_helper(thr, env, NULL, name, throw_flag);
}

DUK_INTERNAL
duk_bool_t duk_js_getvar_activation(duk_hthread* thr, duk_activation* act,
                                    duk_hstring* name, duk_bool_t throw_flag) {
  DUK_ASSERT(act != NULL);
 ...
 *    8.7.2 PutValue (V,W)  [see especially step 3.b, undefined -> automatic
 * global in non-strict mode] 8.12.4 [[CanPut]] (P) 8.12.5 [[Put]] (P)
 *
...
DUK_LOCAL
void duk__putvar_helper(duk_hthread* thr, duk_hobject* env, duk_activation* act,
                        duk_hstring* name, duk_tval* val, duk_bool_t strict) {
  duk__id_lookup_result ref;
  duk_tval tv_tmp_obj;
  duk_tval tv_tmp_key;
  duk_bool_t parents;
  ...
  DUK_DDD(DUK_DDDPRINT(
      "putvar: thr=%p, env=%p, act=%p, name=%!O, val=%p, strict=%ld "
      "(env -> %!dO, val -> %!T)",
      (void*)thr, (void*)env, (void*)act, (duk_heaphdr*)name, (void*)val,
      (long)strict, (duk_heaphdr*)env, (duk_tval*)val));
  ...
  DUK_ASSERT(!strict || (name != DUK_HTHREAD_STRING_EVAL(thr) &&
  ...
  if (strict) {
    DUK_DDD(DUK_DDDPRINT(
        "identifier binding not found, strict => reference error"));
    DUK_ERROR_FMT1(thr, DUK_ERR_REFERENCE_ERROR, "identifier '%s' undefined",
                   (const char*)DUK_HSTRING_GET_DATA(name));
  ...
  DUK_DDD(DUK_DDDPRINT(
      "identifier binding not found, not strict => set to global"));

  DUK_TVAL_SET_OBJECT(&tv_tmp_obj, thr->builtins[DUK_BIDX_GLOBAL]);
  DUK_TVAL_SET_STRING(&tv_tmp_key, name);
  (void)duk_hobject_putprop(thr, &tv_tmp_obj, &tv_tmp_key, val,
                            0); /* 0 = no throw */
...
DUK_INTERNAL
void duk_js_putvar_envrec(duk_hthread* thr, duk_hobject* env, duk_hstring* name,
                          duk_tval* val, duk_bool_t strict) {
  duk__putvar_helper(thr, env, NULL, name, val, strict);
}

DUK_INTERNAL
void duk_js_putvar_activation(duk_hthread* thr, duk_activation* act,
...
DUK_LOCAL
duk_bool_t duk__delvar_helper(duk_hthread* thr, duk_hobject* env,
                              duk_activation* act, duk_hstring* name) {
  duk__id_lookup_result ref;
  duk_bool_t parents;
...
DUK_INTERNAL
duk_bool_t duk_js_delvar_activation(duk_hthread* thr, duk_activation* act,
                                    duk_hstring* name) {
  DUK_ASSERT(act != NULL);
...
DUK_LOCAL
duk_bool_t duk__declvar_helper(duk_hthread* thr, duk_hobject* env,
                               duk_hstring* name, duk_tval* val,
  ...
  DUK_DDD(
      DUK_DDDPRINT("declvar: thr=%p, env=%p, name=%!O, val=%!T, "
                   "prop_flags=0x%08lx, is_func_decl=%ld "
                   "(env -> %!iO)",
                   (void*)thr, (void*)env, (duk_heaphdr*)name, (duk_tval*)val,
                   (unsigned long)prop_flags, (unsigned int)is_func_decl,
                   (duk_heaphdr*)env));
  ...
  if (duk__get_identifier_reference(thr, env, name, NULL, parents, &ref)) {
     ...
     */
    DUK_ASSERT(DUK_HOBJECT_GET_CLASS_NUMBER(holder) ==
               DUK_HOBJECT_CLASS_GLOBAL);
    DUK_ASSERT(!DUK_HOBJECT_HAS_EXOTIC_ARRAY(
        holder)); /* global object doesn't have array part */
    ...
    if (holder == ref.holder) {
      ...
      DUK_DDD(DUK_DDDPRINT(
          "updated global binding, final result: "
   ...
   * extensibility etc correctly.  Basically we'd want to do a
   * [[DefineOwnProperty]] or Object.defineProperty() here.
   */
...
DUK_INTERNAL
duk_bool_t duk_js_declvar_activation(duk_hthread* thr, duk_activation* act,
                                     duk_hstring* name, duk_tval* val,
                                     duk_small_uint_t prop_flags,
                                     duk_bool_t is_func_decl) {
  duk_hobject* env;
