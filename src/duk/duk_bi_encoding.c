...
#define DUK__CP_CONTINUE \
  (-1)                     /* continue to next byte, no completed codepoint */
  ...
  if (dec_ctx->needed == 0) {
    ...
    if (x >= dec_ctx->lower && x <= dec_ctx->upper) {
      ...
      if (--dec_ctx->needed > 0) {
        ...
        /* got a codepoint */
        duk_codepoint_t ret;
        DUK_ASSERT(dec_ctx->codepoint <=
                   0x10ffffL); /* Decoding rules guarantee. */
...
#  endif
  ...
  if (DUK_UNLIKELY(codepoint > 0x10ffffL)) {
    ...
    if (codepoint <= 0xdbffL) {
      ...
      if (enc_ctx->lead != 0x0000L) {
        codepoint =
            (duk_codepoint_t)(0x010000L + ((enc_ctx->lead - 0xd800L) << 10) +
                              (codepoint - 0xdc00L));
    ...
    if (enc_ctx->lead != 0x0000L) {
      /* unpaired high surrogate: emit replacement character and the input
       * codepoint */
      enc_ctx->lead = 0x0000L;
   ...
   */
  enc_ctx->out +=
      duk_unicode_encode_xutf8((duk_ucodepoint_t)codepoint, enc_ctx->out);
}
...
DUK_LOCAL duk_ret_t duk__decode_helper(duk_hthread* thr,
  ...
  output = (duk_uint8_t*)duk_push_fixed_buffer_nozero(
      thr, 3 + (3 * len)); /* used parts will be always manually written over */
  ...
  if (!stream) {
    if (dec_ctx->needed != 0) {
      ...
      if (dec_ctx->fatal) {
        ...
        out +=
            duk_unicode_encode_cesu8(DUK_UNICODE_CP_REPLACEMENT_CHARACTER, out);
        DUK_ASSERT(out <= output + (3 + (3 * len)));
      }
    }
    duk__utf8_decode_init(
        dec_ctx); /* Initialize decoding state for potential reuse. */
  }
...
duk_bi_textencoder_prototype_encoding_getter(duk_hthread* thr) {
...
DUK_INTERNAL duk_ret_t duk_bi_textdecoder_constructor(duk_hthread* thr) {
   ...
   */
  dec_ctx = (duk__decode_context*)duk_push_fixed_buffer(
      thr, sizeof(duk__decode_context));
...
DUK_INTERNAL duk_ret_t
duk_bi_textdecoder_prototype_shared_getter(duk_hthread* thr) {
  duk__decode_context* dec_ctx;
  duk_int_t magic;
 ...
 * index 0, and decode options (not present for Buffer) at index 1.  Return
 * value is a Duktape/C function return value.
 */
