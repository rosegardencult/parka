 ...
 *
 *  Default algorithm is xoroshiro128+:
 * http://xoroshiro.di.unimi.it/xoroshiro128plus.c with SplitMix64 seed
 * preparation: http://xorshift.di.unimi.it/splitmix64.c.
 *
...
#    define DUK__UPDATE_RND(rnd)                                             \
      do {                                                                   \
        (rnd) += ((rnd) * (rnd)) | 0x05UL;                                   \
        (rnd) = ((rnd)&0xffffffffUL); /* if duk_uint32_t is exactly 32 bits, \
                                         this is a NOP */                    \
      } while (0)
...
DUK_LOCAL DUK_ALWAYS_INLINE duk_uint64_t duk__rnd_rotl(const duk_uint64_t x,
                                                       duk_small_uint_t k) {
  return (x << k) | (x >> (64U - k));
}

DUK_LOCAL DUK_ALWAYS_INLINE duk_uint64_t
duk__xoroshiro128plus(duk_uint64_t* s) {
  duk_uint64_t s0;
  duk_uint64_t s1;
  duk_uint64_t res;
...
DUK_INTERNAL void duk_util_tinyrandom_prepare_seed(duk_hthread* thr) {
  ...
  for (i = 0; i < 64; i++) {
    thr->heap->rnd_state[i & 0x01] =
        duk__rnd_splitmix64(&x); /* Keep last 2 values. */
  }
}
...
DUK_INTERNAL duk_double_t duk_util_tinyrandom_get_double(duk_hthread* thr) {
   ...
   */
  v = (DUK_U64_CONSTANT(0x3ff) << 52U) |
      (duk__xoroshiro128plus((duk_uint64_t*)thr->heap->rnd_state) >> 12U);
  du.ull[0] = v;
