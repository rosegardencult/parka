 ...
 */
#  define DUK_HSTRING_FLAG_ASCII \
    DUK_HEAPHDR_USER_FLAG(0) /* string is ASCII, clen == blen */
#  define DUK_HSTRING_FLAG_ARRIDX \
    DUK_HEAPHDR_USER_FLAG(1) /* string is a valid array index */
#  define DUK_HSTRING_FLAG_SYMBOL \
    DUK_HEAPHDR_USER_FLAG(2) /* string is a symbol (invalid utf-8) */
#  define DUK_HSTRING_FLAG_HIDDEN                                          \
    DUK_HEAPHDR_USER_FLAG(3) /* string is a hidden symbol (implies symbol, \
                                Duktape 1.x internal string) */
#  define DUK_HSTRING_FLAG_RESERVED_WORD \
    DUK_HEAPHDR_USER_FLAG(4) /* string is a reserved word (non-strict) */
#  define DUK_HSTRING_FLAG_STRICT_RESERVED_WORD \
    DUK_HEAPHDR_USER_FLAG(5) /* string is a reserved word (strict) */
#  define DUK_HSTRING_FLAG_EVAL_OR_ARGUMENTS \
    DUK_HEAPHDR_USER_FLAG(6) /* string is 'eval' or 'arguments' */
#  define DUK_HSTRING_FLAG_EXTDATA \
    DUK_HEAPHDR_USER_FLAG(         \
        7) /* string data is external (duk_hstring_external) */
#  define DUK_HSTRING_FLAG_PINNED_LITERAL \
    DUK_HEAPHDR_USER_FLAG(8) /* string is a literal, and pinned */
...
#  define DUK_HSTRING_HAS_SYMBOL(x) \
    DUK_HEAPHDR_CHECK_FLAG_BITS(&(x)->hdr, DUK_HSTRING_FLAG_SYMBOL)
#  define DUK_HSTRING_HAS_HIDDEN(x) \
    DUK_HEAPHDR_CHECK_FLAG_BITS(&(x)->hdr, DUK_HSTRING_FLAG_HIDDEN)
...
#  define DUK_HSTRING_HAS_STRICT_RESERVED_WORD(x) \
    DUK_HEAPHDR_CHECK_FLAG_BITS(&(x)->hdr,        \
...
#  define DUK_HSTRING_HAS_PINNED_LITERAL(x) \
    DUK_HEAPHDR_CHECK_FLAG_BITS(&(x)->hdr, DUK_HSTRING_FLAG_PINNED_LITERAL)
...
#  define DUK_HSTRING_SET_SYMBOL(x) \
    DUK_HEAPHDR_SET_FLAG_BITS(&(x)->hdr, DUK_HSTRING_FLAG_SYMBOL)
#  define DUK_HSTRING_SET_HIDDEN(x) \
    DUK_HEAPHDR_SET_FLAG_BITS(&(x)->hdr, DUK_HSTRING_FLAG_HIDDEN)
...
#  define DUK_HSTRING_SET_PINNED_LITERAL(x) \
    DUK_HEAPHDR_SET_FLAG_BITS(&(x)->hdr, DUK_HSTRING_FLAG_PINNED_LITERAL)
...
#  define DUK_HSTRING_CLEAR_SYMBOL(x) \
    DUK_HEAPHDR_CLEAR_FLAG_BITS(&(x)->hdr, DUK_HSTRING_FLAG_SYMBOL)
#  define DUK_HSTRING_CLEAR_HIDDEN(x) \
    DUK_HEAPHDR_CLEAR_FLAG_BITS(&(x)->hdr, DUK_HSTRING_FLAG_HIDDEN)
...
#  define DUK_HSTRING_CLEAR_STRICT_RESERVED_WORD(x) \
    DUK_HEAPHDR_CLEAR_FLAG_BITS(&(x)->hdr,          \
                                DUK_HSTRING_FLAG_STRICT_RESERVED_WORD)
...
#  define DUK_HSTRING_CLEAR_PINNED_LITERAL(x) \
    DUK_HEAPHDR_CLEAR_FLAG_BITS(&(x)->hdr, DUK_HSTRING_FLAG_PINNED_LITERAL)
...
#    define DUK_HSTRING_GET_ARRIDX_FAST(h)                \
      (DUK_HSTRING_HAS_ARRIDX((h))                        \
           ? duk_js_to_arrayindex_hstring_fast_known((h)) \
...
DUK_INTERNAL_DECL duk_ucodepoint_t
duk_hstring_char_code_at_raw(duk_hthread* thr, duk_hstring* h, duk_uint_t pos,
                             duk_bool_t surrogate_aware);
DUK_INTERNAL_DECL duk_bool_t duk_hstring_equals_ascii_cstring(duk_hstring* h,
                                                              const char* cstr);
