...
DUK_LOCAL void duk__run_global_torture_finalizer(duk_hthread* thr) {
  ...
  if (thr->heap->call_recursion_depth + 5 >= thr->heap->call_recursion_limit ||
    ...
    DUK_D(
        DUK_DPRINT("skip global torture finalizer, too little headroom for "
                   "call recursion or call stack size"));
    return;
  }
...
#  endif
  ...
  if (heap->pf_prevent_count != 0) {
    DUK_DDD(
        DUK_DDDPRINT("skip finalize_list processing: pf_prevent_count != 0"));
    return;
  }
...
#  endif
    ...
    DUK_DD(DUK_DDPRINT("processing finalize_list entry: %p -> %!iO",
                       (void*)curr, curr));

    DUK_ASSERT(DUK_HEAPHDR_GET_TYPE(curr) ==
               DUK_HTYPE_OBJECT); /* Only objects have finalizers. */
    DUK_ASSERT(!DUK_HEAPHDR_HAS_REACHABLE(curr));
    DUK_ASSERT(!DUK_HEAPHDR_HAS_TEMPROOT(curr));
    DUK_ASSERT(DUK_HEAPHDR_HAS_FINALIZABLE(
        curr)); /* All objects on finalize_list will have this flag (except
                   object being finalized right now). */
    DUK_ASSERT(!DUK_HEAPHDR_HAS_FINALIZED(curr)); /* Queueing code ensures. */
    DUK_ASSERT(!DUK_HEAPHDR_HAS_READONLY(
        curr)); /* ROM objects never get freed (or finalized). */
...
#  if defined(DUK_USE_REFERENCE_COUNTING)
      ...
      had_zero_refcount = (DUK_HEAPHDR_GET_REFCOUNT(curr) ==
...
#  if defined(DUK_USE_REFERENCE_COUNTING)
      DUK_DD(DUK_DDPRINT("refcount after finalizer (includes bump): %ld",
                         (long)DUK_HEAPHDR_GET_REFCOUNT(curr)));
      if (DUK_HEAPHDR_GET_REFCOUNT(curr) ==
          1) { /* Only artificial bump in refcount? */
#    if defined(DUK_USE_DEBUG)
        if (had_zero_refcount) {
          DUK_DD(
              DUK_DDPRINT("finalized object's refcount is zero -> free "
                          "immediately (refcount queued)"));
        } else {
          DUK_DD(
              DUK_DDPRINT("finalized object's refcount is zero -> free "
                          "immediately (mark-and-sweep queued)"));
        }
#    endif
        queue_back = 0;
      } else
...
#  endif
       ...
       */
      DUK_D(
          DUK_DPRINT("skip finalizers flag set, queue object to heap_allocated "
                     "without finalizing"));
      DUK_ASSERT(!DUK_HEAPHDR_HAS_FINALIZED(curr));
#  if defined(DUK_USE_REFERENCE_COUNTING)
      queue_back = 1;
...
#  if defined(DUK_USE_DEBUG)
  DUK_DD(DUK_DDPRINT("duk_heap_process_finalize_list: %ld finalizers called",
                     (long)count));
#  endif
}
...
DUK_LOCAL duk_ret_t duk__finalize_helper(duk_hthread* thr, void* udata) {
  ...
  duk_get_prop_stridx_short(
      thr, -1, DUK_STRIDX_INT_FINALIZER); /* -> [... obj finalizer] */
  duk_dup_m2(thr);
...
#  endif
  ...
  DUK_DD(
      DUK_DDPRINT("running duk_hobject finalizer for object: %p", (void*)obj));
...
#  endif
  DUK_HEAPHDR_SET_FINALIZED(
      (duk_heaphdr*)
          obj); /* ensure never re-entered until rescue cycle complete */
...
#  endif /* DUK_USE_ES6_PROXY */
  ...
  rc = duk_safe_call(thr, duk__finalize_helper, NULL /*udata*/, 0 /*nargs*/,
                     1 /*nrets*/);    /* -> [... obj retval/error] */
