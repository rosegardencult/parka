 ...
 */
DUK_LOCAL DUK_NOINLINE_PERF DUK_COLD void* duk__heap_mem_alloc_slowpath(
    duk_heap* heap, duk_size_t size) {
  void* res;
  duk_small_int_t i;
  ...
  DUK_D(
      DUK_DPRINT("first alloc attempt failed or voluntary GC limit reached, "
                 "attempt to gc and retry"));
  ...
  DUK_D(DUK_DPRINT("duk_heap_mem_alloc() failed even after gc, alloc size %ld",
                   (long)size));
  return NULL;
}
...
DUK_INTERNAL DUK_INLINE_PERF DUK_HOT void* duk_heap_mem_alloc(duk_heap* heap,
  ...
  if (heap->ms_prevent_count == 0) {
    DUK_DDD(DUK_DDDPRINT(
        "gc torture enabled, pretend that first alloc attempt fails"));
    res = NULL;
    DUK_UNREF(res);
    goto slowpath;
  }
...
slowpath:
  ...
  if (size == 0) {
    DUK_D(
        DUK_DPRINT("first alloc attempt returned NULL for zero size alloc, use "
                   "slow path to deal with it"));
  } else {
...
DUK_INTERNAL DUK_INLINE_PERF DUK_HOT void* duk_heap_mem_alloc_zeroed(
    duk_heap* heap, duk_size_t size) {
  void* res;
...
DUK_INTERNAL DUK_INLINE_PERF DUK_HOT void* duk_heap_mem_alloc_checked(
    duk_hthread* thr, duk_size_t size) {
  void* res;
...
DUK_INTERNAL DUK_INLINE_PERF DUK_HOT void* duk_heap_mem_alloc_checked_zeroed(
    duk_hthread* thr, duk_size_t size) {
  void* res;
...
/* Slow path: voluntary GC triggered, first realloc attempt failed, or zero
 * size. */
DUK_LOCAL DUK_NOINLINE_PERF DUK_COLD void* duk__heap_mem_realloc_slowpath(
    duk_heap* heap, void* ptr, duk_size_t newsize) {
  void* res;
  duk_small_int_t i;
  ...
  for (i = 0; i < DUK_HEAP_ALLOC_FAIL_MARKANDSWEEP_LIMIT; i++) {
    ...
    if (res || newsize == 0) {
      DUK_D(
          DUK_DPRINT("duk_heap_mem_realloc() succeeded after gc (pass %ld), "
                     "alloc size %ld",
  ...
  DUK_D(
      DUK_DPRINT("duk_heap_mem_realloc() failed even after gc, alloc size %ld",
                 (long)newsize));
  return NULL;
}

DUK_INTERNAL DUK_INLINE_PERF DUK_HOT void* duk_heap_mem_realloc(
    duk_heap* heap, void* ptr, duk_size_t newsize) {
  void* res;
  ...
  if (heap->ms_prevent_count == 0) {
    DUK_DDD(DUK_DDDPRINT(
        "gc torture enabled, pretend that first realloc attempt fails"));
    res = NULL;
    DUK_UNREF(res);
    goto slowpath;
  }
...
slowpath:
  ...
  if (newsize == 0) {
    DUK_D(
        DUK_DPRINT("first realloc attempt returned NULL for zero size realloc, "
                   "use slow path to deal with it"));
  } else {
    DUK_D(DUK_DPRINT("first realloc attempt failed, attempt to gc and retry"));
  }
...
/* Slow path: voluntary GC triggered, first realloc attempt failed, or zero
 * size. */
DUK_LOCAL DUK_NOINLINE_PERF DUK_COLD void*
duk__heap_mem_realloc_indirect_slowpath(duk_heap* heap, duk_mem_getptr cb,
                                        void* ud, duk_size_t newsize) {
  void* res;
  duk_small_int_t i;
  ...
  for (i = 0; i < DUK_HEAP_ALLOC_FAIL_MARKANDSWEEP_LIMIT; i++) {
    ...
    if (ptr_pre != ptr_post) {
      DUK_DD(DUK_DDPRINT(
          "realloc base pointer changed by mark-and-sweep: %p -> %p",
    ...
    if (res || newsize == 0) {
      DUK_D(
          DUK_DPRINT("duk_heap_mem_realloc_indirect() succeeded after gc (pass "
                     "%ld), alloc size %ld",
                     (long)(i + 1), (long)newsize));
      return res;
    }
...
DUK_INTERNAL DUK_INLINE_PERF DUK_HOT void* duk_heap_mem_realloc_indirect(
    duk_heap* heap, duk_mem_getptr cb, void* ud, duk_size_t newsize) {
  void* res;
  ...
  if (heap->ms_prevent_count == 0) {
    DUK_DDD(
        DUK_DDDPRINT("gc torture enabled, pretend that first indirect realloc "
                     "attempt fails"));
    res = NULL;
    DUK_UNREF(res);
    goto slowpath;
...
slowpath:
  ...
  if (newsize == 0) {
    DUK_D(
        DUK_DPRINT("first indirect realloc attempt returned NULL for zero size "
                   "realloc, use slow path to deal with it"));
  } else {
    DUK_D(DUK_DPRINT(
        "first indirect realloc attempt failed, attempt to gc and retry"));
  }
