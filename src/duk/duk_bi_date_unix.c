...
DUK_INTERNAL duk_double_t duk_bi_date_get_now_time(void) {
  ...
  return ((duk_double_t)t) * 1000.0;
    ...
    defined(DUK_USE_DATE_TZO_GMTIME_S)
...
#  endif
  ...
  if (!duk_bi_date_timeval_in_leeway_range(d)) {
    DUK_DD(
        DUK_DDPRINT("timeval not within valid range, skip tzoffset computation "
  ...
  duk_bi_date_timeval_to_parts(d, parts, dparts,
                               DUK_DATE_FLAG_EQUIVYEAR /*flags*/);
  DUK_ASSERT(parts[DUK_DATE_IDX_YEAR] >= 1970 &&
             parts[DUK_DATE_IDX_YEAR] <= 2038);
...
#  endif
  ...
  DUK_DDD(DUK_DDDPRINT(
      "localtime result: "
      "tm={sec:%ld,min:%ld,hour:%ld,mday:%ld,mon:%ld,year:%ld,"
      "wday:%ld,yday:%ld,isdst:%ld}",
...
DUK_INTERNAL duk_bool_t duk_bi_date_parse_string_strptime(duk_hthread* thr,
                                                          const char* str) {
  struct tm tm;
  time_t t;
  ...
  if (strptime((const char*)buf, "%c", &tm) != NULL) {
    DUK_DDD(DUK_DDDPRINT(
        ...
        "wday:%ld,yday:%ld,isdst:%ld}",
        (long)tm.tm_sec, (long)tm.tm_min, (long)tm.tm_hour, (long)tm.tm_mday,
        (long)tm.tm_mon, (long)tm.tm_year, (long)tm.tm_wday, (long)tm.tm_yday,
        (long)tm.tm_isdst));
...
DUK_INTERNAL duk_bool_t duk_bi_date_parse_string_getdate(duk_hthread* thr,
                                                         const char* str) {
  struct tm tm;
  duk_small_int_t rc;
  time_t t;
...
DUK_INTERNAL duk_double_t duk_bi_date_get_monotonic_time_clock_gettime(void) {
  ...
  if (clock_gettime(CLOCK_MONOTONIC, &ts) == 0) {
    return (duk_double_t)ts.tv_sec * 1000.0 +
           (duk_double_t)ts.tv_nsec / 1000000.0;
  } else {
