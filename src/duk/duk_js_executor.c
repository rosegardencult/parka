...
DUK_LOCAL DUK_EXEC_ALWAYS_INLINE_PERF duk_double_t
...
DUK_LOCAL DUK_EXEC_ALWAYS_INLINE_PERF duk_double_t
duk__compute_exp(duk_double_t d1, duk_double_t d2) {
  return (duk_double_t)duk_js_arith_pow((double)d1, (double)d2);
}
#endif

DUK_LOCAL DUK_EXEC_ALWAYS_INLINE_PERF void duk__vm_arith_add(
  ...
  duk_to_primitive(
      thr, -2, DUK_HINT_NONE); /* side effects -> don't use tv_x, tv_y after */
...
DUK_LOCAL DUK_EXEC_ALWAYS_INLINE_PERF void duk__vm_arith_binary_op(
    duk_hthread* thr, duk_tval* tv_x, duk_tval* tv_y, duk_uint_fast_t idx_z,
...
DUK_LOCAL DUK_EXEC_ALWAYS_INLINE_PERF void duk__vm_bitwise_binary_op(
    duk_hthread* thr, duk_tval* tv_x, duk_tval* tv_y,
    duk_small_uint_fast_t idx_z, duk_small_uint_fast_t opcode) {
  /*
...
DUK_LOCAL DUK_EXEC_ALWAYS_INLINE_PERF void duk__vm_arith_unary_op(
    duk_hthread* thr, duk_uint_fast_t idx_src, duk_uint_fast_t idx_dst,
    duk_small_uint_fast_t opcode) {
  /*
   *  Arithmetic operations other than '+' have number-only semantics
   *  and are implemented here.  The separate switch-case here means a
...
#if defined(DUK_USE_FASTINT)
    tv = DUK_GET_TVAL_POSIDX(thr, (duk_idx_t)idx_dst);
    DUK_TVAL_SET_NUMBER_CHKFAST_UPDREF(thr, tv,
                                       du.d); /* always 'fast', i.e. inlined */
    return;
#endif
  } else {
...
DUK_LOCAL DUK_EXEC_ALWAYS_INLINE_PERF void duk__vm_bitwise_not(
    duk_hthread* thr, duk_uint_fast_t idx_src, duk_uint_fast_t idx_dst) {
  /*
   *  E5 Section 11.4.8
   */
...
DUK_LOCAL DUK_EXEC_ALWAYS_INLINE_PERF void duk__vm_logical_not(
    duk_hthread* thr, duk_uint_fast_t idx_src, duk_uint_fast_t idx_dst) {
  /*
   *  E5 Section 11.4.9
   */
...
skip_fastint:
  ...
  if (DUK_TVAL_IS_NUMBER(tv_src)) {
    ...
    off_dst =
        (duk_size_t)((duk_uint8_t*)tv_dst - (duk_uint8_t*)thr->valstack_bottom);
    bc = (duk_idx_t)(tv_src -
                     thr->valstack_bottom); /* XXX: pass index explicitly? */
...
DUK_LOCAL DUK_EXEC_ALWAYS_INLINE_PERF void duk__prepost_incdec_var_helper(
    duk_hthread* thr, duk_small_uint_t idx_dst, duk_tval* tv_id,
    duk_small_uint_t op, duk_small_uint_t is_strict) {
  duk_activation* act;
  duk_double_t x, y;
  duk_hstring* name;
...
DUK_LOCAL void duk__reconfig_valstack_ecma_return(duk_hthread* thr) {
  ...
  thr->valstack_bottom =
      (duk_tval*)(void*)((duk_uint8_t*)thr->valstack + act->bottom_byteoff);
  ...
  thr->valstack_end =
      (duk_tval*)(void*)((duk_uint8_t*)thr->valstack + act->reserve_byteoff);
...
DUK_LOCAL void duk__handle_catch_part1(
    duk_hthread* thr, duk_tval* tv_val_unstable, duk_small_uint_t lj_type,
  ...
  if (DUK_CAT_HAS_CATCH_BINDING_ENABLED(cat)) {
    DUK_DDD(DUK_DDDPRINT(
        "catcher has an automatic catch binding, handle in part 2"));
...
DUK_LOCAL void duk__handle_catch_part2(duk_hthread* thr) {
  ...
  DUK_HOBJECT_INCREF(thr,
                     (duk_hobject*)new_env); /* reachable through activation */
...
DUK_LOCAL duk_small_uint_t
duk__handle_longjmp(duk_hthread* thr, duk_activation* entry_act,
                    volatile duk_bool_t* out_delayed_catch_setup) {
...
check_longjmp:
  ...
  DUK_DD(
      DUK_DDPRINT("handling longjmp: type=%ld, value1=%!T, value2=%!T, "
                  "iserror=%ld, top=%ld",
  ...
  switch (thr->heap->lj.type) {
    ...
    case DUK_LJ_TYPE_RESUME: {
      ...
      /* duk_bi_duk_object_yield() and duk_bi_duk_object_resume() ensure all of
       * these are met */

      DUK_ASSERT(
          thr->state ==
          DUK_HTHREAD_STATE_RUNNING); /* unchanged by Duktape.Thread.resume() */
      DUK_ASSERT(
          thr->callstack_top >=
      ...
      if (thr->heap->lj.iserror) {
        ...
        DUK_DD(
            DUK_DDPRINT("-> resume with an error, converted to a throw in the "
                        "resumee, propagate"));
        goto check_longjmp;
...
#  endif
        ...
        setup_rc =
            duk_handle_call_unprotected_nargs(resumee, 1 /*nargs*/, call_flags);
        if (setup_rc == 0) {
...
#  endif
      ...
      if (thr->heap->lj.iserror) {
        ...
        DUK_DD(
            DUK_DDPRINT("-> yield an error, converted to a throw in the "
                        "resumer, propagate"));
        goto check_longjmp;
      } else {
...
#  endif
    ...
    case DUK_LJ_TYPE_THROW: {
      ...
      for (;;) {
        ...
        for (;;) {
          ...
          if (DUK_CAT_HAS_CATCH_ENABLED(cat)) {
            ...
            duk__handle_catch_part1(thr, &thr->heap->lj.value1,
                                    DUK_LJ_TYPE_THROW, out_delayed_catch_setup);

            DUK_DD(DUK_DDPRINT(
                "-> throw caught by a 'catch' clause, restart execution"));
            retval = DUK__LONGJMP_RESTART;
            goto wipe_and_return;
          }
        ...
        if (act == entry_act) {
           ...
           */
          DUK_D(
              DUK_DPRINT("-> throw propagated up to entry level, rethrow and "
                         "exit bytecode executor"));
      ...
      DUK_DD(
          DUK_DDPRINT("-> throw not caught by current thread, yield error to "
                      "resumer and recheck longjmp"));

      /* Not caught by current thread, thread terminates (yield error to
       * resumer); note that this may cause a cascade if the resumer terminates
       * with an uncaught exception etc (this is OK, but needs careful testing).
       */
    ...
    default: {
      ...
      DUK_D(
          DUK_DPRINT("caught unknown longjmp type %ld, treat as internal error",
 ...
 */
DUK_LOCAL DUK_EXEC_NOINLINE_PERF void duk__handle_break_or_continue(
    duk_hthread* thr, duk_uint_t label_id, duk_small_uint_t lj_type) {
  duk_activation* act;
  duk_catcher* cat;
  ...
  for (;;) {
    ...
    if (DUK_CAT_GET_TYPE(cat) == DUK_CAT_TYPE_LABEL &&
      ...
      DUK_DD(
          DUK_DDPRINT("-> break/continue caught by a label catcher (in the "
                      "same function), restart execution"));
      return;
    }
  ...
  /* Should never happen, but be robust. */
  DUK_D(
      DUK_DPRINT("-> break/continue not caught by anything in the current "
...
DUK_LOCAL duk_small_uint_t duk__handle_return(duk_hthread* thr,
  ...
  if (thr->callstack_top >= 2) {
    ...
    DUK_DD(
        DUK_DDPRINT("-> return not intercepted, restart execution in caller"));
    return DUK__RETHAND_RESTART;
  }
  ...
  DUK_ASSERT(thr->resumer->callstack_curr->parent != NULL);
  DUK_ASSERT(
      DUK_ACT_GET_FUNC(thr->resumer->callstack_curr) != NULL &&
  ...
  DUK_ASSERT(
      DUK_ACT_GET_FUNC(thr->resumer->callstack_curr->parent) != NULL &&
      DUK_HOBJECT_IS_COMPFUNC(DUK_ACT_GET_FUNC(
          thr->resumer->callstack_curr->parent))); /* an ECMAScript function */
  ...
  duk_push_hthread(resumer,
                   thr); /* Make 'thr' reachable again, before side effects. */
...
#  endif
  ...
  DUK_DD(
      DUK_DDPRINT("-> return not caught, thread terminated; handle like yield, "
...
#  define DUK__INT_NOACTION                          \
    0 /* no specific action, resume normal execution \
       */
#  define DUK__INT_RESTART \
    1 /* must "goto restart_execution", e.g. breakpoints changed */

#  if defined(DUK_USE_DEBUGGER_SUPPORT)
DUK_LOCAL void duk__interrupt_handle_debugger(
    duk_hthread* thr, duk_bool_t* out_immediate,
  ...
  if (duk_debug_is_attached(thr->heap)) {
    ...
    if (processed_messages) {
      DUK_D(
          DUK_DPRINT("processed debug messages, restart execution to recheck "
                     "possibly changed breakpoints"));
...
DUK_LOCAL DUK_EXEC_NOINLINE_PERF DUK_COLD duk_small_uint_t
...
#  if defined(DUK_USE_DEBUG)
  ...
  DUK_DD(DUK_DDPRINT(
      "execution interrupt, counter=%ld, init=%ld, "
...
DUK_LOCAL void duk__executor_recheck_debugger(duk_hthread* thr,
  ...
  tv_tmp = duk_hobject_find_entry_tval_ptr_stridx(thr->heap, (duk_hobject*)fun,
  ...
  if (tv_tmp && DUK_TVAL_IS_STRING(tv_tmp)) {
    ...
    for (bp_idx = 0; bp_idx < heap->dbg_breakpoint_count; bp_idx++) {
      ...
      if (bp->filename == filename && bp->line >= fun->start_line &&
        ...
        bp_match = 1;
        DUK_DD(
            DUK_DDPRINT("breakpoint filename and line match: "
...
DUK_LOCAL DUK_EXEC_NOINLINE_PERF void duk__handle_op_initset_initget(
    duk_hthread* thr, duk_uint_fast32_t ins) {
...
DUK_LOCAL DUK_EXEC_NOINLINE_PERF void duk__handle_op_trycatch(
    duk_hthread* thr, duk_uint_fast32_t ins, duk_instr_t* curr_pc) {
  duk_activation* act;
  duk_catcher* cat;
  duk_tval* tv1;
  ...
  if (a & DUK_BC_TRYCATCH_FLAG_CATCH_BINDING) {
     ...
     */
    cat = act->cat; /* XXX: better to relookup? not mandatory because 'cat' is
                       stable */
  ...
  DUK_DDD(
      DUK_DDDPRINT("TRYCATCH catcher: flags=0x%08lx, pc_base=%ld, "
...
DUK_LOCAL DUK_EXEC_NOINLINE_PERF duk_instr_t* duk__handle_op_endtry(
    duk_hthread* thr, duk_uint_fast32_t ins) {
  duk_activation* act;
  duk_catcher* cat;
  duk_tval* tv1;
  ...
  DUK_DDD(
      DUK_DDDPRINT("ENDTRY: clearing catch active flag (regardless of whether "
                   "it was set or not)"));
  DUK_CAT_CLEAR_CATCH_ENABLED(cat);
  ...
  if (DUK_CAT_HAS_FINALLY_ENABLED(cat)) {
    DUK_DDD(
        DUK_DDDPRINT("ENDTRY: finally part is active, jump through 2nd jump "
                     "slot with 'normal continuation'"));
    ...
    DUK_DDD(
        DUK_DDDPRINT("ENDTRY: no finally part, dismantle catcher, jump through "
                     "2nd jump slot (to end of statement)"));
...
DUK_LOCAL DUK_EXEC_NOINLINE_PERF duk_instr_t* duk__handle_op_endcatch(
    duk_hthread* thr, duk_uint_fast32_t ins) {
  duk_activation* act;
  duk_catcher* cat;
  duk_tval* tv1;
  ...
  DUK_ASSERT(cat != NULL);
  DUK_ASSERT(
      !DUK_CAT_HAS_CATCH_ENABLED(cat)); /* cleared before entering catch part */
  ...
  if (DUK_CAT_HAS_FINALLY_ENABLED(cat)) {
    DUK_DDD(
        DUK_DDDPRINT("ENDCATCH: finally part is active, jump through 2nd jump "
                     "slot with 'normal continuation'"));
...
DUK_LOCAL DUK_EXEC_NOINLINE_PERF duk_small_uint_t duk__handle_op_endfin(
  ...
  switch (cont_type) {
    ...
    case DUK_LJ_TYPE_RETURN: {
      DUK_DDD(
          DUK_DDDPRINT("ENDFIN: finally part finishing with 'return' "
...
DUK_LOCAL DUK_EXEC_NOINLINE_PERF void duk__handle_op_initenum(
    duk_hthread* thr, duk_uint_fast32_t ins) {
  duk_small_uint_t b;
  duk_small_uint_t c;
...
DUK_LOCAL DUK_EXEC_NOINLINE_PERF duk_small_uint_t
...
DUK_LOCAL duk_bool_t duk__executor_handle_call(duk_hthread* thr, duk_idx_t idx,
  ...
  duk_set_top_unsafe(
      thr, (duk_idx_t)(idx + nargs + 2)); /* [ ... func this arg1 ... argN ] */
  ...
  if (rc != 0) {
     ...
     */
    DUK_DDD(
        DUK_DDDPRINT("ecma-to-ecma call setup possible, restart execution"));
...
DUK_LOCAL void duk__handle_executor_error(
    duk_heap* heap, duk_activation* entry_act,
    duk_int_t entry_call_recursion_depth, duk_jmpbuf* entry_jmpbuf_ptr,
    volatile duk_bool_t* out_delayed_catch_setup) {
  ...
  DUK_DD(DUK_DDPRINT("executor error handled, pf_prevent_count updated to %ld",
  ...
  for (;;) {
    ...
    if (DUK_SETJMP(our_jmpbuf.jb) == 0) {
      ...
      if (DUK_UNLIKELY(delayed_catch_setup != 0)) {
        ...
        DUK_DDD(DUK_DDDPRINT("top after delayed catch setup: %ld",
                             (long)duk_get_top(entry_thread)));
      }
      ...
      duk__handle_executor_error(heap, entry_act, entry_call_recursion_depth,
                                 entry_jmpbuf_ptr, &delayed_catch_setup);
    }
#if defined(DUK_USE_CPP_EXCEPTIONS)
      ...
      try {
        ...
        DUK_ERROR_FMT1(heap->curr_thread, DUK_ERR_TYPE_ERROR,
                       "caught invalid c++ std::exception '%s' (perhaps thrown "
                       "by user code)",
                       what);
        DUK_WO_NORETURN(return;);
      } catch (duk_internal_exception exc) {
      ...
      DUK_D(
          DUK_DPRINT("unexpected c++ exception (perhaps thrown by user code)"));
      DUK_STATS_INC(exec_thr->heap, stats_exec_throw);
      try {
        DUK_ASSERT(heap->curr_thread != NULL);
        DUK_ERROR_TYPE(
...
DUK_LOCAL DUK_NOINLINE DUK_HOT void duk__js_execute_bytecode_inner(
    duk_hthread* entry_thread, duk_activation* entry_act) {
...
restart_execution:
  ...
  DUK_DD(
      DUK_DDPRINT("restarting execution, thr %p, act idx %ld, fun %p,"
...
#  if defined(DUK_USE_DEBUGGER_SUPPORT)
      if (thr->heap->dbg_force_restart) {
        DUK_DD(DUK_DDPRINT(
            "dbg_force_restart flag forced restart execution")); /* GH-303 */
...
#  endif
...
#if defined(DUK_USE_ASSERTIONS) || defined(DUK_USE_DEBUG)
    {
      ...
      DUK_DDD(DUK_DDDPRINT(
          ...
          (long)(DUK__FUN() ? DUK__FUN()->nregs : -1), (duk_instr_t)*curr_pc));
    }
#endif
...
#  endif
      ...
      case DUK_OP_LDTHIS: {
        ...
        tv2 =
            thr->valstack_bottom - 1; /* 'this binding' is just under bottom */
...
#  endif /* DUK_USE_ES7_EXP_OPERATOR */
      ...
      case DUK_OP_BAND_CC: {
        duk__vm_bitwise_binary_op(thr, DUK__CONSTP_B(ins), DUK__CONSTP_C(ins),
                                  DUK_DEC_A(ins), DUK_OP_BAND);
        break;
      }
      case DUK_OP_BOR_RR: {
  ...
  {                                       \
    ...
    DUK__REPLACE_BOOL_A_BREAK(tmp);       \
      ...
      case DUK_OP_POSTDECR: {
        duk__prepost_incdec_reg_helper(thr, DUK__REGP_A(ins), DUK__REGP_BC(ins),
                                       op);
        break;
      }
      case DUK_OP_PREINCV:
      ...
      case DUK_OP_POSTDECV: {
        duk__prepost_incdec_var_helper(thr, DUK_DEC_A(ins), DUK__CONSTP_BC(ins),
                                       op, DUK__STRICT());
        break;
      }
#else  /* DUK_USE_EXEC_PREFER_SIZE */
      ...
      case DUK_OP_PREINCV: {
        duk__prepost_incdec_var_helper(thr, DUK_DEC_A(ins), DUK__CONSTP_BC(ins),
                                       DUK_OP_PREINCV, DUK__STRICT());
        break;
      }
      case DUK_OP_PREDECV: {
        duk__prepost_incdec_var_helper(thr, DUK_DEC_A(ins), DUK__CONSTP_BC(ins),
                                       DUK_OP_PREDECV, DUK__STRICT());
        break;
      }
      case DUK_OP_POSTINCV: {
        duk__prepost_incdec_var_helper(thr, DUK_DEC_A(ins), DUK__CONSTP_BC(ins),
                                       DUK_OP_POSTINCV, DUK__STRICT());
        break;
      }
      case DUK_OP_POSTDECV: {
        duk__prepost_incdec_var_helper(thr, DUK_DEC_A(ins), DUK__CONSTP_BC(ins),
                                       DUK_OP_POSTDECV, DUK__STRICT());
        break;
      }
#endif /* DUK_USE_EXEC_PREFER_SIZE */
...
#  endif
      ...
      case DUK_OP_REGEXP_CC: {
        ...
        duk_push_tval(thr, DUK__REGCONSTP_C(ins));
        duk_push_tval(thr, DUK__REGCONSTP_B(
                               ins)); /* -> [ ... escaped_source bytecode ] */
      ...
      case DUK_OP_CSVAR_CC: {
         ...
         * B -> identifier name, usually constant but can be a register due to
         * shuffling
         */
        ...
        duk_replace(
            thr,
            (duk_idx_t)
                idx); /* variable value (function, we hope, not checked here) */
        break;
      }
      ...
      case DUK_OP_CLOSURE: {
        ...
        DUK_ASSERT((duk_uint_t)bc < (duk_uint_t)DUK_HCOMPFUNC_GET_FUNCS_COUNT(
                                        thr->heap, DUK__FUN()));
      ...
      case DUK_OP_JUMP: {
         ...
         */
        curr_pc +=
            (duk_int_fast_t)DUK_DEC_ABC(ins) - (duk_int_fast_t)DUK_BC_JUMP_BIAS;
        break;
      }
...
#  if defined(DUK_USE_REFERENCE_COUNTING)
        ...
        DUK_ASSERT(
            !DUK_TVAL_IS_HEAP_ALLOCATED(tv)); /* no INCREF for this constant */
#  endif
        thr->valstack_top++;
        DUK__RETURN_SHARED();
      }
      case DUK_OP_RETUNDEF: {
        DUK__SYNC_AND_NULL_CURR_PC();
        thr->valstack_top++; /* value at valstack top is already undefined by
                                valstack policy */
      ...
      case DUK_OP_LABEL: {
        ...
        cat->flags =
            (duk_uint32_t)(DUK_CAT_TYPE_LABEL | (bc << DUK_CAT_LABEL_SHIFT));
    ...
    defined(DUK_USE_ASSERTIONS)
      ...
      case DUK_OP_THROW: {
        ...
        DUK_DDD(
            DUK_DDDPRINT("THROW ERROR (BYTECODE): %!dT (before throw augment)",
                         (duk_tval*)duk_get_tval(thr, -1)));
#if defined(DUK_USE_AUGMENT_ERROR_THROW)
        duk_err_augment_error_throw(thr);
        DUK_DDD(
            DUK_DDDPRINT("THROW ERROR (BYTECODE): %!dT (after throw augment)",
                         (duk_tval*)duk_get_tval(thr, -1)));
#endif
      ...
      case DUK_OP_CALL7: {
         ...
         * BC -> base register for call (base -> func, base+1 -> this, base+2 ->
         * arg1 ... base+2+N-1 -> argN)
         */
...
#  endif
      ...
      case DUK_OP_MPUTARRI: {
         ...
         * valueN) or register containing first register number if indirect C ->
        ...
        do {
           ...
           * and finally set 'length' manually in the end (as already happens
           * now).
           */
      ...
      case DUK_OP_DEBUGGER: {
        ...
        if (duk_debug_is_attached(thr->heap)) {
          ...
          DUK_D(
              DUK_DPRINT("DEBUGGER statement ignored, debugger not attached"));
        }
#else
