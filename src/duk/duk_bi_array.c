...
DUK_LOCAL duk_harray* duk__arraypart_fastpath_this(duk_hthread* thr) {
  ...
  DUK_ASSERT(thr->valstack_bottom >
             thr->valstack); /* because call in progress */
  ...
  DUK_ASSERT(h != NULL);
  flags_mask = DUK_HOBJECT_FLAG_ARRAY_PART | DUK_HOBJECT_FLAG_EXOTIC_ARRAY |
  ...
  DUK_ASSERT(DUK_HOBJECT_GET_ASIZE(h) == 0 ||
             DUK_HOBJECT_A_GET_BASE(thr->heap, h) != NULL);
...
DUK_INTERNAL duk_ret_t duk_bi_array_constructor(duk_hthread* thr) {
  ...
  if (nargs == 1 && duk_is_number(thr, 0)) {
    /* XXX: expensive check (also shared elsewhere - so add a shared internal
     * API call?) */
...
DUK_INTERNAL duk_ret_t duk_bi_array_prototype_to_string(duk_hthread* thr) {
  ...
  if (!duk_is_callable(thr, -1)) {
    ...
    duk_set_top(thr, 0);
    return duk_bi_object_prototype_to_string(
        thr); /* has access to 'this' binding */
  }
...
#  endif
    ...
    need_has_check =
        (DUK_HOBJECT_IS_PROXY(h) != 0); /* Always 0 w/o Proxy support. */
...
DUK_INTERNAL duk_ret_t duk_bi_array_prototype_join_shared(duk_hthread* thr) {
  ...
  DUK_DDD(DUK_DDDPRINT("sep=%!T, this=%!T, len=%lu",
                       ...
                       (duk_tval*)duk_get_tval(thr, 1), (unsigned long)len));

  /* The extra (+4) is tight. */
  valstack_required = (duk_idx_t)(
      (len >= DUK__ARRAY_MID_JOIN_LIMIT ? DUK__ARRAY_MID_JOIN_LIMIT : len) + 4);
  ...
  for (;;) {
    ...
    if (count >= DUK__ARRAY_MID_JOIN_LIMIT || /* intermediate join to avoid
                                                 valstack overflow */
...
#  endif
  ...
  /* XXX: Merge fastpath check into a related call (push this, coerce length,
   * etc)? */
...
DUK_LOCAL duk_ret_t duk__array_push_fastpath(duk_hthread* thr,
  ...
  if (DUK_UNLIKELY(len + (duk_uint32_t)n < len)) {
    ...
    DUK_DCERROR_RANGE_INVALID_LENGTH(
        thr); /* != 0 return value returned as is by caller */
  }
...
DUK_LOCAL duk_small_int_t duk__array_sort_compare(duk_hthread* thr,
  ...
  /* Fast exit if indices are identical.  This is valid for a non-existent
   * property, for an undefined value, and almost always for ToString() coerced
   * comparison of arbitrary values (corner cases where this is not the case
   * include e.g. a an object with varying ToString() coercion).
   *
   * The specification does not prohibit "caching" of values read from the
   * array, so assuming equality for comparing an index with itself falls into
   * the category of "caching".
   *
   * Also, compareFn may be inconsistent, so skipping a call to compareFn here
   * may have an effect on the final result.  The specification does not require
   * any specific behavior for inconsistent compare functions, so again, this
   * fast path is OK.
   */

  if (idx1 == idx2) {
    DUK_DDD(
        DUK_DDDPRINT("duk__array_sort_compare: idx1=%ld, idx2=%ld -> indices "
                     "identical, quick exit",
                     (long)idx1, (long)idx2));
    return 0;
  }
  ...
  DUK_DDD(DUK_DDDPRINT(
      "duk__array_sort_compare: idx1=%ld, idx2=%ld, have1=%ld, have2=%ld, "
      "val1=%!T, val2=%!T",
  ...
  if (!duk_is_undefined(thr, idx_fn)) {
    ...
    duk_pop_nodecref_unsafe(thr);
    DUK_DDD(DUK_DDDPRINT("-> result %ld (from comparefn, after coercion)",
                         (long)ret));
    return ret;
  }
...
DUK_LOCAL void duk__array_sort_swap(duk_hthread* thr, duk_int_t l,
                                    duk_int_t r) {
...
DUK_LOCAL void duk__debuglog_qsort_state(duk_hthread* thr, duk_int_t lo,
                                         duk_int_t hi, duk_int_t pivot) {
  char buf[4096];
  char* ptr = buf;
  duk_int_t i, n;
  ...
  *ptr++ = '\0';
  ...
  DUK_DDD(DUK_DDDPRINT("%s   (lo=%ld, hi=%ld, pivot=%ld)", (const char*)buf,
                       (long)lo, (long)hi, (long)pivot));
}
#  endif
...
DUK_LOCAL void duk__array_qsort(duk_hthread* thr, duk_int_t lo, duk_int_t hi) {
  ...
  DUK_DDD(DUK_DDDPRINT("duk__array_qsort: lo=%ld, hi=%ld, obj=%!T", (long)lo,
                       (long)hi, (duk_tval*)duk_get_tval(thr, 1)));
  ...
  p = lo + (duk_int_t)(DUK_UTIL_GET_RANDOM_DOUBLE(thr) *
                       (duk_double_t)(hi - lo + 1));
  DUK_ASSERT(p >= lo && p <= hi);
  DUK_DDD(DUK_DDDPRINT("lo=%ld, hi=%ld, chose pivot p=%ld", (long)lo, (long)hi,
                       (long)p));

  /* move pivot out of the way */
  duk__array_sort_swap(thr, p, lo);
  p = lo;
  DUK_DDD(DUK_DDDPRINT("pivot moved out of the way: %!T",
                       (duk_tval*)duk_get_tval(thr, 1)));
  ...
  for (;;) {
    ...
    for (;;) {
      DUK_DDD(DUK_DDDPRINT("left scan: l=%ld, r=%ld, p=%ld", (long)l, (long)r,
                           (long)p));
      if (l >= hi) {
        break;
      }
    ...
    for (;;) {
      DUK_DDD(DUK_DDDPRINT("right scan: l=%ld, r=%ld, p=%ld", (long)l, (long)r,
                           (long)p));
      if (r <= lo) {
        break;
      }
  ...
  DUK_DDD(DUK_DDDPRINT("before final pivot swap: %!T",
                       (duk_tval*)duk_get_tval(thr, 1)));
...
#  endif
  ...
  DUK_DDD(DUK_DDDPRINT("recurse: pivot=%ld, obj=%!T", (long)r,
                       (duk_tval*)duk_get_tval(thr, 1)));
 ...
 * for an array part.  Uses signed ints so does not handle full array range
 * correctly.
 */
...
#  endif
  ...
  if (((duk_double_t)len) - ((duk_double_t)del_count) +
          ((duk_double_t)item_count) >
      (duk_double_t)DUK_UINT32_MAX) {
    DUK_D(DUK_DPRINT(
  ...
  for (i = 0; i < del_count; i++) {
    if (duk_get_prop_index(thr, -3, (duk_uarridx_t)(act_start + i))) {
      duk_xdef_prop_index_wec(
          thr, -2,
          (duk_uarridx_t)i); /* throw flag irrelevant (false in std alg) */
    } else {
      duk_pop_undefined(thr);
    }
  }
  ...
  if (item_count < del_count) {
     ...
     *    [ A B C F G H ]        (actual result at this point, C will be
     * replaced)
     */
  ...
  /* Step 16: update length; note that the final length may be above 32 bit
   * range (but we checked above that this isn't the case here)
   */
...
DUK_INTERNAL duk_ret_t duk_bi_array_prototype_unshift(duk_hthread* thr) {
  ...
  while (i > 0) {
    ...
    if (duk_get_prop_index(thr, -2, (duk_uarridx_t)i)) {
      ...
      duk_put_prop_index(
          ...
          (duk_uarridx_t)(
              i + (duk_uint32_t)
                      nargs)); /* -> [ ... ToObject(this) ToUint32(length) ] */
    } else {
      /* fromPresent = false */
      /* [ ... ToObject(this) ToUint32(length) val ] */
      duk_pop_undefined(thr);
  ...
  for (i = 0; i < (duk_uint32_t)nargs; i++) {
    ...
    duk_dup(
        thr,
        (duk_idx_t)i); /* -> [ ... ToObject(this) ToUint32(length) arg[i] ] */
  ...
  duk_dup_top(
      thr); /* -> [ ... ToObject(this) ToUint32(length) final_len final_len ] */
  duk_put_prop_stridx_short(thr, -4, DUK_STRIDX_LENGTH);
  return 1;
}
...
DUK_INTERNAL duk_ret_t duk_bi_array_prototype_indexof_shared(duk_hthread* thr) {
  ...
  duk_int_t i, len;
  duk_int_t from_idx;
  duk_small_int_t idx_step = duk_get_current_magic(
      thr); /* idx_step is +1 for indexOf, -1 for lastIndexOf */
   ...
   *
   *   - lastIndexOf: clamp to [-len-1, len-1], negative handling -> [-1,
   * len-1], if clamped result is -1, for-loop bails out immediately
   *
  ...
  if (nargs >= 2) {
     ...
     * (if clamped to -len-1 -> fromIndex becomes -1, terminates for-loop
     * directly)
     */
    from_idx = duk_to_int_clamped(thr, 1, (idx_step > 0 ? -len : -len - 1),
...
/* XXX: This helper is a bit awkward because the handling for the different
 * iteration callers is quite different.  This now compiles to a bit less than
 * 500 bytes, so with 5 callers the net result is about 100 bytes / caller.
 */
...
DUK_INTERNAL duk_ret_t duk_bi_array_prototype_iter_shared(duk_hthread* thr) {
  ...
  for (i = 0; i < len; i++) {
    ...
    switch (iter_type) {
      ...
      case DUK__ITER_MAP:
        duk_dup_top(thr);
        duk_xdef_prop_index_wec(thr, 4,
                                (duk_uarridx_t)i); /* retval to result[i] */
        res_length = i + 1;
        break;
  ...
  switch (iter_type) {
    ...
    case DUK__ITER_FILTER:
      DUK_ASSERT_TOP(thr, 5);
      DUK_ASSERT(duk_is_array(
          thr, -1)); /* topmost element is the result array already */
      duk_push_u32(thr, res_length);
...
DUK_INTERNAL duk_ret_t duk_bi_array_prototype_reduce_shared(duk_hthread* thr) {
  ...
  duk_uint32_t i, len;
  duk_small_int_t idx_step = duk_get_current_magic(
      thr); /* idx_step is +1 for reduce, -1 for reduceRight */
  ...
  DUK_DDD(DUK_DDDPRINT("have_acc=%ld, acc=%!T", (long)have_acc,
  ...
  for (i = (idx_step >= 0 ? 0 : len - 1);
    ...
    if (!have_acc) {
      ...
      duk_dup_2(thr);
      DUK_DDD(DUK_DDDPRINT(
          "calling reduce function: func=%!T, prev=%!T, curr=%!T, idx=%!T, "
          "obj=%!T",
