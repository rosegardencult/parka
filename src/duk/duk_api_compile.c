...
DUK_EXTERNAL duk_int_t duk_eval_raw(duk_hthread* thr, const char* src_buffer,
  ...
  rc = duk_compile_raw(
      thr, src_buffer, src_length,
      flags |
          DUK_COMPILE_EVAL); /* may be safe, or non-safe depending on flags */
...
DUK_LOCAL duk_ret_t duk__do_compile(duk_hthread* thr, void* udata) {
  ...
  if (flags & DUK_COMPILE_NOFILENAME) {
    ...
    duk_push_hstring_stridx(
        thr, (flags & DUK_COMPILE_EVAL) ? DUK_STRIDX_EVAL : DUK_STRIDX_INPUT);
  }
  ...
  duk_js_push_closure(thr, h_templ, thr->builtins[DUK_BIDX_GLOBAL_ENV],
                      thr->builtins[DUK_BIDX_GLOBAL_ENV], 1 /*add_auto_proto*/);
...
DUK_EXTERNAL duk_int_t duk_compile_raw(duk_hthread* thr, const char* src_buffer,
