...
/* XXX: return type should probably be duk_size_t, or explicit checks are needed
 * for maximum size.
 */
DUK_LOCAL duk_uint32_t duk__insert_u32(duk_re_compiler_ctx* re_ctx,
  ...
  DUK_ASSERT(len >= 0);
  DUK_BW_INSERT_ENSURE_BYTES(re_ctx->thr, &re_ctx->bw, offset, buf,
                             (duk_size_t)len);
...
/* special helper for emitting u16 lists (used for character ranges for built-in
 * char classes) */
...
DUK_LOCAL void duk__insert_slice(duk_re_compiler_ctx* re_ctx,
                                 duk_uint32_t offset, duk_uint32_t data_offset,
                                 duk_uint32_t data_length) {
  DUK_BW_INSERT_ENSURE_SLICE(re_ctx->thr, &re_ctx->bw, offset, data_offset,
                             data_length);
}
...
DUK_LOCAL void duk__remove_slice(duk_re_compiler_ctx* re_ctx,
                                 duk_uint32_t data_offset,
                                 duk_uint32_t data_length) {
  DUK_BW_REMOVE_ENSURE_SLICE(re_ctx->thr, &re_ctx->bw, data_offset,
                             data_length);
}
...
DUK_LOCAL duk_uint32_t duk__append_jump_offset(duk_re_compiler_ctx* re_ctx,
                                               duk_int32_t skip) {
  return (duk_uint32_t)duk__insert_jump_offset(
      re_ctx, (duk_uint32_t)DUK__RE_BUFLEN(re_ctx), skip);
}
...
DUK_LOCAL void duk__regexp_emit_range(duk_re_compiler_ctx* re_ctx,
                                      duk_codepoint_t r1, duk_codepoint_t r2) {
  DUK_ASSERT(r2 >= r1);
...
duk__re_canon_next_discontinuity(duk_codepoint_t start, duk_codepoint_t end) {
...
DUK_LOCAL void duk__regexp_generate_ranges(void* userdata, duk_codepoint_t r1,
                                           duk_codepoint_t r2,
                                           duk_bool_t direct) {
  ...
  if (direct || (re_ctx->re_flags & DUK_RE_FLAG_IGNORE_CASE) == 0) {
    DUK_DD(DUK_DDPRINT("direct or not case sensitive, emit range: [%ld,%ld]",
                       (long)r1, (long)r2));
    duk__regexp_emit_range(re_ctx, r1, r2);
    return;
  }

  DUK_DD(DUK_DDPRINT("case sensitive, process range: [%ld,%ld]", (long)r1,
                     (long)r2));
...
DUK_LOCAL const duk_uint16_t* const duk__re_range_lookup1[3] = {
    duk_unicode_re_ranges_digit, duk_unicode_re_ranges_white,
...
DUK_LOCAL void duk__parse_disjunction(duk_re_compiler_ctx* re_ctx,
  ...
  duk_int32_t atom_start_offset =
      -1; /* negative -> no atom matched on previous round */
  duk_int32_t atom_char_length = 0; /* negative -> complex atom */
  duk_uint32_t atom_start_captures =
      re_ctx->captures; /* value of re_ctx->captures at start of atom */
...
#  endif
  ...
  for (;;) {
     ...
     */
    duk_int32_t
        new_atom_char_length; /* char length of the atom parsed in this loop */
    duk_int32_t new_atom_start_offset;    /* bytecode start offset of the atom
                                           * parsed in this loop    (allows
                                           * quantifiers to copy the atom bytecode)
                                           */
    duk_uint32_t new_atom_start_captures; /* re_ctx->captures at the start of
                                             the atom parsed in this loop */

    duk_lexer_parse_re_token(&re_ctx->lex, &re_ctx->curr_token);

    DUK_DD(DUK_DDPRINT(
        "re token: %ld (num=%ld, char=%c)", (long)re_ctx->curr_token.t,
        (long)re_ctx->curr_token.num,
        (re_ctx->curr_token.num >= 0x20 && re_ctx->curr_token.num <= 0x7e)
            ? (int)re_ctx->curr_token.num
            : (int)'?'));
    ...
    switch (re_ctx->curr_token.t) {
      case DUK_RETOK_DISJUNCTION: {
        /*
         *  The handling here is a bit tricky.  If a previous '|' has been
         * processed, we have a pending split1 and a pending jump (for a
         * previous match).  These need to be back-patched carefully.  See docs
      ...
      case DUK_RETOK_QUANTIFIER: {
        ...
        if (atom_char_length >= 0) {
           ...
           *  Simple atom
           *
           *  If atom_char_length is zero, we'll have unbounded execution time
           * for e.g.
           *  /()*x/.exec('x').  We can't just skip the match because it might
           * have some side effects (for instance, if we allowed captures in
           * simple atoms, the capture needs to happen).  The simple solution
           * below is to force the quantifier to match at most once, since the
           * additional matches have no effect.
           *
           *  With a simple atom there can be no capture groups, so no captures
           * need to be reset.
           */
          ...
          atom_code_length = (duk_int32_t)(DUK__RE_BUFLEN(re_ctx) -
                                           (duk_size_t)atom_start_offset);
           ...
           *  quantifiers.  This would need some sort of a 'progress'
           * instruction.
           *
           *  XXX: impose limit on maximum result size, i.e. atom_code_len *
           * atom_copies?
           */
          duk_int32_t atom_code_length;
          ...
          /* pre-check how many atom copies we're willing to make (atom_copies
           * not needed below) */
          atom_copies = (re_ctx->curr_token.qmax == DUK_RE_QUANTIFIER_INFINITE)
                            ? re_ctx->curr_token.qmin
          ...
          if (atom_start_captures != re_ctx->captures) {
            ...
            DUK_DDD(DUK_DDDPRINT(
                "must wipe ]atom_start_captures,re_ctx->captures]: ]%ld,%ld]",
                (long)atom_start_captures, (long)re_ctx->captures));

            /* insert (DUK_REOP_WIPERANGE, start, count) in reverse order so the
             * order ends up right */
            duk__insert_u32(re_ctx, (duk_uint32_t)atom_start_offset,
                            (re_ctx->captures - atom_start_captures) * 2U);
            duk__insert_u32(re_ctx, (duk_uint32_t)atom_start_offset,
                            (atom_start_captures + 1) * 2);
            duk__insert_u32(re_ctx, (duk_uint32_t)atom_start_offset,
                            DUK_REOP_WIPERANGE);
          } else {
            DUK_DDD(
                DUK_DDDPRINT("no need to wipe captures: atom_start_captures == "
                             "re_ctx->captures == %ld",
                             (long)atom_start_captures));
          }

          atom_code_length =
              (duk_int32_t)DUK__RE_BUFLEN(re_ctx) - atom_start_offset;

          /* insert the required matches (qmin) by copying the atom */
          tmp_qmin = re_ctx->curr_token.qmin;
          tmp_qmax = re_ctx->curr_token.qmax;
          while (tmp_qmin > 0) {
            duk__append_slice(re_ctx, (duk_uint32_t)atom_start_offset,
          ...
          if (tmp_qmax == DUK_RE_QUANTIFIER_INFINITE) {
            ...
            duk__append_jump_offset(re_ctx,
                                    -atom_code_length - 1); /* -1 for opcode */
          } else {
            /*
             *  The remaining matches are emitted as sequence of SPLITs and atom
             *  copies; the SPLITs skip the remaining copies and match the
             * sequel. This sequence needs to be emitted starting from the last
             * copy because the SPLITs are variable length due to the variable
             * length skip offset.  This causes a lot of memory copying now.
             *
             *  Example structure (greedy, match maximum # atoms):
             *
             *      SPLIT1 LSEQ
             *      (atom)
             *      SPLIT1 LSEQ    ; <- the byte length of this instruction is
             * needed (atom)         ; to encode the above SPLIT1 correctly
             *      ...
             *   LSEQ:
             */
            duk_uint32_t offset = (duk_uint32_t)DUK__RE_BUFLEN(re_ctx);
            while (tmp_qmax > 0) {
              duk__insert_slice(re_ctx, offset, (duk_uint32_t)atom_start_offset,
                                (duk_uint32_t)atom_code_length);
              if (re_ctx->curr_token.greedy) {
                duk__insert_u32(re_ctx, offset,
                                DUK_REOP_SPLIT1); /* prefer direct */
              } else {
                duk__insert_u32(re_ctx, offset,
                                DUK_REOP_SPLIT2); /* prefer jump */
              }
              duk__insert_jump_offset(
                  re_ctx, offset + 1, /* +1 for opcode */
          ...
          duk__remove_slice(re_ctx, (duk_uint32_t)atom_start_offset,
                            (duk_uint32_t)atom_code_length);
        }
      ...
      case DUK_RETOK_ASSERT_START_NEG_LOOKAHEAD: {
        duk_uint32_t offset;
        duk_uint32_t opcode =
            (re_ctx->curr_token.t == DUK_RETOK_ASSERT_START_POS_LOOKAHEAD)
                ? DUK_REOP_LOOKPOS
                : DUK_REOP_LOOKNEG;
      ...
      case DUK_RETOK_ATOM_CHAR: {
        ...
        if (re_ctx->re_flags & DUK_RE_FLAG_IGNORE_CASE) {
          ch = (duk_uint32_t)duk_unicode_re_canonicalize_char(
              re_ctx->thr, (duk_codepoint_t)ch);
        }
      ...
      case DUK_RETOK_ATOM_NOT_WORD_CHAR: {
        ...
        re_op = (re_ctx->curr_token.t & 0x01) ? DUK_REOP_RANGES
                                              : DUK_REOP_INVRANGES;
        ...
        idx = (duk_small_uint_t)(
            (re_ctx->curr_token.t - DUK_RETOK_ATOM_DIGIT) >> 1U);
        DUK_ASSERT(idx <= 2U); /* Assume continuous token numbers; also checks
                                  negative underflow. */

        duk__append_range_atom_matcher(re_ctx, re_op,
                                       duk__re_range_lookup1[idx],
                                       duk__re_range_lookup2[idx]);
        break;
      }
      ...
      case DUK_RETOK_ATOM_START_CAPTURE_GROUP: {
        ...
        duk__parse_disjunction(
            re_ctx, 0, &tmp_disj); /* retval (sub-atom char length) unused,
      ...
      case DUK_RETOK_ATOM_START_CHARCLASS_INVERTED: {
        ...
        new_atom_start_offset = (duk_int32_t)DUK__RE_BUFLEN(re_ctx);
        duk__append_reop(
            re_ctx, (re_ctx->curr_token.t == DUK_RETOK_ATOM_START_CHARCLASS)
                        ? DUK_REOP_RANGES
                        : DUK_REOP_INVRANGES);
        offset = (duk_uint32_t)DUK__RE_BUFLEN(
            re_ctx); /* patch in range count later */

        /* parse ranges until character class ends */
        re_ctx->nranges = 0; /* note: ctx-wide temporary */
        duk_lexer_parse_re_ranges(&re_ctx->lex, duk__regexp_generate_ranges,
                                  (void*)re_ctx);
  ...
  if (unpatched_disjunction_jump >= 0) {
    ...
    offset += duk__insert_jump_offset(
        re_ctx, offset, (duk_int32_t)(DUK__RE_BUFLEN(re_ctx) - offset));
    /* offset is now target of the pending split (right after jump) */
    duk__insert_jump_offset(re_ctx, (duk_uint32_t)unpatched_disjunction_split,
                            (duk_int32_t)offset - unpatched_disjunction_split);
  }
 ...
 *
 *  Sets up a regexp compilation context, and calls duk__parse_disjunction() to
 * do the actual parsing.  Handles generation of the compiled regexp header and
 * the "boilerplate" capture of the matching substring (save 0 and 1).  Also
 * does some global level regexp checks after recursive compilation has
 * finished.
 *
 *  An escaped version of the regexp source, suitable for use as a RegExp
 * instance 'source' property (see E5 Section 15.10.3), is also left on the
 * stack.
 *
...
DUK_INTERNAL void duk_regexp_compile(duk_hthread* thr) {
  ...
  DUK_LEXER_INITCTX(
      &re_ctx.lex); /* duplicate zeroing, expect for (possible) NULL inits */
  re_ctx.thr = thr;
  re_ctx.lex.thr = thr;
  ...
  (void)duk_buffer_to_string(thr,
                             -1); /* Safe because flags is at most 7 bit. */
...
DUK_INTERNAL void duk_regexp_create_instance(duk_hthread* thr) {
  ...
  DUK_HOBJECT_SET_PROTOTYPE_UPDREF(thr, h,
                                   thr->builtins[DUK_BIDX_REGEXP_PROTOTYPE]);

  duk_xdef_prop_stridx_short(thr, -3, DUK_STRIDX_INT_BYTECODE,
  ...
  duk_xdef_prop_stridx_short(thr, -2, DUK_STRIDX_INT_SOURCE,
                             DUK_PROPDESC_FLAGS_NONE);

  /* [ ... regexp_object ] */

  duk_push_int(thr, 0);
  duk_xdef_prop_stridx_short(thr, -2, DUK_STRIDX_LAST_INDEX,
