...
#  define DUK_HEAP_FLAG_MARKANDSWEEP_RECLIMIT_REACHED                          \
    (1U << 0) /* mark-and-sweep marking reached a recursion limit and must use \
                 multi-pass marking */
#  define DUK_HEAP_FLAG_INTERRUPT_RUNNING                                     \
    (1U << 1) /* executor interrupt running (used to avoid nested interrupts) \
               */
#  define DUK_HEAP_FLAG_FINALIZER_NORESCUE \
    (1U                                    \
     << 2) /* heap destruction ongoing, finalizer rescue no longer possible */
#  define DUK_HEAP_FLAG_DEBUGGER_PAUSED                                       \
    (1U << 3) /* debugger is paused: talk with debug client until step/resume \
               */
...
#  define DUK_HEAP_HAS_DEBUGGER_PAUSED(heap) \
    DUK__HEAP_HAS_FLAGS((heap), DUK_HEAP_FLAG_DEBUGGER_PAUSED)
...
#  define DUK_HEAP_SET_DEBUGGER_PAUSED(heap) \
    DUK__HEAP_SET_FLAGS((heap), DUK_HEAP_FLAG_DEBUGGER_PAUSED)
...
#  define DUK_HEAP_CLEAR_DEBUGGER_PAUSED(heap) \
    DUK__HEAP_CLEAR_FLAGS((heap), DUK_HEAP_FLAG_DEBUGGER_PAUSED)
...
#  define DUK_LJ_TYPE_YIELD \
    2 /* value1 -> yield value, iserror -> error / normal */
#  define DUK_LJ_TYPE_RESUME                                          \
    3 /* value1 -> resume value, value2 -> resumee thread, iserror -> \
         error/normal */
#  define DUK_LJ_TYPE_BREAK                                                   \
    4 /* value1 -> label number, pseudo-type to indicate a break continuation \
         (for ENDFIN) */
#  define DUK_LJ_TYPE_CONTINUE                                      \
    5 /* value1 -> label number, pseudo-type to indicate a continue \
         continuation (for ENDFIN) */
#  define DUK_LJ_TYPE_RETURN                                                   \
    6 /* value1 -> return value, pseudo-type to indicate a return continuation \
         (for ENDFIN) */
#  define DUK_LJ_TYPE_NORMAL                                                  \
    7 /* no value, pseudo-type to indicate a normal continuation (for ENDFIN) \
       */
...
#  define DUK_HEAP_STRINGCACHE_NOCACHE_LIMIT \
    16 /* strings up to the this length are not cached */
...
#  define DUK_PAUSE_FLAG_ONE_OPCODE \
    (1U << 0) /* pause when a single opcode has been executed */
#  define DUK_PAUSE_FLAG_ONE_OPCODE_ACTIVE                             \
    (1U << 1) /* one opcode pause actually active; artifact of current \
                 implementation */
#  define DUK_PAUSE_FLAG_LINE_CHANGE \
    (1U << 2) /* pause when current line number changes */
#  define DUK_PAUSE_FLAG_FUNC_ENTRY \
    (1U << 3) /* pause when entering a function */
#  define DUK_PAUSE_FLAG_FUNC_EXIT \
    (1U << 4) /* pause when exiting current function */
#  define DUK_PAUSE_FLAG_CAUGHT_ERROR \
    (1U << 5) /* pause when about to throw an error that is caught */
#  define DUK_PAUSE_FLAG_UNCAUGHT_ERROR \
    (1U << 6) /* pause when about to throw an error that won't be caught */
...
#  if defined(DUK_USE_DEBUGGER_SUPPORT)
  /* Callbacks and udata; dbg_read_cb != NULL is used to indicate attached
   * state. */
  ...
  duk_bool_t dbg_processing; /* currently processing messages or breakpoints:
                                don't enter message processing recursively (e.g.
                                no breakpoints when processing debugger eval) */
  duk_bool_t
      dbg_state_dirty; /* resend state next time executor is about to run */
  duk_bool_t
      dbg_force_restart; /* force executor restart to recheck breakpoints; used
                            to handle function returns (see GH-303) */
  duk_bool_t dbg_detaching; /* debugger detaching; used to avoid calling detach
                               handler recursively */
  duk_small_uint_t dbg_pause_flags; /* flags for automatic pause behavior */
  duk_activation* dbg_pause_act; /* activation related to pause behavior (pause
  ...
  duk_uint_t dbg_exec_counter; /* cumulative opcode execution count (overflows
                                  are OK) */
  duk_uint_t dbg_last_counter; /* value of dbg_exec_counter when we last did a
                                  Date-based check */
  duk_double_t dbg_last_time; /* time when status/peek was last done (Date-based
                                 rate limit) */
...
#  if defined(DUK_USE_ASSERTIONS)
  duk_bool_t dbg_calling_transport; /* transport call in progress, calling into
                                       Duktape forbidden */
#  endif
...
#  endif
  duk_bool_t
      st_resizing; /* string table is being resized; avoid recursive resize */
...
DUK_INTERNAL_DECL void duk_heap_insert_into_heap_allocated(duk_heap* heap,
                                                           duk_heaphdr* hdr);
#  if defined(DUK_USE_REFERENCE_COUNTING)
DUK_INTERNAL_DECL void duk_heap_remove_from_heap_allocated(duk_heap* heap,
                                                           duk_heaphdr* hdr);
#  endif
#  if defined(DUK_USE_FINALIZER_SUPPORT)
DUK_INTERNAL_DECL void duk_heap_insert_into_finalize_list(duk_heap* heap,
                                                          duk_heaphdr* hdr);
DUK_INTERNAL_DECL void duk_heap_remove_from_finalize_list(duk_heap* heap,
                                                          duk_heaphdr* hdr);
#  endif
#  if defined(DUK_USE_ASSERTIONS)
DUK_INTERNAL_DECL duk_bool_t duk_heap_in_heap_allocated(duk_heap* heap,
                                                        duk_heaphdr* ptr);
#  endif
#  if defined(DUK_USE_INTERRUPT_COUNTER)
DUK_INTERNAL_DECL void duk_heap_switch_thread(duk_heap* heap,
                                              duk_hthread* new_thr);
#  endif

DUK_INTERNAL_DECL duk_hstring* duk_heap_strtable_intern(duk_heap* heap,
                                                        const duk_uint8_t* str,
                                                        duk_uint32_t blen);
DUK_INTERNAL_DECL duk_hstring* duk_heap_strtable_intern_checked(
    duk_hthread* thr, const duk_uint8_t* str, duk_uint32_t len);
#  if defined(DUK_USE_LITCACHE_SIZE)
DUK_INTERNAL_DECL duk_hstring* duk_heap_strtable_intern_literal_checked(
    duk_hthread* thr, const duk_uint8_t* str, duk_uint32_t blen);
#  endif
DUK_INTERNAL_DECL duk_hstring* duk_heap_strtable_intern_u32(duk_heap* heap,
                                                            duk_uint32_t val);
DUK_INTERNAL_DECL duk_hstring* duk_heap_strtable_intern_u32_checked(
    duk_hthread* thr, duk_uint32_t val);
#  if defined(DUK_USE_REFERENCE_COUNTING)
DUK_INTERNAL_DECL void duk_heap_strtable_unlink(duk_heap* heap, duk_hstring* h);
#  endif
DUK_INTERNAL_DECL void duk_heap_strtable_unlink_prev(duk_heap* heap,
                                                     duk_hstring* h,
                                                     duk_hstring* prev);
...
DUK_INTERNAL_DECL duk_uint_fast32_t duk_heap_strcache_offset_char2byte(
    duk_hthread* thr, duk_hstring* h, duk_uint_fast32_t char_offset);
...
DUK_INTERNAL_DECL void* duk_heap_mem_alloc_zeroed(duk_heap* heap,
                                                  duk_size_t size);
DUK_INTERNAL_DECL void* duk_heap_mem_alloc_checked(duk_hthread* thr,
                                                   duk_size_t size);
DUK_INTERNAL_DECL void* duk_heap_mem_alloc_checked_zeroed(duk_hthread* thr,
                                                          duk_size_t size);
...
DUK_INTERNAL_DECL void duk_heap_mark_and_sweep(duk_heap* heap,
                                               duk_small_uint_t flags);
